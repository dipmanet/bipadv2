...
import { LegendItem, LayerHierarchy, LayerMap, EnumItem } from "#types";
...
export const hazardTypesList = (listWithHazard: WithHazard[], hazardTypes: Obj<HazardType>) => {
	...
	const group = groupList(listWithHazardFiltered as Required<WithHazard>[], (item) => item.hazard);
...
export const lossMetrics = [
	...
	{ key: "estimatedLoss", label: "Estimated loss (NPR)", labelNe: "अनुमानित आर्थिक क्षेति (रु )" },
	{
		...
		labelNe: "पूर्वाधारको क्षेति",
	},
	{ key: "livestockDestroyedCount", label: "Livestock destroyed", labelNe: "पशुचौपाया क्षति" },
	{ key: "peopleInjuredCount", label: "Injured people", labelNe: "घाइते संख्या" },
...
export const alertToPolygonGeojson = (alertList: Alert[], hazards: Obj<HazardType>) => {
	const geojson = {
		...
		features: alertList
			...
			.map((alert) => {
				const { id, title, polygon, description, createdOn } = alert;
	...
	const geojson = {
		...
		features: alertList
			...
			.map((alert) => {
				...
				const geometry = polygon ? centroid(polygon as AllGeoJSON).geometry : point;
...
export const eventToPolygonGeojson = (eventList: Event[], hazards: Obj<HazardType>) => {
	const geojson = {
		...
		features: eventList
			...
			.map((event) => {
				const { id, title, polygon, description, severity, hazard } = event;
...
export const incidentPointToGeojson = (incidentList: Incident[], hazards: Obj<HazardType>) => ({
	...
	features: incidentList
		...
		.map((incident) => {
			const { id, point, hazard: hazardId, incidentOn, severityValue } = incident;
...
export const incidentPolygonToGeojson = (incidentList: Incident[], hazards: Obj<HazardType>) => ({
	...
	features: incidentList
		...
		.sort((a, b) => b.severityValue - a.severityValue)
		.map((incident) => {
			const { id, polygon, hazard, incidentOn, severityValue } = incident;
			return {
				id,
				type: "Feature",
				geometry: { ...polygon },
...
export const regionLabelToGeojson = (adminLevels: Geo[]) => {
	const geojson = {
		type: "FeatureCollection",
		features: adminLevels.map((level) => ({
			id: level.id,
			type: "Feature",
			geometry: {
				...level.centroid,
...
export function getWardFilter(
	...
	if (selectedMunicipalityId) {
		return [
			"match",
			["id"],
			wards.filter((w) => w.municipality === selectedMunicipalityId).map((w) => w.id),
			true,
			false,
		];
	}
	if (selectedDistrictId) {
		return [
			"match",
			["id"],
			wards.filter((w) => w.district === selectedDistrictId).map((w) => w.id),
			true,
			false,
		];
	}
	if (selectedProvinceId) {
		return [
			"match",
			["id"],
			wards.filter((w) => w.province === selectedProvinceId).map((w) => w.id),
			true,
			false,
		];
	}
...
export function getMunicipalityFilter(
	...
	if (selectedDistrictId) {
		return [
			"match",
			["id"],
			municipalities.filter((m) => m.district === selectedDistrictId).map((m) => m.id),
			true,
			false,
		];
	}
	if (selectedProvinceId) {
		return [
			"match",
			["id"],
			municipalities.filter((m) => m.province === selectedProvinceId).map((m) => m.id),
			true,
			false,
		];
	}
...
export function getDistrictFilter(
	...
	if (selectedProvinceId) {
		return [
			"match",
			["id"],
			districts.filter((d) => d.province === selectedProvinceId).map((d) => d.id),
			true,
			false,
		];
	}
...
export const generatePaint = (colorDomain: string[], minValue: number, maxValue: number) => {
	...
	if (colors.length !== 0) {
		const fillColor = ["step", ["feature-state", "value"], ...colors.slice(0, -1)];
...
export const houseHoldSummaryLGProfileData = (
	lgProfileData,
...
export const literacyRatioLGProfileData = (
	lgProfileData,
...
export const sexRatioLGProfileData = (
	lgProfileData,
	sexRatioTotalPopulationLGProfile,
	language
) => [
...
export const summationLGProfileBuildingFoundationData = (lgProfileData) =>
	(lgProfileData.buildingFoundationType.bambooTimber
		? lgProfileData.buildingFoundationType.bambooTimber
		: 0) +
	...
	(lgProfileData.buildingFoundationType.reinforcedConcrete
		? lgProfileData.buildingFoundationType.reinforcedConcrete
		: 0) +
...
export const summationLGProfileBuildingTypeData = (lgProfileData) =>
	(lgProfileData.buildingType.AdobeMudConstruction
		? lgProfileData.buildingType.AdobeMudConstruction
		: 0) +
...
export const summationLGProfileAgricultureProductData = (lgProfileData) =>
	...
	(lgProfileData.majorAgriProducts.vegetableCrop
		? lgProfileData.majorAgriProducts.vegetableCrop
		: 0) +
...
export const summationLGProfileAgriculturePracticeData = (lgProfileData) =>
	(lgProfileData.agriculturePractice.totalNumberOfHouseholdEngagedInAgriculture
		? lgProfileData.agriculturePractice.totalNumberOfHouseholdEngagedInAgriculture
		: 0) +
	(lgProfileData.agriculturePractice.householdHavingOwnLandForAgriculture
		? lgProfileData.agriculturePractice.householdHavingOwnLandForAgriculture
		: 0) +
	(lgProfileData.agriculturePractice.householdRaisingLivestockForAgriculturePurposes
		? lgProfileData.agriculturePractice.householdRaisingLivestockForAgriculturePurposes
		: 0);
...
export const summationLGProfileDrinkingWaterData = (lgProfileData) =>
	...
	(lgProfileData.drinkingWater.tubeWellHandPump
		? lgProfileData.drinkingWater.tubeWellHandPump
		: 0) +
...
export const summationLGProfileMajorOccupationData = (lgProfileData) =>
	...
	(lgProfileData.majorOccupation.foreignEmployment
		? lgProfileData.majorOccupation.foreignEmployment
		: 0) +
...
export const summationLGProfileHouseHoldData = (lgProfileData) =>
	(lgProfileData.noOfHouseholds.femaleHeadedHouseholds
		? lgProfileData.noOfHouseholds.femaleHeadedHouseholds
		: 0) +
	(lgProfileData.noOfHouseholds.householdWithMemberAged60
		? lgProfileData.noOfHouseholds.householdWithMemberAged60
		: 0) +
	(lgProfileData.noOfHouseholds.numberOfHouseholdsWithDifferentlyAbleIndividual
		? lgProfileData.noOfHouseholds.numberOfHouseholdsWithDifferentlyAbleIndividual
		: 0);
export const summationLGProfileDisabilityData = (lgProfileData) =>
	(lgProfileData.disability.physicalDisabilities
		? lgProfileData.disability.physicalDisabilities
		: 0) +
	(lgProfileData.disability.visualDisabilities ? lgProfileData.disability.visualDisabilities : 0) +
	(lgProfileData.disability.hearingDisabilities
		? lgProfileData.disability.hearingDisabilities
		: 0) +
	(lgProfileData.disability.hearingVisualDisabilities
		? lgProfileData.disability.hearingVisualDisabilities
		: 0) +
	(lgProfileData.disability.speakingDisabilities
		? lgProfileData.disability.speakingDisabilities
		: 0) +
	(lgProfileData.disability.mentalDisabilities ? lgProfileData.disability.mentalDisabilities : 0) +
	(lgProfileData.disability.intellectualDisabilities
		? lgProfileData.disability.intellectualDisabilities
		: 0) +
	(lgProfileData.disability.multipleDisabilities
		? lgProfileData.disability.multipleDisabilities
		: 0);
export const summationLGProfileSocialSecurityData = (lgProfileData) =>
	(lgProfileData.socialSecurityBenefitAvailed.elderCitizen
		? lgProfileData.socialSecurityBenefitAvailed.elderCitizen
		: 0) +
	(lgProfileData.socialSecurityBenefitAvailed.singleWomen
		? lgProfileData.socialSecurityBenefitAvailed.singleWomen
		: 0) +
	(lgProfileData.socialSecurityBenefitAvailed.differentlyAble
		? lgProfileData.socialSecurityBenefitAvailed.differentlyAble
		: 0) +
	(lgProfileData.socialSecurityBenefitAvailed.extinctCaste
		? lgProfileData.socialSecurityBenefitAvailed.extinctCaste
		: 0) +
	(lgProfileData.socialSecurityBenefitAvailed.childSecurity
		? lgProfileData.socialSecurityBenefitAvailed.childSecurity
		: 0) +
	(lgProfileData.socialSecurityBenefitAvailed.pension
		? lgProfileData.socialSecurityBenefitAvailed.pension
		: 0) +
	(lgProfileData.socialSecurityBenefitAvailed.others
		? lgProfileData.socialSecurityBenefitAvailed.others
		: 0) +
	(lgProfileData.socialSecurityBenefitAvailed.notAvailed
		? lgProfileData.socialSecurityBenefitAvailed.notAvailed
		: 0) +
	(lgProfileData.socialSecurityBenefitAvailed.notApplicable
		? lgProfileData.socialSecurityBenefitAvailed.notApplicable
		: 0);
export const summationLGProfileMigrationData = (lgProfileData) =>
	...
	(lgProfileData.migration.notPresentInsideNepal
		? lgProfileData.migration.notPresentInsideNepal
		: 0) +
	(lgProfileData.migration.notPresentOutsideNepal
		? lgProfileData.migration.notPresentOutsideNepal
		: 0) +
...
export const SummationLGProfileEducationLevelData = (lgProfileData) =>
	(lgProfileData.educationLevel.primary ? lgProfileData.educationLevel.primary : 0) +
	(lgProfileData.educationLevel.lowerSecondary ? lgProfileData.educationLevel.lowerSecondary : 0) +
	(lgProfileData.educationLevel.slcOrEquivalent
		? lgProfileData.educationLevel.slcOrEquivalent
		: 0) +
	(lgProfileData.educationLevel.intermediateOrEquivalent
		? lgProfileData.educationLevel.intermediateOrEquivalent
		: 0) +
	(lgProfileData.educationLevel.bachelorOrEquivalent
		? lgProfileData.educationLevel.bachelorOrEquivalent
		: 0) +
	(lgProfileData.educationLevel.mastersPhd ? lgProfileData.educationLevel.mastersPhd : 0) +
	(lgProfileData.educationLevel.noEducation ? lgProfileData.educationLevel.noEducation : 0) +
	(lgProfileData.educationLevel.otherNonformal ? lgProfileData.educationLevel.otherNonformal : 0) +
	(lgProfileData.educationLevel.notEligible ? lgProfileData.educationLevel.notEligible : 0) +
	(lgProfileData.educationLevel.unknown ? lgProfileData.educationLevel.unknown : 0);
export const LGProfileBuildingFoundationData = (
	lgProfileData,
	summationLGProfileBuildingFoundation,
	language
) => [
	{
		key: "bambooTimber",
		label: language === "en" ? "Bamboo/Timber" : "बांस/टिम्बर",
		value: lgProfileData.buildingFoundationType.bambooTimber
			? lgProfileData.buildingFoundationType.bambooTimber
			: 0,
		percentage: lgProfileData.buildingFoundationType.bambooTimber
			? (lgProfileData.buildingFoundationType.bambooTimber / summationLGProfileBuildingFoundation) *
			  100
			: 0,
	},
	{
		...
		value: lgProfileData.buildingFoundationType.cement
			? lgProfileData.buildingFoundationType.cement
			: 0,
		percentage: lgProfileData.buildingFoundationType.cement
			? (lgProfileData.buildingFoundationType.cement / summationLGProfileBuildingFoundation) * 100
			: 0,
	},
	{
		key: "mud",
		...
		percentage: lgProfileData.buildingFoundationType.mud
			? (lgProfileData.buildingFoundationType.mud / summationLGProfileBuildingFoundation) * 100
			: 0,
	},
	{
		key: "reinforcedConcrete",
		label: language === "en" ? "Re Inforced Concrete" : "पुन: बलियो कंक्रीट",
		value: lgProfileData.buildingFoundationType.reinforcedConcrete
			? lgProfileData.buildingFoundationType.reinforcedConcrete
			: 0,
		percentage: lgProfileData.buildingFoundationType.reinforcedConcrete
			? (lgProfileData.buildingFoundationType.reinforcedConcrete /
					summationLGProfileBuildingFoundation) *
			  100
			: 0,
	},
	{
		key: "other",
		label: language === "en" ? "Other" : "अन्य",
		value: lgProfileData.buildingFoundationType.other
			? lgProfileData.buildingFoundationType.other
			: 0,
		percentage: lgProfileData.buildingFoundationType.other
			? (lgProfileData.buildingFoundationType.other / summationLGProfileBuildingFoundation) * 100
			: 0,
	},
];
export const LGProfileBuildingTypeData = (
	lgProfileData,
	summationLGProfileBuildingType,
	language
) => [
	{
		key: "AdobeMudConstruction",
		label: language === "en" ? "Adobe Mud Construction" : "एडोब मड निर्माण",
		value: lgProfileData.buildingType.AdobeMudConstruction
			? lgProfileData.buildingType.AdobeMudConstruction
			: 0,
		percentage: lgProfileData.buildingType.AdobeMudConstruction
			? (lgProfileData.buildingType.AdobeMudConstruction / summationLGProfileBuildingType) * 100
			: 0,
	},
	{
		key: "bamboo",
		...
		percentage: lgProfileData.buildingType.bamboo
			? (lgProfileData.buildingType.bamboo / summationLGProfileBuildingType) * 100
			: 0,
	},
	{
		key: "timber",
		...
		percentage: lgProfileData.buildingType.timber
			? (lgProfileData.buildingType.timber / summationLGProfileBuildingType) * 100
			: 0,
	},
	{
		key: "cementBrick",
		...
		percentage: lgProfileData.buildingType.cementBrick
			? (lgProfileData.buildingType.cementBrick / summationLGProfileBuildingType) * 100
			: 0,
	},
	{
		key: "cementStone",
		...
		percentage: lgProfileData.buildingType.cementStone
			? (lgProfileData.buildingType.cementStone / summationLGProfileBuildingType) * 100
			: 0,
	},
	{
		key: "mudBrick",
		...
		percentage: lgProfileData.buildingType.mudBrick
			? (lgProfileData.buildingType.mudBrick / summationLGProfileBuildingType) * 100
			: 0,
	},
	{
		key: "mudStone",
		...
		percentage: lgProfileData.buildingType.mudStone
			? (lgProfileData.buildingType.mudStone / summationLGProfileBuildingType) * 100
			: 0,
	},
	{
		key: "rcEngg",
		...
		percentage: lgProfileData.buildingType.rcEngg
			? (lgProfileData.buildingType.rcEngg / summationLGProfileBuildingType) * 100
			: 0,
	},
	{
		key: "rcNonEngg",
		...
		percentage: lgProfileData.buildingType.rcNonEngg
			? (lgProfileData.buildingType.rcNonEngg / summationLGProfileBuildingType) * 100
			: 0,
	},
	{
		key: "other",
		...
		percentage: lgProfileData.buildingType.other
			? (lgProfileData.buildingType.other / summationLGProfileBuildingType) * 100
			: 0,
	},
];

export const LGProfileAgricultureProductData = (
	lgProfileData,
	summationLGProfileAgricultureProduct,
	language
) => [
	{
		key: "cereals",
		...
		percentage: lgProfileData.majorAgriProducts.cereals
			? (lgProfileData.majorAgriProducts.cereals / summationLGProfileAgricultureProduct) * 100
			: 0,
	},
	{
		key: "oilseeds",
		...
		percentage: lgProfileData.majorAgriProducts.oilseeds
			? (lgProfileData.majorAgriProducts.oilseeds / summationLGProfileAgricultureProduct) * 100
			: 0,
	},
	{
		key: "vegetableCrop",
		label: language === "en" ? "Vegetable Crop" : "तरकारी बाली",
		value: lgProfileData.majorAgriProducts.vegetableCrop
			? lgProfileData.majorAgriProducts.vegetableCrop
			: 0,
		percentage: lgProfileData.majorAgriProducts.vegetableCrop
			? (lgProfileData.majorAgriProducts.vegetableCrop / summationLGProfileAgricultureProduct) * 100
			: 0,
	},
	{
		key: "spicyCrop",
		label: language === "en" ? "Spicy Crop" : "मसालेदार बाली",
		value: lgProfileData.majorAgriProducts.spicyCrop
			? lgProfileData.majorAgriProducts.spicyCrop
			: 0,
		percentage: lgProfileData.majorAgriProducts.spicyCrop
			? (lgProfileData.majorAgriProducts.spicyCrop / summationLGProfileAgricultureProduct) * 100
			: 0,
	},
	{
		key: "cashCrop",
		...
		percentage: lgProfileData.majorAgriProducts.cashCrop
			? (lgProfileData.majorAgriProducts.cashCrop / summationLGProfileAgricultureProduct) * 100
			: 0,
	},
];
export const LGProfileAgriculturePracticeData = (
	lgProfileData,
	summationLGProfileAgriculturePractice,
	language
) => [
	{
		...
		label:
			language === "en"
				? "Total Number Of Household Engaged In Agriculture"
				: "कृषिमा संलग्‍न परिवारको कुल संख्या",
		value: lgProfileData.agriculturePractice.totalNumberOfHouseholdEngagedInAgriculture
			? lgProfileData.agriculturePractice.totalNumberOfHouseholdEngagedInAgriculture
			: 0,
		percentage: lgProfileData.agriculturePractice.totalNumberOfHouseholdEngagedInAgriculture
			? (
					(lgProfileData.agriculturePractice.totalNumberOfHouseholdEngagedInAgriculture /
						summationLGProfileAgriculturePractice) *
					100
	...
	},
	{
		...
		label:
			language === "en"
				? "Household Having Own Land For Agriculture"
				: "कृषिका लागि आफ्नै जग्गा भएका घरपरिवार",
		value: lgProfileData.agriculturePractice.householdHavingOwnLandForAgriculture
			? lgProfileData.agriculturePractice.householdHavingOwnLandForAgriculture
			: 0,
		percentage: lgProfileData.agriculturePractice.householdHavingOwnLandForAgriculture
			? (
					(lgProfileData.agriculturePractice.householdHavingOwnLandForAgriculture /
						summationLGProfileAgriculturePractice) *
					100
	...
	},
	{
		...
		label:
			language === "en"
				? "Household Raising Livestock For Agriculture Purposes"
				: "कृषि उद्देश्यका लागि घरेलु पशुपालन",
		value: lgProfileData.agriculturePractice.householdRaisingLivestockForAgriculturePurposes
			? lgProfileData.agriculturePractice.householdRaisingLivestockForAgriculturePurposes
			: 0,
		percentage: lgProfileData.agriculturePractice.householdRaisingLivestockForAgriculturePurposes
			? (
					(lgProfileData.agriculturePractice.householdRaisingLivestockForAgriculturePurposes /
						summationLGProfileAgriculturePractice) *
					100
...
export const LGProfileResidentHouseholdData = (
	lgProfileData,
...
) => [
	{
		...
		percentage: lgProfileData.buildingsByResidentHh["0"]
			? (lgProfileData.buildingsByResidentHh["0"] / summationLGProfileResidentHousehold) * 100
			: 0,
	},
	{
		key: "1",
		...
		percentage: lgProfileData.buildingsByResidentHh["1"]
			? (lgProfileData.buildingsByResidentHh["1"] / summationLGProfileResidentHousehold) * 100
			: 0,
	},
	{
		key: "2",
		...
		percentage: lgProfileData.buildingsByResidentHh["2"]
			? (lgProfileData.buildingsByResidentHh["2"] / summationLGProfileResidentHousehold) * 100
			: 0,
	},
	{
		key: "3",
		...
		percentage: lgProfileData.buildingsByResidentHh["3"]
			? (lgProfileData.buildingsByResidentHh["3"] / summationLGProfileResidentHousehold) * 100
			: 0,
	},
	{
		key: "4+",
		label: "4+",
		value: lgProfileData.buildingsByResidentHh["4+"]
			? lgProfileData.buildingsByResidentHh["4+"]
			: 0,
		percentage: lgProfileData.buildingsByResidentHh["4+"]
			? (lgProfileData.buildingsByResidentHh["4+"] / summationLGProfileResidentHousehold) * 100
			: 0,
	},
];

export const LGProfileDrinkingWaterData = (
	lgProfileData,
	summationLGProfileDrinkingWater,
	language
) => [
	{
		key: "pipeWaterAtHome",
		label: language === "en" ? "Pipe Water At Home" : "घरमा पाइप पानी",
		value: lgProfileData.drinkingWater.pipeWaterAtHome
			? lgProfileData.drinkingWater.pipeWaterAtHome
			: 0,
		percentage: lgProfileData.drinkingWater.pipeWaterAtHome
			? (lgProfileData.drinkingWater.pipeWaterAtHome / summationLGProfileDrinkingWater) * 100
			: 0,
	},
	{
		key: "deepBoring",
		...
		percentage: lgProfileData.drinkingWater.deepBoring
			? (lgProfileData.drinkingWater.deepBoring / summationLGProfileDrinkingWater) * 100
			: 0,
	},
	{
		key: "tubeWellHandPump",
		label: language === "en" ? "TubeWell HandPump" : "ट्यूबवेल ह्यान्डपम्प",
		value: lgProfileData.drinkingWater.tubeWellHandPump
			? lgProfileData.drinkingWater.tubeWellHandPump
			: 0,
		percentage: lgProfileData.drinkingWater.tubeWellHandPump
			? (lgProfileData.drinkingWater.tubeWellHandPump / summationLGProfileDrinkingWater) * 100
			: 0,
	},
	{
		key: "coveredWell",
		...
		percentage: lgProfileData.drinkingWater.coveredWell
			? (lgProfileData.drinkingWater.coveredWell / summationLGProfileDrinkingWater) * 100
			: 0,
	},
	{
		key: "openWell",
		...
		percentage: lgProfileData.drinkingWater.openWell
			? (lgProfileData.drinkingWater.openWell / summationLGProfileDrinkingWater) * 100
			: 0,
	},
	{
		key: "groundWater",
		...
		percentage: lgProfileData.drinkingWater.groundWater
			? (lgProfileData.drinkingWater.groundWater / summationLGProfileDrinkingWater) * 100
			: 0,
	},
	{
		key: "river",
		...
		percentage: lgProfileData.drinkingWater.river
			? (lgProfileData.drinkingWater.river / summationLGProfileDrinkingWater) * 100
			: 0,
	},
];
export const LGProfileAverageMajorOccupationData = (
	lgProfileData,
	summationLGProfileMajorOccupation,
	language
) => [
	{
		key: "notEligible",
		label: language === "en" ? "Not Eligible" : "अयाेग्य",
		value: lgProfileData.majorOccupation.notEligible
			? lgProfileData.majorOccupation.notEligible
			: 0,
		percentage: lgProfileData.majorOccupation.notEligible
			? (lgProfileData.majorOccupation.notEligible / summationLGProfileMajorOccupation) * 100
			: 0,
	},
	{
		key: "unknown",
		...
		percentage: lgProfileData.majorOccupation.unknown
			? (lgProfileData.majorOccupation.unknown / summationLGProfileMajorOccupation) * 100
			: 0,
	},
	{
		key: "business",
		...
		percentage: lgProfileData.majorOccupation.business
			? (lgProfileData.majorOccupation.business / summationLGProfileMajorOccupation) * 100
			: 0,
	},
	{
		key: "service",
		...
		percentage: lgProfileData.majorOccupation.service
			? (lgProfileData.majorOccupation.service / summationLGProfileMajorOccupation) * 100
			: 0,
	},
	{
		key: "foreignEmployment",
		label: language === "en" ? "Foreign Employment" : "वैदेशिक रोजगारी",
		value: lgProfileData.majorOccupation.foreignEmployment
			? lgProfileData.majorOccupation.foreignEmployment
			: 0,
		percentage: lgProfileData.majorOccupation.foreignEmployment
			? (lgProfileData.majorOccupation.foreignEmployment / summationLGProfileMajorOccupation) * 100
			: 0,
	},
	{
		key: "student",
		...
		percentage: lgProfileData.majorOccupation.student
			? (lgProfileData.majorOccupation.student / summationLGProfileMajorOccupation) * 100
			: 0,
	},
	{
		key: "others",
		...
		percentage: lgProfileData.majorOccupation.others
			? (lgProfileData.majorOccupation.others / summationLGProfileMajorOccupation) * 100
			: 0,
	},
];

export const LGProfileAverageMonthlyIncomeData = (
	lgProfileData,
	summationLGProfileAverageMonthlyIncome,
	language
) => [
	{
		key: "lessThanNpr15",
		label: language === "en" ? "Less Than Npr 15,000" : "रु १५,००० भन्दा कम",
		value: lgProfileData.householdIncome.lessThanNpr15
			? lgProfileData.householdIncome.lessThanNpr15
			: 0,
		percentage: lgProfileData.householdIncome.lessThanNpr15
			? (lgProfileData.householdIncome.lessThanNpr15 / summationLGProfileAverageMonthlyIncome) * 100
			: 0,
	},
	{
		key: "15To30",
		...
		percentage: lgProfileData.householdIncome["15To30"]
			? (lgProfileData.householdIncome["15To30"] / summationLGProfileAverageMonthlyIncome) * 100
			: 0,
	},
	{
		key: "30To60",
		...
		percentage: lgProfileData.householdIncome["30To60"]
			? (lgProfileData.householdIncome["30To60"] / summationLGProfileAverageMonthlyIncome) * 100
			: 0,
	},
	{
		key: "60To120",
		...
		percentage: lgProfileData.householdIncome["60To120"]
			? (lgProfileData.householdIncome["60To120"] / summationLGProfileAverageMonthlyIncome) * 100
			: 0,
	},
	{
		key: "120To240",
		label: language === "en" ? "NPR120,000 to NPR240,000" : "रु १२०,००० देखि रु २४०,०००",
		value: lgProfileData.householdIncome["120To240"]
			? lgProfileData.householdIncome["120To240"]
			: 0,
		percentage: lgProfileData.householdIncome["120To240"]
			? (lgProfileData.householdIncome["120To240"] / summationLGProfileAverageMonthlyIncome) * 100
			: 0,
	},
	{
		key: "moreThan240",
		label: language === "en" ? "More Than Npr 240,000" : "रु २४०,००० भन्‍दा माथि",
		value: lgProfileData.householdIncome.moreThan240
			? lgProfileData.householdIncome.moreThan240
			: 0,
		percentage: lgProfileData.householdIncome.moreThan240
			? (lgProfileData.householdIncome.moreThan240 / summationLGProfileAverageMonthlyIncome) * 100
			: 0,
	},
];
...
export const LGProfileHouseHoldData = (lgProfileData, summationLGProfileHouseHold, language) => [
	{
		...
		value: lgProfileData.noOfHouseholds.femaleHeadedHouseholds
			? lgProfileData.noOfHouseholds.femaleHeadedHouseholds
			: 0,
		percentage: lgProfileData.noOfHouseholds.femaleHeadedHouseholds
			? (lgProfileData.noOfHouseholds.femaleHeadedHouseholds / summationLGProfileHouseHold) * 100
			: 0,
	},
	{
		key: "householdWithMemberAged60",
		label: language === "en" ? "Household With Member Aged 60" : "६० वर्ष पुगेका सदस्य भएको परिवार",
		value: lgProfileData.noOfHouseholds.householdWithMemberAged60
			? lgProfileData.noOfHouseholds.householdWithMemberAged60
			: 0,
		percentage: lgProfileData.noOfHouseholds.householdWithMemberAged60
			? (lgProfileData.noOfHouseholds.householdWithMemberAged60 / summationLGProfileHouseHold) * 100
			: 0,
	},
	{
		...
		label:
			language === "en"
				? "Number Of Households With Differently Able Individual"
				: "फरक रूपमा सक्षम व्यक्ति भएका परिवारहरूको संख्या",
		value: lgProfileData.noOfHouseholds.numberOfHouseholdsWithDifferentlyAbleIndividual
			? lgProfileData.noOfHouseholds.numberOfHouseholdsWithDifferentlyAbleIndividual
			: 0,
		percentage: lgProfileData.noOfHouseholds.numberOfHouseholdsWithDifferentlyAbleIndividual
			? (lgProfileData.noOfHouseholds.numberOfHouseholdsWithDifferentlyAbleIndividual /
...
export const LGProfileDisabilityData = (lgProfileData, summationLGProfileDisability, language) => [
	{
		...
		value: lgProfileData.disability.physicalDisabilities
			? lgProfileData.disability.physicalDisabilities
			: 0,
		percentage: lgProfileData.disability.physicalDisabilities
			? (lgProfileData.disability.physicalDisabilities / summationLGProfileDisability) * 100
			: 0,
	},
	{
		key: "visualDisabilities",
		label: language === "en" ? "Visual Disabilities" : "भिजुअल अपाङ्गता",
		value: lgProfileData.disability.visualDisabilities
			? lgProfileData.disability.visualDisabilities
			: 0,
		percentage: lgProfileData.disability.visualDisabilities
			? (lgProfileData.disability.visualDisabilities / summationLGProfileDisability) * 100
			: 0,
	},
	{
		key: "hearingDisabilities",
		label: language === "en" ? "Hearing Disabilities" : "सुन्‍ने अपाङ्गता",
		value: lgProfileData.disability.hearingDisabilities
			? lgProfileData.disability.hearingDisabilities
			: 0,
		percentage: lgProfileData.disability.hearingDisabilities
			? (lgProfileData.disability.hearingDisabilities / summationLGProfileDisability) * 100
			: 0,
	},
	{
		key: "hearingVisualDisabilities",
		label: language === "en" ? "Hearing Visual Disabilities" : "सुन्‍ने भिजुअल अपाङ्गता",
		value: lgProfileData.disability.hearingVisualDisabilities
			? lgProfileData.disability.hearingVisualDisabilities
			: 0,
		percentage: lgProfileData.disability.hearingVisualDisabilities
			? (lgProfileData.disability.hearingVisualDisabilities / summationLGProfileDisability) * 100
			: 0,
	},
	{
		key: "speakingDisabilities",
		label: language === "en" ? "Speaking Disabilities" : "बोल्ने अपाङ्गता",
		value: lgProfileData.disability.speakingDisabilities
			? lgProfileData.disability.speakingDisabilities
			: 0,
		percentage: lgProfileData.disability.speakingDisabilities
			? (lgProfileData.disability.speakingDisabilities / summationLGProfileDisability) * 100
			: 0,
	},
	{
		key: "mentalDisabilities",
		label: language === "en" ? "Mental Disabilities" : "मानसिक अपाङ्गता",
		value: lgProfileData.disability.mentalDisabilities
			? lgProfileData.disability.mentalDisabilities
			: 0,
		percentage: lgProfileData.disability.mentalDisabilities
			? (lgProfileData.disability.mentalDisabilities / summationLGProfileDisability) * 100
			: 0,
	},
	{
		key: "intellectualDisabilities",
		label: language === "en" ? "Intellectual Disabilities" : "बौद्धिक अपाङ्गता",
		value: lgProfileData.disability.intellectualDisabilities
			? lgProfileData.disability.intellectualDisabilities
			: 0,
		percentage: lgProfileData.disability.intellectualDisabilities
			? (lgProfileData.disability.intellectualDisabilities / summationLGProfileDisability) * 100
			: 0,
	},
	{
		key: "multipleDisabilities",
		label: language === "en" ? "Multiple Disabilities" : "बहु अपाङ्गता",
		value: lgProfileData.disability.multipleDisabilities
			? lgProfileData.disability.multipleDisabilities
			: 0,
		percentage: lgProfileData.disability.multipleDisabilities
			? (lgProfileData.disability.multipleDisabilities / summationLGProfileDisability) * 100
			: 0,
	},
];
export const LGProfileSocialSecurityData = (
	lgProfileData,
...
) => [
	{
		...
		value: lgProfileData.socialSecurityBenefitAvailed.elderCitizen
			? lgProfileData.socialSecurityBenefitAvailed.elderCitizen
			: 0,
		percentage: lgProfileData.socialSecurityBenefitAvailed.elderCitizen
			? (lgProfileData.socialSecurityBenefitAvailed.elderCitizen /
					summationLGProfileSocialSecurity) *
			  100
			: 0,
	},
	{
		...
		value: lgProfileData.socialSecurityBenefitAvailed.singleWomen
			? lgProfileData.socialSecurityBenefitAvailed.singleWomen
			: 0,
		percentage: lgProfileData.socialSecurityBenefitAvailed.singleWomen
			? (lgProfileData.socialSecurityBenefitAvailed.singleWomen /
					summationLGProfileSocialSecurity) *
			  100
			: 0,
	},
	{
		...
		value: lgProfileData.socialSecurityBenefitAvailed.differentlyAble
			? lgProfileData.socialSecurityBenefitAvailed.differentlyAble
			: 0,
		percentage: lgProfileData.socialSecurityBenefitAvailed.differentlyAble
			? (lgProfileData.socialSecurityBenefitAvailed.differentlyAble /
					summationLGProfileSocialSecurity) *
			  100
			: 0,
	},
	{
		...
		value: lgProfileData.socialSecurityBenefitAvailed.extinctCaste
			? lgProfileData.socialSecurityBenefitAvailed.extinctCaste
			: 0,
		percentage: lgProfileData.socialSecurityBenefitAvailed.extinctCaste
			? (lgProfileData.socialSecurityBenefitAvailed.extinctCaste /
					summationLGProfileSocialSecurity) *
			  100
			: 0,
	},
	{
		...
		value: lgProfileData.socialSecurityBenefitAvailed.childSecurity
			? lgProfileData.socialSecurityBenefitAvailed.childSecurity
			: 0,
		percentage: lgProfileData.socialSecurityBenefitAvailed.childSecurity
			? (lgProfileData.socialSecurityBenefitAvailed.childSecurity /
					summationLGProfileSocialSecurity) *
			  100
			: 0,
	},
	{
		...
		value: lgProfileData.socialSecurityBenefitAvailed.pension
			? lgProfileData.socialSecurityBenefitAvailed.pension
			: 0,
		percentage: lgProfileData.socialSecurityBenefitAvailed.pension
			? (lgProfileData.socialSecurityBenefitAvailed.pension / summationLGProfileSocialSecurity) *
			  100
			: 0,
	},
	{
		key: "others",
		label: language === "en" ? "Others" : "अन्‍य",
		value: lgProfileData.socialSecurityBenefitAvailed.others
			? lgProfileData.socialSecurityBenefitAvailed.others
			: 0,
		percentage: lgProfileData.socialSecurityBenefitAvailed.others
			? (lgProfileData.socialSecurityBenefitAvailed.others / summationLGProfileSocialSecurity) * 100
			: 0,
	},
	{
		key: "notAvailed",
		label: language === "en" ? "Not Availed" : "उपलब्ध छैन",
		value: lgProfileData.socialSecurityBenefitAvailed.notAvailed
			? lgProfileData.socialSecurityBenefitAvailed.notAvailed
			: 0,
		percentage: lgProfileData.socialSecurityBenefitAvailed.notAvailed
			? (lgProfileData.socialSecurityBenefitAvailed.notAvailed / summationLGProfileSocialSecurity) *
			  100
			: 0,
	},
	{
		...
		value: lgProfileData.socialSecurityBenefitAvailed.notApplicable
			? lgProfileData.socialSecurityBenefitAvailed.notApplicable
			: 0,
		percentage: lgProfileData.socialSecurityBenefitAvailed.notApplicable
			? (lgProfileData.socialSecurityBenefitAvailed.notApplicable /
					summationLGProfileSocialSecurity) *
			  100
			: 0,
...
export const LGProfileMigrationData = (lgProfileData, summationLGProfileMigration, language) => [
	{
		...
		percentage: lgProfileData.migration.mostlyPresent
			? (lgProfileData.migration.mostlyPresent / summationLGProfileMigration) * 100
			: 0,
	},
	{
		key: "notPresentInsideNepal",
		label: language === "en" ? "Not present,inside Nepal" : "नेपाल भित्र, उपस्‍थित छेैन ",
		value: lgProfileData.migration.notPresentInsideNepal
			? lgProfileData.migration.notPresentInsideNepal
			: 0,
		percentage: lgProfileData.migration.notPresentInsideNepal
			? (lgProfileData.migration.notPresentInsideNepal / summationLGProfileMigration) * 100
			: 0,
	},
	{
		key: "notPresentOutsideNepal",
		label: language === "en" ? "Not Present,Outside Nepal" : "नेपाल बाहिर,उपस्‍थित छेैन ",
		value: lgProfileData.migration.notPresentOutsideNepal
			? lgProfileData.migration.notPresentOutsideNepal
			: 0,
		percentage: lgProfileData.migration.notPresentOutsideNepal
			? (lgProfileData.migration.notPresentOutsideNepal / summationLGProfileMigration) * 100
			: 0,
	},
	{
		key: "notKnown",
		...
		percentage: lgProfileData.migration.notKnown
			? (lgProfileData.migration.notKnown / summationLGProfileMigration) * 100
			: 0,
	},
];

export const LGProfileEducationLevelData = (
	lgProfileData,
	SummationLGProfileEducationLevel,
	language
) => [
	{
		key: "primary",
		...
		percentage: lgProfileData.educationLevel.primary
			? (lgProfileData.educationLevel.primary / SummationLGProfileEducationLevel) * 100
			: 0,
	},
	{
		key: "lowerSecondary",
		label: language === "en" ? "Lower Secondary" : "निम्न माध्यमिक",
		value: lgProfileData.educationLevel.lowerSecondary
			? lgProfileData.educationLevel.lowerSecondary
			: 0,
		percentage: lgProfileData.educationLevel.lowerSecondary
			? (lgProfileData.educationLevel.lowerSecondary / SummationLGProfileEducationLevel) * 100
			: 0,
	},
	{
		key: "slcOrEquivalent",
		label: language === "en" ? "SLC or equivalent" : "एसएलसी वा सो सरह",
		value: lgProfileData.educationLevel.slcOrEquivalent
			? lgProfileData.educationLevel.slcOrEquivalent
			: 0,
		percentage: lgProfileData.educationLevel.slcOrEquivalent
			? (lgProfileData.educationLevel.slcOrEquivalent / SummationLGProfileEducationLevel) * 100
			: 0,
	},
	{
		key: "intermediateOrEquivalent",
		label: language === "en" ? "Intermediate or equivalent" : "मध्यामिक वा सो सरह",
		value: lgProfileData.educationLevel.intermediateOrEquivalent
			? lgProfileData.educationLevel.intermediateOrEquivalent
			: 0,
		percentage: lgProfileData.educationLevel.intermediateOrEquivalent
			? (lgProfileData.educationLevel.intermediateOrEquivalent / SummationLGProfileEducationLevel) *
			  100
			: 0,
	},
	{
		...
		value: lgProfileData.educationLevel.bachelorOrEquivalent
			? lgProfileData.educationLevel.bachelorOrEquivalent
			: 0,
		percentage: lgProfileData.educationLevel.bachelorOrEquivalent
			? (lgProfileData.educationLevel.bachelorOrEquivalent / SummationLGProfileEducationLevel) * 100
			: 0,
	},
	{
		key: "mastersPhd",
		...
		percentage: lgProfileData.educationLevel.mastersPhd
			? (lgProfileData.educationLevel.mastersPhd / SummationLGProfileEducationLevel) * 100
			: 0,
	},
	{
		key: "noEducation",
		...
		percentage: lgProfileData.educationLevel.noEducation
			? (lgProfileData.educationLevel.noEducation / SummationLGProfileEducationLevel) * 100
			: 0,
	},
	{
		key: "otherNonformal",
		label: language === "en" ? "Other/Non-formal" : "अन्य/अनौपचारिक",
		value: lgProfileData.educationLevel.otherNonformal
			? lgProfileData.educationLevel.otherNonformal
			: 0,
		percentage: lgProfileData.educationLevel.otherNonformal
			? (lgProfileData.educationLevel.otherNonformal / SummationLGProfileEducationLevel) * 100
			: 0,
	},
	{
		key: "notEligible",
		label:
			language === "en"
				...
				: "योग्य छैन (५ वर्ष भन्दा कम उमेर)",
		value: lgProfileData.educationLevel.notEligible ? lgProfileData.educationLevel.notEligible : 0,
		percentage: lgProfileData.educationLevel.notEligible
			? (lgProfileData.educationLevel.notEligible / SummationLGProfileEducationLevel) * 100
			: 0,
	},
	{
		key: "unknown",
		...
		percentage: lgProfileData.educationLevel.unknown
			? (lgProfileData.educationLevel.unknown / SummationLGProfileEducationLevel) * 100
			: 0,
	},
];
...
export const generatePaintByQuantile = (
	...
	nonEmptyData.map((d) =>
		intervals.push(Math.max(...d) === 0 ? Math.max(...d) + 1 : Math.max(...d))
	);
	...
	if (colors.length !== 0) {
		const fillColor =
			adminLevel === 3
				? ["step", ["feature-state", "value"], ...colors, colorDomain[0]]
				: ["step", ["feature-state", "value"], ...colors.slice(0, -1)];
