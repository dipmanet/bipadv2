...
const NepDataProfile = (props) => {
	...
	const province = provinces.map((data) => ({
		...data,
		label: language === "en" ? data.title_en : data.title_ne,
		value: data.id,
	}));
	const district =
		selectedProvince &&
		districts
			...
			.map((dist) => ({
				...dist,
				label: language === "en" ? dist.title_en : dist.title_ne,
				value: dist.id,
			}));
	const municipality =
		selectedDistrict &&
		municipalities
			...
			.map((mun) => ({
				...mun,
				label: language === "en" ? mun.title_en : mun.title_ne,
				value: mun.id,
			}));
				...
				<div className={_cs(className, styles.documents)}>
					...
					<div className={styles.dropdowns}>
						...
						<Select
							...
							options={reportType}
							placeholder={
								language === "en" ? "Select Report Type" : "प्रतिवेदनको प्रकार चयन गर्नुहोस्"
							}
						/>
					</div>
					<div className={styles.dropdowns}>
