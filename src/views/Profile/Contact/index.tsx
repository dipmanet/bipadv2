...
import { convertTableToCsv, prepareColumns, defaultState } from "#utils/table";
...
import { Contact, Municipality, Region } from "#store/atom/page/types";
import {
...
class ContactPage extends React.PureComponent<Props, State> {
	...
	public constructor(props: Props) {
		...
		const {
			requests: { municipalityContactRequest },
		} = this.props;
		...
		if (!contactList.length) {
			...
			if (user) {
				const {
					user: {
						profile: { province, district, municipality },
					},
					region,
				} = this.props;
	...
	private getPositionOptions = memoize((contactList: Contact[]) => {
		const contactPositionList = [...new Set(contactList.map((d) => d.position).filter((d) => !!d))];
	...
	private getPointsGeoJson = memoize((contactList: Contact[], municipalityList: Municipality[]) => {
		...
		const pointList = contactList
			...
			.map((d) => d.point || municipalityBounds[d.municipality]);
	...
	private getFilteredContactList = memoize(
		(contactList: Contact[], region, municipalityList: Municipality[], filterOptions) => {
			const { committee, training, position, drrFocalPersonOnly } = filterOptions;
	...
	private handleContactSortUp = (contactDetail) => {
		...
		const {
			...
			municipalityList,
			className,
			filters: { faramValues: filterValues },
			requests: {
				municipalityContactPatchSelectedID,
				municipalityContactPatchAlternateID,

				municipalityContactRequest: { pending = false } = {},
			},
		} = this.props;
		...
		const indexOfSelectedContact = filteredContactList.findIndex(
			(item) => item.id === contactDetail.id
		);

		const moveElement = (filteredList, indexInitial, indexDestination) => {
			filteredList.splice(indexDestination, 0, filteredList.splice(indexInitial, 1)[0]);

			return filteredList;
		};
		filteredContactList = moveElement(
			filteredContactList,
			indexOfSelectedContact,
			indexOfSelectedContact - 1
		);
	...
	private AlertTableEmptyComponent = () => (
		<Message>
			<div className={styles.alertEmpty}>There are no alerts at the moment.</div>
		</Message>
	);
	...
	private handleSearch = (value) => {
		...
		if (value === "" || value === null) {
			...
			const filteredData = filteredContactList.filter(
				(item, i) => item.position.toLowerCase().indexOf(value.toLowerCase()) !== -1
			);
	...
	private editDeleteButton = (contactId, contact) => {
		const {
			user,
			region,
			requests: { municipalityContactDeleteRequest },
			language: { language },
		} = this.props;
		...
		return filterPermissionGranted ? (
			<Translation>
				{(t) => (
					<>
						...
						<table className={styles.contacts}>
							...
							<tbody>
								{searchActivated ? (
									...
									filteredContactList.map((item, i) => (
										<tr key={item.id}>
											<td>{i + 1}</td>
											<td>{item.name}</td>
											<td>{item.position}</td>
							...
							{searchActivated ? (
								!sortData.length ? (
									...
									sortData.map((item, i) => (
										<div
											key={item.id}
											className={
												item.id === selectedContactFromLabelListId
													? _cs(styles.column, styles.active)
													: styles.column
											}
											role="button"
											tabIndex={0}
											...
											<div className={styles.contactImageProfile}>
												...
												<div className={styles.contactNameDetails}>
													...
													<span>
														{item.position},{item.committee}
													</span>
												</div>
											</div>
											<div className={styles.contactPhoneDetails}>
												<span style={{ color: "black" }}>{item.mobileNumber || "-"}</span>
												<ScalableVectorGraphcis className={styles.icon} src={phoneContactLogo} />
											</div>
											<div className={styles.contactEmailDetails}>
												<span
													style={{ color: "black", wordBreak: "break-word", marginRight: "10px" }}>
													{item.email || "-"}
												</span>
												<ScalableVectorGraphcis className={styles.icon} src={emailContactLogo} />
											</div>
											<div
												className={styles.contactDetailsFooter}
												style={item.trainings.length ? {} : { display: "flex" }}>
												<div
													style={
													...
													<p>
														{item.trainings.length
															? item.trainings.map((data, index) => (
																	<>
																		{item.trainings.length - 1 === index &&
																		item.trainings.length !== 1
																			? " & "
																			: ""}
																		{this.trainingTitleName(data.title)}

																		{item.trainings.length - 1 === index ||
																		item.trainings.length - 2 === index
																			? ""
																			: ","}
																	</>
															  ))
								...
								filteredContactList.map((item, i) => (
									<div
										...
										onKeyDown={undefined}>
										<div className={styles.contactImageProfile}>
											<img className={styles.image} src={item.image} alt="profile" loading="lazy" />
											<div className={styles.contactNameDetails}>
												<span style={{ fontWeight: "bold" }}>{item.name.toUpperCase()}</span>
												<span>
													{item.position},{item.committee}
	...
	public render() {
		...
		if (isSortByOrdering) {
			...
			filteredContactList = filteredContactListWithoutArrayIndex.map((item, i) => ({
				...item,
				indexValue: i,
			}));
								...
								<div style={{ marginLeft: "10px", marginRight: "10px", display: "flex" }}>
									...
									<Cloak hiddenIf={(p) => !p.add_contact}>
										<AccentModalButton
											...
											iconName="add"
											transparent
											modal={<ContactEditForm onAddSuccess={this.handleContactAdd} />}>
											{t("Add Contact")}
										</AccentModalButton>
									</Cloak>
								</div>
...
export default connect(mapStateToProps)(
	createConnectedRequestCoordinator<ReduxProps>()(createRequestClient(requests)(ContactPage))
);
