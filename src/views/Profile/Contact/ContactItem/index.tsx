...
import { Contact, Municipality } from "#store/atom/page/types";
import {
...
import { trainingValues, committeeValues } from "../utils";
...
const Detail = (p: { label: string; value: string; className?: string }) => {
	const { label, value, className } = p;
...
const IconDetail = (p: { value: string; iconName: string; className?: string }) => {
	const { value, iconName, className } = p;
...
const ContactItem = (props: Props) => {
	...
	const trainingValueString = useMemo(
		() => trainings.map((d) => trainingValues[d.title]).join(", ") || "-",
		[trainings]
	);
	...
	return (
		<div className={_cs(styles.contactDetails, isDrrFocalPerson && styles.focalPerson)}>
...
export default connect(mapStateToProps)(
	createConnectedRequestCoordinator<Props>()(createRequestClient(requests)(ContactItem))
);
