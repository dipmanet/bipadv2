...
import { regionSelector, regionNameSelector, languageSelector, userSelector } from "#selectors";
...
const categoryLabelSelector = (d: DocumentCategory, language) =>
	language === "en" ? d.title : d.titleNe;
const regionKeySelector = (d: RegionOption) => d.id;
const regionLabelSelector = (d: RegionOption, language) =>
	language === "en" ? d.title : d.titleNe;
...
class Document extends React.PureComponent<Props, State> {
	...
	private handleSearch = (value) => {
		...
		if (value === "" || value === null) {
			...
			const filteredData = expandedDocuments.filter(
				(item, i) => item.title.toLowerCase().indexOf(value.toLowerCase()) !== -1
			);
	...
	private listComponent = () => {
		const {
			...
			searchKeyword,
			searchActivated,
			selectedContactFromLabelListId,
		} = this.state;
		const {
			user,
			region,
			requests: {
				documentDeleteRequest: { pending },
			},
			language: { language },
		} = this.props;
		...
		const dataList =
			...
			splittedData.map((item) => {
				const individualItem = item.map((d) => {
					...
					const listViewDate = d.publishedDate
						? convertDateAccToLanguage(d.publishedDate, language)
						: "Unavailable";
					return {
						...d,
						listViewDate,
			...
			splittedData.map((item, i) => (
				<Translation>
					{(t) => (
						<div
							key={i}
							style={{ display: "flex", width: "100%", height: "220px", marginBottom: "40px" }}>
							<div
								className={
									i > mainColumnIndex
										? clickedColumn === "col1"
											? _cs(styles.extraMargin)
											: styles.normalMargin
										: styles.normalMargin
								}
								style={{ width: "33.33%" }}>
								{item.map((data, idx) => (
									<div key={idx}>
										{idx === 0 ? (
											<div
												className={
													data.id === selectedContactFromLabelListId
														? _cs(styles.active, styles.col1Div)
														: styles.col1Div
												}>
												<div
													role="button"
													...
													<div
														...
														<div
															...
															<div style={{ display: "flex" }}>
																...
																<div style={{ marginLeft: "2px" }}> {data.listViewDate}</div>
															</div>
														</div>
														<div
															style={{
															...
															<ScalableVectorGraphics className={styles.iconTitle} src={bookIcon} />
														</div>
													</div>
													<div
														className="col1"
														style={{ borderTop: "1px solid #e1e1e1", padding: "10px" }}>
														<div style={{ display: "flex" }}>
															<div style={{ fontWeight: "bold" }}>{t("Region")}:</div>
															<div style={{ marginLeft: "2px" }}> {data.region}</div>
														</div>
														<div style={{ display: "flex" }}>
															<div style={{ fontWeight: "bold" }}>{t("Published By")}:</div>
												...
												<div className={styles.footer}>
													<div
														...
														<div
															...
															style={this.colourForCategory(data.categoryName)}>
															{data.categoryName}
															<div className={styles.tableTooltip}>{data.categoryName}</div>
													...
													<div>
														<a
															...
															title={t("Download")}>
															<ScalableVectorGraphics className={styles.icon} src={downloadCloud} />
														</a>
									...
									<div key={idx}>
										{idx === 1 ? (
											<div
												className={
													data.id === selectedContactFromLabelListId
														? _cs(styles.active, styles.col2Div)
														: styles.col2Div
												}>
												<div
													role="button"
									...
									<div key={idx}>
										{idx === 2 ? (
											<div
												className={
													data.id === selectedContactFromLabelListId
														? _cs(styles.active, styles.col3Div)
														: styles.col3Div
												}>
												<div
													role="button"
												...
												<td>
													<div
														...
														<a
															...
															<ScalableVectorGraphics className={styles.icon} src={DownloadIcon} />
														</a>
														{filterPermissionGranted ? (
															<>
																{" "}
	...
	public render() {
		...
		expandedDocuments = this.getCategoryExpandedDocuments(filteredDocuments, documentCategories);
...
export default connect(mapStateToProps)(createRequestClient(requestOptions)(Document));
