...
import { setEventListAction, setDocumentCategoryListAction } from "#actionCreators";
...
const labelSelector = (d: PageType.Field, language: string) =>
	language === "en" ? d.title : d.titleNe === undefined ? d.title : d.titleNe;
...
const requests: { [key: string]: ClientAttributes<ReduxProps, Params> } = {
	...
	documentCategoriesGetRequest: {
		...
		onSuccess: ({ response, props: { setDocumentCategoryList } }) => {
			interface Response {
				results: PageType.DocumentCategory[];
			}
	...
	addDocumentPostRequest: {
		url: ({ props }) => (props.value ? `/document/${props.value.id}/` : "/document/"),
...
class AddDocumentForm extends React.PureComponent<Props, State> {
	public constructor(props: Props) {
		...
		const { file, province, district, municipality, ...otherValues } = props.value || {};
		...
		this.schema = {
			...
			validation: (faramValues: FaramValues) => {
				...
				if (isDefined(region) && isDefined(stepwiseRegion)) {
					...
					const currentRegion = regionList.find((item) => item.id === stepwiseRegion.adminLevel);
	...
	private handleFaramValidationSuccess = (_: unknown, faramValues: FaramValues) => {
		...
		const { publishedDate, stepwiseRegion, ...others } = faramValues;
	...
	public render() {
		...
		return (
			<Translation>
				{(t) => (
					<Modal className={_cs(styles.addDocumentModal, className)}>
						...
						<Faram
							...
							disabled={pending}>
							<ModalHeader
								title={value ? t("Edit Document") : t("Add Document")}
								rightComponent={
									<DangerButton
										transparent
										iconName="close"
							...
							<ModalBody>
								...
								/>
								<TextInput faramElementName="publishedBy" label={t("Published By")} />
								...
								<SelectInput
									...
									placeholder={language === "en" ? "Select an option" : "विकल्प चयन गर्नुहोस्"}
								/>
								<StepwiseRegionSelectInput faramElementName="stepwiseRegion" wardsHidden />
								...
								{!value && (
									...
									>
										<Icon name="upload" />
										<span className={styles.load}>{t("Add document")}</span>
									</RawFileInput>
								)}
							</ModalBody>
							<ModalFooter>
