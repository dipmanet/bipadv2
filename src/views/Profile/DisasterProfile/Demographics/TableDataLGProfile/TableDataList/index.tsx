			...
			<div
				...
				<table className={_cs(styles.contacts, language === "np" && styles.languageFont)}>
					<thead>
						<tr>
							{selectedTableHeader &&
								selectedTableHeader.map((item) =>
									item.subCategory.length ? (
										<th
											key={item.category}
											style={{ textAlign: "center" }}
											colSpan={item.subCategory.length}
											scope="colgroup">
						...
						<tr>
							{language === "en"
								? selectedTableHeader &&
								  selectedTableHeader.map((data) =>
										data.subCategory
											? data.subCategory.map((i) => (
													<th key={i} scope="col">
														{i}
													</th>
											  ))
						...
						{lgProfileWardLevelData.map((item) => (
							<tr key={item.ward}>
								<td style={{ position: "sticky", left: "0", background: "white" }}>
									{language === "en" ? "Ward" : "वार्ड"} {item.ward}
								</td>
								{selectedTableHeader.map((i) =>
									i.key.length
										? i.key.map((keys) => (
												<td key={keys}>
													{item[i.keyHeader][keys] ? item[i.keyHeader][keys] : "-"}
												</td>
										  ))
										: ""
								)}
