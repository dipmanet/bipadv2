		...
		<div style={{ display: "flex", justifyContent: "center", marginLeft: "50px" }}>
			{payload.map((entry, index) => (
				<div
					...
					/>
					<Translation>
						{(t) => <h2>{t(entry.value.charAt(0).toUpperCase() + entry.value.slice(1))}</h2>}
					</Translation>
				</div>
			))}
		</div>
...
const BarchartVisualization = ({ item, category, percentage, language }) => {
	...
	useEffect(() => {
		if (category) {
			const finalData = item.map((d) => ({ ...d, female: d.femaleForPyramidGraphVisualization }));
		...
		!category ? (
			<ResponsiveContainer height={item.length * 85} width={"100%"}>
				...
				>
					...
					<XAxis dataKey="percentage" type="number" tick={<CustomizedAxisTick />} />
					...
					<Bar
						...
						label={
							percentage ? (
								<CustomizedLabelLiteracyRate language={language} />
							) : (
								<CustomizedLabel language={language} />
							)
						}>
						{data.map((hazard) => (
			...
			<ResponsiveContainer height={510} width={"100%"}>
				<BarChart
					...
					<YAxis width={64} dataKey="label" type="category" />
