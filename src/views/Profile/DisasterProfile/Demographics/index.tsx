...
import { Region, Municipality } from "#store/atom/page/types";
...
const colorGrade = ["#E5EFFA", "#CBE0F6", "#B1D1F2", "#97C1EE", "#7DB2E9", "#63A3E5", "#4993E1"];
			...
			<Translation>
				{(t) => (
					<div
						...
						<p>
							{t("Value")} : {NumberWithCommas(payload[0].value)}
						</p>
						{payload[0].payload.percent ? (
							<p>
								{t("Percentage")} : {payload[0].payload.percent.toFixed(2)}%
							</p>
						) : (
							""
						)}
					...
					{payload.map((entry: { color: any; value: string }, index: any) => (
						<div
							...
							<h2 style={{ marginTop: "5px" }}>
								{t(entry.value.charAt(0).toUpperCase() + entry.value.slice(1))}
							</h2>
						</div>
					))}
				</div>
		...
		<div style={{ display: "flex", justifyContent: "center", marginLeft: "50px" }}>
			{payload.map((entry: { color: any }, index: any) => (
				<div
					...
					style={{ display: "flex", alignItems: "center", margin: "10px" }}>
					<div style={{ height: "15px", width: "15px", backgroundColor: `${entry.color}` }} />
					<Translation>{(t) => <h2 style={{ marginTop: "5px" }}>{t("Age Group")}</h2>}</Translation>
				</div>
			))}
		</div>
	);
...
class Demographics extends React.PureComponent<Props> {
	public constructor(props: Props | Readonly<Props>) {
		...
		const {
			requests: { demographicsGetRequest },
	...
	public componentDidUpdate(prevProps: { closedVisualization: any }) {
		...
		if (adminLevel === 3) {
			...
			const selectedFederal = districts.find(
				(d: { id: number | undefined }) => d.id === geoarea
			).title_en;
			this.setState({ selectedFederalName: selectedFederal });
		} else if (adminLevel === 1) {
			const selectedFederal = provinces.find(
				(d: { id: number | undefined }) => d.id === geoarea
			).title_en;
			this.setState({ selectedFederalName: selectedFederal });
		} else {
	...
	private getPopulationData_municipality = (data: DemographicsData[], region: Region) => {
		...
		if (!doesObjectHaveNoData(region)) {
			...
			if (adminLevel === 1) {
				...
				selectedMunicipalities = municipalities.filter((v) => v.id === geoarea).map((v) => v.id);
			}
			filteredData = data
				.map((i) => ({
					...i,
		...
		const demographics = filteredData.reduce(
			(acc, value, i) => {
				const {
	...
	private getPopulationData_district = (data: DemographicsData[], region: Region) => {
		...
		if (!doesObjectHaveNoData(region)) {
			...
			let selectedMunicipalities: number[] = [];
			if (adminLevel === 1) {
				selectedMunicipalities = districts.filter((v) => v.province === geoarea).map((v) => v.id);
			} else if (adminLevel === 2) {
				selectedMunicipalities = municipalities
					.filter((v) => v.district === geoarea)
					.map((v) => v.id);
			...
			filteredData = data
				...
				.filter((d) =>
					selectedMunicipalities.includes(adminLevel === 1 ? d.district : d.municipality)
				);
		}
		...
		demographics.changed_literacyRate /= filteredData.length;
		demographics.femaleLiteracyRate =
			(demographics.temp_total_femaleLiterate / demographics.temp_total_femaleAboveFiveYears) * 100;
		demographics.maleLiteracyRate =
			(demographics.temp_total_maleLiterate / demographics.temp_total_maleAboveFiveYears) * 100;
		demographics.literacyRate =
			(demographics.temp_total_totalLiterate / demographics.temp_total_totalAboveFiveYears) * 100;
	...
	private getPopulationSummary = (data: SummaryData) => {
		const {
			...
			malePopulation,
			femalePopulation,
			changed_malePopulation,
			changed_femalePopulation,
		} = data;
		const {
			language: { language },
		} = this.props;
		const changed_total_population =
			totalPopulation - (changed_malePopulation + changed_femalePopulation);
		return [
			{
	...
	private getAgeGroupSummary = (data: SummaryData) => {
		const {
			ageGroupPopulation: { male, female },
		} = data;
		...
		return keys.map((v) => ({
			...
			malePercentage: Number(
				(
					(male[v as keyof AgeGroup] / (male[v as keyof AgeGroup] + female[v as keyof AgeGroup])) *
					100
				).toFixed(2)
			),
			femalePercentage: -Number(
				(
					(female[v as keyof AgeGroup] /
						(male[v as keyof AgeGroup] + female[v as keyof AgeGroup])) *
					100
				).toFixed(2)
			),
		}));
	...
	private getMapState = (data: any[], selectedAttribute: string | number) => {
		...
		if (selectedDataType === 2) {
			if (adminLevel === 3) {
				const filteredWardList = wards.filter(
					(i: { municipality: number | undefined }) => i.municipality === geoarea
				);
				const value = data.find(
					(d: { municipality: number | undefined }) => d.municipality === geoarea
				);
			...
			if (adminLevel === 1) {
				...
				const districtWiseMuniList = selectedProvinceMunicipalities.map((mun: any[]) => {
					const finaldata = mun.map((dat: { id: any; district: any }) => {
						const datas = data.filter(
							(itm: { municipality: any }) => itm.municipality === dat.id
						)[0];
				...
				const finalsummationData = districtWiseMuniList.map((mun: any[]) => {
					const femaleLiteracyRate = (
						mun.reduce(
							(acc: any, currValue: { femaleLiteracyRate: any }) =>
								acc + currValue.femaleLiteracyRate ? currValue.femaleLiteracyRate : 0,
							0
						) / mun.length
					).toFixed(2);
					const femalePopulation = mun.reduce(
						(acc: any, currValue: { femalePopulation: any }) =>
							acc + currValue.femalePopulation ? currValue.femalePopulation : 0,
						0
					);
					const householdCount = mun.reduce(
						(acc: any, currValue: { householdCount: any }) =>
							acc + currValue.householdCount ? currValue.householdCount : 0,
						0
					);
					const literacyRate = (
						mun.reduce(
							(acc: any, currValue: { literacyRate: any }) =>
								acc + currValue.literacyRate ? currValue.literacyRate : 0,
							0
						) / mun.length
					).toFixed(2);
					const maleLiteracyRate = (
						mun.reduce(
							(acc: any, currValue: { maleLiteracyRate: any }) =>
								acc + currValue.maleLiteracyRate ? currValue.maleLiteracyRate : 0,
							0
						) / mun.length
					).toFixed(2);
					const malePopulation = mun.reduce(
						(acc: any, currValue: { malePopulation: any }) =>
							acc + currValue.malePopulation ? currValue.malePopulation : 0,
						0
					);
					const totalPopulation = mun.reduce(
						(acc: any, currValue: { totalPopulation: any }) =>
							acc + currValue.totalPopulation ? currValue.totalPopulation : 0,
						0
					);
					const district = mun.reduce(
						(acc: any, currValue: { district: any }) => currValue.district,
						0
					);
					return {
						femaleLiteracyRate,
				...
				const mapState = finalsummationData.map(
					(d: { [x: string]: string | number; district: any }) => ({
						id: d.district,
						value: +d[selectedAttribute] || 0,
					})
				);
			...
			const filtered_undefined_array = provinceWiseMuniList.map((d) =>
				d.filter((x) => x !== undefined)
			);
			...
			const finalsummationDataProvince = filtered_undefined_array.map((mun: any[]) => {
				const femaleLiteracyRate = (
					mun.reduce(
						(acc: any, currValue: { femaleLiteracyRate: any }) =>
							acc + currValue.femaleLiteracyRate,
						0
					) / mun.length
				).toFixed(2);
				const femalePopulation = mun.reduce(
					(acc: any, currValue: { femalePopulation: any }) => acc + currValue.femalePopulation,
					0
				);
				const householdCount = mun.reduce(
					(acc: any, currValue: { householdCount: any }) => acc + currValue.householdCount,
					0
				);
				const literacyRate = (
					mun.reduce(
						(acc: any, currValue: { literacyRate: any }) => acc + currValue.literacyRate,
						0
					) / mun.length
				).toFixed(2);
				const maleLiteracyRate = (
					mun.reduce(
						(acc: any, currValue: { maleLiteracyRate: any }) => acc + currValue.maleLiteracyRate,
						0
					) / mun.length
				).toFixed(2);
				const malePopulation = mun.reduce(
					(acc: any, currValue: { malePopulation: any }) => acc + currValue.malePopulation,
					0
				);
				const totalPopulation = mun.reduce(
					(acc: any, currValue: { totalPopulation: any }) => acc + currValue.totalPopulation,
					0
				);
				const province = mun.reduce(
					(acc: any, currValue: { province: any }) => currValue.province,
					0
				);
				return {
					femaleLiteracyRate,
			...
			const mapState = finalsummationDataProvince.map(
				(d: { [x: string]: string | number; district: any }) => ({
					id: d.province,
					value: +d[selectedAttribute] || 0,
				})
		...
		if (selectedDataType === 1) {
			...
			if (adminLevel === 1) {
				...
				const districtWiseMuniList = selectedProvinceMunicipalities.map((mun: any[]) => {
					const finaldata = mun.map((dat: { id: any; district: any }) => {
						const datas = data.filter(
							(itm: { municipality: any }) => itm.district === dat.district
						)[0];
				...
				const finalsummationData = districtWiseMuniList.map((mun: any[]) => {
					const femaleLiteracyRate = (
						mun.reduce(
							(acc: any, currValue: { femaleAboveFiveYears: any; femaleLiterate: any }) =>
								acc + currValue.femaleLiterate
									? currValue.femaleLiterate / currValue.femaleAboveFiveYears
									: 0,
							0
						) / mun.length
					).toFixed(2);
					...
					const totalLiterate = mun.reduce(
						(acc: any, currValue: { totalLiterate: any }) =>
							acc + currValue.totalLiterate ? currValue.totalLiterate : 0,
						0
					);
					const totalAboveFiveYears = mun.reduce(
						(acc: any, currValue: { totalAboveFiveYears: any }) =>
							acc + currValue.totalAboveFiveYears ? currValue.totalAboveFiveYears : 0,
						0
					);
			...
			const filtered_undefined_array = provinceWiseDistList.map((d) =>
				d.filter((x) => x !== undefined)
			);
			...
			const finalsummationDataProvince = filtered_undefined_array.map((mun: any[]) => {
				const femaleLiteracyRate = (
					mun.reduce(
						(acc: any, currValue: { femaleLiterate: any; femaleAboveFiveYears: any }) =>
							acc + currValue.femaleLiterate / currValue.femaleAboveFiveYears,
						0
					) / mun.length
				).toFixed(2);
				const femalePopulation = mun.reduce(
	...
	public render() {
		...
		const colors =
			attributes[selectedAttribute].type === "positive"
				? [...colorGrade].reverse()
				: [...colorGrade];
		...
		const demographics =
			selectedDataType === 1
				? this.getPopulationData_district(data, region)
				: this.getPopulationData_municipality(data, region);

		const populationSummary = this.getPopulationSummary(demographics);

		const sexRatio = populationSummary.filter((v) =>
			["malePopulation", "femalePopulation"].includes(v.key)
		);
		const sexRatioTotalPopulationLGProfile =
			lgProfileData.gender.male + lgProfileData.gender.female + lgProfileData.gender.other;
		const sexRatioLGProfile = sexRatioLGProfileData(
			lgProfileData,
			sexRatioTotalPopulationLGProfile,
			language
		);
		const literatePeopleTotalPopulationLGProfile =
			lgProfileData.literacyRate.male +
			lgProfileData.literacyRate.female +
			lgProfileData.literacyRate.other;
		const literacyRateLGProfile =
			(literatePeopleTotalPopulationLGProfile / sexRatioTotalPopulationLGProfile) * 100;
		const literacyRatioLGProfile = literacyRatioLGProfileData(
			lgProfileData,
			literatePeopleTotalPopulationLGProfile,
			language
		);
		const houseHoldSummaryLGProfile = houseHoldSummaryLGProfileData(
			lgProfileData,
			sexRatioTotalPopulationLGProfile,
			language
		);
		...
		const finalSexRatio = [
			{
				label: "genderRatio",
				male: sexRatio.find((d) => d.label === "Male" || d.label === "पुरुष").value,
				female: sexRatio.find((d) => d.label === "Female" || d.label === "महिला").value,
			},
		];
		const finalSexRatioPercentage = [
			{
				male: sexRatio.find((d) => d.label === "Male" || d.label === "पुरुष").percent,
				female: sexRatio.find((d) => d.label === "Female" || d.label === "महिला").percent,
			},
		];
		const literacySummary = this.getLiteracySummary(demographics);

		const literacyRatio = literacySummary.filter((v) =>
			["maleLiteracyRate", "femaleLiteracyRate"].includes(v.key)
		);
		const finalLiteracyRate = [
			{
				label: "literacyRate",
				male: literacyRatio.find((d) => d.label === "Male" || d.label === "पुरुष").value,
				female: literacyRatio.find((d) => d.label === "Female" || d.label === "महिला").value,
			},
		];

		const householdSummary = this.getHouseholdSummary(demographics);

		const finalHouseholdSummary = [
			{
				label: "houseHoldInfo",
				totalPopulation: householdSummary.householdSummary.find((d) => d.key === "totalPopulation")
					.value,
				householdCount: householdSummary.householdSummary.find((d) => d.key === "householdCount")
					.value,
			},
		];
			...
			<>
				{
					<h3
						...
						{`${
							adminLevel === 3
								? municipalityName(datas.feature.properties.municipality, language)
								: language === "en"
								? datas.feature.properties.title_en
								: datas.feature.properties.title_ne
						}`}
					</h3>
				}
		...
		const dateRangeOption =
			region && region.adminLevel === 3
				? pastDateRangeOptions(language)
				: pastDateRangeOptions(language).filter((i) => i.key !== 3);
		if (setProfile) {
			setProfile((prevProfile: Profile) => {
		...
		const majorOccupationList =
			houseHoldInformation &&
			houseHoldInformation.majorOccupations &&
			JSON.parse(houseHoldInformation.majorOccupations);
		const supportingOccupationList =
			houseHoldInformation &&
			houseHoldInformation.supportingOccupations &&
			JSON.parse(houseHoldInformation.supportingOccupations);
		const lgProfileAgeGroup = lgProfileData && lgProfileAgeGroupData(lgProfileData);

		const filteredLGProfileAgeGroup =
			lgProfileAgeGroup && lgProfileAgeGroup.filter((i) => i.value !== 0);
		const SummationLGProfileEducationLevel =
			lgProfileData && SummationLGProfileEducationLevelData(lgProfileData);

		const LGProfileEducationLevel =
			lgProfileData &&
			LGProfileEducationLevelData(lgProfileData, SummationLGProfileEducationLevel, language);
		...
		const LGProfileMigration = LGProfileMigrationData(
			lgProfileData,
			summationLGProfileMigration,
			language
		);
		...
		const LGProfileSocialSecurity = LGProfileSocialSecurityData(
			lgProfileData,
		...
		const LGProfileDisability = LGProfileDisabilityData(
			lgProfileData,
		...
		const LGProfileHouseHold = LGProfileHouseHoldData(
			lgProfileData,
		...
		const summationLGProfileAverageMonthlyIncome =
			summationLGProfileAverageMonthlyIncomeData(lgProfileData);

		const LGProfileAverageMonthlyIncome = LGProfileAverageMonthlyIncomeData(
			lgProfileData,
			summationLGProfileAverageMonthlyIncome,
			language
		);
		const filteredLGProfileAverageMonthlyIncome = LGProfileAverageMonthlyIncome.filter(
			(i) => i.value !== 0
		);

		const summationLGProfileMajorOccupation = summationLGProfileMajorOccupationData(lgProfileData);

		const LGProfileAverageMajorOccupation = LGProfileAverageMajorOccupationData(
			lgProfileData,
			summationLGProfileMajorOccupation,
			language
		);
		const filteredLGProfileAverageMajorOccupation = LGProfileAverageMajorOccupation.filter(
			(i) => i.value !== 0
		);
		const summationLGProfileDrinkingWater = summationLGProfileDrinkingWaterData(lgProfileData);

		const LGProfileDrinkingWater = LGProfileDrinkingWaterData(
			lgProfileData,
			summationLGProfileDrinkingWater,
			language
		);
		const filteredLGProfileDrinkingWater = LGProfileDrinkingWater.filter((i) => i.value !== 0);
		const summationLGProfileResidentHousehold =
			summationLGProfileResidentHouseholdData(lgProfileData);

		const LGProfileResidentHousehold = LGProfileResidentHouseholdData(
			lgProfileData,
			summationLGProfileResidentHousehold
		);
		const filteredLGProfileResidentHousehold = LGProfileResidentHousehold.filter(
			(i) => i.value !== 0
		);
		const summationLGProfileAgriculturePractice =
			summationLGProfileAgriculturePracticeData(lgProfileData);
		const LGProfileAgriculturePractice = LGProfileAgriculturePracticeData(
			lgProfileData,
			summationLGProfileAgriculturePractice,
			language
		);
		const filteredLGProfileAgriculturePractice = LGProfileAgriculturePractice.filter(
			(i) => i.value !== 0
		);
		const summationLGProfileAgricultureProduct =
			summationLGProfileAgricultureProductData(lgProfileData);
		const LGProfileAgricultureProduct = LGProfileAgricultureProductData(
			lgProfileData,
			summationLGProfileAgricultureProduct,
			language
		);
		const filteredLGProfileAgricultureProduct = LGProfileAgricultureProduct.filter(
			(i) => i.value !== 0
		);
		const summationLGProfileBuildingType = summationLGProfileBuildingTypeData(lgProfileData);
		const LGProfileBuildingType = LGProfileBuildingTypeData(
			lgProfileData,
			summationLGProfileBuildingType,
			language
		);
		const filteredLGProfileBuildingType = LGProfileBuildingType.filter((i) => i.value !== 0);
		const summationLGProfileBuildingFoundation =
			summationLGProfileBuildingFoundationData(lgProfileData);
		const LGProfileBuildingFoundation = LGProfileBuildingFoundationData(
			lgProfileData,
			summationLGProfileBuildingFoundation,
			language
		);
		const filteredLGProfileBuildingFoundation = LGProfileBuildingFoundation.filter(
			(i) => i.value !== 0
		);
		const disablestats = houseHoldInformation && JSON.parse(houseHoldInformation.disabilityStat);
		const totalDisableCount =
			disablestats && disablestats.length
				? disablestats.reduce(
						(total: any, currentValue: { totalPeople: any }) =>
							total + currentValue.totalPeople || 0,
						0
				  )
				: "-";
				...
				{!closedVisualization ? (
					<Modal className={styles.contactFormModal}>
						<Translation>
							{(t) => (
								<ModalBody
									className={_cs(styles.modalBody, language === "np" && styles.languageFont)}>
									...
									<div className={styles.categoryName}>
										<div className={styles.categoryLogo}>
											...
											<h3>
												{selectedDataType === 1
													? t("Demography (Census 2021)")
													: selectedDataType === 2
													? t("Demography (Census 2011)")
													: ""}
											</h3>
										</div>
									</div>
									{selectedDataType !== 3 ? (
											...
											<div>
												<div className={styles.barChartSection}>
													<div className={styles.percentageValue}>
														...
														{language === "en" ? (
															<h1>{sexRatio.sort((a, b) => b.percent - a.percent)[0].percent}%</h1>
														) : (
															<h1>
																कुल जनसंख्यामा{" "}
														...
														<>
															{language === "en" ? (
																...
																<span>
																	{sexRatio.sort((a, b) => b.percent - a.percent)[0].label} छन्
																</span>
															)}
														</>
													</div>
												...
												<div className={styles.barChartSection}>
													<div className={styles.percentageValue}>
														{/* <h1>Education Institution</h1> */}
														<h1>{literacyRatio.sort((a, b) => b.value - a.value)[0].value}%</h1>

														<>
															{language === "en" ? (
																<span>
																	{literacyRatio.sort((a, b) => b.value - a.value)[0].label} are
																	literate
																</span>
															) : (
																<span>
																	{literacyRatio.sort((a, b) => b.value - a.value)[0].label} साक्षर
																	छन्
																</span>
															)}
														</>
												...
												<div className={styles.barChartSection}>
													<div className={styles.percentageValue}>
														...
														<h1>
															{NumberWithCommas(
																householdSummary.householdSummary.find(
																	(i) => i.key === "householdCount"
																).value
															)}
														</h1>
                                                        ...
                                                        fontSize: '25px' }}
                                                         /> */}
															<div id="houseHold">
																...
																<BarchartVisualization
																	item={householdSummary.householdSummary}
																	language={language}
																/>
															</div>
														</div>
										...
										<div>
											<h3 style={{ marginBottom: "30px", color: "#4785DE", fontSize: "20px" }}>
												{t("Demographics")}
											</h3>
											{filteredLGProfileEducationLevel.length ? (
												<div className={styles.barChartSection}>
													<div className={styles.percentageValue}>
														{/* <h1>Education Institution</h1> */}
														<h1>{t("Education Attainment")}</h1>
														<h2 style={{ fontSize: "22px", color: "black" }}>
															{
																filteredLGProfileEducationLevel.sort(
															...
															<div id="filteredLGProfileEducationLevel">
																<div
																	...
																	<Button
																		...
																		transparent
																		onClick={() =>
																			this.handleSaveClick("filteredLGProfileEducationLevel")
																		}
																		iconName="download"
																	/>
																</div>

																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileEducationLevel}
																/>
															</div>
														</div>
													</div>
											...
											{filteredLGProfileMigration.length ? (
												<div className={styles.barChartSection}>
													<div className={styles.percentageValue}>
														...
														<h2 style={{ fontSize: "22px", color: "black" }}>
															{
																filteredLGProfileMigration.sort(
																	(a, b) => b.percentage - a.percentage
																)[0].percentage
															}
															%
															...
															<div id="filteredLGProfileMigration">
																<div
																	...
																	<Button
																		...
																		transparent
																		onClick={() =>
																			this.handleSaveClick("filteredLGProfileMigration")
																		}
																		iconName="download"
																	/>
																</div>
																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileMigration}
																/>
															</div>
														</div>
													</div>
															...
															<div id="filteredLGProfileSocialSecurity">
																<div
																	...
																	<Button
																		...
																		transparent
																		onClick={() =>
																			this.handleSaveClick("filteredLGProfileSocialSecurity")
																		}
																		iconName="download"
																	/>
																</div>
																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileSocialSecurity}
																/>
															</div>
														</div>
													</div>
															...
															<div id="filteredLGProfileDisability">
																<div
																	...
																	<Button
																		...
																		transparent
																		onClick={() =>
																			this.handleSaveClick("filteredLGProfileDisability")
																		}
																		iconName="download"
																	/>
																</div>
																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileDisability}
																/>
															</div>
														</div>
													</div>
															...
															<div id="filteredLGProfileHouseHold">
																<div
																	...
																	<Button
																		...
																		transparent
																		onClick={() =>
																			this.handleSaveClick("filteredLGProfileHouseHold")
																		}
																		iconName="download"
																	/>
																</div>
																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileHouseHold}
																/>
															</div>
														</div>
													</div>
											...
											{filteredLGProfileAverageMonthlyIncome.length ? (
												<div className={styles.barChartSection}>
													<div className={styles.percentageValue}>
														...
														<h2 style={{ fontSize: "22px", color: "black" }}>
															{
																filteredLGProfileAverageMonthlyIncome.sort(
																	(a, b) => b.percentage - a.percentage
																)[0].label
															}
														</h2>
														...
														<>
															<span>
																{language === "en" ? "on average per month" : "औसत प्रति महिना"}
															</span>
														</>
													</div>
															...
															<div id="filteredLGProfileAverageMonthlyIncome">
																<div
																	...
																	<Button
																		...
																		transparent
																		onClick={() =>
																			this.handleSaveClick("filteredLGProfileAverageMonthlyIncome")
																		}
																		iconName="download"
																	/>
																</div>
																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileAverageMonthlyIncome}
																/>
															</div>
														</div>
													</div>
											...
											{filteredLGProfileAverageMajorOccupation.length ? (
												<div className={styles.barChartSection}>
													<div className={styles.percentageValue}>
														...
														<h2 style={{ fontSize: "22px", color: "black" }}>
															{
																filteredLGProfileAverageMajorOccupation.sort(
																	(a, b) => b.percentage - a.percentage
																)[0].value
															}
														</h2>
                                                        ...
                                                        /> */}
															<div id="filteredLGProfileAverageMajorOccupation">
																<div
																	...
																	<h3>{language === "en" ? "Major occupation" : "प्रमुख पेशा"} </h3>
																	<Button
																		title={t("Download Chart")}
																		className={styles.chartDownload}
																		transparent
																		onClick={() =>
																			this.handleSaveClick(
																				"filteredLGProfileAverageMajorOccupation"
																			)
																		}
																		iconName="download"
																	/>
																...
																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileAverageMajorOccupation}
																/>
															</div>
														</div>
													</div>
											...
											{filteredLGProfileDrinkingWater.length ? (
												<div className={styles.barChartSection}>
													<div className={styles.percentageValue}>
														{/* <h1>Education Institution</h1> */}
														<h1>{t("Drinking Water")}</h1>
														<h2 style={{ fontSize: "22px", color: "black" }}>
															{
																filteredLGProfileDrinkingWater.sort(
																	(a, b) => b.percentage - a.percentage
																)[0].value
															}
														</h2>
															...
															<div id="filteredLGProfileDrinkingWater">
																<div
																	...
																	<Button
																		...
																		transparent
																		onClick={() =>
																			this.handleSaveClick("filteredLGProfileDrinkingWater")
																		}
																		iconName="download"
																	/>
																</div>
																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileDrinkingWater}
																/>
															</div>
														</div>
													</div>
															...
															<div id="filteredLGProfileResidentHousehold">
																<div
																	...
																	<Button
																		...
																		transparent
																		onClick={() =>
																			this.handleSaveClick("filteredLGProfileResidentHousehold")
																		}
																		iconName="download"
																	/>
																</div>
																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileResidentHousehold}
																/>
															</div>
														</div>
													</div>
											...
											{filteredLGProfileAgriculturePractice.length ? (
												<div className={styles.barChartSection}>
													<div className={styles.percentageValue}>
														...
														<h2 style={{ fontSize: "22px", color: "black" }}>
															{
																filteredLGProfileAgriculturePractice.sort(
																	(a, b) => b.percentage - a.percentage
																)[0].percentage
															}
															%
															...
															<div id="filteredLGProfileAgriculturePractice">
																<div
																	...
																	<Button
																		...
																		transparent
																		onClick={() =>
																			this.handleSaveClick("filteredLGProfileAgriculturePractice")
																		}
																		iconName="download"
																	/>
																</div>
																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileAgriculturePractice}
																/>
															</div>
														</div>
													</div>
											...
											{filteredLGProfileAgricultureProduct.length ? (
												<div className={styles.barChartSection}>
													<div className={styles.percentageValue}>
														{/* <h1>Education Institution</h1> */}
														<h1>{t("Agricultural Products")}</h1>
														<>
															{language === "en" ? (
																<span>
																	{
																		filteredLGProfileAgricultureProduct.sort(
																			(a, b) => b.percentage - a.percentage
																		)[0].label
																	}{" "}
                                                        ...
                                                        /> */}
															<div id="filteredLGProfileAgricultureProduct">
																<div
																	...
																	<h3>{t("Major Agricultural products")} </h3>
																	<Button
																		title={t("Download Chart")}
																		className={styles.chartDownload}
																		transparent
																		onClick={() =>
																			this.handleSaveClick("filteredLGProfileAgricultureProduct")
																		}
																		iconName="download"
																	/>
																</div>
																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileAgricultureProduct}
																/>
															</div>
														</div>
													</div>
											...
											{filteredLGProfileBuildingType.length ? (
												<div className={styles.barChartSection}>
													<div className={styles.percentageValue}>
														{/* <h1>Education Institution</h1> */}
														<h1>{t("Majority of building type")}</h1>
														<h2 style={{ fontSize: "22px", color: "black" }}>
															{
																filteredLGProfileBuildingType.sort(
																	(a, b) => b.percentage - a.percentage
																)[0].percentage
															}
															%
															...
															<div id="filteredLGProfileBuildingType">
																<div
																	...
																	<Button
																		...
																		transparent
																		onClick={() =>
																			this.handleSaveClick("filteredLGProfileBuildingType")
																		}
																		iconName="download"
																	/>
																</div>
																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileBuildingType}
																/>
															</div>
														</div>
													</div>
											...
											{filteredLGProfileBuildingFoundation.length ? (
												<div className={styles.barChartSection}>
													<div className={styles.percentageValue}>
														{/* <h1>Education Institution</h1> */}
														<h1>{t("Majority of building")}</h1>
														<h2 style={{ fontSize: "22px", color: "black" }}>
															{
																filteredLGProfileBuildingFoundation.sort(
																	(a, b) => b.percentage - a.percentage
																)[0].percentage
															}
															%
															...
															<div id="filteredLGProfileBuildingFoundation">
																<div
																	...
																	<Button
																		...
																		transparent
																		onClick={() =>
																			this.handleSaveClick("filteredLGProfileBuildingFoundation")
																		}
																		iconName="download"
																	/>
																</div>
																<LGProfileVisualization
																	language={language}
																	percentage
																	item={filteredLGProfileBuildingFoundation}
																/>
															</div>
														</div>
													</div>
									...
									<div>
										<div className={styles.dataDetails}>
											<div style={{ padding: "10px" }}>
												...
												<div
													className={
														selectedAttribute === "totalPopulation" ? styles.demographyHeading : ""
													}
												...
												{sexRatio && sexRatio.length ? (
													<div
														...
														<h2
															...
															{NumberWithCommas(
																populationSummary.find((i) => i.key === "totalPopulation").value
															)}
														</h2>
														{selectedDataType === 1 ? (
															populationSummary.find((i) => i.key === "changed_total_population")
																.value > 0 ? (
																<div style={{ display: "flex", alignItems: "center" }}>
																	...
																	<h2
																		...
																		{NumberWithCommas(
																			populationSummary.find(
																				(i) => i.key === "changed_total_population"
																			).value
																		)}{" "}
																		Since 2011
																	</h2>
												...
												<div
													...
													<ResponsiveContainer>
														<BarChart
															...
															<YAxis width={yAxisWidth} dataKey="label" type="category" />
															<Tooltip content={<CustomTooltip />} />
															<Bar dataKey="value" fill="#dcdcde" barSize={25}>
																{sexRatio.map((v) => (
																	<Cell key={v.key} fill={v.color} />
																))}
															</Bar>
														</BarChart>
													</ResponsiveContainer>
										...
										<div className={styles.dataDetails}>
											<div style={{ padding: "10px" }}>
												<div
													className={
														selectedAttribute === "literacyRate" ? styles.demographyHeading : ""
													}
													style={{ cursor: "pointer" }}
													role="button"
													tabIndex={0}
												...
												{literacyRatio && literacyRatio.length ? (
													<div
														...
														<h2
															...
															{NumberWithCommas(
																literacySummary.find((i) => i.key === "literacyRate").value
															)}
															%
														</h2>
														{selectedDataType === 1 ? (
															literacySummary.find((i) => i.key === "changed_literacyRate").value >
															0 ? (
																<div style={{ display: "flex", alignItems: "center" }}>
																	<img
																		src={Positive}
																	...
																	<h2
																		...
																		{NumberWithCommas(
																			literacySummary.find((i) => i.key === "changed_literacyRate")
																				.value
																		)}
																		% Since 2011
																	</h2>
										...
										<div className={styles.dataDetails}>
											<div style={{ padding: "10px" }}>
												<div
													className={
														selectedAttribute === "householdCount" ? styles.demographyHeading : ""
													}
													style={{
														cursor: "pointer",
														display: "flex",
												...
												<div
													...
													<ResponsiveContainer>
														<BarChart
															...
															<Bar dataKey="male" fill="#2A7BBB" stackId="a" barSize={25} />
															<Bar dataKey="female" fill="#83A4D3" stackId="a" barSize={25} />
														</BarChart>
													</ResponsiveContainer>
												</div>
											) : (
												...
												<div
													...
													<ResponsiveContainer>
														<BarChart
															...
															<Bar dataKey="value" fill="#2A7BBB" stackId="a" barSize={25} />
														</BarChart>
													</ResponsiveContainer>
												</div>
											) : (
				...
				{selectedDataType === 1 || selectedDataType === 2 ? (
					<>
						...
						<div className={_cs("map-legend-container", styles.legendContainer)}>
							<ChoroplethLegend className={styles.legend} legend={legend} minValue={min} />
						</div>
					</>
				) : (
					<>
										...
										{(t) => (
											<div style={{ margin: "10px" }}>
												<div style={{ marginTop: "20px", marginBottom: "20px" }}>
													<h1>
														{t("Household ID")}: {houseHoldInformation.houseNo}
													</h1>
													<table id={styles.customers}>
														<tr>
															...
															<td>
																<b>{houseHoldInformation.totalFamilyMembers}</b>
															</td>
														</tr>
														<tr>
															<td>{t("Ethinicity")}</td>
															...
															<td>
																<b>{houseHoldInformation.ethnicity}</b>
															</td>
														</tr>
														<tr>
															<td>{t("Religion")}</td>
															...
															<td>
																<b>{houseHoldInformation.religion}</b>
															</td>
														</tr>
													</table>
												</div>
												<div style={{ height: "200px", overflowY: "scroll" }}>
													<div
														...
														onClick={() =>
															this.setState({
																enableSensitivePopulationDiv: !enableSensitivePopulationDiv,
															})
														}
														onKeyDown={undefined}
														style={{
														...
														<div>
															<table id={styles.customers}>
																<tr>
																	...
																	<td>
																		<b>{houseHoldInformation.peopleAboveSixty || "-"}</b>
																	</td>
																</tr>
																<tr>
																	...
																	<td>
																		<b>
																			{houseHoldInformation.totalPregnantAndLactatingWomen || "-"}
																		</b>
																	</td>
																</tr>
																{/* <tr>
																...
																<tr>
																	...
																	<td>
																		<b>{houseHoldInformation.peopleBelowFive || "-"}</b>
																	</td>
																</tr>
															</table>
														</div>
													...
													<div
														...
														onClick={() =>
															this.setState({
																enableBuildingStructureDiv: !enableBuildingStructureDiv,
															})
														}
														onKeyDown={undefined}
														style={{
														...
														<div>
															<table id={styles.customers}>
																<tr>
																	...
																	<td>
																		<b>
																			{houseHoldInformation.buildingAge} {t("Years")}
																		</b>
																	</td>
																</tr>
																<tr>
																	...
																	<td>
																		<b>{houseHoldInformation.numberOfStory}</b>
																	</td>
																</tr>
																<tr>
																	<td>{t("Roof Type")}</td>
																	...
																	<td>
																		<b>{houseHoldInformation.materialUsedForRoof}</b>
																	</td>
																</tr>
																<tr>
																	...
																	<td>
																		<b>{houseHoldInformation.foundationType}</b>
																	</td>
																</tr>
															</table>
														</div>
													...
													<div
														...
														tabIndex={0}
														onClick={() =>
															this.setState({ enableEconomicAspectDiv: !enableEconomicAspectDiv })
														}
														onKeyDown={undefined}
														style={{
															marginTop: "10px",
														...
														<div>
															<table id={styles.customers}>
																<tr>
																	...
																	<td>
																		{majorOccupationList && majorOccupationList.length
																			? majorOccupationList.map(
																					(
																						i:
																							| string
																							| number
																...
																<tr>
																	...
																	<td>
																		{supportingOccupationList && supportingOccupationList.length
																			? supportingOccupationList.map(
																					(
																						i:
																							| string
																							| number
																...
																<tr>
																	...
																	<td>
																		<b>{houseHoldInformation.annualIncome}</b>
																	</td>
																</tr>
															</table>
														</div>
...
export default compose(connect(mapStateToProps), createRequestClient(requestOptions))(Demographics);
