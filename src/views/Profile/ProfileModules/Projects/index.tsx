...
import { _cs, mapToList, isNotDefined, listToMap, getHexFromString } from "@togglecorp/fujs";
...
import { regionsSelector, regionLevelSelector, projectsProfileFiltersSelector } from "#selectors";

import Loading from "#components/Loading";
import PageContext from "#components/PageContext";
import { TitleContext } from "#components/TitleContext";

import { createConnectedRequestCoordinator, createRequestClient } from "#request";
import { isAnyRequestPending, getResults } from "#utils/request";
...
const unflatten = (nodes, idSelector, parentSelector) => {
	...
	const valueList = mapToList(value, (val) => val);
...
const sanitize = (projectList, regions, ndrrsapMap) => {
	const mapped = projectList.map((project) => {
		...
		output.forEach((item) => {
			const { location, category, drrcycle, ndrrsap } = item;
...
const filter = (projectList, filters) =>
	projectList.filter(
		(project) =>
...
const wsEndpoint = import.meta.env.VITE_APP_PROJECT_SERVER_URL || "http://165.22.215.64/pims";
...
export class Profile extends React.PureComponent<Props, State> {
	...
	public render() {
		...
		const selectedPriority =
			...
			priorityOptions.find((item) => ndrrsapKeySelector(item) === faramValues.priority);
		if (selectedPriority) {
		...
		const projectOrganizationPieData = projectOrganizationGrouped
			.map((item) => ({
				key: item.key,
				label: item.key,
						...
						<header className={styles.header}>
							<div className={styles.backButtonProfile}>
								<Button
									...
									transparent
								/>
								<Translation>{(t) => <h1>{t("Projects")}</h1>}</Translation>
							</div>
						</header>
						<ProjectsProfile
