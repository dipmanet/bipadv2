...
const NepDataProfile = (props) => {
	...
	const handleSubmit = () => {
		...
		window.open(
			`${baseUrl}?report_type=${selectedReportType.id}` +
				`&language=${selectedLanguage.value}` +
				`&province=${selectedProvince ? selectedProvince.id : ""}` +
				`&district=${selectedDistrict ? selectedDistrict.id : ""}` +
				`&municipality=${selectedMunicipality ? selectedMunicipality.id : ""}` +
	...
	return (
		<div className={_cs(className, styles.documents)}>
			...
			<div className={styles.content}>
				<p>
					The Nep Dat Profile generates disaster reports at national, provincial district, and
					municipal levels for the selected time frame. The report is generated in both English and
					Nepali versions. Using the data available in the BIPAD portal and DRR portal, the analysis
					is done to present data in terms of the number of disaster incidents, people deaths due to
					disaster incidents, estimated economic loss, hazard calendar, seasonal hazards, and most
					recurrent disasters.
				</p>
			</div>
			...
			<div className={styles.dropdowns}>
				<PrimaryButton
					...
					disabled={
						!(selectedReportType && selectedLanguage && selectedStartDate && selectedEndDate)
					}
					onClick={handleSubmit}>
					<div style={{ marginRight: "10px" }}>
						<ScalableVectorGraphics src={nepDatDownload} />
					</div>
					GENERATE REPORT
				</PrimaryButton>
			</div>
...
export default connect(mapStateToProps)(
	createConnectedRequestCoordinator<ReduxProps>()(createRequestClient(requests)(NepDataProfile))
);
