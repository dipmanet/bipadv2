...
const RiskAndImpact = (props: Props) => {
	...
	const { ibfPage, riskClassName, toggleRisk, setToggleRisk, handleWidth, setIbfPage } = props;
	...
	const filteredRiskAndImpactParams = returnPeriod
		? riskAndImpactParams
		: riskAndImpactParams.slice(0, 1);
	...
	return (
		<>
			<div
				className={_cs(style.container, riskClassName, toggleRisk ? style.hidden : style.visible)}>
				<div className={style.btnContainer}>
					{filteredRiskAndImpactParams.length > 0 &&
						filteredRiskAndImpactParams.map(({ btnName, btnActive, countActive }) => (
							<button
								key={btnName}
								type="button"
								className={_cs(
									style.button,
									contentShowingParam === btnActive && style.activeBtnStyle
								)}
								onClick={() => {
				...
				<div className={style.mainContentContainer}>
					{contentShowingParam === "risk" && householdJson.length > 0 && filter.municipality && (
						<ExistingRisk
					...
					{contentShowingParam === "exposed" &&
						returnPeriod &&
						returnPeriod !== 0 &&
						mystationdata.length > 0 && (
							<PotentiallyExposed overallFloodHazard={overallFloodHazard} />
						)}
					{contentShowingParam === "impacted" &&
						householdJson.length > 0 &&
						returnPeriod &&
						returnPeriod !== 0 && (
							<PotentiallyImpacted
				...
				{isCountActive && (
					<CountLegend isActive={contentShowingParam} getImpactedValue={impactedValueHandler} />
				)}
			</div>
		</>
	);
