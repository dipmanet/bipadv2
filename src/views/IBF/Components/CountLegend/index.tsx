...
const CountLegend = (props: Props) => {
    ...
    const householdRisk = householdJson.map((household) => {
        ...
        const HistoricalImpactsAndDamage = valueToScore('historicalImpactssndDamage', historicalImpactsandDamage);
        ...
        const HasAccessToDrinkingWater = valueToScore('hasAccessToDrinkingWater', hasAccessToDrinkingWater);
        const HasAccessToEarlyWarningInformation = valueToScore('hasAccessToEarlyWarningInformation', hasAccessToEarlyWarningInformation);
        const HasInvolvmentToCommunityGroup = valueToScore('hasInvolvmentToCommunityGroup', hasInvolvmentToCommunityGroup);
        const HasAccessToFinancialServices = valueToScore('hasAccessToFinancialServices', hasAccessToFinancialServices);
        const HasAvailabilityOfSocialSecurity = valueToScore('hasAvailabilityOfSocialSecurity', hasAvailabilityOfSocialSecurity);
    ...
    return (
        <div className={style.riskSuperCon}>
            <div className={style.riskContainer}>
                <h4 className={style.title}>{`${riskScore[0] && Object.keys(riskScore[0]).length > 0 ? riskScore[0].title : ''}`}</h4>
                <div className={style.rangeContainer}>
                    {
                        existingRiskRange[selectedIndicator] && existingRiskRange[selectedIndicator].map(data => (
                            <>
                                <div
                                    key={data.indicator}
