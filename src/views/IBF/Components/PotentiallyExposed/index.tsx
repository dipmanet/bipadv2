    ...
    useEffect(() => {
        if (overallFloodHazard
            ...
            if (overallFloodHazard[0].types.building) {
                house = overallFloodHazard[0].types.building
                    .filter(
            ...
            if (overallFloodHazard[0].types.building) {
                chartbuildingdata = overallFloodHazard[0].types.building
                    .filter(
                        item => Object.keys(item)[0] !== 'yes',
                    ).map(
                        item => ({
                            name: capitalize(Object.keys(item)[0] === 'farmAuxiliary' ? 'farmhouse' : Object.keys(item)[0]),
                            total: Object.keys(item)[0] === 'house'
                                ? countBuilding(house)
            ...
            const chartlanddata = overallFloodHazard[0].types.landuse
                .filter(
                    item => Object.keys(item)[0] !== 'farmyard' && Object.keys(item)[0] !== 'meadow',
                )
                .map(
                    item => ({
                        name: capitalize(Object.keys(item)[0] === 'farmland' ? 'farmland'
                            : Object.keys(item)[0] === 'grass' ? 'greenland'
                                : Object.keys(item)[0]),
                        total: Object.keys(item)[0] === 'farmland'
                            ? Number((Number(countLand(farmland)) * 1e-6).toFixed(2))
                            : Object.keys(item)[0] === 'grass'
                                ? Number((Number(countLand(grassland)) * 1e-6).toFixed(2))
                                : Number((Number(item[Object.keys(item)[0]].totalArea) * 1e-6)
                                    .toFixed(2)),
                    }),
                );
    ...
    return (
        <div className={styles.exposedContainer}>
            ...
            <div className={styles.content}>
                <Tabs>
                    {
                        chartData.length > 0 && chartData.map(dataItem => (
