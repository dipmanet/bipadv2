...
const Summary = (props: Props) => {
	const { ibfPage, summaryClassName, toggleSummary, setToggleSummary, handleWidth } = props;

	const { selectedStation, stationDetail, filter } = ibfPage;
	...
	const districtRow = totalMunicipality.map((item) =>
		utils.getTotalDistrictRow(item, municipality, ward)
	);

	const municipalityRow = utils.getMunicipalityRow(
		mystationdata,
		filter.municipality,
		municipality,
		ward
	...
	return (
		<>
			<div
				...
				<div>
					{filter.municipality ? (
						<Table theadData={theadMunicipalityData} tbodyData={[municipalityRow]} />
					) : filter.district ? (
						<Table theadData={theadDistrictData} tbodyData={districtRow} />
					) : (
