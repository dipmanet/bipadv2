  ...
  return (
    <div className={style.housePopUpContainer}>
      <div className={style.popUpTitle}>
        ...
        {user && (
          <button className={style.editBtn} type="button" onClick={editHandler}>
            Edit
          </button>
        )}
      </div>
      <div className={style.fieldContainer}>
        {Object.keys(stateShcema).map((houseKey) => {
          if (
            stateShcema[houseKey].view &&
            stateShcema[houseKey].view === "private" &&
            user
          ) {
            return (
              <div className={style.popUpData} key={houseKey}>
                <div>{stateShcema[houseKey].label}</div>
                <div>{getValue(houseKey, houseProperties)}</div>
              </div>
            );
          }
          if (
          ...
          if (
            ...
            stateShcema[houseKey].label === "Income Source"
          ) {
            return null;
          }
          return (
