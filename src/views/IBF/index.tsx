...
const Ibf = (props: Props) => {
	const {
		ibfPage: { stations, stationDetail, selectedStation, filter, wtChange, householdJson },
		setIbfPage,
	} = props;
	...
	useEffect(() => {
		const getCall = async () => {
			if (wtChange === 1) {
				setPending(true);
				const indicatorData = await getRequest("ibf-vulnerability-indicator", {
		...
		<>
			...
			<div className={style.ibfContainer}>
				...
				{stations && stations.features && Object.keys(stationDetail).length > 0 && (
					<>
						<Navigation setFormOpen={setFormOpen} />
						{isSelectionActive && (
							<div
								className={_cs(
									style.mapSelection,
						...
						{viewDashboard && (
							<>
											...
											<>
												{filter.district && (
													<ForDrag
														...
														defaultPosition={defaultValues.summaryDefaultPosition}>
														<SummaryCon handleWidthToggle={handleWidthToggle} />
													</ForDrag>
												)}
												{filter.municipality && (
													<ForDrag
														widthToggle={widthToggle}
														defaultPosition={defaultValues.impactDefaultPosition}>
														<RiskAndImpactCon handleWidthToggle={handleWidthToggle} />
													</ForDrag>
												)}
											</>
										)}
								...
								<LegendDrag defaultPosition={defaultValues.legendDefaultPosition}>
									<Legend />
								</LegendDrag>
								<Calendar />
								{isFormOpen && (
									<ForDrag
										widthToggle={widthToggle}
										defaultPosition={defaultValues.householdFormPosition}>
										<HouseForm
