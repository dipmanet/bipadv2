...
const countHouse = (max, min, household) => {
	const count = household.response.results.filter(
		(item) => item.normalized_risk_score < max && item.normalized_risk_score >= min
	).length;
	return count;
};
...
const Existing = (props) => {
	const {
		ibfPage: { selectedStation, stationDetail },
		district,
		municipality,
	} = props;
				...
				{score && (
					<div className={style.info}>
						<div className={style.title}>
							{`Existing Risk at ${munName} Municipality, ${utils.getDistrictName(
								district,
						...
						<div className={style.blurpContainer}>
							<div className={style.blurp}>
								<div>
									<img className={style.icon} src={riskScore} alt="source" />
								</div>
							...
							<div className={style.blurp}>
								<div>
									<img className={style.icon} src={hazardScore} alt="link" />
								</div>
								<div className={style.text}>
							...
							<div className={style.blurp}>
								<div>
									<img className={style.icon} src={vulnerabilityScore} alt="source" />
								</div>
								<div className={style.text}>
							...
							<div className={style.blurp}>
								<div>
									<img className={style.icon} src={lackOfCoping} alt="event" />
								</div>
								<div className={style.text}>
									{`Lack of Coping Capacity: ${Number(
										score.response.avg_lack_of_coping_capacity
									).toFixed(2)}`}
								</div>
							</div>
						</div>
						...
						<table id={`existing-table-${props.mun}`} className={style.table}>
							...
							<tbody className={style.body}>
								...
								<tr className={style.row}>
									...
									<td className={style.col2}>{countHouse(4.9, 3.5, household)}</td>
								</tr>
								<tr className={style.row}>
...
export default connect(
	mapStateToProps,
	undefined
)(createConnectedRequestCoordinator<ReduxProps>()(createRequestClient(requests)(Existing)));
