...
const countImpactedHouse = (max, min, household, returnPeriod) => {
    let count;
    if (returnPeriod === 5) {
        count = (household.response.results
            .filter(item => item.impact_score_five <= max
                && item.impact_score_five >= min)).length;
    } else if (returnPeriod === 20) {
        count = (household.response.results
            .filter(item => item.impact_score_twenty <= max
                && item.impact_score_twennty >= min)).length;
    }
    return count;
};
                ...
                {household && (
                    <>
                        <div className={style.title}>
                            {`Impact Level at ${munName} Municipality, ${utils.getDistrictName(district, mystationdata[0].district)}`}
                        </div>
