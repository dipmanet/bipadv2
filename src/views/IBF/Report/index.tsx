...
const Report = (props) => {
    ...
    if (Object.keys(selectedStation).length > 0) {
        filteredStation = stations.features
    ...
    const effectiveDate = new Date(mydate.setDate(mydate.getDate() + leadTime)).toLocaleString('en-NP', options);
    ...
    useEffect(() => {
        ...
        props.requests.floodHazard.do(
            { returnPeriod, district: selectedDistrict.id, setSummary, setPending },
        );
    ...
    return (
        <Modal className={style.modal}>
            ...
            <ModalHeader
                ...
                rightComponent={(
                    <>
                        <button className={style.download} onClick={handleBtnClick} type="button">Download</button>
                        <DangerButton
                            transparent
                            iconName="close"
