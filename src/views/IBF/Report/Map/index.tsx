...
import { ibfPageSelector, districtsSelector, municipalitiesSelector } from "#selectors";
		...
		map.on("load", () => {
			...
			map.addLayer({
				...
				layout: {
					"icon-image": [
						...
						"Location-20",
						["all", ["==", ["get", "exceed_five", ["at", leadTime, ["get", "calculation"]]], true]],
						"Location-5",
						"Location",
					],
					"icon-size": 0.35,
			...
			if (props.effectedDristict) {
				mapRef.current.setFilter("district-centroid", [
					"all",
					["==", ["get", "title"], getTitle("district", props.effectedDristict[0].id)],
				]);
			...
			if (props.effectedMunicipality) {
				...
				if (props.effectedMunicipality.length === 1) {
					munbbox = utils.getBboxOfMunicipality(props.effectedMunicipality, municipality);
					mapRef.current.fitBounds([munbbox[0], munbbox[1], munbbox[2], munbbox[3]], {
						padding: 50,
					});
				}
				mapRef.current.setFilter(
					"municipality-local",
					getFiltlerExpression(props.effectedMunicipality)
				);
				mapRef.current.setFilter(
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(createConnectedRequestCoordinator<ReduxProps>()(createRequestClient(requests)(Map)));
