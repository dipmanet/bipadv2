...
const Calendar = (props: any) => {
	const {
		ibfPage: { stations, selectedStation, calendarData },
	} = props;

	const [selected, setSelected] = useState(-1);
	const [containerStyle, setContainerStyle] = useState<ContainerStyleType>(
		containerStyleInitialValue
	);
	...
	const clickHandler = (index: number) => {
		...
		if (Object.keys(selectedStation).length > 0) {
			const sel = stations.features.filter(
				(item) => item.properties.station_id === selectedStation.properties.station_id
			);
	...
	return (
		<>
			<div style={containerStyle.bg} className={style.calendarContainer}>
				...
				{Object.keys(containerStyle).length > 0 && containerStyle.text && (
				...
				<div className={style.buttonContainer}>
					...
					/>
					{calendarData &&
						calendarData.map((calendarItem: any, calendarDataIndex: any, calendarArray: any) => (
							<CalendarButton
