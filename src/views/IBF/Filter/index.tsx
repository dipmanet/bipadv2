...
const Filter = (props: Props) => {
	...
	const { ibfPage, district, municipality, ward, isFormOpen } = props;
	...
	useEffect(() => {
		if (wardState) {
			const toPushArray = wardState.filter((wardItem) => wardItem && wardItem.isChecked === true);
	...
	return (
		<>
			...
			>
				...
				{filter.district && filter.municipality === "" && <ReactTooltip />}
				{wardBool && (
							...
							wardState
								...
								.map((wardItem: any) => (
									<li key={wardItem.id} className={style.wardList}>
										...
										/>
										<label htmlFor={wardItem.id} className={style.label}>
											{wardItem.title}
										</label>
									</li>
								))}
