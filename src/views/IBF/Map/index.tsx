	...
	useEffect(() => {
		if (mapRef.current && mapRef.current.isStyleLoaded()) {
			if (filter.municipality) {
				...
				mapRef.current.setFilter("municipality-local", [
					"all",
					["==", ["get", "title"], getTitle("municipality", filter.municipality)],
				]);
		...
		if (mapRef.current && mapRef.current.isStyleLoaded()) {
			if (filter.ward.length > 0) {
				...
				if (filter.ward.length === 1) {
					...
					mapRef.current.setFilter("ward-local", [
						"all",
						["==", ["get", "id"], filter.ward.map((wardItem) => wardItem.id)[0]],
					]);
				...
				if (filter.ward.length > 1) {
					...
					mapRef.current.setFilter("ward-local", [
						"all",
						["in", ["get", "id"], ["literal", filter.ward.map((wardItem) => wardItem.id)]],
					]);
				}
	...
	useEffect(() => {
		...
		const markerHandler = (e: any) => {
			if (mapRef.current && e) {
				...
				const marker = new mapboxgl.Marker(grayCircle).setLngLat(e.lngLat).addTo(mapRef.current);
