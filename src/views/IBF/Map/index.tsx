...
const Map = (props: Props) => {
  ...
  const getMunicipalityBbox = () => {
    const { bbox } = municipality.filter(
      (item) => item.id === Number(filter.municipality)
    )[0];
    return bbox;
  };
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      markerLocations.map((img) => {
        map.loadImage(img.image, (error: any, image: any) => {
      ...
      map.on("click", "placesInitial", (e) => {
        const uniqueDistrictArray = getDistrictArray(
          stationDetail,
          e.features[0]
        );
  ...
  const getHouseSource = (filteredData: any) => {
    ...
    const householdGeoJSON = {
      ...
      features:
        nullToZerodata.length > 0 &&
        nullToZerodata.map((item: any) => ({
          id: item.id,
          type: "Feature",
          geometry: item.point,
          properties: { ...item },
      ...
      if (householdTemp.length > 0) {
        ...
        mapRef.current.on("click", "household-layer", (e) => {
          ...
          if (mapRef.current && e) {
            ...
            const popupHouse = new mapboxgl.Popup()
              .setLngLat(e.lngLat)
  ...
  useEffect(() => {
    if (mapRef.current && mapRef.current.isStyleLoaded()) {
      if (filter.district) {
        mapRef.current.setFilter("district-local", [
          "all",
          ["==", ["get", "title"], getTitle("district", filter.district)],
        ]);
        mapRef.current.setFilter("district-centroid", [
          "all",
          ["==", ["get", "title"], getTitle("district", filter.district)],
        ]);
        mapRef.current.setFilter("municipality-local", [
          "all",
          ["==", ["get", "district"], Number(filter.district)],
        ]);
        mapRef.current.setFilter("municipality-centroid", [
          "all",
          ["==", ["get", "district"], Number(filter.district)],
        ]);
      ...
      if (filter.municipality) {
        ...
        mapRef.current.setFilter("municipality-local", [
          ...
          [
            "==",
            ["get", "title"],
            getTitle("municipality", filter.municipality),
          ],
        ]);
        mapRef.current.setFilter("ward-local", [
          "all",
          ["==", ["get", "municipality"], Number(filter.municipality)],
        ]);
        mapRef.current.setFilter("ward-centroid", [
          "all",
          ["==", ["get", "municipality"], Number(filter.municipality)],
        ]);
        mapRef.current.setLayoutProperty(
    ...
    if (mapRef.current && mapRef.current.isStyleLoaded()) {
      if (filter.ward.length > 0) {
        ...
        if (filter.ward.length === 1) {
          const { bbox } = ward.filter(
            (item) => item.id === Number(filter.ward[0].id)
          )[0];
          mapRef.current.fitBounds(
            [
              [bbox[0], bbox[1]],
          ...
          mapRef.current.setFilter("ward-local", [
            ...
            [
              "==",
              ["get", "id"],
              filter.ward.map((wardItem) => wardItem.id)[0],
            ],
          ]);
          mapRef.current.setFilter("household-layer", [
            "all",
            ["==", ["get", "ward"], filter.ward[0].id],
          ]);
        }
        ...
        if (filter.ward.length > 1) {
          ...
          mapRef.current.setFilter("household-layer", [
            ...
            [
              "in",
              ["get", "ward"],
              ["literal", filter.ward.map((wardItem) => wardItem.id)],
            ],
          ]);
          mapRef.current.setFilter("ward-local", [
            "all",
        ...
        const wardLevelHouseData = wardHouseData.filter((houseData) =>
          tempWard.includes(houseData.ward)
        );
      ...
      if (filter.ward.length === 0 && filter.municipality) {
        ...
        mapRef.current.setFilter("household-layer", [
          "all",
          ["==", ["get", "municipality"], filter.municipality],
        ]);
        mapRef.current.setFilter("ward-local", [
          "all",
          ["==", ["get", "municipality"], Number(filter.municipality)],
  ...
  useEffect(() => {
    ...
    if (selectedLegend) {
      ...
      const filteredData = householdJson.filter(
        legendFilterForCsv[selectedLegend]
      );
  ...
  return (
    <>
      ...
      <div
        style={props.myStyle ? props.myStyle : mapContainer}
        ref={mapContainerRef}
      />
    </>
  );
