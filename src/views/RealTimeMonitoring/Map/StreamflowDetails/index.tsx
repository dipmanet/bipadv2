...
class StreamflowDetails extends React.PureComponent<Props> {
	private getChartData = (data: FlowData[] = []) =>
		data.map((d) => {
			...
			return {
				...
				min: values.minValue,
				max: values.maxValue,
				flow: [values.minValue, values.maxValue],
				std: [values.stdDevLowerValue, values.stdDevUpperValue],
			};
		});
	...
	public render() {
		const { id, handleModalClose, requests } = this.props;
					...
					{!pending && (
						<div className={styles.streamflow}>
								...
								<Table celled structured className={styles.returnPeriodTable}>
									<Table.Row>
										...
										{returnPeriod.map((value) => (
											<Table.Cell className={styles.headerTitle}>{value.value}</Table.Cell>
										))}
									</Table.Row>
									<Table.Row>
										<Table.HeaderCell className={styles.headerTitle}>
										...
										{returnPeriod.map((value) => (
											<Table.Cell className={styles.headerTitle}>{value.label}</Table.Cell>
										))}
									</Table.Row>
								</Table>
							</div>
							...
							<header className={styles.header}>
								<h3 className={styles.heading}>Streamflow</h3>
							</header>
							<PlotlyComponent className="chart" data={data} layout={layout1} config={config} />
						</div>
					)}
				</ModalBody>
			</Modal>
