...
class Earthquake extends React.PureComponent<Props> {
	...
	public render() {
		const { realTimeEarthquake, closeModal, language } = this.props;

		const earthquakeHeader = this.earthquakeHeader(language);

		const formattedTableData = convertNormalTableToCsv(realTimeEarthquake, earthquakeHeader);
		return (
			<Translation>
				{(t) => (
					<Modal
						...
						<ModalFooter>
							...
							<DownloadButton value={formattedTableData} name="Earthquake.csv">
								{t("Download")}
							</DownloadButton>
						</ModalFooter>
					</Modal>
