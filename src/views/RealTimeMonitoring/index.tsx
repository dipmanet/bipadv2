...
const mapDispatchToProps = (dispatch: Dispatch): PropsFromDispatch => ({
	...
	setDataArchiveRainStations: (params) => dispatch(setDataArchiveRainStationAction(params)),
});
...
const requestOptions: { [key: string]: ClientAttributes<ReduxProps, Params> } = {
	...
	realTimeRiverRequest: {
		...
		onSuccess: ({ response, props: { setRealTimeRiverList } }) => {
			interface Response {
				results: PageType.RealTimeRiver[];
			}
	...
	realTimeEarthquakeRequest: {
		...
		onSuccess: ({ response, props: { setRealTimeEarthquakeList } }) => {
			interface Response {
				results: PageType.RealTimeEarthquake[];
			}
	...
	realTimePollutionRequest: {
		...
		onSuccess: ({ response, props: { setRealTimePollutionList } }) => {
			interface Response {
				results: PageType.RealTimePollution[];
			}
		...
		onSuccess: ({ params, response, props: { setDataArchiveRainStations } }) => {
			...
			if (params.stationFilter !== undefined) {
				setDataArchiveRainStations({
					dataArchiveRainStations: dataArchiveRainStations.filter(
						(item) => item.id === params.stationFilter
					),
				});
			} else if (params.basinFilter !== undefined) {
...
const legendLabelSelector = (
	d: {
		label: string;
		labelNe: string;
	},
	lang
) => (lang === "en" ? d.label : d.labelNe);
...
class RealTimeMonitoring extends React.PureComponent<Props, State> {
	...
	private renderLegend = () => {
		const {
			filters: { realtimeSources, otherSources },
			realTimeRiverList,
		...
		const autoRiverLegends = getAutoRealTimeRiverLegends(realTimeRiverList, newRiverLegendItems);
		const autoPollutionLegends = getPollutionLegends(realTimePollutionList, pollutionLegendItems);
		return (
			<>
				{showRain && (
					<div className={styles.legendContainer}>
						<header className={styles.header}>
							...
							<h4 className={styles.heading}>
								<Translation>{(t) => <span>{t("Rain")}</span>}</Translation>
							</h4>
						</header>
						{duration === 24 && (
							<Legend
						...
						<div className={styles.sourceDetails}>
							<div className={styles.label}>
								<Translation>{(t) => <span>{t("Source")}</span>}</Translation>:
							</div>
							<a
				...
				{showRiver && (
					<div className={styles.legendContainer}>
						<header className={styles.header}>
							<ScalableVectorGraphics className={styles.legendIcon} src={RiverIcon} alt="River" />
							<h4 className={styles.heading}>
								<Translation>{(t) => <span>{t("River")}</span>}</Translation>
							</h4>
						</header>
						<Legend
							className={styles.legend}
							...
							data={realTimeRiverList.length > 0 ? autoRiverLegends : noLegend}
							itemClassName={styles.legendItem}
							keySelector={itemSelector}
							// iconSelector={iconSelector}
				...
				{showEarthquake && (
					<div className={styles.legendContainer}>
						<header className={styles.header}>
							...
							/>
							<h4 className={styles.heading}>
								<Translation>{(t) => <span>{t("Earthquake (ML)")}</span>}</Translation>
							</h4>
						</header>
						<Legend
							className={styles.legend}
					...
					<div className={styles.legendContainer}>
						<header className={styles.header}>
							...
							<h4 className={styles.heading}>
								...
								<Translation>{(t) => <span>{t("Pollution (AQI Value)")}</span>}</Translation>
							</h4>
						</header>
						<Legend
							className={styles.legend}
							// data={pollutionLegendItems}
							data={realTimePollutionList.length > 0 ? autoPollutionLegends : noLegend}
							itemClassName={styles.legendItem}
							keySelector={itemSelector}
							// labelSelector={legendLabelSelector}
							labelSelector={(d) => legendLabelSelector(d, language)}
				...
				{showFire && (
					<div className={styles.legendContainer}>
						<header className={styles.header}>
							<ScalableVectorGraphics className={styles.legendIcon} src={FireIcon} />
							<h4 className={styles.heading}>
								<Translation>{(t) => <span>{t("Forest Fire")}</span>}</Translation>
							</h4>
						</header>
						<Legend
							className={styles.legend}
							...
							<header className={styles.header}>
								<svg className={styles.legendIcon}>
									<line
										...
										x2="20"
										y2="5"
										style={{ stroke: "#7cb5ec", strokeWidth: 5 }}
									/>
								</svg>
								<h4 className={styles.heading}>
									<Translation>{(t) => <span>{t("Streamflow")}</span>}</Translation>
								</h4>
							</header>
							<div className={styles.sourceDetails}>
								<div className={styles.label}>
	...
	public render() {
		...
		const { hoveredHazardId, source } = this.state;
...
just been deselected */
		const validateActiveView = (firstCondition: boolean, secondCondition: boolean | undefined) => {
										...
										<div className={styles.tabs}>
											{showRain && (
												<div
													className={_cs(
														styles.tab,
														validActiveView === "rainwatch" && styles.active
													)}
													onClick={this.handleRainWatchButtonClick}
													role="presentation">
													<div className={styles.value}>{realTimeRainList.length}</div>
											...
											{showRiver && (
												<div
													className={_cs(
														styles.tab,
														validActiveView === "riverwatch" && styles.active
													)}
													onClick={this.handleRiverWatchButtonClick}
													role="presentation">
													<div className={styles.value}>{realTimeRiverList.length}</div>
													<div className={styles.title}>
											...
											{showEarthquake && (
												<div
													className={_cs(
														styles.tab,
														validActiveView === "earthquake" && styles.active
													)}
													onClick={this.handleEarthquakeButtonClick}
													role="presentation">
													<div className={styles.value}>{realTimeEarthquakeList.length}</div>
													<div className={styles.title}>
														<div className={_cs(styles.icon, styles.eventIcon)} />
														<div className={styles.text}>{t("Earthquake")}</div>
													</div>
												</div>
											)}
											{showPollution && (
												<div
													className={_cs(
														styles.tab,
														validActiveView === "pollution" && styles.active
													)}
													onClick={this.handlePollutionButtonClick}
													role="presentation">
													<div className={styles.value}>{realTimePollutionList.length}</div>
													<div className={styles.title}>
														<div className={_cs(styles.icon, styles.eventIcon)} />
														<div className={styles.text}>{t("Pollution")}</div>
													</div>
												</div>
											)}
											{showFire && (
												<div
													...
													role="presentation">
													<div className={styles.value}>{realTimeFireList.length}</div>
													<div className={styles.title}>
														<div className={_cs(styles.icon, styles.eventIcon)} />
														<div className={styles.text}>{t("Fire")}</div>
													</div>
												</div>
											)}
										</div>
									...
									<div className={styles.content}>
										...
										{!validActiveView && (
											<div className={styles.message}>
												<Message>Select a category to see details</Message>
											</div>
										)}
									</div>
								</>
