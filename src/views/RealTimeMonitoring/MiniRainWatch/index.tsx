...
import { setRealTimeDurationAction } from "#actionCreators";
	...
	private getRainHeader = (duration: number, language: string) => [
		{
			...
			modifier: (row: RealTimeRain) => {
				...
				return basin ? <div style={{ width: "60px" }}>{basin}</div> : undefined;
			},
		},
		{
			key: "title",
			...
			<div className={_cs(className, styles.rainWatch)}>
				<header className={styles.header}>
					...
					<ModalButton
						...
						iconName="table"
						title="Show all data"
						modal={<RainWatch realTimeRain={realTimeRain} />}
					/>
				</header>
...
export default compose(connect(mapStateToProps, mapDispatchToProps))(MiniRainWatch);
