...
import { compareString, compareNumber, _cs, Maybe } from "@togglecorp/fujs";
		...
		this.pollutionHeader = (language: string) => [
			{
				...
				order: 1,
				sortable: true,
				comparator: (a: { name: Maybe<string> }, b: { name: Maybe<string> }) =>
					compareString(a.name, b.name),
				modifier: (row: RealTimePollutionExtended) => {
					const { name } = row;

					return name ? <div>{name}</div> : undefined;
				},
			},
			{
				key: "modifiedOn",
				...
				order: 2,
				sortable: true,
				comparator: (a: { modifiedOn: Maybe<string> }, b: { modifiedOn: Maybe<string> }) =>
					compareString(a.modifiedOn, b.modifiedOn),
				modifier: (row: RealTimePollutionExtended) => {
					const { dateTime } = row;
			...
			},
			{
				...
				order: 4,
				sortable: true,
				comparator: (a: { aqi: Maybe<number> }, b: { aqi: Maybe<number> }) =>
					compareNumber(a.aqi, b.aqi),
				modifier: (row: RealTimePollutionExtended) => {
					const { aqi } = row;
			...
			<div className={_cs(className, styles.pollution)}>
				<header className={styles.header}>
					...
					<ModalButton
						...
						iconName="table"
						title="Show all data"
						modal={<Pollution realTimePollution={realTimePollution} language={language} />}
					/>
				</header>
