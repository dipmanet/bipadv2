		...
		<div className={styles.activeLayer}>
			<header className={styles.header}>
				...
				{layer && layer.bbox && layer.bbox.length > 0 && (
					<img src={Bbox} alt="" className={styles.bboxIcon} onClick={zoomToBbox} role="button" />
				)}
			</header>
			<div className={styles.content}>
				...
				{showLegend && <LayerLegend layer={layer} language={language} />}
			</div>
		</div>
	);
};
...
class ActiveLayers extends React.PureComponent<Props, State> {
	...
	private handleToggleOpacityInputVisibilityButtonClick = () => {
		this.setState(({ showOpacityInput }) => ({ showOpacityInput: !showOpacityInput }));
	};
	...
	private getData = memoize((data) => [...data].reverse());
	...
	public render() {
		...
		const { showLegend, showOpacityInput } = this.state;
			...
			<Translation>
				{(t) => (
					<div className={_cs(styles.activeLayers, className)}>
						<header className={styles.header}>
							<h3 className={styles.heading}>{t("Active layers")}</h3>
							<div className={styles.actions}>
								...
								<Button
									...
									className={_cs(styles.toggleLegendVisibilityButton, showLegend && styles.active)}
									iconName="list"
								/>
							</div>
						</header>
