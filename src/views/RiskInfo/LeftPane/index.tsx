...
import { _cs, listToGroupList, listToMap } from "@togglecorp/fujs";
...
import { setHashToBrowser, getHashFromBrowser } from "#rscg/HashManager";
...
class RiskInfoLeftPane extends React.PureComponent<Props, State> {
	...
	private getGroupedLayers = (layerList: Layer[], layerGroupList: LayerGroup[]) => {
		...
		const groupedLayerList = listToGroupList(groupExpandedLayerList, (d) => d.category);
	...
	public render() {
		...
		return (
			<div
				className={_cs(styles.leftPane, className, activeAttribute && styles.hasActiveAttribute)}>
				...
				<header className={styles.header}>
					{activeAttribute ? (
						...
						<>
							<Translation>
								{(t) => <h2 className={styles.heading}>{t("Risk Info")}</h2>}
							</Translation>
