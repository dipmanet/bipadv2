...
import { _cs, listToMap, isDefined } from "@togglecorp/fujs";
...
import { getResults, getResponse, isAnyRequestPending } from "#utils/request";

import { generatePaint, getLayerHierarchy } from "#utils/domain";
...
const requestOptions: { [key: string]: ClientAttributes<Props, Params> } = {
	...
	durhamLandslideDistrictRequest: {
		url: `${import.meta.env.VITE_APP_GEO_SERVER_URL}/geoserver/Bipad/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=Bipad%3Adurham_landslide_hazard_risk_district&outputFormat=application%2Fjson&propertyName=district_d,District_r`,
		method: methods.GET,
		onMount: true,
	},
	durhamLandslideMunicipalityRequest: {
		url: `${import.meta.env.VITE_APP_GEO_SERVER_URL}/geoserver/Bipad/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=Bipad%3Adurham_landslide_hazard_risk_municipality&outputFormat=application%2Fjson&propertyName=Palika_ris,municipali`,
		method: methods.GET,
		onMount: true,
	},
	durhamLandslideWardRequest: {
		url: `${import.meta.env.VITE_APP_GEO_SERVER_URL}/geoserver/Bipad/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=Bipad%3Adurham_landslide_hazard_risk_ward&outputFormat=application%2Fjson&propertyName=ward_id,Ward_War_4`,
		method: methods.GET,
		onMount: true,
	},
};
...
const RiskTooltip = ({ layer, feature }) => (
	<div className={styles.riskTooltip}>
		...
		<div className={styles.content}>
			<RiskTooltipOutput label="Risk score:" value={feature.state.value} />
			<RiskTooltipOutput label="Rank:" value={layer.rankMap[feature.id]} />
		</div>
	</div>
);
...
class Risk extends React.PureComponent<Props, State> {
	...
	public render() {
		const { className, requests, layerList, layerGroupList } = this.props;

		const layers = this.getHierarchy(layerList, layerGroupList);
		const { showMetricSettings, showOpacitySettings, opacityValue, metricValues } = this.state;

		const pending = isAnyRequestPending(requests);
		const riskDataRaw = getResults(requests, "riskGetRequest") as RiskData[];
		const districtLandslideRaw = getResponse(
			requests,
			"durhamLandslideDistrictRequest"
		) as LandslideDataGeoJson;
		const municipalityLandslideRaw = getResponse(
			requests,
			"durhamLandslideMunicipalityRequest"
		) as LandslideDataGeoJson;
		const wardLandslideRaw = getResponse(
			requests,
			"durhamLandslideWardRequest"
		) as LandslideDataGeoJson;
		...
		const riskLayer = transformRiskDataToLayer(
			riskData,
			earthquakeLayer[0],
			<>
				<DataTableModalButton
					modal={<RiskTable title="Durham earthquake risk data" data={riskDataRaw} />}
		...
		const RiskLayerSelectionItem = (p) => {
			...
			if (layer.layername === "durham_earthquake_risk_score") {
				return (
					<React.Fragment>
						<LayerSelectionItem data={riskLayer} disabled={pending} />
						<div className={styles.options}>
							{showMetricSettings && (
								<div className={styles.metricSettings}>
									<header className={styles.header}>
										<h4 className={styles.heading}>Configure risk parameters</h4>
										<Button
									...
									<div className={styles.content}>
										{metricKeys.map((m) => (
											<div key={m} className={styles.metricInput}>
												<div className={styles.label}>{metrices[m]}</div>
												<RangeInput
													classNames={{
				...
				<LayerSelectionItem
					...
					data={
						landslideLayerToDataMap[layer.layername]
							? transformLandslideDataToLayer(landslideLayerToDataMap[layer.layername], layer)
							: layer
					}
					disabled={pending}
				/>
