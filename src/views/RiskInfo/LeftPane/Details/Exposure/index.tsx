...
class Exposure extends React.PureComponent<Props, State> {
	...
	public componentWillUnmount() {
		...
		const { previousMapStyle, selectedId } = this.state;
	...
	private handleLayerUnselect = () => {
		const { selectedId, previousMapStyle } = this.state;
	...
	public render() {
		const { className, layerList, layerGroupList } = this.props;
		...
		return (
			<div className={_cs(styles.exposure, className)}>
                ...
                /> */}
				<LayerSelection className={_cs(styles.hazard, className)} layerList={exposureLayer} />
			</div>
		);
	}
}
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(createConnectedRequestCoordinator<ReduxProps>()(createRequestClient()(Exposure)));
