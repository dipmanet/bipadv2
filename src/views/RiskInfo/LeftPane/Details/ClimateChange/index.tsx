...
import { NapData, NapValue, Scenario } from "#types";
...
class ClimateChange extends React.PureComponent<Props, State> {
	...
	private getLayer = memoize((layerGroupList, measurementType) => {
		...
		const metadata =
			measurementType === "temperature" ? temperature.metadata : precipitation.metadata;
	...
	private getChartData = (measurementType: string, district?: number) => {
		...
		const nationalTemperature = getResults(
			requests,
			"napNationalTemperatureGetRequest"
		) as NapData[];
		const nationalPrecipitation = getResults(
			requests,
			"napNationalPrecipitationGetRequest"
		) as NapData[];
		...
		if (district) {
			...
			const selectedData =
				measurementType === "temperature" ? districtTemperature : districtPrecipitation;
	...
	private getMapState = (
		...
		const timePeriod =
			timePeriodOptions.find((option) => option.key === timePeriodKey) || timePeriodOptions[0];
		const { startYear, endYear } = timePeriod;
		const { startYear: referenceStart, endYear: referenceEnd } =
			timePeriodOptions.find((option) => option.key === "reference-period") || timePeriodOptions[0];

		const filter = ({ year }: NapValue) => year >= startYear && year <= endYear;
		const referenceFilter = ({ year }: NapValue) => year >= referenceStart && year <= referenceEnd;
		...
		const filteredData = napData.map((data) => {
			...
			return {
				id: district,
				value:
					measurementType === "temperature" && timePeriodKey !== "reference-period"
	...
	private transformClimateChangeToLayer = ({
		...
		if (selectedMeasurement && selectedTimePeriod) {
			precipitationChangeInPercent =
				selectedMeasurement.key === "precipitation" &&
				selectedTimePeriod.key !== "reference-period";
		}

		let temperatureChangeInPercent = false;
		if (selectedMeasurement && selectedTimePeriod) {
			temperatureChangeInPercent =
				selectedMeasurement.key === "temperature" && selectedTimePeriod.key !== "reference-period";
		}
		...
		const legendTitle = `
            ...
            ${
							!precipitationChangeInPercent && !temperatureChangeInPercent && selectedMeasurement
								? selectedMeasurement.legendTitle
								: ""
						}
	...
	private handleChange = (value: boolean) => {
		const { addLayer, removeLayer } = this.context;

		this.setState({
			isActive: value,
		});

		const { timePeriodKey, measurementType, scenario } = this.state;
	...
	public render() {
		...
		const yAxisLabel =
			selectedOption && language === "en" ? selectedOption.axisLabel : selectedOption.axisLabelNe;
		const chartName = climateChangeSelectedDistrict.title || "Nepal";
		const chartTitle =
			selectedOption && language === "en"
				? `${selectedOption.chartTitle} ${chartName}`
				...
				<Translation>
					{(t) => (
						<div className={_cs(styles.climateChange, className)}>
								...
								{!pending && (
									<>
										<DataTableModalButton
											...
											modal={<ClimateChangeTable data={flatData} title={t(measurementType)} />}
							...
							<div className={styles.shortDescription}>{layer.shortDescription}</div>
							...
							<div className={styles.externalLinks}>
								...
								<div className={styles.content}>
									<a
										...
										<Icon className={styles.icon} name="externalLink" />
										<div className={styles.text}>{t("Temperature data")}</div>
									</a>
									<a
										className={styles.externalLink}
										rel="noopener noreferrer"
										...
										<Icon className={styles.icon} name="externalLink" />
										<div className={styles.text}>{t("Precipitation data")}</div>
									</a>
								</div>
							</div>
							{!pending && isActive && (
								<div className={styles.bottom} id="climateChange">
									<div className={styles.heading}>
										<div className={styles.header}>{chartTitle}</div>
