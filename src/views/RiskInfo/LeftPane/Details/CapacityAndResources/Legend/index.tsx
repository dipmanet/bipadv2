...
const CapacityAndResourcesLegend = (props: Props) => {
	...
	const showLayerControls = activeLegends.some(
		(legend) => legend.key === "openspace" || legend.key === "communityspace"
	);
	...
	if (openspaceOn && communityspaceOn) {
		...
		activeLegends.filter((e) => String(e.key) !== "openspace" && String(e.key) === "communityspace")
			.length > 0
	) {
	...
	return (
		<React.Fragment>
			...
			<div className={_cs(styles.wrapper, "map-legend-container")}>
				<Translation>
					{(t) => <div className={styles.title}>{t("Capacity and Resources")}</div>}
				</Translation>
				<Legend
