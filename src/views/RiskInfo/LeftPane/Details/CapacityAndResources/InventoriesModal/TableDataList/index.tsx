...
const TableDataList = ({
	...
	const idToTitle = (array, id, lang) => {
		...
		const selectedData =
			id && array.length ? (lang === "en" ? data.title : data.titleNe || data.title) : "-";
		return selectedData;
	};
						...
						{selectedCategory === 1 ? (
							<table className={_cs(styles.contacts, language === "np" && styles.languageFont)}>
								<thead style={{ position: "sticky", top: "0" }}>
									<tr>
										<th colSpan="10" scope="colgroup" style={{ textAlign: "center" }}>
											{t("Items")}
										</th>
										...
										<tr key={data.id}>
											<td>
												<a href={data.item.image} target="_blank" rel="noopener noreferrer">
													<img
														src={data.item.image || inventoryDefaultImage}
														alt="item-pic"
														style={{
															height: "120px",
															width: "120px",
											...
											<td>{idToTitle(unitList, data.item.unitLink, language)}</td>
											...
											<td>
												{" "}
												{data.item.clusters.map(
													(d, index) =>
														`${idToTitle(clusterList, d, language)}${
															data.item.clusters.length - 1 === index ? "" : " , "
														}`
												)}
											</td>
											<td>
												{data.item.categories.map(
													(d, index) =>
														`${idToTitle(categoryList, d, language)}${
															data.item.categories.length - 1 === index ? "" : " , "
														}`
												)}
											</td>

											<td>
												{data.item.hazards.map(
													(d, index) =>
														`${idToTitle(hazard, d, language)}${
															data.item.hazards.length - 1 === index ? "" : " , "
														}`
												)}{" "}
											</td>
											<td>{data.quantity || "-"}</td>
										...
										<tr key={item.id}>
											...
											<td>
												{item.organization.map(
													(d, index) => `${idToTitle(organizationList, d, language)}
                                                                    ${
								...
								<tbody>
									{stockOutList.map((item) => (
										<tr key={item.id}>
											...
											<td>
												{language === "en"
													? item.transferedToResource
														? item.transferedToResource.title
														: "-"
													: item.transferedToResource
													? item.transferedToResource.titleNe
													: "-"}
											</td>
											<td>{idToTitle(itemList, item.item, language)}</td>
											<td>{item.rate}</td>
										...
										<tr key={item.id}>
											<td>
												<a href={item.image} target="_blank" rel="noopener noreferrer">
													<img
														src={item.image || inventoryDefaultImage}
														alt="item-pic"
														style={{
															height: "120px",
															width: "120px",
											...
											<td>{idToTitle(unitList, item.unitLink, language)}</td>
