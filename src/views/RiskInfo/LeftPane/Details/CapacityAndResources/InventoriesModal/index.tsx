...
const InventoryItem = (props: InventoryItemProps) => {
	...
	const { item, quantity, description, createdOn, id } = data;
	...
	return (
		<div className={_cs(className, styles.inventory)}>
			<TextOutput label="Title" value={item.title} />
			<TextOutput label="Category" value={item.category} />
			{isDefined(quantity) && (
				<TextOutput
					label="Quantity"
					value={<Numeral className={styles.quantity} value={quantity} suffix={item.unit} />}
				/>
			)}
			<TextOutput
				label="Created On"
				value={<FormattedDate className={styles.createdOn} value={createdOn} mode="yyyy-MM-dd" />}
			/>
			<TextOutput label="Description" value={description || "No description"} />
			<div className={styles.actionButtons}>
				<Cloak hiddenIf={(p) => !p.change_inventory}>
					<ModalButton
						className={styles.button}
						modal={<AddInventoryForm onUpdate={onUpdate} value={data} resourceId={resourceId} />}
						disabled={disabled}
						iconName="edit"
						transparent>
						Edit
...
class InventoriesModal extends React.PureComponent<Props, State> {
	...
	private handleRefreshStockIn = () => {
		this.props.requests.stockInGetRequest.do();
	};
	...
	public render() {
		const {
			...
			closeModal,
			requests: {
				inventoriesGetRequest: { pending, response },
				clusterGetRequest: { pending: clusterPending, response: clusterResponse },
				categoryGetRequest: { pending: categoryPending, response: categoryResponse },
				unitGetRequest: { pending: unitPending, response: unitResponse },
				itemGetRequest: { pending: itemPending, response: itemResponse },
				hazardGetRequest: { pending: hazardPending, response: hazardResponse },
				organizationGetRequest: { pending: organizationPending, response: organizationResponse },
				stockOutGetRequest: { pending: stockOutPending, response: stockOutResponse },
				stockInGetRequest: { pending: stockInPending, response: stockInResponse },
			},
			resourceId,
			palikaRedirect,
					...
					{(t) => (
                        ...
                        /> */}
								...
								{isDefined(palikaRedirect.inventoryItem) && (
									<button onClick={this.handleReturnToPalika} type="button">
...
export default connect(
	...
	createConnectedRequestCoordinator<PropsWithRedux>()(
		createRequestClient(requests)(InventoriesModal)
	)
);
