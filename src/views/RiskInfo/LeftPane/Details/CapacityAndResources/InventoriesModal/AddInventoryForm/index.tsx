...
const requests: { [key: string]: ClientAttributes<ReduxProps, Params> } = {
    ...
    */
	addInventoryPostRequest: {
		url: ({ props }) => (props.value ? `/inventory/${props.value.id}/` : "/inventory/"),
...
class AddInventoryForm extends React.PureComponent<Props, State> {
	...
	public render() {
		const {
			...
			// resourceList,
			requests: {
				addInventoryPostRequest: { pending },
			},
			value,
		} = this.props;
		...
		return (
			...
			>
				...
				<Faram
					...
					disabled={pending}>
					<ModalHeader
						title={value ? "Edit Inventory" : "Add Inventory"}
						rightComponent={
					...
					<ModalBody>
						...
						<TextArea faramElementName="description" label="Description" />
						{/*
                        <SelectInput
