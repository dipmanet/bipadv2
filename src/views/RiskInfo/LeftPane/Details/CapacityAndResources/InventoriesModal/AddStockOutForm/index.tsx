...
const requests: { [key: string]: ClientAttributes<ReduxProps, Params> } = {
    ...
    */
	addStockOutPostRequest: {
		url: ({ props }) =>
			props.value ? `/inventory-stockout/${props.value.id}/` : "/inventory-stockout/",
...
class AddStockOutForm extends React.PureComponent<Props, State> {
	...
	private multipleSelectFunctionType = (array, language) => {
		const data =
			...
			array.map((item) => ({
				value: item.id,
				label: language === "en" ? item.title : item.titleNe || item.title,
			}));
		return data;
	};
	...
	public render() {
		const {
			...
			// resourceList,
			requests: {
				addStockOutPostRequest: { pending },
			},
			value,
			unitList,
			categoriesList,
				...
				<Faram
					...
					<Translation>
						{(t) => (
							<>
								<ModalHeader
									title={value ? t("Edit Stock Out") : t("Add Stock Out")}
									rightComponent={
										<DangerButton
											transparent
											iconName="close"
								...
								<ModalBody>
									...
									<NumberInput faramElementName="rate" label={t("Rate")} />
									<NumberInput faramElementName="quantity" label={t("Quantity")} />
									...
									<NumberInput faramElementName="referenceNumber" label={t("Reference Number")} />

									<TextArea faramElementName="remarks" label={t("remarks")} />
