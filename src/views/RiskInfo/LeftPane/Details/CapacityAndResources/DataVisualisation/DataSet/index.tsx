...
const TableData = ({ selectedResourceData, resourceType, language: { language } }) => {
		...
		selector:
			item.key === "title"
				...
				: (row) =>
						item.key === "point1"
							? row.point
								? row.point.coordinates[1]
								: ""
							: item.key === "point2"
							? row.point
								? row.point.coordinates[0]
								: ""
							: row[item.key]
	...
	const handleDownload = useCallback((datas) => {
		...
		const csvData = datas.map((itm) =>
			tableHeader.map((data) =>
				data.key === "point1"
					? itm.point
						? itm.point.coordinates[1]
						: ""
					: data.key === "point2"
					? itm.point
						? itm.point.coordinates[0]
						: ""
					: itm[data.key]
					? typeof itm[data.key] === "boolean"
						? itm[data.key] === true
							? "Yes"
							: ""
						: itm[data.key]
	...
	const Export = ({ onExport }) => (
		<button
			...
			{language === "en" ? "Download" : "डाउनलोड गर्नुहोस्"}
		</button>
	);
	// const Export = ({ onExport }) => <DownloadButton onClick={e => onExport(e.target.value)}>Download</DownloadButton>;

	const actionsMemo = React.useMemo(
		() => <Export onExport={() => handleDownload(selectedResourceData)} />,
		[handleDownload, selectedResourceData]
	);
	const actionsMemo1 = React.useMemo(
		() => <Export onExport={() => handleDownload(selectedResourceData)} />,
		[handleDownload, selectedResourceData]
	);
	React.useEffect(() => {
