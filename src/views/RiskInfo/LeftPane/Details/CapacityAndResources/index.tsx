...
const ResourceTooltip = (props: ResourceTooltipProps) => {
	...
	if (
		...
		if (totalAreaKey) {
			totalAreaKey.value = totalAreaKey && totalAreaKey.value && `${totalAreaKey.value} sq.m`;
		}

		const usableAreaKey = filtered && filtered.find((el) => el.label === "usableArea");
		if (usableAreaKey) {
			usableAreaKey.value = usableAreaKey && usableAreaKey.value && `${usableAreaKey.value} sq.m`;
		}
			...
			{(t) => (
				<div className={styles.resourceTooltip}>
					...
					<div className={styles.content}>
						{picture ? (
							<img src={picture} alt="" style={{ maxHeight: "150px", width: "100%" }} />
						) : (
							""
						)}
						<table>
									...
									<tr key={item.label}>
										...
										<td>
											{item.value && typeof item.value === "string"
												? camelCaseToSentence(item.value)
												: item.value}
										</td>
									</tr>
								) : (
	...
	resourceGetRequest: {
		url: ({ params }) => {
			...
			const finalInventoryItems =
				params.inventoryItems && params.inventoryItems.map((i) => `inventory_item=${i}`).join("&");
			...
			return params.filterClickCheckCondition
				...
				: `/resource/?resource_type=${resource_type[0]}${
						finalInventoryItems
							? result2.length
								? `&${finalInventoryItems}`
								: finalInventoryItems
							: ""
				  }&${a.length ? a[0] : ""}&limit=-1&meta=true`;
		...
		onSuccess: ({ params, response }) => {
			...
			if (params && params.setResourceList && params.setIndividualResourceList) {
				...
				if (params.resourceType) {
					params.setIndividualResourceList(
						params.resourceType,
						resources.results,
						params.resourceType,
						resources.results
...
class CapacityAndResources extends React.PureComponent<Props, State> {
	...
	public componentDidMount() {
		const { palikaRedirect, setPalikaRedirect, handleCarActive, filters, setFilters, user, wards } =
			this.props;
	...
	public componentDidUpdate(
		prevProps: { filters: { faramValues: { region: any } } },
		prevState: { PreserveresourceCollection: any },
		snapshot: any
	) {
		const {
			faramValues: { region },
		...
		if (prevProps.filters.faramValues.region !== this.props.filters.faramValues.region) {
			...
			if (carKeys.length) {
				...
				tempResourcelistKey.map((item, index) => (tempResourceCollection[item] = []));
		...
		if (prevState.PreserveresourceCollection !== this.state.PreserveresourceCollection) {
			if (isFilterClicked) {
				...
				const filtering = PreserveresourceCollection[selectedSubCategoryName].filter(
					(d: { [x: string]: any }) =>
						selectedSubCategorynameList.includes(d[selectCategoryForinitialFilter[0].attribute])
				);
				...
				if (checked === "Checked") {
					...
					const filteredFinalCollection = {
						...finalCollection,
						[selectedSubCategoryName]: finalFiltering,
					};
	...
	private getWarehouseSubCategory = (data: any) => {
		...
		const updatedCapacityResource = capacity_resource.map((item: { id: number }) =>
			item.id === 29 ? { ...item, subCategory: finalData } : item
		);
	...
	private handleToggleClick = (
		...
		value: boolean,
		typeName: undefined,
		filteredSubCategoriesLvl2ResourceType: any[] | undefined,
		lvl2UncheckCondition: boolean | undefined
	) => {
		const {
			activeLayersIndication,
			resourceCollection,
		...
		const temp =
			filteredSubCategoriesLvl2ResourceType || key
				? { ...activeLayersIndication }
				: { ...initialActiveLayersIndication };
		...
		if (filteredSubCategoriesLvl2ResourceType) {
			const data = filteredSubCategoriesLvl2ResourceType.map(
				(item: string | number) => (temp[item] = lvl2UncheckCondition)
			);
		}
		...
		const checkingResourceCollection =
			filteredSubCategoriesLvl2ResourceType &&
			filteredSubCategoriesLvl2ResourceType
				.map((item: string | number) => !!resourceCollection[item].length)
		...
		if (
			typeName &&
			checkingResourceCollection &&
			checkingResourceCollection.length !== filteredSubCategoriesLvl2ResourceType.length
		) {
			const newArr = [];
	...
	private setIndividualResourceList = (
		key: toggleValues,
		resourceList: PageType.Resource[],
		resourceType: any[],
		Result: any[]
		...
		if (isFilterClicked) {
			...
			const mainCat = carKeys.map((i: string) =>
				capacity_resource
					.filter((d: { resourceType: string }) => d.resourceType === i)
					.map((name: { name: any }) => name.name)
			);
			const subCat = carKeys.map((i: string) =>
				capacity_resource
					.filter((d: { resourceType: string }) => d.resourceType === i)
					.map((itm: { subCategory: any[] }) =>
						itm.subCategory.map((subCate: { id: any }) => subCate.id)
					)
			);
			...
			const temp = { ...PreserveresourceCollection };
			if (lvl2TypeName) {
				const resourceLists = resourceType.map(
					(item: string | number) =>
						(temp[item] = Result.filter(
							(data: { resourceType: any }) => data.resourceType === item
						))
				);
			} else {
			...
			const subCatList =
				categoryLevel === 2
					? lvl2TypeName
						? capacity_resource
								.filter((item: { name: any }) => item.name === lvl2catName)
								.map((data: { Category: any }) => data.Category)[0]
								.filter((item: { resourceType: any }) =>
									filteredSubCategoriesLvl2ResourceType.includes(item.resourceType)
								)
								.map((r: { subCategory: any }) => r.subCategory)
						: capacity_resource
								.filter((item: { name: any }) => item.name === lvl2catName)
								.map((data: { Category: any }) => data.Category)[0]
								.filter(
									(item: { resourceType: any }) => item.resourceType === selectedSubCategoryName
								)[0]
								.subCategory.map((data: { type: any }) => data.type)
					: capacity_resource
							.filter(
								(item: { resourceType: any }) => item.resourceType === selectedSubCategoryName
							)[0]
			...
			const subCatName = lvl2TypeName
				? RawsubCatName.map((i: { type: any }) => i.type)
				: subCatList;

			const filteredOtherSubCat =
				!lvl2TypeName &&
				temp[selectedSubCategoryName].filter(
					(item: { [x: string]: any }) =>
						!subCatName.includes(item[selectCategoryForinitialFilter[0].attribute])
				);

			const finalFilteredOtherSubCat =
				!lvl2TypeName &&
				temp[selectedSubCategoryName].map((data: { [x: string]: any }) => {
					const filteredSubCatOther = filteredOtherSubCat.filter(
						(itm: { [x: string]: any }) =>
							itm[selectCategoryForinitialFilter[0].attribute] ===
							data[selectCategoryForinitialFilter[0].attribute]
					);
					return {
						...data,
						[selectCategoryForinitialFilter[0].attribute]: filteredSubCatOther.length
							? "Other"
							: data[selectCategoryForinitialFilter[0].attribute],
					};
				});
			const final = lvl2TypeName
				? { ...temp }
				: { ...temp, [selectedSubCategoryName]: finalFilteredOtherSubCat };
			...
			if (keying.length === 0) {
				...
				this.setState({
					reserveListForOtherFilter: {
						...reserveListForOtherFilter,
						[selectedSubCategoryName]: final[selectedSubCategoryName],
					},
					PreserveresourceCollection: {
						...reserveListForOtherFilter,
						[selectedSubCategoryName]: final[selectedSubCategoryName],
	...
	private getNewResourceCollection = (
		...
		const newSingleResource = [...singleResource, resource];
	...
	private handleResourceAdd = (resource: PageType.Resource) => {
		const { resourceList, resourceCollection } = this.state;
		const newResourceList = [resource, ...resourceList];
	...
	private handleResourceEdit = (
		resourceId: PageType.Resource["id"],
		resource: PageType.Resource
	) => {
		const { resourceList, resourceCollection } = this.state;
		...
		const newSingleResource = produce(
			singleResource,
			(safeSingleResource: { [x: string]: PageType.Resource }) => {
	...
	private handleClusterClick = (feature: unknown, lngLat: [number, number]) => {
		...
		if (source && map && clusterId) {
			map.getSource(source).getClusterExpansionZoom(clusterId, (error: string, zoom: number) => {
				if (!error) {
	...
	private handleResourceClick = (feature: unknown, lngLat: [number, number]) => {
		const {
			properties: { id, title, description, ward, resourceType, point },
		} = feature;
	...
	private handleLayerUnselect = () => {
		const { map } = this.context;
		const nepalBounds = [
			80.05858661752784, 26.347836996368667, 88.20166918432409, 30.44702867091792,
		];
	...
	private handlePolygonCreate = (features: MapboxGeoJSONFeature[], draw: Draw) => {
		const {
			requests: { polygonResourceDetailGetRequest },
		} = this.props;
	...
	private handleShowCommunitypaceDetails = (communityspaceDeleted?: boolean) => {
		const {
			resourceInfo: { resourceType },
			showInventoryModal,
		} = this.state;
	...
	private handleSubCategory = (
		...
		if (boolean !== undefined) {
			if (mainCategoryCheckboxChecked.find((item: any) => item === selectedResource)) {
				this.setState({
					resourceCategory: mainCategoryCheckboxChecked.filter(
						(item: any) => item !== selectedResource
					),
				});
			}
	...
	private handleMainCategoryCheckBox = (
		checkedCategory: string,
		resourceType: string,
		level: number,
		lvl2catName: string,
		typeName: string | undefined,
		showVisualization: undefined
	) => {
		const {
			mainCategoryCheckboxChecked,
			subCategoryCheckboxChecked,
		...
		if (mainCategoryCheckboxChecked.find((item: any) => item === checkedCategory)) {
			...
			const filteredSubCategories =
				level === 2
					? capacity_resource
							.filter((item: { name: string }) => item.name === lvl2catName)
							.map((data: { Category: any }) => data.Category)[0]
							.map((finalData: { id: any }) => finalData.id)
					: capacity_resource
							.filter((item: { name: string }) => item.name === checkedCategory)
							.map((data: { subCategory: any }) => data.subCategory)[0]
							.map((finalData: { id: any }) => finalData.id);
			const removedSubCategoryInUncheck = subCategoryCheckboxChecked.filter(
				(item: any) => !filteredSubCategories.includes(item)
			);
			...
			const finalCategoryCheckBoxCheckedLvl2 = typeName
				? mainCategoryCheckboxChecked.filter((item: any) => !test.includes(item))
				: mainCategoryCheckboxChecked;
			const filteredSubCategoriesLvl2ResourceType =
				typeName &&
				capacity_resource
					.filter((item: { name: string }) => item.name === lvl2catName)
					.map((data: { Category: any }) => data.Category)[0]
					.map((i: { resourceType: any }) => i.resourceType);

			// testing
			const fullCategoryNameList =
				level === 2 &&
				capacity_resource
					.filter((item: { name: string }) => item.name === lvl2catName)
					.map((data: { Category: any }) => data.Category)[0]
			...
			const mainCheckBoxChecked = mainCategoryCheckboxChecked.find(
				(item: any) => item === checkedCategory
			)
			...
			const comparisonFullCategoryData =
				fullCategoryNameList &&
				mainCheckBoxChecked.filter((item: any) => fullCategoryNameList.includes(item));

			if (
				fullCategoryNameList &&
				fullCategoryNameList.length !== comparisonFullCategoryData.length
			) {
				this.setState({
					mainCategoryCheckboxChecked: finalCategoryCheckBoxCheckedLvl2
						.filter((item: any) => item !== checkedCategory)
						.filter((data: any) => data !== lvl2catName),
					subCategoryCheckboxChecked: removedSubCategoryInUncheck,

					selectedCategoryName: checkedCategory,
					indeterminantConditionArray: comparisonFullCategoryData.length
						? [...indeterminantConditionArray, lvl2catName]
						: indeterminantConditionArray.filter((data: any) => data !== lvl2catName),
				});
			} else {
				this.setState({
					mainCategoryCheckboxChecked: finalCategoryCheckBoxCheckedLvl2.filter(
						(item: any) => item !== checkedCategory
					),
					subCategoryCheckboxChecked: removedSubCategoryInUncheck,

					selectedCategoryName: checkedCategory,
					indeterminantConditionArray: indeterminantConditionArray.filter(
						(item: any) => item !== lvl2catName
					),
				});
			}
		...
		if (mainCategoryCheckboxChecked.length === 0) {
				...
				level === 2
					? typeName
						...
						: capacity_resource
								.filter((item: { name: string }) => item.name === lvl2catName)
								.map((data: { Category: any }) => data.Category)[0]
								.filter((item: { name: any }) => item.name === checkedCategory)
			...
			const comparisonFullCategoryData =
				fullCategoryNameList &&
				mainCheckBoxChecked.filter((item) => fullCategoryNameList.includes(item));
			...
			if (test) {
				...
				this.setState({
					mainCategoryCheckboxChecked: datas,
					indeterminantConditionArray: [...indeterminantConditionArray, lvl2catName],
				});
			} else {
				this.setState({
					mainCategoryCheckboxChecked: [
				...
				if (filteredSubCategoriesLvl2ResourceType) {
					filteredSubCategoriesLvl2ResourceType.map((item: string | number) =>
						this.setState({
							resourceCollection: {
								...resourceCollection,
								[item]: PreserveresourceCollection[item],
							},
						})
					);
					this.setState({
					...
					this.setState({
						...
						resourceCollection: {
							...resourceCollection,
							[resourceType]: PreserveresourceCollection[resourceType],
						},
					});
				}
				this.setState({
	...
	private handleSubCategoryCheckbox = (
		id: number,
		checkedCategory: string,
		resourceType: string
	) => {
		...
		const filteredSubCategory = capacity_resource
			.filter((item: { name: string }) => item.name === checkedCategory)
			.map((data: { subCategory: any }) => data.subCategory)[0]
			.map((finalData: { id: any }) => finalData.id);
		this.setState({
		...
		if (subCategoryCheckboxChecked.find((item: any) => item === id)) {
			const filteredSubCategories = capacity_resource
				.filter((item: { name: string }) => item.name === checkedCategory)
				.map((data: { subCategory: any }) => data.subCategory)[0]
				.map((finalData: { id: any }) => finalData.id);
			...
			const removedSubCategoryInUncheckSameGroup = filteredSubCategories.filter((itm: any) =>
				removedSubCategoryInUncheck.includes(itm)
			);

			const selectedCategory = capacity_resource.filter(
				(item: { name: string }) => item.name === checkedCategory
			);
			const selectedSubCategorynameList = selectedCategory
				.map((data: { subCategory: any }) => data.subCategory)[0]
				.filter((ide: { id: any }) => removedSubCategoryInUncheck.includes(ide.id))
				.map((d: { name: any }) => d.name);

			if (resourceType === "warehouse") {
				const filtering = PreserveresourceCollection[resourceType].filter(
					(d: { categories: any[] }) =>
						d.categories.some((category: any) => selectedSubCategorynameList.includes(category))
				);
				...
				const filtering = PreserveresourceCollection[resourceType].filter(
					(d: { [x: string]: string }) =>
						selectedSubCategorynameList.includes(d[selectedCategory[0].attribute])
				);
			...
			const remainingCheckedSubCategory = removedSubCategoryInUncheck.filter((item: number) =>
				filteredSubCategories.includes(item)
			);

			const data = resourceCollection[resourceType].filter((item: { type: any }) =>
				removedSubCategoryInUncheck.includes(item.type)
			);
			...
			if (removedSubCategoryInUncheck.length === 0) {
				...
				removedSubCategoryInUncheckSameGroup.length > 0 &&
				removedSubCategoryInUncheckSameGroup.length !== filteredSubCategories.length
			) {
				this.setState({
			...
			if (filteredSubCategories.length !== removedSubCategoryInUncheckSameGroup.length) {
				this.setState({
					mainCategoryCheckboxChecked: mainCategoryCheckboxChecked.filter(
						(item: any) => item !== checkedCategory
					),
					// checked: CHECKBOX_STATES.Indeterminate,
					selectedCategoryName: checkedCategory,
					subCategoryCheckboxChecked: removedSubCategoryInUncheck,
					indeterminantConditionArray: [
						...new Set([...indeterminantConditionArray, checkedCategory]),
					],
				});
			}
			if (
				filteredSubCategories.length === removedSubCategoryInUncheckSameGroup.length ||
				remainingCheckedSubCategory.length === 0
			) {
				this.setState({
					indeterminantConditionArray: indeterminantConditionArray.filter(
						(item: any) => item !== checkedCategory
		...
		if (subCategoryCheckboxChecked.length === 0) {
			...
			const filteredAddedSubCategoryInUncheck = addSubCategoryInUncheck.filter((item) =>
				filteredSubCategories.includes(item)
			);
			...
			const selectedSubCategorynameList = selectedCategory
				.map((data: { subCategory: any }) => data.subCategory)[0]
				.filter((ide: { id: any }) => addSubCategoryInUncheck.includes(ide.id))
				.map((d: { name: any }) => d.name);
			...
			if (addSubCategoryInUncheck.length === 0) {
				...
				addSubCategoryInUncheck.length > 0 &&
				addSubCategoryInUncheck.length !== filteredSubCategories.length
			) {
				this.setState({
					checked: CHECKBOX_STATES.Indeterminate,
				});
	...
	private getCheckedIndexArr = () => {
		...
		const indeterminateArray = capacity_resource.map((item: { name: any }) => item.name);
		const data =
			mainCategoryCheckboxChecked.length &&
			mainCategoryCheckboxChecked.map((item: string, i: any) => indeterminateArray.indexOf(item));
	...
	private resourceProfileImage = (level: number, name: string) => {
		...
		const ResourceCategoryLevel2 = capacity_resource.filter(
			(i: { name: string }) => i.name === name
		)[0].Category;

		if (level === 1) {
			const selectedResourceProfileImage = sidepanelLogo.filter(
				(i) => i.name === ResourceCategory.resourceType
			)[0].image;
		...
		if (level === 2) {
			const selectedResourceProfileImage = sidepanelLogo.filter((item) =>
				ResourceCategoryLevel2.map((data: { resourceType: string }) => {
	...
	private handleVisualization = (
		...
		this.handleMainCategoryCheckBox(
			checkedCategory,
			resourceType,
			level,
			lvl2catName,
	...
	private handleSearchResource = (resourceType: string, id: number, name: string) => {
		const { showSearchModal, PreserveresourceCollection, showTooltip, selectedCategoryId } =
			this.state;
		const data = PreserveresourceCollection[resourceType].filter(
			(i: { resourceType: any }) => i.resourceType === resourceType
		);
	...
	public render() {
		...
		const {
			resourceDetailGetRequest: { response, pending: resourceDetailPending },
			resourceGetRequest: { pending: resourceGetPending },
			polygonResourceDetailGetRequest: { pending: polygonResourceGetPending },
		} = requests;
		...
		const pending =
			resourceDetailPending || resourceGetPending || polygonResourceGetPending || droneImagePending;
		...
		const communityspaceGeoJson = this.getGeojson(resourceCollection.communityspace);
		...
		const filteredCheckedSubCategory = filterSubCategory.filter((item: any) =>
			subCategoryCheckboxChecked.includes(item)
		);
		const showIndeterminateButton = !!(
			filteredCheckedSubCategory.length && filterSubCategory !== filteredCheckedSubCategory
		);
		...
		return (
			<>
				...
				<div className={_cs(styles.capacityAndResources, className)} id="capacityAndResources">
									...
									<header className={styles.header}>
										<div className={styles.actions}>
											...
											<DangerButton
												...
												disabled={
													!Object.values(activeLayersIndication).some(Boolean) && !activeLayerKey
												}
									...
									<Translation>
										{(t) => (
											<div key={item.name}>
												<div
													className={
														resourceCategory.find((res: string) => res === item.name)
													...
													<div style={{ marginTop: "5px" }}>
														<Checkbox
															...
															checkedCategory={
																!!mainCategoryCheckboxChecked.find(
																	(data: string) => data === item.name
																)
															}
													...
													<div
														...
														<div
															style={{
																...
																alignItems: "center",
																marginRight:
																	item.Category || item.subCategory.length ? "0px" : "26px",
															}}>
															{item.level === 1 ? (
																<>
																	<div style={{ position: "relative" }}>
																		<button
																			...
																			style={{
																				cursor: "pointer",
																				backgroundColor: resourceCategory.find(
																					(res: string) => res === item.name
																				)
																					? "#ddf2fd"
																					: "white",
																				border: "none",
																	...
																	{item.resourceType === "warehouse" ? (
																		<div style={{ height: "14px", width: "73px" }} />
																	) : (
																		<button
																			type="button"
																			style={{
															...
															{item.Category || item.subCategory.length ? (
																resourceCategory.find((res: string) => res === item.name) ? (
																	<Icon name="dropdown" className={styles.inputIconDropdown} />
																) : (
																	<Icon name="dropRight" className={styles.inputIconDropdown} />
																)
															) : (
																""
															)}
																...
																(data: {
																	...
																	<ul
																		...
																		<div
																			...
																			tabIndex={0}
																			onClick={() =>
																				this.handleSubCategory(data.name, showSubCategory)
																			}
																			onKeyDown={undefined}>
																			<input
																				type="checkbox"
																		...
																		<button
																			...
																			<Icon name="table" className={styles.inputIcon} />
																		</button>
																	</ul>
																)
														  )
																	...
																	<ul key={data.id}>
																		<div style={{ display: "flex", alignItems: "center" }}>
																			<input
																				...
																				checked={
																					!!subCategoryCheckboxChecked.find(
																						(i: number) => i === data.id
																					)
																				}
																				onChange={
																					disableCheckbox
																						? ""
																						: () =>
																								this.handleSubCategoryCheckbox(
																									data.id,
																									item.name,
																									item.resourceType
																								)
					...
					<MapImage url={HealthIcon} name="health" />
					<MapImage url={FinanceIcon} name="finance" />
					<MapImage url={FoodWarehouseIcon} name="governance" />
								...
								<>
									<MapSource
										...
										<MapLayer
											...
											layerOptions={{
												type: "circle",
												paint: mapStyles.resourceCluster.evacuationcentre,
												filter: ["has", "point_count"],
											}}
										/>
										<MapLayer
										...
										<MapLayer
											...
											layerOptions={{
												type: "circle",
												filter: ["!", ["has", "point_count"]],
												paint: mapStyles.resourcePoint.evacuationcentre,
											}}
										/>
										<MapLayer
								...
								<>
									<MapSource
										...
										<MapLayer
											...
											layerOptions={{
												type: "circle",
												paint: mapStyles.resourceCluster.warehouse,
												filter: ["has", "point_count"],
											}}
										/>
										<MapLayer
										...
										<MapLayer
											...
											layerOptions={{
												type: "circle",
												filter: ["!", ["has", "point_count"]],
												paint: mapStyles.resourcePoint.warehouse,
											}}
										/>
										<MapLayer
								...
								<>
									<MapSource
										...
										<MapLayer
											...
											layerOptions={{
												type: "circle",
												paint: mapStyles.resourceCluster.communityspace,
												filter: ["has", "point_count"],
											}}
										/>
										<MapLayer
										...
										<MapLayer
											...
											layerOptions={{
												type: "circle",
												filter: ["!", ["has", "point_count"]],
												paint: mapStyles.resourcePoint.communityspace,
											}}
										/>
										<MapLayer
										...
										{resourceLngLat && resourceInfo && (
											<MapTooltip
												...
												<ResourceTooltip
													...
													{...resourceDetails}
													onEditClick={this.handleEditClick}
													onShowInventoryClick={() => this.handleShowCommunitypaceDetails()}
									...
									{resourceInfo && <PolygonBoundaryCommunity resourceInfo={resourceInfo} />}
								</>
							)}
							{/* openspace */}
								...
								<>
									<MapSource
										...
										<MapLayer
											...
											layerOptions={{
												type: "circle",
												paint: mapStyles.resourceCluster.openspace,
												filter: ["has", "point_count"],
											}}
										/>
										<MapLayer
										...
										<MapLayer
											...
											layerOptions={{
												type: "circle",
												filter: ["!", ["has", "point_count"]],
												paint: mapStyles.resourcePoint.openspace,
											}}
										/>
										<MapLayer
										...
										{resourceLngLat && resourceInfo && (
											<MapTooltip
												...
												<ResourceTooltip
													...
													{...resourceDetails}
													onEditClick={this.handleEditClick}
													onShowInventoryClick={() => this.handleShowOpenspaceDetailsClick()}
													authenticated={authenticated}
													wardsRef={wardsRef}
													filterPermissionGranted={filterPermissionGranted}
												/>
									...
									{resourceInfo && (
										<>
											<PolygonBoundary resourceInfo={resourceInfo} />
										</>
									)}
								</>
							)}
				...
				{showInventoryModal && isDefined(resourceDetails) && isDefined(resourceDetails.id) && (
					<InventoriesModal
