...
class BoundaryComponent extends React.PureComponent<Props, State> {
	...
	private opacitySlideHandler = (e) => {
		...
		if (map.getLayer("polygon-layer")) {
			map.setPaintProperty("polygon-layer", "fill-opacity", parseInt(value, 10) / 100);
		}
		if (map.getLayer("community-sorce-layer")) {
			map.setPaintProperty("community-sorce-layer", "fill-opacity", parseInt(value, 10) / 100);
		}
	};
	...
	public render() {
		...
		// let title = '';
		if (response) {
			geoserverUrl = response.results && response.results[0] && response.results[0].geoserverUrl;
			layerName = response.results && response.results[0] && response.results[0].layerName;
			workspace = response.results && response.results[0] && response.results[0].workspace;
				...
				<div className={styles.mapLegendWrap}>
					...
					<div className={styles.legendWrap}>
						...
						<div className={styles.wrapList}>
							...
							<div className={styles.listItem}>
								<img alt="drone" src={droneIcon} className={styles.dImg} />
								...
								{geoserverUrlNotFound && (
									<span className={styles.droneWarning}>Drone image not available!</span>
								)}
							</div>
						</div>
					</div>
