...
class Details extends React.PureComponent<Props, State> {
	...
	private handleImageChange = (e) => {
		const { name } = e.target;
		this.setState({ [name]: e.target.files[0] }, () => this.submitShapeFile());
	};
	...
	private submitDetails = () => {
		const { coverImage, geoserverUrl, layerName, workspace, objectId } = this.state;

		const { openspaceId, closeModal, LoadingSuccessHalt, faramValueSetNull } = this.props;
	...
	private setSingleDetails = (details: DetailsResponse) => {
		const { id, geoserverUrl, layerName, workspace } = details;
		this.setState({
			objectId: id,
			geoserverUrl,
			layerName,
	...
	public render() {
		...
		const { objectId, openspacePostError, geoserverUrl, layerName, workspace } = this.state;
		return (
			<Translation>
				{(t) => (
					<React.Fragment>
						{resourceId === undefined && (
							<div
								...
								<span className={styles.detailsLabel}>{t("Add shape file(.zip extension)")}</span>
								<input
									name="shapeFile"
						...
						{openspacePostError && <NonFieldErrors faramElement errors={["Some error occured!"]} />}
                        ...
                        >
							...
							<PrimaryButton onClick={() => this.submitDetails()}>
								{t("Save and Continue")}
							</PrimaryButton>
						</div>
					</React.Fragment>
