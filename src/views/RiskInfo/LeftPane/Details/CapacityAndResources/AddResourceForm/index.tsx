...
const selectLabel = (d: PageType.Field, language: string) =>
	language === "en" ? d.label : d.labelNe;
...
class AddResourceForm extends React.PureComponent<Props, State> {
	public constructor(props: Props) {
		...
		const { resourceId, resourceDetails } = this.props;

		let faramValues = {};
		if (resourceDetails) {
			const { point, ward } = resourceDetails;
	...
	private handleFaramChange = (faramValues: FaramValues, faramErrors: FaramErrors) => {
		...
		if (faramValues.noOfMaleEmployee || faramValues.noOfFemaleEmployee) {
			const { noOfMaleEmployee, noOfOtherEmployee, noOfFemaleEmployee, noOfEmployee } = faramValues;
			this.setState({
				faramValues: {
					...faramValues,
					noOfEmployee:
						(noOfMaleEmployee || 0) + (noOfFemaleEmployee || 0) + (noOfOtherEmployee || 0),
		...
		if (faramValues.noOfMaleStudent || faramValues.noOfFemaleStudent) {
			const { noOfMaleStudent, noOfFemaleStudent, noOfOtherStudent } = faramValues;
			this.setState({
				faramValues: {
					...faramValues,
					noOfStudent: (noOfMaleStudent || 0) + (noOfFemaleStudent || 0) + (noOfOtherStudent || 0),
	...
	private handleFaramValidationSuccess = (_: FaramValues, faramValues: FaramValues) => {
		...
		const { setPalikaRedirect, palikaRedirect, wards } = this.props;
		...
		const {
			requests: { addResourcePostRequest, editResourcePutRequest },
		} = this.props;
	...
	private filterEnumItem = (resourceEnums: ModelEnum[], resourceType: string) => {
	...
	public render() {
		const {
			...
			modalPos,
			requests: {
				editResourcePutRequest: { pending: editResourcePending },
				addResourcePostRequest: { pending },
				addResourcePostRequest,
			},
			palikaRedirect,
			handleClearDataAfterAddition,
		...
		const { faramValues, faramErrors, pristine, resourceId, addResourcePending } = this.state;
						...
						<Faram
							...
							{faramValues.resourceType !==
								("sanitation" ||
									...
									"helipad" ||
									"bridge" ||
									"electricity") && <h1>{t("INSTITUTION DETAILS")}</h1>}
							{faramValues.resourceType ===
								("roadway" || "helipad" || "waterway" || "bridge" || "airway") && (
								<h1>{t("GENERAL DETAILS")}</h1>
							)}
							<TextInput faramElementName="title" label={t("Name")} />
							...
							{selectedAttribute.length && selectedType && selectedType.length ? (
								<SelectInput
									...
									labelSelector={typeSelector}
									label={
										faramValues.resourceType === "health"
											? t("Facility Type")
											: faramValues.resourceType === "electricity"
											? language === "en"
												? "Component"
												: "प्रकार"
											: faramValues.resourceType === "watersupply"
											? t("Scale")
											: t("Type")
									}
									autoFocus
							...
							{faramValues.type === "Fire Engine" ? (
								<NumberInput faramElementName="numberOfFireEngine" label="Number of Fire Engine" />
							) : (
								""
							)}
							{faramValues.type === "Fire Bike" ? (
								<NumberInput faramElementName="numberOfFireBike" label="Number of Fire Bike" />
							) : (
								""
							)}
							{faramValues.type === "Other" ? (
							...
							{!hideButtons && (
								...
								// Close
								//     </DangerButton>
								<div className={pristine ? styles.submitButnDisabled : styles.submitButn}>
									<PrimaryButton
										type="submit"
										disabled={pristine}
