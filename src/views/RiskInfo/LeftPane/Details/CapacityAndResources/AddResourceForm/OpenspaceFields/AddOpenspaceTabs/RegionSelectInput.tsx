...
class RegionSelectInput extends React.PureComponent<Props, State> {
	...
	private handleRegionChange = (newValue: Region) => {
		const { onChange, districtList, municipalityList, wardList } = this.props;
	...
	public render() {
		...
		const { selectedProvinceId, selectedDistrictId, selectedMunicipalityId } = this.state;

		const { provinceOptions, districtOptions, municipalityOptions } = this.getRegionOptions(
			provinceList,
			districtList,
			municipalityList,
			wardList,
...
export default connect(mapStateToProps, mapDispatchToProps)(FaramInputElement(RegionSelectInput));
