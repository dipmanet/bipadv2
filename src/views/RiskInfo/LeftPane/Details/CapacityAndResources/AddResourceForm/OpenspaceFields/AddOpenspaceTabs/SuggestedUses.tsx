...
class SuggestedUses extends React.PureComponent<Props, State> {
	...
	private handleEditSuggestedChange = (
		e: ChangeEvent<HTMLInputElement>,
		id: number,
		use: SuggestedUse
	) => {
		const {
			requests: { addSuggestedUsesRequest, deleteSuggestedUsesRequest, singleSuggestedUsesRequest },
			openspaceId,
		} = this.props;
		const { singleSuggestedUses } = this.state;
		let matchedId = [];
		const matchedIds =
			singleSuggestedUses && singleSuggestedUses.filter((s) => s.suggestedUse === use.id);
	...
	private handleSuggestedChange = (e: ChangeEvent<HTMLInputElement>, id: number) => {
		...
		this.setState((prevState) => ({
			checkedSuggestedUses: prevState.checkedSuggestedUses.find((item) => item.suggestedUse === id)
	...
	public render() {
		...
		const { suggestedUsesList, singleSuggestedUses, pristine, singleSuggestedUsesIds, error } =
			this.state;
		return (
			<React.Fragment>
				...
				{error && <NonFieldErrors faramElement errors={["Some error occured!"]} />}
