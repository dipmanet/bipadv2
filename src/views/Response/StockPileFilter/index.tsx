...
import { setInventoryCategoryListActionRP, setInventoryItemListActionRP } from "#actionCreators";
...
class StockPileFilter extends React.PureComponent {
	...
	getItemsUnits = memoize((inventoryItemList) =>
		listToMap(
			inventoryItemList,
			(item) => item.id,
			(item) => item.unit
			...
			<Translation>
				{(t) => (
					<Faram
						...
						value={faramValues}
						error={faramErrors}>
						<h3 className={styles.heading}>{t("Stockpile Items")}</h3>
						<SelectInput
							key="item"
							label={t("Item")}
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(createConnectedRequestCoordinator()(createRequestClient(requests)(StockPileFilter)));
