...
import { setIncidentActionIP, setResourceListActionRP } from "#actionCreators";
...
const requests = {
	responseRequest: {
		url: ({ props: { incidentId } }) => `/incident/${incidentId}/response/`,
	...
	incidentRequest: {
		url: ({ props: { incidentId } }) => `/incident/${incidentId}/`,
		query: {
...
class Response extends React.PureComponent<Props, State> {
	...
	public render() {
		...
		const { leftPaneExpanded, rightPaneExpanded, stockPileFilter, filter } = this.state;
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(createConnectedRequestCoordinator()(createRequestClient(requests)(Response)));
