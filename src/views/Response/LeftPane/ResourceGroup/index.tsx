...
export default class ResourceGroup extends React.PureComponent {
	...
	handleFilterButtonClick = () => {
		const { type, isFilterShown, onFilterShowClick } = this.props;
				...
				{(t) => (
					<div className={styles.summary}>
						...
						}
						{
							...
							Object.keys(aggregatedAttributes)
								.sort()
								.map((label) => (
									<TextOutput
										key={label}
						...
						}
						{
							...
							Object.keys(inventoriesSummary)
								.sort()
								.map((itemName) => (
									<TextOutput
										key={itemName}
			...
			<div className={_cs(className, styles.resourceGroup)}>
				<div className={styles.header}>
					...
					<h3 className={styles.heading}>
						<div className={styles.resourceName}>{heading}</div>
						<div className={styles.countDetails}>
							<div className={styles.numItems}>{itemsCount}</div>
							<div className={styles.separator}>/</div>
							<div className={styles.total}>{totalSize}</div>
						</div>
					</h3>
