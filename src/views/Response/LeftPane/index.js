...
const getFilterInputElement = (filterParam, show, elementProps = {}, language) => {
	const { key, type: paramType, label, labelNe, filter } = filterParam;
	...
	if (type === "number") {
		return <NumberInput {...commonProps} title={label} separator=" " />;
	}
	if (type === "boolean") {
...
class LeftPane extends React.Component {
	...
	renderFilter = () => {
		...
		const { key, filterParams } = filterItem;

		return (
			<div className={styles.resourceFilter}>
				<FaramGroup key={key} faramElementName={key}>
				...
				{(t) => (
					<div className={_cs(className, styles.filter)}>
						<div className={styles.resourceListContainer}>
							...
							/>
							<header className={styles.header}>
								<h3 className={styles.heading}>{t("Resources")}</h3>
							</header>
							<ListView
								data={resourceKeys}
