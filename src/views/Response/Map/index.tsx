...
import { incidentPointToGeojson, incidentPolygonToGeojson, resourceToGeojson } from "#utils/domain";
...
class ResponseMap extends React.PureComponent {
	...
	render() {
		...
		const { resourceLngLat, resource } = this.state;
		...
		return (
			<React.Fragment>
				...
				{resourceImages.map((image) => (
					<SVGMapIcon src={image.icon} name={image.name} fillColor="#222222" />
				))}
				<MapSource
					sourceKey="resource"
					sourceOptions={{
					...
					{resourceLngLat && resource && (
						<MapTooltip
							...
							<ResourceItem {...resource} showDetails language={language} />
						</MapTooltip>
					)}
				</MapSource>
