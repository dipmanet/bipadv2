...
import { _cs, compareDate } from "@togglecorp/fujs";
import memoize from "memoize-one";
import PropTypes from "prop-types";

import { patchIncidentActionIP, setIncidentActionIP } from "#actionCreators";
	...
	<Translation>
		{(t) => (
			<Modal className={styles.incidentTableModal}>
				<ModalHeader
					title={t("Incidents")}
					rightComponent={<Button iconName="close" onClick={closeModal} transparent />}
				/>
				<ModalBody className={styles.body}>
					<IncidentTable className={styles.table} incidentList={incidentList} />
				</ModalBody>
			</Modal>
		)}
	</Translation>
...
class LeftPane extends React.PureComponent {
	...
	render() {
		...
		const { lossServerId, incidentServerId, activeView, changedStartDate, changedEndDate } =
			this.state;
					...
					startDate={
						language === "en"
							? changedStartDate
								? BSToAD(startDate)
								: startDate
							: convertDateAccToLanguage(startDate, language)
					}
					endDate={
						language === "en"
							? changedEndDate
								? BSToAD(endDate)
								: endDate
							: convertDateAccToLanguage(endDate, language)
					}
				/>
				...
				<header className={styles.header}>
					<div className={styles.tabs}>
						<div
							...
							role="presentation">
							<div className={styles.value}>{pending ? 0 : incidentList.length}</div>
					...
					<div className={styles.actions}>
						<Cloak hiddenIf={(p) => !p.add_incident}>
							<AccentModalButton
								...
								<Translation>{(t) => <span>{t("New incident")}</span>}</Translation>
							</AccentModalButton>
						</Cloak>
						<ModalButton
							...
							iconName="table"
							transparent
							modal={<IncidentTableModal incidentList={incidentList} />}
						/>
					</div>
				</header>
