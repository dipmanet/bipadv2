...
import { groupList, saveChart } from "#utils/common";
...
class Visualizations extends React.PureComponent {
	...
	getHazardSummary = memoize((incidentList, language) => {
		...
		return freqCount
			.map((h) => ({
				label:
					language === "en" ? (hazardTypes[h.key] || {}).title : (hazardTypes[h.key] || {}).titleNe,
		...
		return (
			<div className={styles.visualizations}>
				<div className={styles.hazardStatisticsChart}>
					...
					<div className={styles.chart} id="hazardSummary">
						<h4>
							<Translation>{(t) => <span>{t("Number of Incidents")}</span>}</Translation>
						</h4>
						<ResponsiveContainer
							height={
								hazardSummary.length < 3 ? hazardSummary.length * 100 : hazardSummary.length * 50
							}>
							<BarChart
								layout="vertical"
								data={hazardSummary}
				...
				<div className={styles.severitySummary}>
					...
					<div className={styles.chart} id="hazardSeverity">
						...
						<ResponsiveContainer
							height={
								lifeLossSummary.length < 3
									? lifeLossSummary.length * 100
									: lifeLossSummary.length * 50
							}>
							<BarChart
								layout="vertical"
