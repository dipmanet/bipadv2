...
class AddFamilyLoss extends React.PureComponent<Props, State> {
	...
	private handleFaramValidationSuccess = (faramValues: FaramValues) => {
		const {
			requests: { addFamilyLossRequest },
			lossServerId,
		} = this.props;

		const { belowPoverty: id } = faramValues;

		const selected = belowPovertyOptions.find((v) => v.id === id);
					...
					<Modal className={className}>
						...
						<Faram
							...
							<ModalBody className={styles.modalBody}>
								...
								<TextInput faramElementName="ownerName" label={t("Owner Name")} autoFocus />
								<SelectInput
							...
							<ModalFooter>
								<DangerButton onClick={closeModal}>{t("Cancel")}</DangerButton>
