...
import { populateFormat, breakFormat, encodeDate, _cs } from "@togglecorp/fujs";
...
const requests: { [key: string]: ClientAttributes<ReduxProps, Params> } = {
	incidentRequest: {
		url: ({ props: { incidentServerId } }) =>
			incidentServerId ? `/incident/${incidentServerId}/` : "/incident/",
		method: ({ props: { incidentServerId } }) => (incidentServerId ? methods.PATCH : methods.POST),
		query: {
...
const getLocationDetails = (incidentDetails: {}) => {
	const { wards = [], point, ward, municipality } = incidentDetails || {};
...
class AddIncidentForm extends React.PureComponent<Props, State> {
	public constructor(props: Props) {
		...
		const { incidentDetails = {} } = this.props;
		...
		this.views = {
			general: {
				component: () => {
					...
					const {
						requests: {
							incidentRequest: { pending: incidentPending },
						},
					} = this.props;
								...
								<Cloak hiddenIf={isIncidentHidden}>
									<Faram
										...
										<ModalFooter className={styles.footer}>
											<PrimaryButton type="submit" pending={incidentPending} disabled={pristine}>
												{t("Save")}
											</PrimaryButton>
										</ModalFooter>
									</Faram>
			...
			loss: {
				component: () => {
					const { onLossChange, lossServerId, incidentServerId } = this.props;
				...
				component: () => {
					...
					return (
						<Cloak hiddenIf={isPeopleLossHidden}>
							<PeopleLossList className={styles.peopleLossList} lossServerId={lossServerId} />
						</Cloak>
					);
				},
			},
				...
				component: () => {
					...
					return (
						<Cloak hiddenIf={isFamilyLossHidden}>
							<FamiliesLossList className={styles.peopleLossList} lossServerId={lossServerId} />
						</Cloak>
					);
				},
			},
				...
				component: () => {
					...
					return (
						<Cloak hiddenIf={isLivestockLossHidden}>
							<LivestockLossList className={styles.peopleLossList} lossServerId={lossServerId} />
						</Cloak>
					);
				},
			},
	...
	private handleFaramValidationSuccess = (faramValues: FaramValues) => {
		...
		const { incidentOnDate, incidentOnTime, reportedOnDate, reportedOnTime, location, ...others } =
			faramValues;
			...
			<Translation>
				{(t) => (
					<Modal
						...
						/>
						<MultiViewContainer views={this.views} active={currentView} />
					</Modal>
				)}
			</Translation>
		);
