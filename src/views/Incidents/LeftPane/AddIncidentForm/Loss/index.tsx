...
const requests: { [key: string]: ClientAttributes<ReduxProps, Params> } = {
	...
	incidentPatchRequest: {
		...
		body: ({ params: { body } = { body: {} } }) => body,
		onSuccess: ({ props, params: { loss }, response: incidentResponse }) => {
...
class AddLoss extends React.PureComponent<Props, State> {
	...
	public render() {
		const {
			className,
			requests: {
				lossEditRequest: { pending: lossEditPending },
				lossGetRequest: { pending: lossGetPending },
				incidentPatchRequest: { pending: incidentPending },
			},
		} = this.props;
				...
				{(t) => (
					<Faram
						...
						<ModalBody className={styles.body}>
							...
							<NumberInput faramElementName="estimatedLoss" label={t("Estimated loss")} />
						</ModalBody>
