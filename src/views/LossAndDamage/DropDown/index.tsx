...
const Dropdown = (props: DropDownProps) => {
	...
	const [dropDownPlaceHolder, setdropDownPlaceHolder] = useState<string | undefined | null>(
		placeholder
	);
	...
	const searchHandler = (e) => {
		...
		const filteredDropdown = dropdownOption.filter((x) =>
			x.label.split(" ")[0].toLowerCase().includes(str)
		);
		setShowOption(true);
	...
	return (
		<>
			<div
				className={className ? _cs(styles.selectContainer, className) : styles.selectContainer}
				ref={optionShowRef}>
				{label && (
					<Translation>
						{(t) => (
							<p className={styles.labelText} style={!icon ? { width: "98%" } : {}}>
								{t(label)}
							</p>
						)}
					</Translation>
				...
				<div
					...
					<div className={styles.selectDiv} style={dropDownPlaceHolder ? { opacity: 0.5 } : {}}>
						{icon &&
						...
						<div className={styles.selectField}>
							<Translation>
								{(t) => (
									<p className={styles.selectItem}>{t(dropDownPlaceHolder) || t(selectName)}</p>
								)}
							</Translation>

							{deleteicon && (
								<span className={styles.crossIcon} onClick={clearButtonHandler}>
									<Icon name="times" />
								</span>
							)}
						...
						<div className={styles.dropDown}>
							{dropDownValues.map((item, index) => (
								<div
									...
									<Translation>
										{(t) => <p className={styles.optionName}>{t(item.label)}</p>}
									</Translation>
								</div>
							))}
						</div>
