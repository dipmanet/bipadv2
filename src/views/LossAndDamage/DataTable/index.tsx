...
const DataTable = ({
	...
	const summaryCalculate = (dat: any[]) => {
		if (focus.id !== 1) {
			const keys = [
				...new Set(
					dat.map((item: { [s: string]: unknown } | ArrayLike<unknown>) => Object.values(item)[0])
				),
			];
			const reducedData = [];
			const groupedData = [];
			...
			for (const individualArray of groupedData) {
				...
				const titleValue = Object.entries(individualArray[0]).filter(
			...
			for (const element of incidentList) {
				...
				for (const elemCur of currentVal) {
					...
					if (elemCur.key.split("").includes(".")) {
						...
						const elementDesc =
							language === "en" ? elemCur.key : elemCur.keyNe ? elemCur.keyNe : elemCur.key;
	...
	const BodyHeader = () => (
		<div
			...
			{bodyheader[Object.keys(bodyheader).filter((item) => item === focus.name)[0]].map(
				(dat: {
					type: string;
					name: {} | null | undefined;
					id: React.Key | null | undefined;
					nameNe: {} | null | undefined;
				}) => (
					<div
						...
						tabIndex={0}
						key={dat.id}>
						<p className={styles.bodyHeaderItem}>
	...
	const Row = ({ index, style }) => (
		<div style={style} className={index % 2 === 0 ? styles.colorOne : styles.colorTwo}>
			<div className={styles.bodyWrapper}>
				{Object.values(data[index]).map((item, idx) => {
					...
					return (
						<Translation>
							{(t) => (
								<p className={styles.bodyItem} key={`${index}-${`${idx}-`}`}>
									{typeof item === "number"
										...
										: t(item)}
									<span className={styles.toolTipItem}>
										{typeof item === "number" ? formatNumeralAccLang(item, language) : t(item)}
									</span>
								</p>
							)}
						</Translation>
			...
			<div
				...
				{totalTableData.map((item) => (
					<p className={styles.bodyItem} key={item.id}>
						{typeof item.val === "number" ? formatNumeralAccLang(item.val, language) : item.val}
					</p>
				))}
			</div>
		);
	...
	return (
		<>
			{/* <Spinner /> */}
			<Modal
				className={_cs(styles.lossAndDamageTableModal, language === "np" && styles.languageFont)}>
				<ModalHeader
					title={<Header />}
					rightComponent={
						<div style={{ display: "flex", marginTop: "20px" }}>
							<DownloadButton value={data} name="incidents">
