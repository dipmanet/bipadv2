...
const BarChartVisual = (props: BarchartProps) => {
	...
	const distributionCalculate = (typeKey: { name: string; id: number }[], type: string) => {
		...
		for (let i = 0; i < filteredData.length; i++) {
			...
			regiondata.push({
				[language === "en" ? typeKey[i].name : typeKey[i].nameNe]: regionWiseData,
			});
		}
		...
		if (regionRadio.adminLevel || regionRadio.name) {
			switch (true) {
				case regionRadio.adminLevel === 1 || regionRadio.name === "province":
					setChartData(
						distributionCalculate(provinceIndex, "province").sort((a, b) => b.value - a.value)
					);
					break;
	...
	function nameReturn(region: RadioValue) {
		if (region.name === "district" || region.name === "municipality" || region.name === "ward") {
			...
			return language === "en" ? "Ward-wise distribution (Top 10)" : "वडा अनुसार (शीर्ष १०)";
		}
		if (region.name === "province") {
			return language === "en" ? `${regionRadio.name}-wise distribution` : "प्रदेश अनुसार";
		}
		if (region.adminLevel === 1) {
			return language === "en" ? "Province-Wise distribution" : "प्रदेश अनुसार";
		}
		if (region.adminLevel === 2) {
			return language === "en" ? "District-Wise distribution" : "जिल्‍ला अनुसार";
		}
		return language === "en" ? "Municipality-Wise distribution" : "नगरपालिका अनुसार";
	}
				...
				{barChartData && barChartData.length > 0 && (
					<ResponsiveContainer width={fullScreen.width} height={fullScreen.height}>
						<BarChart
							...
							/>
							<YAxis axisLine={false} tickLine={false} tick={<CustomizedLabel />} dx={-30} />
