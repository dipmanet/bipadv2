...
import { getGroupMethod, getGroupedIncidents, getAggregatedStats } from "../common";
			...
			<Translation>
				{(t) => (
					<p
						...
						{language === "en"
							? `No of ${t(currentSelection)}: ${formatNumeralAccLang(
									feature.state.value,
									language
							  )}`
...
export const generateMapState = memoize((geoareas, groupedIncidentMapping, metricFn) => {
	const value = geoareas.map((geoarea) => ({
		id: geoarea.id,
		value: groupedIncidentMapping ? metricFn(groupedIncidentMapping[geoarea.id]) : 0,
	}));
	return value;
});
...
export const generatePaintLegendByInterval = (data, parts, color, language) => {
	...
	const max = arrayData.length ? Math.floor(arrayData.reduce((a, b) => (a > b ? a : b))) : null;
	const min = arrayData.length ? Math.floor(arrayData.reduce((a, b) => (a < b ? a : b))) : null;
	...
	for (let index = 1; index <= parts; index++) {
		if (index === 1) {
			...
			colorLegend.push({
				name: `${formatNumeralAccLang(min, language)} - ${formatNumeralAccLang(dat, language)}`,
				range: [min, dat],
				color: color[index - 1],
			});
		} else if (dat <= max) {
			...
			dat += 1;
			colorLegend.push({
				name: `${formatNumeralAccLang(dat, language)} - ${
					dat + interval > max
	...
	const paintColor = {
		"fill-color": ["step", ["feature-state", "value"], ...colorInterval.slice(0, -1)],
		"fill-opacity": [
			"case",
