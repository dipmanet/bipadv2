...
const NumeralCell = ({ value }) => (
	<Numeral className={styles.numeral} value={value} precision={0} />
);
...
export default class TabularView extends React.PureComponent {
	...
	render() {
		const { className, lossAndDamageList } = this.props;
		...
		return (
			<div className={_cs(className, styles.tabularView)}>
				...
				<div className={styles.downloadLinkContainer}>
					<DownloadButton onClick={this.handleClick} value={lossAndDamageListForExport}>
						Download csv
					</DownloadButton>
				</div>
			</div>
