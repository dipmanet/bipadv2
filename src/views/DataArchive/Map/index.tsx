...
const DataArchiveToolTip = ({ renderer: Renderer, params }) => <Renderer {...params} />;
...
class DataArchiveMap extends React.PureComponent {
	...
	earthquakeTooltipRenderer = ({ address, description, eventOn, magnitude }) => (
		<div className={styles.tooltip}>
			<div className={styles.header}>
				<h3>{address}</h3>
				<span className={this.magnitudeClassSelector(magnitude)}>{magnitude} ML</span>
			</div>
			...
			<div className={styles.eventOn}>
				...
				<div className={styles.value}>
					<FormattedDate value={eventOn} mode="yyyy-MM-dd hh:mm" />
				</div>
			</div>
		</div>
	);
	...
	render() {
		...
		const { tooltipRenderer, tooltipParams, coordinates } = this.state;

		const earthquakeFeatureCollection = this.getEarthquakeFeatureCollection(data);
		const pollutionFeatureCollection = this.getPollutionFeatureCollection(data);
		...
		return (
			<div className={styles.dataArchiveMap}>
				<CommonMap sourceKey="realtime" boundsPadding={24} />
				{coordinates && (
					<MapTooltip
						coordinates={coordinates}
						tooltipOptions={tooltipOptions}
						onHide={this.handleTooltipClose}>
						<DataArchiveToolTip renderer={tooltipRenderer} params={tooltipParams} />
					</MapTooltip>
				)}
				<MapSource
