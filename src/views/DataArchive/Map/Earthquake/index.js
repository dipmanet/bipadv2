...
const EarthquakeToolTip = ({ renderer: Renderer, params }) => <Renderer {...params} />;
...
class EarthquakeMap extends React.PureComponent {
	...
	render() {
		...
		const { regionLevel, provinceLevel, districtLevel, municipalityLevel } =
			getRegionDetails(region);
		const regionBoundings = getRegionBoundings(region, provinces, districts, municipalities);
		return (
			<div className={styles.dataArchiveEarthquakeMap}>
				...
				{coordinates && (
					<MapTooltip
						...
						<EarthquakeToolTip renderer={tooltipRenderer} params={tooltipParams} />
					</MapTooltip>
				)}
				<MapSource
