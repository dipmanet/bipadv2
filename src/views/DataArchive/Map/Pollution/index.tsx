...
const PollutionToolTip = ({ renderer: Renderer, params }) => <Renderer {...params} />;
...
const nepalBounds = [80.05858661752784, 26.347836996368667, 88.20166918432409, 30.44702867091792];
...
class PollutionMap extends React.PureComponent {
	...
	handlePollutionClick = (feature, lngLat) => {
		const {
			properties: { title, aqi, description, dateTime: measuredOn, station: stationId },
			geometry,
		} = feature;
		this.setState({
	...
	render() {
		const { data, pollutionFilters, rightPaneExpanded, leftPaneExpanded } = this.props;
			...
			<div className={styles.dataArchivePollutionMap}>
				...
				{coordinates && (
					<MapTooltip
						...
						<PollutionToolTip renderer={tooltipRenderer} params={tooltipParams} />
					</MapTooltip>
				)}
				<MapSource
