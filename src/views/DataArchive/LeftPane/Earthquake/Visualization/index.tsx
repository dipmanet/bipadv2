...
const withFederalsList = (earthquakeList: PageType.DataArchiveEarthquake[]) => {
	const withFederals = earthquakeList.map((earthquakeItem) => {
		const provinceTitle =
			earthquakeItem && earthquakeItem.province ? earthquakeItem.province.title : "";
		const districtTitle =
			earthquakeItem && earthquakeItem.district ? earthquakeItem.district.title : "";
		const municipalityTitle =
			earthquakeItem && earthquakeItem.municipality ? earthquakeItem.municipality.title : "";
...
const getMagCount = (
	...
	const sortedData = visualData.sort((a, b) => (a.eventOn < b.eventOn ? 1 : -1));
	return sortedData;
};
...
const EarthquakeViz = (props: Props) => {
	...
	const earthquakeTemporalSummary = getEarthquakeTemporalSummary(earthquakeWithTemporals, "year");
