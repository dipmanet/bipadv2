...
import { setDataArchiveEarthquakeListAction } from "#actionCreators";
...
const requestOptions: { [key: string]: ClientAttributes<ReduxProps, Params> } = {
	dataArchiveEarthquakeRequest: {
		...
		onSuccess: ({ response, props: { setDataArchiveEarthquakeList } }) => {
			interface Response {
				results: PageType.DataArchiveEarthquake[];
			}
...
const Earthquake = (props: Props) => {
	...
	const { earthquakeList, requests, eqFilters, provinces, districts, municipalities } = props;
	const pending = isAnyRequestPending(requests);
	const filteredEarthquakes = filterByMagnitudeRange(eqFilters.magnitude, earthquakeList);

	const { setData } = useContext(DataArchiveContext);
	...
	if (setDataArchive) {
		setDataArchive((prevState: DataArchive) => {
			if (
				prevState.mainModule !== "Earthquake" ||
	...
	return (
		<div className={styles.earthquake}>
			...
			<TopBar eqFilters={eqFilters} earthquakeList={filteredEarthquakes} />
			...
			{activeView === "data" &&
				groupedEarthquakeList.map((group) => {
					const { key, value } = group;
					if (value.length > 1) {
						return <EarthquakeGroup address={key} data={value} key={key} />;
					}
					return <EarthquakeItem key={key} data={value[0]} />;
				})}
			{activeView === "visualizations" && (
				<EarthquakeViz earthquakeList={filteredEarthquakes} eqFilters={eqFilters} />
			)}
			{activeView === "charts" && (
				<ScatterChartViz
