...
import { setDataArchiveRainListAction, setDataArchiveRainStationAction } from "#actionCreators";
...
const requestOptions: { [key: string]: ClientAttributes<ReduxProps, Params> } = {
	dataArchiveRainRequest: {
		...
		onSuccess: ({ params, response, props: { setDataArchiveRainList } }) => {
			interface Response {
				results: PageType.DataArchiveRain[];
			}
			const { results: dataArchiveRainList = [] } = response as Response;

			if (params.basinFilter !== undefined) {
				setDataArchiveRainList({
					dataArchiveRainList: dataArchiveRainList.filter(
...
const Rain = (props: Props) => {
	...
	useEffect(() => {
		if (rainFilters.basin) {
			if (props.rainStations && props.rainStations.length > 0) {
				props.setDataArchiveRainStations({
					dataArchiveRainStations: props.rainStations.filter(
						(item) => item.basin === rainFilters.basin.title
					),
				});
			}
		}
		...
		if (rainFilters.basin) {
			if (props.rainList && props.rainList.length > 0) {
				props.setDataArchiveRainList({
					dataArchiveRainList: props.rainList.filter(
						(item) => item.basin === rainFilters.basin.title
					),
				});
			}
		...
		if (rainFilters.station) {
			if (props.rainStations && props.rainStations.length > 0) {
				props.setDataArchiveRainStations({
					dataArchiveRainStations: props.rainStations.filter(
						(item) => item.id === rainFilters.station.id
					),
				});
			}
		}
	...
	return (
		<div className={styles.rain}>
			...
			<TopBar rainList={rainList} startDate={startDate} endDate={endDate} />
			<div className={styles.header}>
				<Header dataCount={rainList && rainList.length} />
			...
			{groupedRainList.map((group) => {
				...
				if (value.length > 1) {
					return <RainGroup title={key} data={value} key={key} />;
				}
