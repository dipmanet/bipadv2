...
const noLegend = [{ color: "transparent", label: "No legends to display", style: styles.noSymbol }];
...
export const getAutoRealTimeRiverLegends = (
	...
	const extractedLegendItems = dataList.map((item) =>
		`${item.status} and ${item.steady || "steady"}`.toUpperCase()
	);
...
const RiverLegend = (props: Props) => {
	...
	const autoRiverLegends = getAutoRealTimeRiverLegends(props.riverStation || [], riverLegendItems);

	return (
		<div className={styles.legendContainer}>
			<header className={styles.header}>
				<ScalableVectorGraphics className={styles.legendIcon} src={RiverIcon} alt="River" />
				<h4 className={styles.heading}>River</h4>
			</header>
			<Legend
				className={styles.legend}
				data={(props.riverStation || []).length ? autoRiverLegends : noLegend}
