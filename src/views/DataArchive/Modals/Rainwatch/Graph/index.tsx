...
const getChartTitle = (chartTitle: string, stationName: string, date: string) =>
...
const Graph = (props: Props) => {
	...
	useEffect(() => {
		if (filterWiseChartData && filterWiseChartData.length > 0) {
			...
			const datawithCumulative = filterWiseChartData.map((item) => {
				if (!isNaN(item.accHourly)) {
					cumulative += item.accHourly;
				} else {
					cumulative += 0;
				}

				if (!isNaN(item.accDaily)) {
	...
	useEffect(() => {
		if (cumulativeData && cumulativeData.length > 0) {
			...
			const uniquemonthArr = [
				...new Set(
					cumulativeData.map((item) => `${item.key.split("-")[0]}-${item.key.split("-")[1]}`)
				),
			];
	...
	const calculatedTitle = getChartTitle(chartTitle || "", stationName, date);
	...
	return (
		<div className={styles.visualizations}>
			<div className={styles.periodIntervalChart}>
				...
				<div className={styles.chartWrapper} id={downloadId || DEFAULT_DOWNLOAD_ID}>
					<h4 className={styles.heading}>{calculatedTitle || DEFAULT_CHART_TITLE}</h4>
						...
						<ResponsiveContainer className={styles.container}>
							{periodCode === "monthly" && monthlyChartData.length > 0 ? (
								<ComposedChart
									...
									<YAxis />
									<Tooltip
										content={<CustomTooltip periodCode={periodCode} intervalCode={intervalCode} />}
									/>
								...
								<ComposedChart
									...
									<Tooltip
										content={<CustomTooltip periodCode={periodCode} intervalCode={intervalCode} />}
									/>
									<ReferenceLine y={60} stroke="red" strokeWidth={2} isFront strokeDasharray="3 2">
								...
								<ComposedChart
									...
									<Tooltip
										content={<CustomTooltip periodCode={periodCode} intervalCode={intervalCode} />}
									/>
									<ReferenceLine y={140} stroke="red" strokeWidth={2} isFront strokeDasharray="3 2">
