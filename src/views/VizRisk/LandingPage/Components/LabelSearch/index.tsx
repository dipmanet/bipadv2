...
const LabelSearch = (props) => {
	...
	const availableData =
		vzLabel === "municipality" ? [...vzRiskMunicipalData] : [...vzRiskProvinceData];

	const handleChange = (e: any) => {
		setInputValue(e.target.value);
		const val = e.target.value;
		const filterDataOnChange = availableData.filter(
			(data) =>
	...
	return (
		<div className={styles.inputContainer}>
			<div>
				...
				/>
				<img className={styles.search} src={searchVizRisk} alt="Search" />
					...
					<div className={styles.searchData}>
						{filteredData.map((data, index) => (
							<button
								...
								<p
									className={styles.vizriskNames}
									style={
										cursor === index
											? { backgroundColor: "#00000060" }
											: { backgroundColor: "transparent" }
									}>
									{data.name}-{data.visriskType}
								</p>
							</button>
						))}
					</div>
