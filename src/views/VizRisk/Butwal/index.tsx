...
const { layers, mapCSS, zoom, hillshadeLayerName, incidentsPages, ciPages, incidentsSliderDelay } =
	mapConstants;
...
export const Butwal = (props) => {
		...
		if (filteredIL.length > 0) {
			...
			if (inciTotal.length > 0) {
				const incidentDetails = inciTotal.reduce((a, b) => ({
					...
					peopleMissingCount: (a.peopleMissingCount || 0) + (b.peopleMissingCount || 0),
	...
	const THEME_ID = 101;
	const pickKeyName = htmlData
		.filter((item) => item.key.includes("page3_top_introhtml"))
		.map((item) => item.key)[0];
	...
	const { regions, hazardTypes, hazards } = props;
	useEffect(() => {
	...
	const sanitizedIncidentList = getSanitizedIncidents(incidentList, regions, hazardTypes);
	const pointFeatureCollectionButwal = incidentPointToGeojson(sanitizedIncidentList, hazards);
	...
	useEffect(() => {
		if (pending) {
			if (incidentList.length > 0 && cI.length > 0 && htmlData.length > 0 && jsonData.length > 0) {
				setpending(false);
			}
		}
