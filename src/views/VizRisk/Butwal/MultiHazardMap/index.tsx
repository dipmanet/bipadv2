...
import { generatePaintByQuantile, getWardFilter, incidentPointToGeojson } from "#utils/domain";
...
const MultiHazardMap = (props: Props) => {
	...
	const calculateHeight = (density) => {
		...
		if (density > range2[0] && density < range2[1]) {
			extrusionHeight = 200;
			return extrusionHeight;
		}
		if (density > range3[0] && density < range3[1]) {
			extrusionHeight = 300;
			return extrusionHeight;
		}
		if (density > range4[0] && density < range4[1]) {
			extrusionHeight = 400;
			return extrusionHeight;
		}
		extrusionHeight = 500;
	...
	const updateArea = () => {
		...
		const coordList = dataArr[0].map((position) => [
	...
	useEffect(() => {
		...
		const mapping = wards
			.filter((item) => item.municipality === municipalityId)
			.map((item) => ({
				...item,
	...
	useEffect(() => {
		if (map.current && map.current.isStyleLoaded()) {
			// -----------------------------------------------------First Page-------------------------------------------------
			if (rightElement <= layers.length - 1 && layers[rightElement].length > 0) {
			...
			// -----------------------------------------------------Third Page-------------------------------------------------
			if (
				(ciPages && ciPages.indexOf(rightElement + 1) !== -1 && clickedArr[0] === 1) ||
			...
			if (
				(rightElement === 2 && clickedArr[3] === 1) ||
				(rightElement === 3 && exposureElementsArr[0] === 1)
			) {
				if (map.current) {
			...
			if (
				(rightElement === 0 && legendElement === "Landcover") ||
				(rightElement === 2 && clickedArr[2] === 1) ||
				(rightElement === 3 && exposureElementsArr[3] === 1)
			) {
				if (map.current) {
			...
			if (rightElement === 4) {
				if (map.current) {
					map.current.setLayoutProperty(
						`satelliteImageMain_${selectedYear}`,
						"visibility",
						"visible"
					);
				}
	...
	useEffect(() => {
		if (rightElement === 6) {
			if (vulnerabilityData.length > 0 && map.current) {
				const buildingsD = vulnerabilityData
	...
	const handleBuildingClick = (clicked: boolean, search) => {
		...
		if (searchId) {
			const coordinatesObj = buildings.features.filter(
				(b) => Number(searchId) === Math.round(b.properties.osm_id)
			);
			let coordinate = [];
			...
			<div style={mapCSS} ref={mapContainerRef}>
				...
				<div>
					...
					{rightElement === 3 && hazardLegendClickedArr[2] === 1 && (
						<>
							<p className={_cs(styles.sliderLabel3)}>Seismic Layer Opacity</p>
							<input
