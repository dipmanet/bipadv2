export default {
    0: {
        title: '',
        type: 'hexagon',
        legends: [
            { key: 1, color: 'rgb(209,203,111)', label: 'Landslide' },
        ],
    },
    1: {
        title: '',
        type: 'hexagon',
        legends: [
            { key: 1, color: 'rgb(209,203,111)', label: 'Landslide' },
        ],
    },
    22: {
        title: 'Landslides in Bahrabise',
        type: 'hexagon',
        legends: [
            { key: 1, color: 'rgb(209,203,111)', label: 'Landslide' },
        ],
    },
    13: {
        title: 'Landuse',
        type: 'hexagon',
        legends: [
            { key: 1, color: '#99ca91', label: 'Forest' },
            { key: 2, color: 'rgb(187,245,107)', label: 'Farmland' },
            { key: 3, color: '#fff', label: 'Bridge' },
            { key: 4, color: '#ada29f', label: 'Road' },
            { key: 5, color: '#0670bc', label: 'Water Bodies' },
            { key: 6, color: '#aaa', label: 'Buildings' },
            { key: 7, color: 'rgb(247,246,241)', label: 'Other' },
        ],
    },
    15: {
        title: 'Demography',
        type: 'square',
        legends: [
            { key: 1, color: 'rgb(217,72,1)', label: '4288' },
            { key: 2, color: 'rgb(241,105,19)', label: '3000-4000' },
            { key: 3, color: 'rgb(253,174,107)', label: '2000-3000' },
            { key: 4, color: 'rgb(254, 230, 206)', label: '1000-2000' },
        ],
    },
    8: {
        title: 'LandSlide Susceptibility',
        type: 'square',
        legends: [
            { key: 1, color: 'rgb(8,120,188)', label: '0' },
            { key: 2, color: 'rgb(47,140,197)', label: '0.05' },
            { key: 3, color: '#5FA1CB', label: '0.1' },
            { key: 4, color: 'rgb(120,180,213)', label: '0.15' },
            { key: 5, color: 'rgb(146,198,223)', label: '0.2' },
            { key: 6, color: 'rgb(171,217,233)', label: '0.25' },
            { key: 7, color: 'rgb(188,225,225)', label: '0.3' },
            { key: 8, color: 'rgb(205,233,217)', label: '0.35' },
            { key: 9, color: 'rgb(222,240,208)', label: '0.40' },
            { key: 10, color: 'rgb(239,248,200)', label: '0.45' },
            { key: 11, color: 'rgb(254,251,191)', label: '0.50' },
            { key: 12, color: 'rgb(254,239,172)', label: '0.55' },
            { key: 13, color: 'rgb(252,223,154)', label: '0.60' },
            { key: 14, color: 'rgb(249,207,135)', label: '0.65' },
            { key: 15, color: 'rgb(247,189,115)', label: '0.70' },
            { key: 16, color: 'rgb(245,173,96)', label: '0.75' },
            { key: 17, color: 'rgb(242,144,83)', label: '0.80' },
            { key: 18, color: 'rgb(238,114,69)', label: '0.85' },
            { key: 19, color: 'rgb(231,84,55)', label: '0.90' },
            { key: 20, color: 'rgb(223,55,47)', label: '0.95' },
            { key: 21, color: 'rgb(215,50,45)', label: '1' },

        ],
    },
    9: {
        title: 'LandSlide Risk',
        type: 'square',
        legends: [
            { key: 1, color: 'rgb(46,133,194)', label: '0' },
            { key: 2, color: 'rgb(93,145,185)', label: '0.10' },
            { key: 5, color: 'rgb(158,225,195)', label: '0.20' },
            { key: 7, color: 'rgb(230,245,152)', label: '0.30' },
            { key: 9, color: 'rgb(224,251,140)', label: '0.40' },
            { key: 11, color: 'rgb(252,224,139)', label: '0.50' },
            { key: 13, color: 'rgb(253,255,183)', label: '0.60' },
            { key: 15, color: 'rgb(255,227,124)', label: '0.70' },
            { key: 17, color: 'rgb(255,173,74)', label: '0.80' },
            { key: 17, color: 'rgb(213,98,35)', label: '0.90' },
            { key: 17, color: 'rgb(229,33,67)', label: '1+' },

        ],
    },
};
