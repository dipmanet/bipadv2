	...
	useEffect(() => {
		if (tempSelectedData === "temp2010" && climateDataType === "Temperature") {
			setclimateBarChartData(
				climateDataYearWise.tempDataForMapUpto2010.map((item) => ({
					value: item.value,
		...
		if (tempSelectedData === "temp2045" && climateDataType === "Temperature") {
			setclimateBarChartData(
				climateDataYearWise.tempDataForMapUpto2045.map((item) => ({
					value: item.value,
					name: districtIdToName(item.id),
				}))
			);
		...
		if (tempSelectedData === "temp2065" && climateDataType === "Temperature") {
			setclimateBarChartData(
				climateDataYearWise.tempDataForMapUpto2065.map((item) => ({
					value: item.value,
					name: districtIdToName(item.id),
				}))
			);
		...
		if (prepSelectedData === "prep2010" && climateDataType === "Precipitation") {
			setclimateBarChartData(
				climateDataYearWise.prepDataForMapUpto2010.map((item) => ({
					value: item.value,
					name: districtIdToName(item.id),
				}))
			);
		...
		if (prepSelectedData === "prep2045" && climateDataType === "Precipitation") {
			setclimateBarChartData(
				climateDataYearWise.prepDataForMapUpto2045.map((item) => ({
					value: item.value,
					name: districtIdToName(item.id),
				}))
			);
		...
		if (prepSelectedData === "prep2065" && climateDataType === "Precipitation") {
			setclimateBarChartData(
				climateDataYearWise.prepDataForMapUpto2065.map((item) => ({
					value: item.value,
					name: districtIdToName(item.id),
				}))
			);
				...
				{(leftElement === 4 || leftElement === 6) && (
					<>
						<div className={styles.hazardContainer}>
							...
							{["Flood Hazard", "Landslide Hazard"].map((item, i) => (
								<div
									className={
										legentItemDisabled
											? styles.incidentsLegendsContainer3Disabled
						...
						<ResponsiveContainer className={styles.respContainer} width="100%" height={"60%"}>
							<BarChart
								...
								<XAxis
									...
									<Label
										value={
											climateDataType === "Temperature" ? "Temperature(℃)" : "Precipitaion(mm)"
										}
										offset={-10}
										position="insideBottom"
										style={{
								...
								<Tooltip content={climateTooltip} cursor={{ fill: "#1c333f" }} />
								<Bar
									dataKey="value"
									fill="rgb(0,219,95)"
									barSize={15}
