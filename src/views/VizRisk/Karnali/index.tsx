...
const { layers, mapCSS, zoom, incidentsPages, ciPages, incidentsSliderDelay } = mapConstants;
...
export const Karnali = (props: Props) => {
	...
	useEffect(() => {
		if (pending) {
			if (
				...
				tempData.length > 0 &&
				lossDataFlood.length > 0 &&
				lossDataLandslide.length > 0 &&
	...
	const tempDataTo2010: RCPData[][] = tempData.map((item) =>
		item.rcp45.filter((obtVal) => obtVal.year <= 2010)
	);

	const tempDataForMapUpto2010 = tempData.map((item, i) => ({
		value: meanValueCalculator(tempDataTo2010[i]),
		id: item.district,
	}));

	const tempDataTo2045 = tempData.map((item) =>
		item.rcp45.filter((obtVal) => obtVal.year <= 2045 && obtVal.year >= 2010)
	);

	const tempDataForMapUpto2045 = tempData.map((item, i) => ({
		value: meanValueCalculator(tempDataTo2045[i]),
		id: item.district,
	}));

	const tempDataTo2065 = tempData.map((item) =>
		item.rcp45.filter((obtVal) => obtVal.year >= 2036 && obtVal.year <= 2065)
	);

	const tempDataForMapUpto2065 = tempData.map((item, i) => ({
		value: meanValueCalculator(tempDataTo2065[i]),
		id: item.district,
	}));

	const prepDataTo2010 = precipitationData.map((item) =>
		item.rcp45.filter((obtVal) => obtVal.year <= 2010)
	);

	const prepDataForMapUpto2010 = precipitationData.map((item, i) => ({
		value: meanValueCalculator(prepDataTo2010[i]),
		id: item.district,
	}));

	const prepDataTo2045 = precipitationData.map((item) =>
		item.rcp45.filter((obtVal) => obtVal.year <= 2045 && obtVal.year >= 2010)
	);

	const prepDataForMapUpto2045 = precipitationData.map((item, i) => ({
		value: meanValueCalculator(prepDataTo2045[i]),
		id: item.district,
	}));

	const prepDataTo2065 = precipitationData.map((item) =>
		item.rcp45.filter((obtVal) => obtVal.year >= 2036 && obtVal.year <= 2065)
	);

	const prepDataForMapUpto2065 = precipitationData.map((item, i) => ({
		value: meanValueCalculator(prepDataTo2065[i]),
		id: item.district,
	}));
	...
	const handleNext = () => {
		setCIState(true);
		if (leftElement < leftelements.length) {
	...
	const handleMultipeLegendClicked = (legendClicked: string, i: number) => {
		...
		if (legendClicked === "Population Density" && clickedCur[3] === 1) {
			...
			clickedCur[3] === 1 &&
			(clickedCur[0] === 1 || clickedCur[2] === 1 || clickedCur[1] === 1)
		) {
			clickedCur[3] = 0;
			setclicked(clickedCur);
		} else {
	...
	const handleMultipleExposure = (exposureItem: string, i: number) => {
		...
		if (
			exposureItem === "Population Density" &&
			curExposure[1] === 1 &&
			curExposure[2] === 1 &&
			curExposure[3] === 1 &&
			curExposure[4] === 1
			...
			curExposure = [1, 0, 0, 0];
			setexposureElementsArr(curExposure);
		} else if (curExposure[0] === 1 || curExposure[1] === 1 || curExposure[3] === 1) {
			curExposure[0] = 0;
			setexposureElementsArr(curExposure);
		}
		setexposureElementsArr(curExposure);
	...
	useEffect(() => {
		...
		precipitationDataGetRequest.setDefaultParams({
			setPrecipitationData,
		});
		precipitationDataGetRequest.do();
	...
	const page1TopIntrohtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page1_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page1Legend1TopIntrohtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page1_legend1_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page1Legend2TopIntrohtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page1_legend2_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page1Legend3TopIntrohtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page1_legend3_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page2TopIntrohtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page2_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page3TopIntrohtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page3_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page3Legend1Introhtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page3_legend1_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page3Legend2Introhtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page3_legend2_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page3Legend3Introhtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page3_legend3_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page3Legend4Introhtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page3_legend4_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page4TopIntrohtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page4_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page4Legend1Introhtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page4_legend1_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page4Legend2Introhtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page4_legend2_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page4Legend3Introhtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page4_legend3_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page5TopIntrohtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page5_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page6TopIntrohtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page6_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page7TopIntrohtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page7_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page8TopIntrohtml = htmlData
		.filter((item) => item.key === `${KEYNAME}_page8_top_introhtml_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page1Legend1MidClimateData = jsonData
		.filter(
			(item) => item.key === `${KEYNAME}_page1_legend1_mid_climatedata_${THEME_ID}_${SUFFIXID}`
		)
		.map((item) => item.value);

	const page1Legend1BottomTempData = jsonData
		.filter(
			(item) =>
				item.key === `${KEYNAME}_page1_legend1_bottom_temperaturedata_${THEME_ID}_${SUFFIXID}`
		)
		.map((item) => item.value);

	const page1Legend2MidLandcoverData = jsonData
		.filter(
			(item) => item.key === `${KEYNAME}_page1_legend2_mid_landcoverdata_${THEME_ID}_${SUFFIXID}`
		)
		.map((item) => item.value);

	const page1Legend3MidPopulationData = jsonData
		.filter(
			(item) => item.key === `${KEYNAME}_page1_legend3_mid_populationdata_${THEME_ID}_${SUFFIXID}`
		)
		.map((item) => item.value);

	const page3MidLandcoverData = jsonData
		.filter((item) => item.key === `${KEYNAME}_page3_mid_landcoverdata_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const page5MidUrbanData = jsonData
		.filter((item) => item.key === `${KEYNAME}_page5_mid_urbandata_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const populationGridData = jsonData
		.filter((item) => item.key === `${KEYNAME}_population_grid_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const populationDensityRange = jsonData
		.filter((item) => item.key === `${KEYNAME}_population_density_range_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const satelliteImageYears = jsonData
		.filter((item) => item.key === `${KEYNAME}_satellite_image_year_layer_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const floodHazardLayers = jsonData
		.filter((item) => item.key === `${KEYNAME}_flood_hazard_layers_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const buildingCountData = jsonData
		.filter((item) => item.key === `${KEYNAME}_building_count_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value);

	const mapBoxStyle = jsonData
		.filter((item) => item.key === `${KEYNAME}_mapbox_layer_${THEME_ID}_${SUFFIXID}`)
		.map((item) => item.value.layername);

	const realTimeDataStationName = String(
		jsonData
			.filter((item) => item.key === `${KEYNAME}_realtime_datasource_${THEME_ID}_${SUFFIXID}`)
			.map((item) => item.value.stationName)[0]
	);
		...
		for (let index = 0; index < alertsName.length; index++) {
			...
			const mainAlert = {
				name: alertsName[index]
					? alertsName[index].charAt(0).toUpperCase() + alertsName[index].slice(1)
					: "No data",
				count: realData,
	...
	const totalFloodLossData = lossDataFlood.map((item) => ({
		...
		totalEstimatedLoss: isNaN(item.data && item.data.estimatedLoss)
			? 0
			: item.data && item.data.estimatedLoss,
	}));
	...
	const totalLandslideLossData = lossDataLandslide.map((item) => ({
		...
		totalInfraDamage: isNaN(item.data.infrastructureAffectedCountSum)
			? 0
	...
	return (
		<>
			...
			{pending ? (
				...
				leftElement === 0 &&
				legendElement === "Adminstrative Map" && (
