...
const PopupOnMapClick = (props: Props) => {
	...
	const popUpExposureData =
		houseData && Object.keys(houseData).length > 0
			? [
					{
						name: "Building Type",
	...
	const popUpHazardData =
		houseData && Object.keys(houseData).length > 0
			? [
					{
						name: "Flood return period(yrs)",
						value: houseData.metadata.returnPeriod
							? houseData.metadata.returnPeriod
							: "Likely not to be inundated",
					},
			  ]
			: [];

	const popUpSensitivityData =
		houseData && Object.keys(houseData).length > 0
			? [
					{
						name: "Ethnicity",
						value:
							houseData.metadata.houseOwnerEthnicity &&
					...
					},
					{
						...
						value: houseData.metadata.nearestWardOffice
							? houseData.metadata.nearestWardOffice.toFixed(2)
							: "N/A",
					},
					{
					...
					},
					{
						...
						value: houseData.metadata.nearestMunicipalityOffice
							? houseData.metadata.nearestMunicipalityOffice.toFixed(2)
							: "N/A",
					},
					{
	...
	const popUpAdaptiveCapacityData =
		houseData && Object.keys(houseData).length > 0
			? [
					{
	...
	return (
		<div
			...
			{loading ? (
				...
				<>
					<h2 className={styles.titleType}>House ID :{houseId}</h2>
					<div className={styles.mainContents} style={{ height: 40 }}>
						<h3 style={{ color: data.color, fontSize: 15 }} className={styles.mainTitle}>
