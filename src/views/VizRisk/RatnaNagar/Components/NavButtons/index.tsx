...
const Navbuttons = (props: Props) => {
	const { postionsPerPage, leftElement, onNextClick, onPreviousClick, onButtonClick } = props;

	const { navIdleStatus } = useContext(MainPageDataContext);
	...
	return (
		<div className={leftElement < 9 ? styles.pagination : styles.lastPagePagination}>
			<button
				...
				disabled={(leftElement === 0 && postionsPerPage.page1PositionValue === 1) || !navIdleStatus}
				className={styles.previousNextButton}
				style={{
					cursor:
						(leftElement === 0 && postionsPerPage.page1PositionValue === 1) || !navIdleStatus
							? "not-allowed"
							: "pointer",
				}}>
				<ScalableVectorGraphics className={styles.backButton} src={BackButton} />
			</button>
			<div className={styles.numberContainer} ref={navRef}>
				{new Array(10).fill(0).map((_, indexNumber) => (
					<button
						...
						style={{
							cursor: !navIdleStatus ? "not-allowed" : "pointer",
						}}>
						{
							<svg viewBox="0 0 50 50" width="35px" height="35px" className={styles.circleProgress}>
								<circle
									cx={DIAMETER / 2}
									cy={DIAMETER / 2}
									r={RADIUS}
			...
			<button
				...
				onClick={onNextClick}
				disabled={
					(postionsPerPage.page10PositionValue === 0 && leftElement === 9) || !navIdleStatus
				}
				style={{
					cursor:
						(leftElement === 10 && postionsPerPage.page10PositionValue === 1) || !navIdleStatus
							? "not-allowed"
							: "pointer",
				}}
				className={styles.previousNextButton}>
				<ScalableVectorGraphics className={styles.backButton} src={ForwardButton} />
			</button>
		</div>
	);
};
