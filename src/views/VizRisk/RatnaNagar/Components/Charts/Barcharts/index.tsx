...
const CommonBarChart = (props: Props) => {
	...
	const { requiredQuery, currentHeaderVal, setCurrentRechartsItem } =
		...
		<div className={styles.mainBarChart}>
			<div className={styles.infoReset}>
				<h3 className={styles.barTitle}>
					...
					{requiredQuery && !!currentHeaderVal && Object.values(requiredQuery[currentHeaderVal])[0]
			...
			<h3 className={styles.barTitle}>
				{barTitle === "Flood return period"
					? "Number of households likely to be inundated"
					: barTitle}
			</h3>
			<ResponsiveContainer width={"100%"} height={400}>
				<BarChart
					...
					<Tooltip cursor={{ fill: "#00000050" }} content={CommonToolTip} />
					<Bar
						onClick={(e) =>
							setCurrentRechartsItem((prevState: string) => {
								if (prevState === e.name) {
									return "";
								}
								return e.name;
