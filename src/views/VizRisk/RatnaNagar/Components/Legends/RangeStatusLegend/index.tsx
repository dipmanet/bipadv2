...
const RangeStatusLegend = (props) => {
	...
	const { leftElement, handleReset, handleRangeLegendClick } = useContext(MainPageDataContext);
		...
		<div className={styles.mainStatusLegendContainer}>
			<div className={styles.reloadAndTitle}>
				...
				<p className={styles.titleName}>
					{getCurrentType(leftElement) === "adaptiveCapacity"
						? "Lack of Adaptive Capacity"
						: `${
								getCurrentType(leftElement).charAt(0).toUpperCase() +
								getCurrentType(leftElement).slice(1)
						  } Score`}
				</p>
			</div>
			{rangeData.map((item) => (
				...
				<div
					...
					<p
						className={rangeNames.includes(item.name) ? styles.statusnameActive : styles.statusname}
						style={{
							color: rangeNames.includes(item.name) ? item.color : "white",
						}}>
						{item.status}
					</p>
				</div>
