...
const LeftpaneSlide1 = () => {
	const { mainKey, suffix, keyValueHtmlData, keyValueJsonData } = useContext(MainPageDataContext);

	const htmlData =
		keyValueHtmlData && getDataFromKey(mainKey, "page1_htmldata", suffix, keyValueHtmlData);

	const tempData =
		keyValueJsonData && getDataFromKey(mainKey, "page1_tempdata", suffix, keyValueJsonData);

	const rainFallData =
		keyValueJsonData && getDataFromKey(mainKey, "page1_rainfalldata", suffix, keyValueJsonData);
				...
				<div className={styles.mainLineChart}>
					{tempData && tempData.value && tempData.value.length > 0 && (
						<ResponsiveContainer className={styles.chartContainer} height={300}>
							<LineChart margin={{ top: 0, right: 5, left: 30, bottom: 10 }} data={tempData.value}>
					...
					{rainFallData && rainFallData.value && rainFallData.value.length > 0 && (
						<ResponsiveContainer className={styles.chartContainer} height={300}>
							<LineChart
								...
								<CartesianGrid strokeDasharray="3 3" vertical={false} opacity={0.3} />
								<XAxis dataKey="month" interval="preserveStart" tick={{ fill: "#94bdcf" }} />
								<YAxis
									unit={"mm"}
									axisLine={false}
									// domain={[0, 1150]}
