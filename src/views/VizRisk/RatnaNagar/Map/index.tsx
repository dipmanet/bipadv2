...
const Map = (props: any) => {
	...
	const totalPopulationByWard =
		demographicsData &&
		demographicsData.map((item: { name: any; MalePop: any; FemalePop: any }) => ({
			ward: item.name,
			totalpop: item.MalePop + item.FemalePop,
		}));

	const arrayValue =
		totalPopulationByWard && totalPopulationByWard.map((item: { totalpop: any }) => item.totalpop);
	...
	const dividedSpecificData =
		arrayValue &&
		new Array(Math.ceil(arrayValue.length / divider))
			.fill()
			.map((_) => arrayValue.splice(0, divider));
	const intervals: number[] = [];

	const nonEmptyData =
		dividedSpecificData && dividedSpecificData.filter((r: string | any[]) => r.length > 0);

	if (nonEmptyData) {
		nonEmptyData.map((d: any) =>
	...
	const fillPaint = () => {
		...
		return {
			"fill-color": ["interpolate", ["linear"], ["feature-state", "value"], ...saveArray],
			"fill-opacity": ["case", ["boolean", ["feature-state", "hover"], false], 1, 1],
		};
	};
	...
	useEffect(() => {
		...
		const mapping = wards
			.filter((item: { municipality: any }) => item.municipality === municipalityId)
			.map((item: { title: any }) => ({
				...item,
		...
		mapZoomEffect.current = setTimeout(() => {
			...
			multihazardMap.fitBounds(
				[84.4655766529013, 27.5488495368228, 84.5618032266611, 27.6813334045862],
		...
		multihazardMap.on("style.load", () => {
			// --------------------------------------SLIDE-3 -> CI layer----------------------------------------
			const ciCategory: any = CIData && [
				...new Set(CIData.features.map((item: any) => item.properties.Type)),
			];
			...
			mapImages.forEach((img) => {
				if (map.current) {
					map.current.loadImage(img.url, (error: any, image: any) => {
			...
			multihazardMap.on("mousemove", "ward-fill-local", (e) => {
				if (e.features && e.features.length > 0) {
					...
					const details = demographicsData.filter(
						(item: { name: string }) => item.name === `Ward ${wardno}`
					);
			...
			multihazardMap.addLayer({
				...
				layout: {
					visibility: renderPage === "homepage" ? "visible" : "none",
				},
				paint: {
					"raster-opacity": renderPage === "homepage" ? 1 : innundationOpacity,
				},
			});
	...
	const sideEffect =
		!!currentHeaderVal && !!selectFieldValue && requiredQuery[currentHeaderVal][selectFieldValue];
	...
	useEffect(() => {
		...
		for (let index = 0; index < rangeValues.length; index++) {
			...
			const filteredData = householdData.filter(
				(item) =>
					item[getCurrentType(leftElement)] / 10 >= rangeValues[index] &&
					item[getCurrentType(leftElement)] / 10 <= rangeValues[index + 1]
			);
		...
		for (const [index, data] of householdData.entries()) {
			...
			const checkRightCondition =
				eachData &&
				!!currentHeaderVal &&
				!!selectFieldValue &&
				eachData.filteredMetadata &&
				eachData.filteredMetadata[currentHeaderVal] &&
				eachData.filteredMetadata[currentHeaderVal] &&
				Object.keys(eachData.filteredMetadata[currentHeaderVal]).includes(
					Object.keys(requiredQuery[currentHeaderVal])[0]
				) &&
		...
		if (sideEffect && leftElement === 5) {
			for (const [index, data] of householdData.entries()) {
				...
				if (
					sideEffect === "100 years" &&
					(checkFeildVal === "100 years" ||
						checkFeildVal === "20 years" ||
						checkFeildVal === "5 years")
				) {
					dataForFloodReturnPeriod.push(eachData);
				}
			}
		...
		const checkDataForGeoJson = () => {
			switch (true) {
				case leftElement === 5 &&
					dataForFloodReturnPeriod.length > 0 &&
					rangeValues &&
		...
		const geoJsonMain = {
			...
			features: requiredGeoJsonDataArr.map(
				(item: { [x: string]: number; id: any; point: any }) => ({
					type: "Feature",
					id: item.id,
					geometry: item.point,
					properties: {
		...
		if (map && map.current && leftElement > 4 && leftElement < 9) {
			if (map.current.getLayer("household-point")) {
		...
		<RatnaNagarMapContext.Provider value={mapValue}>
			<div
				...
				{leftElement > 4 && leftElement < 9 && (
					<>
						<RangeStatusLegend rangeNames={rangeNames} setRangeNames={setRangeNames} />
					</>
				)}
				{leftElement === 5 && (
					<>
						<div className={styles.sliderContainer}>
							<p className={styles.sliderLabelRatna}>Flood Layer Opacity</p>
							<input
				...
				{leftElement === 4 && (
					<>
						<div className={styles.innundationSliderContainer}>
							<p className={styles.sliderLabelRatna}>Innundation Layer Opacity</p>
							<input
