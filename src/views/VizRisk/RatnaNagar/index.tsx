...
const Ratnanagar = (props: any) => {
	...
	const [scrollTopValuesPerPage, setScrollTopValuesPerPage] = useState<ScrollTopInitialValues>(
		scrollTopPageInitialValues
	);
	const [postionsPerPage, setPostionsPerPage] =
		useState<PostionInitialValues>(positionInitialValues);
	...
	const handleRangeLegendClick = (rangeData: number[]) => {
		setRangeValues((prevState) =>
			[...new Set([...prevState, ...rangeData])].sort((a: number, b: number) => a - b)
		);
		...
		if (
			...
			setRangeValues((prevState) =>
				prevState.filter((item) => item !== rangeData[0] && item !== rangeData[1])
			);
		}
	};
				...
				<RatnaNagarMapContext.Provider value={mapValue}>
					{pending ? (
						...
						<>
							...
							{leftElement === 0 && <Leftpane1 />}
							{leftElement === 1 && <Leftpane2 />}
							{leftElement === 2 && <Demographic />}
							{leftElement === 3 && (
								<Leftpane3
									clickedCiName={clickedCiName}
							...
							{leftElement === 4 && <Leftpane4 />}
							{leftElement === 5 && <Leftpane5 />}
							{leftElement === 6 && <Leftpane6 />}
							{leftElement === 7 && <Leftpane7 />}
							{leftElement === 8 && <Leftpane8 />}
							{leftElement === 9 && <Leftpane10 />}
						</>
					)}
