	...
	if (d.length > 0) {
		...
		if (arr.length > 0) {
			...
			const medium = arr.filter(
				(v) => v.vulnerabilityScore >= 50 && v.vulnerabilityScore < 60
			).length;
...
export const getfoundationTypeChartData = (d) => {
	if (d.length > 0) {
		const typeArr = [...new Set(d.map((i) => i.foundationType).filter((f) => f !== undefined))];
...
export const getownershipChartData = (d) => {
	if (d.length > 0) {
		const typeArr = [...new Set(d.map((i) => i.ownership).filter((f) => f !== undefined))];
...
export const getsourceofIncomeChartData = (d) => {
	if (d.length > 0) {
		const typeArr = [...new Set(d.map((i) => i.majorOccupation).filter((f) => f !== undefined))];
...
export const getaverageAnnualincomeChartData = (d) => {
	if (d.length > 0) {
		const typeArr = [
			...new Set(d.map((i) => i.averageAnnualIncome).filter((f) => f !== undefined)),
		];
		const arr = [
			{
				name: ">500000",
			...
			},
			{
				name: "300000-500000",
				Total: d.filter(
					(item) => item.averageAnnualIncome >= 300000 && item.averageAnnualIncome < 500000
				).length,
			},
			{
				name: "100000-300000",
				Total: d.filter(
					(item) => item.averageAnnualIncome >= 100000 && item.averageAnnualIncome < 300000
				).length,
			},
			{
				name: "<100000",
