...
class FloodHistoryMap extends React.Component {
	...
	public componentDidMount() {
		...
		if (isDefined(buildings.length > 0)) {
			...
			const updateArea = (e) => {
				...
				const farmlands = this.map.queryRenderedFeatures([point1, point2], {
					layers: ["Farmlands"],
				});
				const forest = this.map.queryRenderedFeatures([point1, point2], { layers: ["Forest"] });
		...
		this.map.on("style.load", () => {
			...
			this.map.setLayoutProperty("National Park Text", "visibility", "none");
			this.map.setLayoutProperty("National Park", "visibility", "none");
	...
	public componentDidUpdate(prevProps) {
		...
		if (
			this.props.singularBuilding !== prevProps.singularBuilding &&
		...
		if (this.props.buildingVul !== prevProps.buildingVul) {
			...
			this.map.getSource("buildingPointSource").setData(buildingpointsData);
			// this.map.setPaintProperty('buildingPoints', 'circle-color', buildingColor);
			// need to remake the source data
		}
	...
	public zoomToBbox = (data) => {
		const coordList = data.map((position) => [parseFloat(position[0]), parseFloat(position[1])]);
		const line = turf.lineString(coordList);
		const bbox = turf.bbox(line);
		this.map.fitBounds(bbox, {
			padding: 20,
	...
	public showPopupOnBldgs = (coordinates, singularBuldingData, msg, showButton, permission) => {
		...
		if (Object.keys(singularBuldingData).length > 2) {
			// popup.remove();
			popup.setLngLat(coordinates).setDOMContent(content).addTo(this.map);
		} else {
			// popupWithData.remove();
			popup.setLngLat(coordinates).setDOMContent(content).addTo(this.map);
		}
	...
	public handleInputChangeFlood = (e) => {
		...
		this.map.setPaintProperty(
			`raster-flood-${this.props.rasterLayer}`,
			"raster-opacity",
			Number(val)
		);
	};
	...
	public getOSMidFromHouseId = (houseID) => {
		const osmId = this.props.buildings.filter(
			(item) => Number(item.houseOwnerId) === Number(houseID)
		);
		if (osmId.length > 0) {
	...
	public handleBuildingClick = (clicked) => {
		...
		if (searchId) {
			...
			const coordinatesObj = this.state.buildingpoints.features.filter(
				(b) => Number(searchId) === b.properties.osmId
			);
			let cood = [];
			...
			<div>
				...
				<div className={styles.sliderandLegendContainer}>
					{this.props.sesmicLayer === "sus" && (
						<>
							<p className={_cs(styles.sliderLabel)}>Layer Opacity</p>
							<input
