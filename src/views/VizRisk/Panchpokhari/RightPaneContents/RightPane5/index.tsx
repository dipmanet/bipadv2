...
class SlideFourPane extends React.PureComponent<Props, State> {
	...
	public getChartData = (clickedItem, incidentFilterYear, incidentList) => {
		...
		return fullhazardTitle.map((item) => ({
			name: item,
			Total: incidentList.features.filter(
				(ht) =>
	...
	public render() {
		...
		const { chartData, fullhazardTitle, nonZeroArr } = this.state;

		return (
			<div className={styles.vrSideBar}>
				{chartData.length > 0 && (
					<>
						<h1>Past Disaster Events in Panch Pokhari Thangpal Rural Municipality</h1>
						<p>
							In the year {incidentFilterYear} , total{" "}
							{chartData.reduce((a, b) => ({ Total: a.Total + b.Total || 0 })).Total} incidents{" "}
							{nonZeroArr.length > 0 ? " of " : ""}
							{this.getDescription()}
							have been reported in Panch Pokhari Thangpal Rural Municipality. These incidents have
							caused {incidentDetailsData.peopleDeathCount} deaths and{" "}
						...
						{fullhazardTitle.length > 0 &&
							fullhazardTitle.map((item) => (
								<div className={styles.hazardItemContainer}>
									<button
										type="button"
										className={clickedItem === item ? styles.legendBtnSelected : styles.legendBtn}
