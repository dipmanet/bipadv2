...
const JugalMap = (props: Props) => {
	...
	useEffect(() => {
		...
		jugalMap.on("style.load", () => {
			...
			const categories: any = [...new Set(CIData.features.map((item) => item.properties.Type))];
			...
			categories.map((l: string) => {
				...
				categories.map((ci: string) =>
					jugalMap.on("mousemove", `unclustered-point-${ci}`, (e: any) => {
						if (e) {
							const { lngLat } = e;
				...
				categories.map((ci: string) =>
					jugalMap.on("mouseleave", `unclustered-point-${ci}`, () => {
			...
			jugalMap.moveLayer("Ward Boundary Line");
			jugalMap.moveLayer("Ward No.");
			...
			<div style={mapCSS} ref={mapContainerRef}>
				{incidentsPages.indexOf(rightElement + 1) !== -1 && (
					<div className={styles.incidentsSlider}>
						<button className={styles.playButton} type="button" onClick={handlePlayPause}>
							{playState ? (
								<Icon name="play" className={styles.playpauseIcon} />
							) : (
								<Icon name="pause" className={styles.playpauseIcon} />
							)}
						</button>
						...
						<div className={styles.rangeWrap}>
							<div
								style={{
									left: `calc(${Number(incidentYear) * 10}% - ${Number(incidentYear) * 2}px)`,
								}}
