						...
						.map((ls, i) => (
							<div key={ls} className={styles.infraIconContainer}>
								<button
									type="button"
									className={
										selectedArr.indexOf(ls) !== -1
											? styles.criticalButtonSelected
											: styles.criticalButton
									}
									onClick={() => handleyearClick(ls)}>
									<div className={styles.section}>
										<span className={styles.text}>{ls}</span>
										<div className={styles.underline} style={{ backgroundColor: colorArr[i] }} />
									</div>
								</button>
							</div>
						))}
					<button type="button" className={styles.navbutton} onClick={() => handleCompareClick()}>
