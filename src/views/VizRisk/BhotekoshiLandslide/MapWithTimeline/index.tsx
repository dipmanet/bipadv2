...
import { framize } from "#utils/common";
...
class FloodHistoryMap extends React.Component {
	...
	public componentDidMount() {
		...
		this.map = new mapboxgl.Map({
			...
			style: import.meta.env.VITE_APP_VIZRISK_BHOTEKOSHI_LANDSLIDE,
			center: [lng, lat],
			zoom,
			minZoom: 2,
			maxZoom: 22,
		...
		const mapping = wards
			.filter((item) => item.municipality === 23003)
			.map((item) => ({
				...item,
		...
		this.map.on("style.load", () => {
			...
			this.map.setLayoutProperty("bahrabiseFell", "visibility", "none");
	...
	public getSusceptibilityLayer = () =>
		[
			...
			"&service=WMS",
			"&request=GetMap",
			"&layers=Bipad:bhotekoshi_durham_susceptibility",
			"&tiled=true",
			"&width=256",
			"&height=256",
			"&srs=EPSG:3857",
	...
	public handleAnimationKeyframe = framize((percent) => {
		const p = percent;
		const radius = p * 20;
		const opacity = 1 - p;
		return {
