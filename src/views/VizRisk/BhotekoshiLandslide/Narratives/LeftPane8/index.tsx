...
const colorArr = ["#808080", "#3d26a6", "#1e96eb", "#80cb57", "#ff5500", "#a80000", "#750000"];

const epocharr = [2014, 2015, 2016, 2017, 2018, 2019, 2020];
...
const LeftPane8 = (props: Props) => {
	...
	const { drawData, landSlide, chartReset, ci, polygonResponse, landslideYear } = props;
	...
	useEffect(() => {
		if (landslideYear.length > 1) {
			...
			arr.push("name");
			const wwcD = wardwiseData.map((ward) =>
				Object.keys(ward)
	...
	return (
		<div className={styles.vrSideBar}>
			...
			<p className={styles.narrativeText}>
				The map shows the spatial distribution of landslides that occurred from 2014 to 2020.
				Research suggests that a significant proportion of landslide risk in the earthquake affected
				areas comes from the reactivation of existing landslides, so this map is useful for
				visualising if and how they have changed.
			</p>
			<p>
				After Gorkha Earthquake 2015, 836 landslide incidents were seen in the post monsoon season.
				Likewise, the higher number of post monsoon landslide incidents till date occurred in the
				year 2018 i.e. 907.
			</p>
			<p className={styles.narrativeText}>Source: Durham University</p>
			<p className={styles.narrativeText}>NO. OF LANDSLIDES</p>
			...
			{lschartData.length > 0 ? (
				...
				<p className={styles.narrativeText}>
					No landslide has occured in the selected area from 2014 to 2020
				</p>
			)}

			<p className={styles.narrativeText}>COMMUNITY INFRASTRUCTURE</p>
