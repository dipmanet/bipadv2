...
const DemoGraphicsLegends = (props) => {
	...
	const { handlePopulationChange, legends } = props;

	const { densityVals, populationVals, colors, densityColors } = legends;
				...
				<div>
					...
					<div className={styles.populationContainer}>
						<div className={styles.populationIndContainer}>
							{colors.map((c, i) => (
								<div key={c} style={getLegendStyle(i, c, colors)} />
							))}
						</div>
						<div className={styles.populationTextContainer}>
							{populationVals.map((d) => (
								<div key={d} className={styles.populationText}>
									{d}
								</div>
							))}
						</div>
					...
					<div className={styles.populationContainer}>
						<div className={styles.populationIndContainer}>
							{densityColors.map((c, i) => (
								<div key={c} style={getLegendStyle(i, c, densityColors)} />
							))}
						</div>
						<div className={styles.populationTextContainer}>
							{densityVals.map((d) => (
