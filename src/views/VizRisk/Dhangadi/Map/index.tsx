...
const colorGrade = ["#ffedb8", "#ffffff"];
...
const slideOneLayers = ["wardNumbers", "water", "waterway", "wardOutline", "municipalityFill"];
...
const slideTwoLayers = [
	...
	"forestDhangadi",
	"WoodforestDhangadi",
	"agriculturallandDhangadi",
	"agriculturelandDhangadiFarmyard",
	"grasslandDhangadi",
	"meadowDhangadi",
	"SandDhangadi",
	"municipalityFill",
...
class FloodHistoryMap extends React.Component {
	...
	public componentDidMount() {
		...
		this.map = new mapboxgl.Map({
			...
			style: import.meta.env.VITE_APP_VIZRISK_DHANGADI_FLOOD,
			center: [lng, lat],
			zoom,
			minZoom: 2,
			maxZoom: 22,
		...
		const slideSixLayers = [
			...
			"settlement-minor-label",
			"road-label-simple",
			"safeshelterDhangadiimage",
			"safeshelterDhangadiIcon",
			...evacClusters,
			...rasterLayers,
			"bridgeDhangadi",
			"water",
			...
			if (nextProps.rightElement !== rightElement) {
				if (nextProps.rightElement === 0) {
					this.map.easeTo({
						...
						zoom: 11,
						duration: 1000,
						center: [80.6547442950228, 28.693149812305652],
					});
					this.resetClusters();
