...
import { encodeDate, encodeTime } from "#utils/common";

import { createRequestClient, NewProps, ClientAttributes, methods } from "#request";

import { hazardTypeListSelector, severityListSelector, languageSelector } from "#selectors";
...
class AddEventForm extends React.PureComponent<Props, State> {
	...
	private handleFaramValidationSuccess = (faramValues: FaramValues) => {
		const {
			requests: { addEventRequest, editEventRequest },
			onUpdate,
			// closeModal,
			data,
		} = this.props;

		const { startedOnDate, startedOnTime, location, ...others } = faramValues;
							...
							<div className={styles.inputRow}>
								<div className={styles.startedOnInputs}>
									<DateInput
										...
										language={language}
									/>
									<TimeInput faramElementName="startedOnTime" />
								</div>
								<SelectInput
...
export default compose(connect(mapStateToProps), createRequestClient(requests))(AddEventForm);
