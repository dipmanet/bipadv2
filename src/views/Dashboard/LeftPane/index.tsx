...
const AlertEmptyComponent = () => (
	<Translation>
		{(t) => <div className={styles.alertEmpty}>{t("There are no alerts at the moment.")}</div>}
	</Translation>
);
...
const EventEmptyComponent = () => (
	<div className={styles.eventEmpty}>
		<Translation>{(t) => <span>{t("There are no major events at the moment.")}</span>}</Translation>
	</div>
);

const AlertTableModal = ({ closeModal, alertList, language }) => (
	<Modal className={_cs(styles.alertTableModal, language === "np" && styles.languageFont)}>
		<Translation>
			{(t) => (
				<ModalHeader
					title={t("Alerts")}
...
class LeftPane extends React.PureComponent {
	...
	groupByHazard = memoize((alerts, hazards) => {
		const freqList = groupList(alerts, (alert) => alert.hazard);
	...
	render() {
		...
		const { showAddAlertModal, showAddEventModal, alertToEdit, eventToEdit, activeView } =
			this.state;
			...
			<div className={_cs(className, styles.leftPane)}>
				...
				<div className={_cs(styles.sourceDetails, "source-tour")}>
					...
					<div className={styles.label}>
						<Translation>{(t) => <span>{t("Data source")}</span>}</Translation>
					</div>
					<div className={styles.value}>
						<div className={styles.source}>
							<div className={styles.text}>
								<Translation>{(t) => <span>{t("Realtime Module")}</span>}</Translation>
							</div>
							{/* <a
				...
				<header className={styles.header}>
					<div className={styles.tabs}>
						<div
							...
							role="presentation">
							<div className={styles.value}>{alertList.length}</div>
							<div className={styles.title}>
								<div className={_cs(styles.icon, styles.alertIcon)} />
								<div className={styles.text}>
									<Translation>{(t) => <span>{t("Alerts")}</span>}</Translation>
								</div>
							</div>
						</div>
						<div
							...
							role="presentation">
							<div className={styles.value}>{eventList.length}</div>
							<div className={styles.title}>
								<div className={_cs(styles.icon, styles.eventIcon)} />
								<div className={styles.text}>
									<Translation>{(t) => <span>{t("Events")}</span>}</Translation>
								</div>
							</div>
						</div>
						<div
							className={_cs(
								styles.tab,
								activeView === "visualizations" && styles.active,
								"visualization-tour"
							)}
							role="presentation"
							onClick={this.handleVisualizationsButtonClick}>
							<Icon className={styles.visualizationIcon} name="bars" />
							<div className={styles.text}>
								<Translation>{(t) => <span>{t("Visualizations")}</span>}</Translation>
							</div>
						</div>
					</div>
					<div className={styles.actions}>
						{activeView === "alerts" && (
							<Cloak hiddenIf={(p) => !p.add_alert}>
								<AccentButton
									...
									iconName="add"
									transparent>
									<Translation>{(t) => <span>{t("New alert")}</span>}</Translation>
								</AccentButton>
							</Cloak>
						)}
						{activeView === "events" && (
							<Cloak hiddenIf={(p) => !p.add_event}>
								<AccentButton
									...
									iconName="add"
									transparent>
									<Translation>{(t) => <span>{t("New event")}</span>}</Translation>
								</AccentButton>
							</Cloak>
						)}
						<AlertTableModalButton
							...
							iconName="table"
							transparent
							modal={<AlertTableModal alertList={alertList} language={language} />}
						/>
					</div>
				</header>
