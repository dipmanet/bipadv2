	...
	<div className={styles.eventTooltip}>
		...
		<div className={styles.content}>
			<TextOutput
				...
				label="Created On"
				value={<FormattedDate value={createdOn} mode="yyyy-MM-dd hh:mm" />}
			/>
			<TextOutput
...
class AlertEventMap extends React.PureComponent {
	...
	getFilter = memoize((timestamp) => [">", ["get", "createdOn"], timestamp]);
	...
	handleAlertClick = (feature, lngLat) => {
		const {
			properties: { title, description, referenceType, referenceData, startedOn, createdDate },
		} = feature;
	...
	handleEventClick = (feature, lngLat) => {
		const {
			properties: { title, severity, createdOn, description },
		} = feature;
		this.setState({
			eventTitle: title,
	...
	getHazardList = (alertList = [], eventList = []) => {
		...
		const alertHazardIdList = alertList.map((v) => v.hazard).filter((v) => v);
		const eventHazardIdList = eventList.map((v) => v.hazard).filter((v) => v);
	...
	render() {
		...
		const eventsConvexFeatureCollection = this.getConvexEventsFeatureCollection(eventList, hazards);
		const eventsPolygonFeatureCollection = this.getPolygonEventsFeatureCollection(
			eventList,
			hazards
		);
		const eventsPointFeatureCollection = this.getPointEventsFeatureCollection(eventList, hazards);
		...
		return (
			<React.Fragment>
				...
				<MapSource
					...
					<MapLayer
						...
						layerOptions={{
							...
							layout: isAlertHovered || isEventHovered ? noneLayout : visibleLayout,
						}}
                    ...
                    */}
					...
					<MapState attributes={alertHoverAttributes} attributeKey="hover" />
				</MapSource>
				<MapSource
				...
				<MapSource
					...
					<MapState attributes={eventHoverAttributes} attributeKey="hover" />
				</MapSource>
			</React.Fragment>
		);
	}
