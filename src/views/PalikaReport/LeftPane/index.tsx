...
const allRepIcons = [dashboardLogo, myreport];

const { menuItems, Data1, Data2 } = SidebarMenu;
			...
			{showReportEdit && (
				<>
					<div className={styles.breadcrumbRow}>
						<button className={styles.breadcrumb} type="button" onClick={handleMyPalikaClick}>
							...
							<Icon
								...
								title={
									drrmLanguage.language === "np"
										? Translations.CreateReportInformationButtom.np
										: Translations.CreateReportInformationButtom.en
								}
							/>
						</h2>
											...
											<button
												...
												<span className={styles.iconandMenu}>
													<ScalableVectorGraphics
														...
														alt="Bullet Point"
													/>
													{drrmLanguage.language === "en" ? item.content : item.contentNp}
												</span>
												<span>
													{drrmProgress >= item.key ? (
														<Icon name="check" className={styles.progressDone} />
													) : (
														<Icon
															name="circle"
															className={
																drrmProgress + 1 === item.key
								...
								return (
									<li>
										<button
											key={item.key}
											className={
												drrmProgress === 9 ? styles.createReportDisabled : styles.createReport
											}
											onClick={() => handleMenuItemClick(item.key)}
											type="button"
											disabled={drrmProgress === item.key}>
			...
			{!showReportEdit && (
				<div className={styles.reportSidebarMainContainer}>
									...
									{isSubmenuClicked && selectedMenuId === item.id
										? item.components.map((data) => (
												<button
													type="button"
													className={
														isIndicatorClicked && selectedSubMenuId === data.id
															? styles.selected
															: styles.notSelected
													}
													onClick={() =>
														handleSelectSubmenu(data.id, data.url, data.title, data.slug, item.slug)
													}>
													<span className={styles.iconandMenu}>
														<ScalableVectorGraphics
															className={styles.bulletPoint}
															...
															alt="Bullet Point"
														/>
														{data.id === 2 && palikaLanguage.language === "en" ? title_en : ""}
														{data.id === 2 && palikaLanguage.language === "np" ? title_ne : ""}{" "}
														{data.title}
													</span>
												</button>
										  ))
					...
					<div className={styles.addButnDiv}>
						{selectedSubMenuId === 2 && (
							<button type="submit" className={styles.addButn} onClick={handleAdd}>
						...
						{user && user.isSuperuser && (
							<>
								<div className={styles.inputContainer}>
									<h5>
										<Gt section={Translations.dashboardSidebarSelectMunicipalityHeader} />
									</h5>
