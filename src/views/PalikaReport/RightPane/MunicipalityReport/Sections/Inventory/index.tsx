...
const Inventory: React.FC<Props> = (props: NewProps) => {
	...
	useEffect(() => {
		if (fetchedData.length > 0) {
			...
			const categories = [
				...new Set(finalInventoriesData.map((inventory) => inventory.item.category)),
			];
			...
			setDataWithIndex(
				finalInventoriesData.map((item, i) => ({ ...item, index: i, selectedRow: true }))
			);
		}
	}, [fetchedData.length]);
	...
	const handleCheckAll = (e) => {
		...
		if (e.target.checked) {
			...
			setCheckedRows([]);
			setDataWithIndex(
				finalInventoriesData.map((item, i) => ({ ...item, index: i, selectedRow: false }))
			);
		}
	};
				...
				<div className={styles.tabsPageContainer}>
					...
					<div className={styles.palikaTable}>
						<table>
							<tbody>
								<tr>
									...
									{!annex && finalInventoriesData.length ? (
										<th>
											<Gt section={Translations.InventoryAction} />
										</th>
									) : null}
								</tr>
								{loader ? (
									...
									<>
										{finalInventoriesData &&
											finalInventoriesData.map((item, i) => (
												<tr>
													<td>
														<input
															type="checkbox"
															...
															<button
																...
																title={
																	drrmLanguage.language === "np"
																		? Translations.InventoryEditTooltip.np
																		: Translations.InventoryEditTooltip.en
																}>
						...
						{!loader && finalInventoriesData.length === 0 && (
							<h2>
								<Gt section={Translations.InventoryNoDataMessage} />
							</h2>
						)}
						{!loader && <>{!annex && <NextPrevBtns handleNextClick={handleNext} />}</>}
					</div>
				</div>
			)}
			{previewDetails && <ReportChart chartData={chartData} />}
		</div>
	);
};
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(createConnectedRequestCoordinator<PropsWithRedux>()(createRequestClient(requests)(Inventory)));
