...
import { setDrrmProgressAction } from "#actionCreators";
...
const Relief = (props: Props) => {
	...
	useEffect(() => {
		if (reliefData) {
			...
			const reliefChart = reliefDateArr.map((d, mainindex) => ({
				...
				"Amount-Distributed":
					reliefData.filter((item) => getMonthFromDate(item.dateOfReliefDistribution) === d)
						.length > 0
						? reliefData
				...
				"रकम वितरित":
					reliefData.filter(
						(nep, i) => getNpValFromDate(nep.dateOfReliefDistribution) === mainindex + 1
					).length > 0
						? reliefData
						...
						? reliefData
								...
								.reduce((a, b) => ({
									numberOfBeneficiaryFamily:
										a.numberOfBeneficiaryFamily + b.numberOfBeneficiaryFamily,
								})).numberOfBeneficiaryFamily
						: 0,
				लाभार्थीहरू:
					reliefData.filter(
	...
	const handleFilteredViewRelief = (response) => {
		setfamiliesBenefited(
			response[0].numberOfBeneficiaryFamily ? response[0].numberOfBeneficiaryFamily : "-"
		);
		setnamesofBeneficiaries(response[0].nameOfBeneficiary ? response[0].nameOfBeneficiary : "-");
		setreliefDate(
			response[0].dateOfReliefDistribution ? ADToBS(response[0].dateOfReliefDistribution) : "-"
		);
	...
	useEffect(() => {
		if (fetchedData.length > 0) {
			...
			const estimatedLoss = fetchedData
				.map((item) => item.loss)
			...
			const missingTotal = fetchedData
				.map((item) => item.loss)
				.filter((item) => item !== undefined)
			...
			const injuredTotal = fetchedData
				.map((item) => item.loss)
				.filter((item) => item !== undefined)
			...
			const hazards = [...new Set(fetchedData.map((item) => item.hazard))].filter(
				(hazar) => hazar !== undefined
			);
			const hazardwiseImpactData = hazards.map((item) => {
				const name =
					drrmLanguage.language === "en" ? hazardTypes[item].title : hazardTypes[item].titleNe;
				const Incidents = fetchedData.filter((inc) => inc.hazard === item).length;
				const PeopleDeath = fetchedData
					.filter((inc) => inc.hazard === item)
				...
				return {
					...
					PeopleDeath,
					घटना: Incidents,
					"मान्छे-मृत्यु": PeopleDeath,
				};
			});

			const deathMaleData = fetchedData
				.map((item) => item.loss)
				.filter((item) => item !== undefined)
			...
			const deathFemaleData = fetchedData
				.map((item) => item.loss)
				.filter((item) => item !== undefined)
			...
			const houseAffectedData = fetchedData
				.map((item) => item.loss)
				.filter((item) => item !== undefined)
			...
			const houseDamagedData = fetchedData
				.map((item) => item.loss)
				.filter((item) => item !== undefined)
			...
			const getWardWiseDatum = (dataA, wardA) =>
				dataA
					.map((item) => item.loss)
					.reduce((a, b) => ({
						ward: wardA,
			...
			const wards = [...new Set(fetchedData.map((item) => item.wards[0]))].filter(
				(ward) => ward !== undefined
			);
			const chartArr = [];
		...
		if (fetchedData.length > 0) {
			return [
				...
				fetchedData.filter((incidentObj) => incidentObj.id === item.incident)[0]
					? ADToBS(
							fetchedData
								.filter((incidentObj) => incidentObj.id === item.incident)[0]
								.incidentOn.split("T")[0]
					  )
					: "-",
			];
		}
				...
				<div className={styles.tabsWardContainer}>
					{!showRelief && (
						<>
							...
							<div className={styles.palikaTable}>
												...
												{finalArr &&
													finalArr.map((item, i) => (
														<tr key={item.item.id}>
															...
															<td>
																{item.item.loss ? item.item.loss.infrastructureDestroyedCount : 0}
															</td>
																...
																{!props.annex && item.item.cause ? (
																	...
																	!props.annex && (
																		<button
																			...
																			title={
																				drrmLanguage.language === "np"
																					? Translations.IncidentAddCauseTooltip.np
																					: Translations.IncidentAddCauseTooltip.en
																			}>
															...
															reliefData.find((data) => data.incident === item.item.id) ? (
																reliefData
																	.filter((data) => data.incident === item.item.id)
																	.map((data) => (
																		<td>
																...
																<td>
																	{!props.annex && (
																		<button
																			...
																			title={
																				drrmLanguage.language === "np"
																					? Translations.IncidentAddReliefTooltip.np
																					: Translations.IncidentAddReliefTooltip.en
																			}>
								...
								{props.annex && (
									<>
										...
										<table style={{ tableLayout: "fixed" }} id="table-to-xls">
											<tbody>
												<tr>
													<th>
												...
												{reliefData &&
													reliefData.map((item, i) => (
														<tr key={item.id}>
															...
															<td>{getIncidentTitle(item)[0]}</td>
															<td>{getIncidentTitle(item)[1]}</td>
														</tr>
													))}
											</tbody>
										</table>
							...
							{!loader && fetchedData.length === 0 && (
								<h2 style={{ display: "flex", justifyContent: "center" }}>
									<Gt section={Translations.OrganizationNoDataMessage} />
								</h2>
							)}
							{!loader && (
								<>
									...
									<tbody>
										...
										<tr key={currentRelief.id}>
											...
											<td>
												{Number(
													currentRelief.loss
														? currentRelief.loss.infrastructureAffectedBridgeCount
														: 0
												) +
													Number(
														currentRelief.loss
															? currentRelief.loss.infrastructureAffectedElectricityCount
															: 0
													) +
													Number(
														currentRelief.loss
															? currentRelief.loss.infrastructureAffectedHouseCount
															: 0
													) +
													Number(
														currentRelief.loss
															? currentRelief.loss.infrastructureAffectedRoadCount
															: 0
													)}
											</td>
											<td>
												{currentRelief.loss ? currentRelief.loss.infrastructureDestroyedCount : 0}
											</td>
			...
			{props.previewDetails && (
				<>
					<div className={styles.budgetPreviewContainer}>
						<h2>
							<Gt section={Translations.DisasterIncidentSummary} />
						</h2>
						<div className={styles.lossRow}>
							<div className={styles.lossSection}>
								...
								<div className={styles.lossElement}>
									...
									/>
									<ul>
										<p className={styles.darkerText}>{getELWithUnit(totalEstimatedLoss)}</p>
										<p className={styles.smallerText}>
					...
					<div className={styles.incidentImpactRow}>
						<div className={styles.incidentSection}>
							...
							<BarChart
								width={300}
								height={250}
								data={hazardwiseImpact.sort((a, b) => b.Incidents - a.Incidents).slice(0, 5)}
							...
							<div className={styles.houseData}>
								...
								<div className={styles.houseRow}>
									...
									<div className={styles.houseElement}>
										...
										<ul>
											<li>
												<span className={styles.darker}>{houseDamaged}</span>
											</li>
											<li>
						...
						<div className={styles.incidentSection}>
							...
							<BarChart
								...
								data={wardWiseImpact
									.sort(
										(a, b) => -(a.Death + a.Injured + a.Missing) + (b.Death + b.Injured + b.Missing)
									)
									.slice(0, 5)}
					...
					<div className={styles.reliefdistRow}>
						...
						<div className={styles.reliefDataMainContainer}>
							<div className={styles.reliefDistribution}>
									...
									<ul>
										<li>
											<span className={styles.biggerText}>
												<Gt section={Translations.Rupees} /> {totreliefAmt}
											</span>
										</li>
										<li>
								...
								<div className={styles.distItem}>
									<ScalableVectorGraphics className={styles.reliefIcon} src={family} alt="Relief" />
									<ul>
										<li>
							...
							<div className={styles.peopleBenefited}>
								<div className={styles.benefitedRow}>
									<div className={styles.distItem}>
										<ScalableVectorGraphics className={styles.reliefIcon} src={male} alt="Relief" />
										<ul>
											<li>
								...
								<div className={styles.lastRow}>
									<ul>
										<li>
											<span className={styles.darkerSmText}>{totJanajatis || 0}</span>
										</li>
										<li>
									...
									<ul>
										<li>
											<span className={styles.darkerSmText}>{totMadhesis}</span>
										</li>
										<li>
											<span className={styles.lighterSmText}>
												<Gt section={Translations.ReliefBenefitedPeopleMadhesi} />
									...
									<ul>
										<li>
											<span className={styles.darkerSmText}>{totMinotiries}</span>
										</li>
										<li>
											<span className={styles.lighterSmText}>
												<Gt section={Translations.ReliefBenefitedPeopleMinorities} />
									...
									<ul>
										<li>
											<span className={styles.darkerSmText}>{totDalits}</span>
										</li>
										<li>
											<span className={styles.lighterSmText}>
												<Gt section={Translations.ReliefBenefitedPeopleDalit} />
				...
				<Modal>
					...
					<ModalBody>
						...
						<h3 style={{ marginLeft: "10px", marginBottom: "20px" }}>
							<Gt section={Translations.IncidentTitle} />: {selectedIncidentTitle}
						</h3>
						<div className={styles.inputContainer}>
							<textarea
								...
								placeholder={
									drrmLanguage.language === "np"
										? "कृपया चयन गरिएको घटनाको लागि कारण निर्दिष्ट गर्नुहोस्"
										: "Kindly specify the Cause For Selected Incident"
								}
								rows={5}
								// cols={7}
						...
						<div className={styles.butnGroup}>
							<button type="button" className={styles.savebtn} onClick={handleBackButton}>
					...
					<ModalBody>
						...
						<div className={styles.inputContainer}>
							...
							<input
								...
								placeholder={
									drrmLanguage.language === "np"
										? "कृपया लाभान्वित भएका परिवारहरूको संख्या निर्दिष्ट गर्नुहोस्"
										: "Kindly specify number of families benefited"
								}
								disabled={disableInput}
							/>
						</div>
						<div className={styles.inputContainer}>
							<span className={styles.dpText}>
								<Gt section={Translations.ReliefBeneficiaryName} />
							</span>
							<textarea
								className={styles.inputElement}
								...
								placeholder={
									drrmLanguage.language === "np"
										? "कृपया लाभार्थीहरूको नाम निर्दिष्ट गर्नुहोस्"
										: "Kindly specify the names of beneficiaries"
								}
								rows={3}
								// cols={7}
								disabled={disableInput}
						...
						<div className={styles.inputContainer}>
							...
							<input
								...
								placeholder={
									drrmLanguage.language === "np"
										? "कृपया राहत राशि निर्दिष्ट गर्नुहोस्"
										: "Kindly specify Relief amount"
								}
								disabled={disableInput}
							/>
						</div>
						...
						<h2 style={{ marginBottom: "10px" }}>
							<strong>
								<Gt section={Translations.ReliefBenefitedPeopleHeading} />
							</strong>
						</h2>
						<div className={styles.inputContainer}>
							<span className={styles.dpText}>
							...
							<input
								...
								placeholder={
									drrmLanguage.language === "np"
										? "कृपया नम्बर निर्दिष्ट गर्नुहोस्"
										: "Kindly specify number"
								}
								disabled={disableInput}
							/>
						</div>
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(createConnectedRequestCoordinator<PropsWithRedux>()(createRequestClient(requests)(Relief)));
