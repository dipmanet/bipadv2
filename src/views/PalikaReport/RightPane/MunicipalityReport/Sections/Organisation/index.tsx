...
const Organisation: React.FC<Props> = (props: Props) => {
	...
	const {
		requests: { PalikaReportOrganizationReport },
		url,
		provinces,
		drrmProgress,
		user,
	...
	useEffect(() => {
		if (fetchedData.length > 0 && chartData.length === 0) {
			const chartDataArr = [
				...new Set(fetchedData.map((org) => org.operatorType || org.otherOperatorType)),
			];
			setChartData(
				chartDataArr.map((item) => ({
					name: item,
					Total: fetchedData.filter(
						(organisation) =>
							organisation.operatorType === item || organisation.otherOperatorType === item
					).length,
				}))
			);
		...
		<div className={drrmLanguage.language === "np" && styles.nep}>
					...
					<div className={styles.palikaTable}>
						<table id="table-to-xls">
							<tbody>
								<tr>
									...
									{!props.annex && fetchedData.length ? (
										<th>
											<Gt section={Translations.OrganizationAction} />
										</th>
									) : null}
								</tr>
								{loader ? (
																...
																<button
																	...
																	title={
																		drrmLanguage.language === "np"
																			? Translations.OrganizationEditTooltip.np
																			: Translations.OrganizationEditTooltip.en
																	}>
						...
						{!props.annex && !loader ? (
							<div className={styles.btnsCont}>
								<NextPrevBtns handlePrevClick={handleNext} handleNextClick={handleNext} />
							</div>
						) : (
							""
						)}
			...
			{props.previewDetails && (
				<div className={styles.budgetPreviewContainer}>
					<h2>
						<Gt section={Translations.OrganizationHeading} /> {municipalityName}
					</h2>
					<BarChart
						width={350}
						height={180}
						...
						<XAxis type="category" dataKey="name" tick={{ fill: "#777", fontSize: "10px" }} />
						<Bar
							dataKey="Total"
							fill="rgb(0,164,109)"
							// barCategoryGap={30}
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(createConnectedRequestCoordinator<PropsWithRedux>()(createRequestClient(requests)(Organisation)));
