...
import { setProgramAndPolicyDataAction, setDrrmProgressAction } from "#actionCreators";
...
const ProgramPolicies = (props: Props) => {
	...
	const handleSubmit = () => {
		...
		if (point) {
			...
			setLoader(false);
			setPostErrors(
				"Please Enter DRR related points for this fiscal year's annual policy and program of the municipality"
			);
		}
	};
		...
		<div className={drrmLanguage.language === "np" && styles.nep}>
					...
					<table>
						<tbody>
							<tr>
								...
								<th>
									<Gt section={Translations.points} />
								</th>
			...
			{props.previewDetails && (
				<div className={styles.budgetPreviewContainer}>
					...
					<ul>
						{finalPolicyData.length > 0 &&
							finalPolicyData.slice(0, 2).map((item) => <li key={item.id}>{item.point}</li>)}
					</ul>
				</div>
			)}
		</div>
...
export default connect(
	...
	createConnectedRequestCoordinator<PropsWithRedux>()(
		createRequestClient(requests)(ProgramPolicies)
	)
);
