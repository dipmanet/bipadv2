		...
		<div className={styles.header}>
			<div className={styles.leftContainer}>
				<div className={styles.logoAndAddress}>
					<ScalableVectorGraphics className={styles.logo} src={govtlogo} alt="Nepal Government" />

					<div className={styles.address}>
						<ul>
							<li className={styles.munTitle}>
								{`${municipalityName}`} <Gt section={Translations.MunicipalitySingle} />
							</li>
							<li className={styles.desc}>
								{`${districtName}`} <Gt section={Translations.dashboardTblHeaderDistrict} />,{" "}
								{`${provinceName}`} <Gt section={Translations.dashboardTblHeaderProvince} />
							</li>
						</ul>
					</div>
				</div>
				<div className={styles.titleAndFY}>
					<ul>
						...
						<li className={styles.fy}>
							<Gt section={Translations.FY} /> {fiscalYearTitle && fiscalYearTitle[0].titleEn}
						</li>
					</ul>
				</div>
			</div>
...
export default connect(
	mapStateToProps,
	undefined
)(createConnectedRequestCoordinator<PropsWithRedux>()(createRequestClient(requests)(Header)));
