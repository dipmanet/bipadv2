...
import { setGeneralDataAction } from "#actionCreators";
			...
			<div className={styles.palikaTable}>
							...
							<>
								<tr>
                                 ...
                                 && tableHeader.map((item, i) => (
									...
									<th>
										<Gt section={Translations.dashboardTblHeaderFiscalYear} />{" "}
										<Icon
											name={iconName}
											className={styles.iconClassName}
									...
									<th>
										<Gt section={Translations.dashboardTblHeaderMunicipality} />{" "}
										<Icon
											name={iconName}
											className={styles.iconClassName}
									...
									<th>
										<Gt section={Translations.dashboardTblHeaderLastPublishedBy} />{" "}
										<Icon name={iconName} className={styles.iconClassName} />
									</th>
									<th>
								...
								{tableData.length > 0 &&
									tableData.map((item, index) => (
										<tr key={item.item.id}>
											...
											<td>
												{drrmLanguage.language === "np" ? item.fiscalYearNp : item.fiscalYear}
											</td>
											<td>{drrmLanguage.language === "np" ? item.provinceNp : item.province}</td>
											<td>{drrmLanguage.language === "np" ? item.districtNp : item.district}</td>
											<td>
												{drrmLanguage.language === "np" ? item.municipalityNp : item.municipality}
											</td>
											...
											<td>
												<div ref={node} className={styles.dropdown}>
													...
													<div
														id="myDropdown"
														className={
															reportId === item.item.id && downloadClicked
																? _cs(styles.dropdownContent, styles.show)
																: styles.dropdownContent
														}>
														<a
															id="summaryReport"
															href="#summary_report"
															onClick={() => window.open(item.item.summaryFileEn)}>
															{" "}
															<Gt section={Translations.dashboardTableSummaryReportDownload} />
														</a>
														<a
															id="fullReport"
															href="#full_report"
															onClick={() => window.open(item.item.fullFileEn)}>
															<Gt section={Translations.dashboardTableFullReportDownload} />
														</a>
													</div>
												</div>
											</td>
							...
							<>
                             ...
                             && tableHeader.map((item, i) => (
                                 <th key={item.i}>{item}</th>
									...
									<th>
										<Gt section={Translations.dashboardTblHeaderTitle} />{" "}
										<Icon
											name={iconName}
											className={styles.iconClassName}
											onClick={handleSortTitle}
								...
								{tableData.length > 0 &&
									tableData.map((data, index) => (
										<tr key={data.id}>
											...
											<td>
												{drrmLanguage.language === "np" ? data.fiscalYearNp : data.fiscalYear}
											</td>
				...
				{tableData && tableData.length === 0 && (
					<p className={styles.dataUnavailable}>
						{" "}
						<Gt section={Translations.DashBoardNoDataMessage} />
					</p>
				)}
			</div>
			{}
...
export default connect(mapStateToProps, mapDispatchToProps)(PalikaReportTable);
