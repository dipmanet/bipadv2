...
const TemperaturesMin = (props: Props) => {
	const { minTemp, handleMinTemp, hideForm, minTempFooter, bulletinEditData } = props;
		...
		<div className={styles.formContainer}>
			<div {...getRootProps({ className: "dropzone" })}>
				...
				<div className={styles.subContainer} style={{ marginLeft: "5px" }}>
					<Translation>{(t) => <p>{t("Daily Min Temperature")}</p>}</Translation>
					...
					{hideForm && minTempFooter && <p className={styles.footerText}>{minTempFooter}</p>}
				</div>
			</div>
		</div>
	);
