...
import { bulletinPageSelector, languageSelector, bulletinEditDataSelector } from "#selectors";
...
const TemperatureMax = (props: Props) => {
	const { maxTemp, handleMaxTemp, maxTempFooter, hideForm, bulletinEditData } = props;
	...
	return (
		<div className={styles.formContainer}>
			<div {...getRootProps({ className: "dropzone" })}>
				...
				<div className={styles.subContainer} style={{ marginRight: "5px" }}>
					<Translation>{(t) => <p>{t("Daily Max Temperature")}</p>}</Translation>
					...
					{hideForm && maxTempFooter && <p className={styles.footerText}>{maxTempFooter}</p>}
				</div>
			</div>
		</div>
	);
