...
import { incidentSummary, peopleLoss, genderWiseLoss, nepaliRef, englishRef } from "../formFields";
...
const Bulletin = (props: Props) => {
	...
	const getRegionDetails = ({ adminLevel, geoarea } = {}) => {
		...
		if (adminLevel === 3) {
			const districtId = districts.filter(
				(d) => d.id === municipalities.find((p) => p.id === geoarea).district
			)[0].id;
			const municipalityId = geoarea;
			const provinceId = districts.filter(
				(d) => d.id === municipalities.find((p) => p.id === geoarea).district
			)[0].province;
			return {
	...
	const handleFormRegion = (region, field, subfield) => {
		...
		handleSameHazardChange(
			{ district, coordinates, municipalityId, districtId, provinceId },
			field,
			"location"
		);
	};
	...
	return (
		<>
			...
			<div className={styles.formContainer}>
				<Translation>{(t) => <h2>{t("Daily Disaster Bulletin")}</h2>}</Translation>
				...
				<div className={styles.formSubContainer} style={{ flexWrap: "unset" }}>
					<div className={styles.formItem}>
						<form
							...
							<label htmlFor="date"> {language === "np" ? "मिती बाट" : "Date from"}</label>
							{Object.keys(bulletinEditData).length > 0 ? (
								<h3 style={{ position: "relative", bottom: "5px" }}>{ADToBS(dateAlt)}</h3>
							) : (
								<NepaliDatePicker
				...
				<div className={styles.formSubContainer} style={{ flexWrap: "unset" }}>
					<div className={styles.formItem}>
						<form
							...
							<label htmlFor="date"> {language === "np" ? "मिती सम्म" : "Date upto"}</label>
							{Object.keys(bulletinEditData).length > 0 ? (
								<h3 style={{ position: "relative", bottom: "5px" }}>{ADToBS(dateAltTo)}</h3>
							) : (
								<NepaliDatePicker
									inputClassName="form-control"
									className={styles.datePick}
						...
						<FormControl style={{ margin: "15px 0" }} fullWidth>
							...
							{filterDataTypeError ? (
								<FormHelperText style={{ color: "#f44336", marginLeft: "14px" }}>
									{language === "np"
										? "कृपया मिति प्रकार फिल्टर प्रविष्ट गर्नुहोस्"
										: "Please enter date type filter"}
								</FormHelperText>
							) : (
								""
							)}
				...
				<div className={styles.formSubContainer}>
					<div className={styles.formItem}>
						<FormControl fullWidth>
							<InputLabel>{language === "np" ? "बुलेटिन नं:" : "Bulletin No:"}</InputLabel>
							<Input
								type="number"
								value={sitRep}
					...
					<Translation>{(t) => <h3>{t("Disaster Hilights")}</h3>}</Translation>
					...
					{Object.keys(incidentSummary).map((field, idx) => (
						<div className={idx > 0 ? styles.formItemHalf : styles.formItem}>
							<FormControl fullWidth>
								<InputLabel>{language === "np" ? nepaliRef[field] : englishRef[field]}</InputLabel>
								<Input
									type="number"
				...
				<div className={styles.formSubContainer}>
					{Object.keys(peopleLoss).map((field) => (
						<>
							<h3> {language === "np" ? nepaliRef[field] : englishRef[field]}</h3>
							{Object.keys(peopleLoss[field]).map((subField) => (
								<div className={styles.formItemThird}>
									<FormControl fullWidth>
										<InputLabel>
											{language === "np" ? nepaliRef[subField] : englishRef[subField]}
										</InputLabel>
										<Input
				...
				<div className={styles.formSubContainer}>
					...
					{addedHazardFields &&
						...
						Object.keys(addedHazardFields).map((field) => (
							<>
								<h3 style={{ width: "90%" }}>{field && addedHazardFields[field].hazard}</h3>
								<button
									disabled={!filterDateType}
								...
								{field &&
									Object.keys(addedHazardFields[field]).map((subField) => {
										if (subField === "coordinates") {
											return (
											...
											<div className={styles.formItemThird}>
												<FormControl fullWidth>
													...
													<Input
														...
														onChange={(e) =>
															handleSameHazardChange(e.target.value, field, subField)
														}
														disableUnderline
														inputProps={{
															disableUnderline: true,
				...
				<div className={styles.formSubContainer}>
					<div className={styles.formItem}>
						<FormControl style={{ margin: "15px 0" }} fullWidth>
							<InputLabel id="hazardInput">
								<Translation>{(t) => <span>{t("Add New Hazard")}</span>}</Translation>
							</InputLabel>
							<Select
								disabled={!filterDateType}
								labelId="hazardLabel"
								...
								{hazardTypes &&
									Object.keys(hazardTypes).map((hT) => (
										<MenuItem
											value={language === "np" ? hazardTypes[hT].titleNe : hazardTypes[hT].title}>
											{language === "np" ? hazardTypes[hT].titleNe : hazardTypes[hT].title}
										</MenuItem>
									))}
							</Select>
						</FormControl>
				...
				{filterDataTypeError ? (
					<FormHelperText
						style={{ color: "#f44336", marginLeft: "14px", marginTop: "20px", fontSize: "16px" }}>
						{language === "np"
							? "कृपया माथि मिति प्रकार फिल्टर प्रविष्ट गर्नुहोस्"
							: "Please enter date type filter"}
					</FormHelperText>
				) : (
					""
