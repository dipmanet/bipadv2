...
const TemperaturesMin = (props: Props) => {
	const { rainSummaryPic, handleRainSummaryPic, hideForm, rainSummaryFooter, bulletinEditData } =
		props;
		...
		<div className={styles.formContainer}>
			<div {...getRootProps({ className: "dropzone" })}>
				...
				<div
					...
					{!hideForm && (
						<Translation>{(t) => <h3>{t("Picture of Daily Rainfall Map")}</h3>}</Translation>
					)}
					...
					{hideForm && rainSummaryFooter && (
						<p className={styles.footerText}>{rainSummaryFooter}</p>
					)}
				</div>
			</div>
		</div>
