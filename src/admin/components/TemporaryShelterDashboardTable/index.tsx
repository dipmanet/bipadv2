			...
			<TableRow>
				...
				{headCells.map((headCell) => {
					const data =
						...
						fetchedData.length &&
						fetchedData.length &&
						fetchedData[0].district === "" &&
						headCell.id === "district" ? (
							""
						) : fetchedData &&
						  fetchedData.length &&
						  fetchedData[0].municipality === "" &&
						  headCell.id === "municipality" ? (
							""
						) : fetchedData &&
						  fetchedData.length &&
						  fetchedData[0].province === "" &&
						  headCell.id === "province" ? (
							""
						) : (
							<TableCell
								align="center"
...
const TemporaryShelterDashboardTable = (props) => {
	...
	useEffect(() => {
		if (fetchedData && fetchedData.lowerLevelAggrigated) {
			...
			fetchedData.lowerLevelAggrigated.data.length &&
			fetchedData.lowerLevelAggrigated.data[0].beneficiaryDistrict_Province_TitleNe
				? ["district", "municipality", "ward"].forEach((key) => {
						delete tableHeader[key];
				  })
				: fetchedData.lowerLevelAggrigated.data.length &&
				  fetchedData.lowerLevelAggrigated.data[0].beneficiaryDistrict_TitleNe
				? ["province", "municipality", "ward"].forEach((key) => {
						delete tableHeader[key];
				  })
				: fetchedData.lowerLevelAggrigated.data.length &&
				  fetchedData.lowerLevelAggrigated.data[0].beneficiaryMunicipality_TitleNe
				? ["province", "district", "ward"].forEach((key) => {
						delete tableHeader[key];
				  })
				: fetchedData.lowerLevelAggrigated.data.length &&
				  fetchedData.lowerLevelAggrigated.data[0].beneficiaryWard_Title
			...
			const tableRows = fetchedData.lowerLevelAggrigated.data.map((row, index) => {
				const epidemicObj = {
					...
					municipality: row.beneficiaryMunicipality_TitleNe
						? row.beneficiaryMunicipality_TitleNe
						: "",
					...
					<div
						...
						<div
							...
							<h1 style={{ fontSize: "30px" }}>
								{fetchedData &&
									fetchedData.totalCounts &&
									englishToNepaliNumber(fetchedData.totalCounts.totalFirstTrancheFormFilled)}
							</h1>
						...
						<div
							...
							<h1 style={{ fontSize: "30px" }}>
								{fetchedData &&
									fetchedData.totalCounts &&
									englishToNepaliNumber(fetchedData.totalCounts.totalFirstTrancheFormUploaded)}
							</h1>
						...
						<div
							...
							<h1 style={{ fontSize: "30px" }}>
								{fetchedData &&
									fetchedData.totalCounts &&
									englishToNepaliNumber(fetchedData.totalCounts.totalSecondTrancheFormFilled)}
							</h1>
						...
						<div
							...
							<h1 style={{ fontSize: "30px" }}>
								{fetchedData &&
									fetchedData.totalCounts &&
									englishToNepaliNumber(fetchedData.totalCounts.totalSecondTrancheFormUploaded)}
							</h1>
							<h3>दोस्रो किस्ता फारम अपलोड गरियो</h3>
						</div>
					</div>
					{fetchedData &&
					fetchedData.lowerLevelAggrigated &&
					fetchedData.lowerLevelAggrigated.data.length ? (
						<div className={styles.rightOptions}>
							<h1 style={{ fontSize: "30px", marginLeft: "27px", marginTop: "24px" }}>
								तल्लो तह अनुसारको डाटा
							</h1>
						</div>
					) : (
						""
