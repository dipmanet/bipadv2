...
function EnhancedTableHead(props: EnhancedTableProps) {
    ...
    (property: string) => (event: React.MouseEvent<unknown>) => onRequestSort(event, property);
	const tableFields = ['title','quantity','unit', 'action'];
	const headCells = tableFields.map((invD:string)=>({
		id: invD,
		numeric: invD === 'quantity' ? true : false,
...
export default function InventoryTable(props: Props) {
	...
	// React.useEffect(() => {
	// 	console.log(covid19GetDataMainGroup);
	// }, [loadingCovid19GetGroup]);
					...
					(
						<Box sx={{ width: '100%'}}>
							<div className={styles.credentialSearch}>
								...
								<input className={styles.credentialSearchBar} type="search" name="" id=""
									value={searchValue}
