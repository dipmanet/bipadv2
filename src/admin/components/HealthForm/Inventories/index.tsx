...
const InstitutionDetails = (props: Props): JSX.Element => {
	...
	const { resourceID,inventoryItem, inventoryData } = useSelector((state : OurState)=> state.health);
	...
	const handleInvAdd = () => {
		...
		const postData =  {itemId: 1,quantity: quantity, description: inventory, resource: resourceID };
		axios
							...
							<FormGroup>
								<FormControlLabel 
									control={<Checkbox 
										...
										checked={inventoryNeeded} onChange={(e=>handleInvNeeded(e,'Yes'))}/>} 
									label={'Yes'} 
								/>
							</FormGroup>
							<FormGroup>
								<FormControlLabel 
									control={<Checkbox 
										disabled={getDisabled('hf_code')}
										checked={!inventoryNeeded} onChange={(e=>handleInvNeeded(e,'No'))}/>} 
									label={'No'} 
								/>
							</FormGroup>
						</FormControl>
