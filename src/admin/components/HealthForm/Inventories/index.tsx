...
const Inventories = (props: Props): JSX.Element => {
	const {
		...
		healthInfrastructurePage: { resourceID, inventoryItem, inventoryData },
	} = props;
	...
	useEffect(() => {
		window.scrollTo({ top: 400, left: 0 });
		axios.get(`${baseUrl}/inventory-item/`).then((data) => {
	...
	return (
		<>
			...
			<div className={styles.rowTitle2}>
				...
				<p>
					Inventories details include what inventories are available at the facility to respond in
					case of disaster.
				</p>
			</div>
			{inventoryData && inventoryData.length === 0 ? (
				<div className={styles.row3}>
					<AccentHeading content={"Please add all inventories below and click Finish"} />
				</div>
			) : (
				<div className={styles.row3}>
					<AccentHeading content={"Please add/edit all inventories below and click Finish"} />
				</div>
			)}
			...
			<div className={styles.containerForm}>
				...
				{(inventoryNeeded || inventoryData.length > 0) && (
					<>
						...
						<div className={styles.btnContainer}>
							<button type="button" onClick={handleInvAdd} disabled={getDisabled("inventory")}>
								+ Add Inventory
							</button>
						</div>
					</>
				...
				<InventoryTable getEditRowData={getEditRowData} openModal={openModal} />
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(createConnectedRequestCoordinator<ReduxProps>()(createRequestClient(requests)(Inventories)));
