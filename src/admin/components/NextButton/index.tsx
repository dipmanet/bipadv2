...
const NextButton = (props) => {
	...
	const {
		...
		userDataMain,
		healthInfrastructurePage: { healthFormEditData, resourceID },
	} = props;
		...
		if (userDataMain.isSuperuser) {
			...
			if (userDataMain.profile.role === "validator") {
				if (Object.keys(healthFormEditData).length > 0) {
					...
					setMenuItems(
						MenuItemsAll.filter((mI) => mI.permission.includes(userDataMain.profile.role))
					);
				}
			} else {
				setMenuItems(
	...
	const checkValidation = (aM) => {
		...
		if (aM === "Institution Details") {
			...
			setHealthInfrastructurePage({
				validationError: "Field Required: Name of the Health Institution",
			});
		...
		if (aM === "Location") {
			if (formData.point && formData.point.coordinates[0] && formData.ward) {
				setHealthInfrastructurePage({ validationError: null });
				// dispatch(setValidationError(null));
				return true;
			}
			setHealthInfrastructurePage({
				validationError:
					"Field required: Province, District, Municipality, Ward, Lattitude and Longitude",
			});
	...
	const handlePatch = () => {
		...
		if (getResourceId()) {
			axios
				.patch(
					`${baseUrl}/resource/${getResourceId()}/?meta=true`,
					getFormData({
						is_verified: formData.is_verified,
						is_approved: formData.is_approved,
						verfication_message: formData.verfication_message,
	...
	const getPrevDisabled = () => {
		if (activeMenu === "Institution Details" || activeMenu === "Inventories") {
			return true;
		}
		return false;
	};
	...
	return (
		<>
			...
			<div className={styles.nextContainer}>
				...
				<button type="button" onClick={returnHandleFunction(activeMenu)} className={styles.nextBtn}>
