...
import { sum, saveChart, encodeDate } from "#utils/common";
import {
				...
				{summaryData &&
					...
					Object.keys(cumulative).length > 0 && (
						<table className={styles.responseTable}>
							<tr>
								<th>
									<Translation>{(t) => <span>{t("Hazard")}</span>}</Translation>
								</th>
								<th>
									<Translation>{(t) => <span>{t("No. of Incidents")}</span>}</Translation>
								</th>
								<th>
								...
								<th>
									<Translation>{(t) => <span>{t("Families Affected")}</span>}</Translation>
								</th>
								<th>
									<Translation>{(t) => <span>{t("Estimated loss (NPR)")}</span>}</Translation>
								</th>
							</tr>
							{summaryData &&
								Object.keys(summaryData).map((hwL, i) => (
									<>
										<tr>
											<td>{hwL}</td>
											<td>{summaryData[hwL].incidents.toLocaleString()}</td>
											<td>{summaryData[hwL].deaths.toLocaleString()}</td>
											<td>{summaryData[hwL].missing.toLocaleString()}</td>
											<td>{summaryData[hwL].injured.toLocaleString()}</td>

											<td>{summaryData[hwL].familiesAffected.toLocaleString()}</td>
											<td>{summaryData[hwL].estimatedLoss.toLocaleString()}</td>
										</tr>
									</>
								))}
							<tr className={styles.lastRow}>
								...
								<td>{cumulative.incidents.toLocaleString()}</td>
								<td>{cumulative.deaths.toLocaleString()}</td>
								<td>{cumulative.missing.toLocaleString()}</td>
								<td>{cumulative.injured.toLocaleString()}</td>

								<td>{cumulative.familiesAffected.toLocaleString()}</td>
								<td>{cumulative.estimatedLoss.toLocaleString()}</td>
							</tr>
						</table>
					)}
			</div>
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(createConnectedRequestCoordinator<ReduxProps>()(createRequestClient(requests)(YearlyData)));
