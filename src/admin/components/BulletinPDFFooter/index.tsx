...
import { bulletinPageSelector, languageSelector } from "#selectors";
		...
		<div className={language === "np" ? styles.footerContainer : styles.footerContainerEnglish}>
			{selectedTemperatureImageType === 1 ? (
				<div className={styles.dailySummary}>
					<Translation>{(t) => <h2>{t("Daily Temperature and Rain Summary")}</h2>}</Translation>
			...
			<div className={styles.footer}>
				<h2>
					{" "}
					<Translation>{(t) => <span>{t("For additional info")}</span>}</Translation>
				</h2>
				...
				<p>
					<Translation>{(t) => <span>{t("GPO Box Number")}</span>}</Translation>: 213213{" "}
				</p>
				<p>
					<Translation>{(t) => <span>{t("Phone")}</span>}</Translation>: +977-1-4211195,
					+977-1-4211196{" "}
				</p>
				<p>
					<Translation>{(t) => <span>{t("Email")}</span>}</Translation>: info@bipad.gov.np
				</p>
			</div>
		</div>
	);
