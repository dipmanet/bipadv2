...
import { bulletinPageSelector, hazardTypeListSelector, languageSelector } from "#selectors";
...
const BulletinPDF = (props: Props) => {
	const { sitRep, incidentSummary, peopleLoss, hilight, hazardWiseLoss, feedback } =
		props.bulletinData;
	...
	const timeConverter = (time) => {
		...
		if (timeHour > 12) {
			currentTime = `${language === "np" ? "अपरान्ह " : ""}${timeHour - 12}:${timeMinute}${
				language === "np" ? " बजे" : " PM"
			}`;
		} else if (stringCheck === "00") {
			currentTime = `${language === "np" ? "पूर्वान्ह " : ""}12:${timeMinute}${
				language === "np" ? " बजे" : " AM"
			}`;
		} else {
			currentTime = `${language === "np" ? "पूर्वान्ह " : ""}${timeHour}:${timeMinute}${
				language === "np" ? " बजे" : " AM"
			}`;
		}
		return currentTime;
			...
			<div style={{ display: "flex", justifyContent: "center", width: "100%", paddingTop: gap }}>
				{payload.map((entry, index) => (
					<div
						...
						/>
						<span>{<Translation>{(t) => <span>{t(`${entry.value}`)}</span>}</Translation>}</span>
					</div>
				))}
			</div>
		);
	...
	useEffect(() => {
		...
		const getHazardColor = (hazardName) => {
			if (language === "np") {
				const h = Object.keys(hazardTypes).filter(
					(k) => hazardTypes[k].titleNe === hazardName || hazardTypes[k].title === hazardName
				);
				return hazardTypes[h[0]] ? hazardTypes[h[0]].color : "#000000";
			}
			const h = Object.keys(hazardTypes).filter(
				(k) => hazardTypes[k].title === hazardName || hazardTypes[k].titleNe === hazardName
			);
			return hazardTypes[h[0]] ? hazardTypes[h[0]].color : "#000000";
		};
	...
	return (
		<div className={language === "np" ? styles.pdfContainer : styles.pdfContainerEnglish}>
			<div className={styles.headerNLoss}>
				<div className={styles.header}>
					<div className={styles.subDiv}>
						<img src={GovLogo} alt="Nepal Government" />
						<div className={styles.govTitles}>
							<ul>
								<li>
									<Translation>{(t) => <span>{t("Nepal Government")}</span>}</Translation>
								</li>
								<li>
									<Translation>{(t) => <span>{t("Ministry of Home Affairs")}</span>}</Translation>
								</li>
								<li>
									<Translation>{(t) => <span>{t("ndrrma")}</span>}</Translation>
								</li>
							</ul>
						</div>
					</div>
					<div className={styles.reportTitles}>
						<h1>
							<Translation>{(t) => <span>{t("Daily Bipad Bulletin")}</span>}</Translation>
						</h1>
						<p style={{ marginTop: "5px", marginBottom: "5px" }}>
							{language === "np" ? `${day} ${month} ${year}` : `${day} ${monthEn} ${year}`} |{" "}
							{language === "np" ? "बुलेटिन नं" : "Bulletin No."} #{sitRep || 0}
						</p>
					</div>
				</div>
			...
			<div className={styles.pratikriyaContainer}>
				<h2>
					{" "}
					<Translation>{(t) => <span>{t("Disaster Hilights")}</span>}</Translation>
				</h2>
			...
			<div className={styles.provinceWiseLoss}>
				<div className={styles.provinceWiseChart}>
					...
					<ResponsiveContainer width="95%" height="100%">
						<BarChart
							...
							<YAxis type="category" dataKey="province" />
							{/* <Tooltip /> */}
				...
				<div className={styles.provinceWiseIncidentsMap}>
					...
					<IncidentMap hazardWiseLoss={hazardWiseLoss} incidentPoints={incidentPoints} />
					<div className={styles.pointSizeLegendContainer}>
						<header className={styles.header}>
							<h4 className={styles.heading}>
								<Translation>{(t) => <span>{t("Death Count")}</span>}</Translation>
							</h4>
						</header>
						<IncidentLegend
						...
						<div className={styles.hazardLegend}>
							<div className={styles.legendTitle}>
								<Translation>{(t) => <span>{t("Hazard Legend")}</span>}</Translation>
							</div>
							<HazardsLegend
