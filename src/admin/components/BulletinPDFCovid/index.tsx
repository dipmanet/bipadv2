...
import {
	...
	provincesRef,
	provincesTitleRef,
} from "src/admin/components/BulletinForm/formFields";
import { Translation } from "react-i18next";
import { bulletinPageSelector, hazardTypesSelector, languageSelector } from "#selectors";
...
const BulletinPDFLoss = (props: Props) => {
	...
	useEffect(() => {
		...
		const getDeathCount = (arr, f) => {
			...
			if (filteredArr && filteredArr.length > 0) {
				const deathObj = filteredArr.reduce((a, b) => ({
					deaths: a.deaths + Number(b.deaths),
				})).deaths;
				return deathObj;
			}
			return 0;
		...
		const uniqueField = [
			...new Set([...uniqueFieldHazard, ...uniqueFieldArr].map((j) => getHazard(j))),
		];

		const hazardsChartObj = uniqueField
			.filter((i) => !!i)
			.map((f) => ({
				hazard: f,
	...
	const bullets = [
		<div
			style={{ width: "10px", height: "10px", backgroundColor: "#A6B2DE", marginRight: "3px" }}
		/>,
		<div
			style={{ width: "10px", height: "10px", backgroundColor: "#3F69C8", marginRight: "3px" }}
		/>,
		<div
			style={{ width: "10px", height: "10px", backgroundColor: "#3457A6", marginRight: "3px" }}
		/>,
	];
	...
	useEffect(() => {
		const cD = Object.keys(covidProvinceWiseTotal).map((c) => ({
			...
			[language === "np" ? "कुल संक्रमित संन्ख्या" : "Total Affected"]:
				covidProvinceWiseTotal[c].totalAffected,
			[language === "np" ? "कुल सक्रिय संक्रमित संन्ख्या" : "Total Active"]: covidProvinceWiseTotal[
				c
			].totalActive
				? covidProvinceWiseTotal[c].totalActive
				: 0,
			[language === "np" ? "कुल मृत्‍यु संन्ख्या" : "Total Deaths"]: covidProvinceWiseTotal[c]
				.totalDeaths
				? covidProvinceWiseTotal[c].totalDeaths
				: 0,
		}));
		...
		<div className={language === "np" ? styles.covidPDFContainer : styles.covidPDFContainerEnglish}>
					...
					<ResponsiveContainer width="100%" height="100%">
						<BarChart
							...
							<XAxis
								...
								tick={{ fontSize: 14 }}
							/>
							<YAxis type="category" dataKey="hazard" tick={{ fontSize: 14 }} />
			...
			<div className={styles.container12}>
				<div className={styles.genderWiseStats}>
					<h2>
						<Translation>{(t) => <span>{t("Genderwise Breakdown of Deaths")}</span>}</Translation>
					</h2>
					{getChartNull(genderWiseLossChart) ? (
						<div className={styles.noDataPie}>
							<Translation>{(t) => <h1>{t("No Deaths Reported")}</h1>}</Translation>
						</div>
					) : (
						<ResponsiveContainer width="100%" height="100%">
							<PieChart width={200} height={150} margin={{ top: 0, bottom: 0, left: 0, right: 0 }}>
								<Pie
