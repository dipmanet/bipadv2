...
const HealthBulkUpload = (props) => {
	...
	const {
		healthInfrastructurePage: { uploadData },
		userDataMain,
		uri,
	} = props;
	...
	const downloadFile = () => {
		axios.get(`${baseUrl}/data_template_file/`).then((file) => {
			const template = file.data.results.filter(
				(item) => item.dataTemplateSlug === "heoc_admin_health_facility_bulk_upload"
			);
					...
					<div className={styles.headTextAres}>
						...
						<div className={styles.ideaDescription}>
							...
							<p className={styles.shortDescription}>
								To upload a list of health institution data, kindly download the template sheet,
								fill the data in the required heading, and upload it below. Do make sure that the
								data are in excel (.xsls) format and headings provided in the template are unchanged
								before uploading it. The bulk data uploaded will be processed, and verified by the
								system before integrating into HEOC and in BIPAD portal and the user will be
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(createConnectedRequestCoordinator<ReduxProps>()(createRequestClient(requests)(HealthBulkUpload)));
