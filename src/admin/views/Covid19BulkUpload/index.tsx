...
const CovidBulkUpload = (props) => {
	...
	const {
		covidPage: { covid19BulkIndividualData, covid19BulkGroupData },
		userDataMain,
		uri,
	} = props;
	...
	const uploadBulkFile = (formData) => {
		...
		if (acceptedFiles.length === 0) {
			...
			if (Object.keys(errors).length === 0 && acceptedFiles[0]) {
				axios
					.post(`${baseUrl}/bulkupload/`, getFormData(individulaBulkData))
					.then(() => {
			...
			if (Object.keys(errors).length === 0 && acceptedFiles[0]) {
				axios
					.post(`${baseUrl}/bulkupload/`, getFormData(groupBulkData))
					.then(() => {
	...
	// template download
	const downloadFile = () => {
		axios.get(`${baseUrl}/data_template_file/`).then((file) => {
			let template;
			if (formtoggler === "Individual Form") {
				template = file.data.results.filter(
					(item) => item.dataTemplateSlug === "heoc_admin_covid19_individual_bulk_upload"
				);
			} else {
				template = file.data.results.filter(
					(item) => item.dataTemplateSlug === "heoc_admin_covid19_group_bulk_upload"
				);
			}
						...
						<div className={styles.ideaDescription}>
							...
							{formtoggler === "Individual Form" ? (
								<p className={styles.shortDescription}>
									...
									fill the data in the required heading, and upload it below. Do make sure that the
									data are in excel (.xsls) format and headings provided in the template are
									unchanged before uploading it. The bulk data uploaded will be processed, and
									verified by the system before integrating into HEOC and the user will be notified
									about the status of the bulk upload.
								</p>
							) : (
								<p className={styles.shortDescription}>
									To upload group COVID data in bulk, kindly download the template sheet, fill the
									data in the required heading, and upload it below. Do make sure that the data are
									in excel (.xsls) format and headings provided in the template are unchanged before
									uploading it. The bulk data uploaded will be processed, and verified by the system
									before integrating into HEOC and the user will be notified about the status of the
									bulk upload.
								</p>
							)}
						</div>
					...
					<div className={styles.maintoggler}>
						{["Individual Form", "Group Form"].map((item) => (
							<button
								...
								className={
									item === formtoggler ? styles.togglerIndGroActive : styles.togglerIndGro
								}>
								{item}
							</button>
						))}
						...
						<div className={styles.app}>
							...
							<div>
								{acceptedFiles.length > 0 ? (
									<>
										<strong>Files:</strong>
										<aside>{acceptedFiles.map((file) => `${file.name} ${file.size}`)}</aside>
									</>
								) : (
									"Add some file"
								)}
					...
					<form className={styles.form}>
						<div className={styles.formArea}>
							...
							<div className={styles.buttonSection}>
								<button
									disabled={getDisabled()}
									onClick={handleSubmit(uploadBulkFile)}
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(createConnectedRequestCoordinator<ReduxProps>()(createRequestClient(requests)(CovidBulkUpload)));
