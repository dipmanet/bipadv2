...
const MapSource = (props: Props) => {
	const { sourceOptions, sourceKey, geoJson, children } = props;
	...
	useEffect(() => {
		...
		const options =
			initialSourceOptions.type === "geojson"
	...
	return (
		<SourceChildContext.Provider value={childrenProps}>{children}</SourceChildContext.Provider>
	);
};
