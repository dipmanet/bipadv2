...
function FloatingContainer(props: Props) {
	...
	const handleMouseDown = useCallback(
		(e: MouseEvent) => {
			...
			const isTargetOrContainsTarget =
				container && (container === e.target || container.contains(e.target as HTMLElement));

			const isTargetParentOrContainedInParent =
				parent && (parent === e.target || parent.contains(e.target as HTMLElement));
		...
		[onBlur, onMouseDown, parentRef, containerRef]
	);
	...
	const myClassName = _cs(classNameFromProps, styles.floatingContainer, "floating-container");

	const [containerId, className] = useHaze(myClassName, showHaze);

	return (
		<Float onInvalidate={handleContainerInvalidate} focusTrap={focusTrap}>
			<div id={containerId} className={className} ref={containerRef}>
				{children}
			</div>
		</Float>
	);
