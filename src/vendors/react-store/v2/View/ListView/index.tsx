...
function ListView<D, P, K extends OptionKey, GP, GK extends OptionKey>(
	...
	if (isEmpty) {
		if (filtered) {
			content = !pending ? <EmptyWhenFilterComponent className={styles.empty} /> : null;
		} else {
			content = EmptyComponent && !pending ? <EmptyComponent className={styles.empty} /> : null;
		}
	} else {
		content = <List {...otherProps} />;
	}
