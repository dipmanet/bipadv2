...
function Options<T, K extends OptionKey>(props: Props<T, K>) {
	...
	const rendererParams = useCallback(
		(key: K, option: T) => {
			const label = optionLabelSelector ? optionLabelSelector(option) : labelSelector(option);
		...
		[focusedKey, labelSelector, onOptionClick, onOptionFocus, optionLabelSelector, value]
	);

	const className = _cs(classNameFromProps, styles.options, "options");
