...
import { calcFloatPositionInMainWindow, defaultLimit, defaultOffset } from "../../../utils/bounds";
...
function SelectInputBase<T, K extends OptionKey = string>(props: SelectInputBaseProps<T, K>) {
	...
	const handleOptionsInvalidate = useCallback(
		(optionsContainer: HTMLDivElement) => {
			...
			const { current: container } = containerRef;
			const parentRect = container ? container.getBoundingClientRect() : boundingClientRect.current;
	...
	const handleFocusChange = useCallback((key: K | undefined) => {
		setFocusKey(key);
	}, []);
	...
	const inputText: string | undefined = !showPopup ? selectedValueLabel : searchValue;

	const isFilled = isTruthyString(inputText);

	const myPlaceholder =
		showPopup && isTruthyString(selectedValueLabel) ? selectedValueLabel : placeholder;
	...
	return (
		<div className={className} title={title}>
			...
			<div ref={containerRef} className={_cs(styles.inputAndActions, "input-and-actions")}>
				<RawInput
					ref={inputRef}
				...
				{showDropdownArrowButton && (
					<Button
						tabIndex={-1}
						iconName={showPopup ? "arrowDropup" : "arrowDropdown"}
