...
export default class FloatingContainer extends React.PureComponent {
	...
	componentDidMount() {
		const { onBlur, closeOnEscape } = this.props;
	...
	handleMouseDown = (e) => {
		const { onBlur, onMouseDown, parent } = this.props;
		...
		const isTargetOrContainsTarget =
			container && (container === e.target || container.contains(e.target));
		const isTargetParentOrContainedInParent =
			parent && (parent === e.target || parent.contains(e.target));
	...
	render() {
		const { className: classNameFromProps, focusTrap, children, showHaze } = this.props;

		const className = _cs(classNameFromProps, styles.floatingContainer, "floating-container");
		...
		return (
			<Float onInvalidate={this.handleContainerInvalidate} focusTrap={focusTrap}>
				{showHaze ? <Haze>{child}</Haze> : child}
			</Float>
		);
	}
}
