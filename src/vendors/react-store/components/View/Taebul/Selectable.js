...
export default (WrappedComponent) => {
	const SelectedComponent = class extends React.Component {
		...
		handleSelectAllClick = () => {
			const { settings, onChange, data, keySelector } = this.props;
				...
				headerRenderer: ({
					...
					return (
						<div className={className}>
							<AccentButton className={buttonIcon} onClick={buttonAction} transparent />
						</div>
					);
				},
		...
		render() {
			const { data, settings, columns, onChange, keySelector, selectClassName, ...otherProps } =
				this.props;

			const newData = this.selectData(data, settings.selectedKeys, keySelector);
			const newColumns = this.modifyColumns(columns, data, selectClassName, keySelector);
