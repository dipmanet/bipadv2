...
const getVirtualizedRenderParams = (columns, scrollOffset, width, itemWidths) => {
	let i = 0;
	let startX = 0;
	let startIndex;
...
export default class Taebul extends React.Component {
	...
	getTotalWidth = memoize((columns, columnWidths, defaultColumnWidth) => {
        ...
        */
		...
		columns.forEach((c) => {
			totalWidth += isDefined(columnWidths[c.key]) ? columnWidths[c.key] : defaultColumnWidth;
		});

		return totalWidth;
	});

	getItemWidths = memoize((columns, columnWidths, defaultColumnWidth) => {
		const itemWidths = {};
		columns.forEach((c) => {
			itemWidths[c.key] = isDefined(columnWidths[c.key]) ? columnWidths[c.key] : defaultColumnWidth;
		});
	...
	calculateRowVirtualizationParams = (
		{ columns, settings: { columnWidths = emptyObject, defaultColumnWidth } },
		scrollLeft
	) => {
		const rowWidth = this.getTotalWidth(columns, columnWidths, defaultColumnWidth);

		const itemWidths = this.getItemWidths(columns, columnWidths, defaultColumnWidth);
	...
	headerRendererParams = (columnKey, column) => {
		...
		const { headerRenderer, headerRendererParams } = column;
	...
	render() {
		const {
			...
			settings: { columnWidths = emptyObject, defaultColumnWidth },
		...
		const minWidth = this.getTotalWidth(columns, columnWidths, defaultColumnWidth);

		// const headClassName = `${styles.head} ${headClassNameFromProps}`;

		return (
			<div id={this.localContainerId} className={className}>
				<ListView
					id={this.localHeadId}
