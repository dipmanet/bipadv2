...
class ResizableHeader extends React.Component {
	static propTypes = {
		_columnKey: PropTypes.oneOfType([PropTypes.string, PropTypes.number]).isRequired,
	...
	handleSeparatorMouseDown = (e) => {
		const { _columnKey: columnKey, _onSeparatorMouseDown: onSeparatorMouseDown } = this.props;
	...
	render() {
		...
		return (
			<div className={className}>
				...
				<div onMouseDown={this.handleSeparatorMouseDown} className={styles.separator} />
			</div>
		);
	}
}
...
export default (WrappedComponent) => {
	...
	const ColumnWidthComponent = class extends React.Component {
		...
		modifyColumns = memoize((columns = [], columnWidths = {}, defaultWidth) => {
			...
			const newColumns = produce(columns, (draftColumns) => {
				draftColumns.forEach((column, index) => {
					...
					const { key: columnKey, headerRendererParams, headerRenderer } = column;
					const width = isFalsy(columnWidths[columnKey]) ? defaultWidth : columnWidths[columnKey];
			...
			this.idleCallback = window.requestIdleCallback(
				() => {
					...
					const newSettings = produce(settings, (draftSettings) => {
						...
						let newValue = isFalsy(value) ? defaultColumnWidth + dx : value + dx;
		...
		handleSeparatorMouseDown = (e, columnKey) => {
			const {
				settings: { columnWidths = {}, defaultColumnWidth = DEFAULT_COLUMN_WIDTH },
			} = this.props;
		...
		render() {
			const { columns, settings, ...otherProps } = this.props;

			const { columnWidths, defaultColumnWidth = DEFAULT_COLUMN_WIDTH } = settings;

			const newColumns = this.modifyColumns(columns, columnWidths, defaultColumnWidth);
			return <WrappedComponent columns={newColumns} settings={settings} {...otherProps} />;
		}
	};
