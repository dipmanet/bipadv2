...
const createNestedSplits = (splits = []) => {
	...
	for (let i = 0; i < splits.length; i += 1) {
		...
		const { start: parentStart, end: parentEnd } = parent;

		const children = [];
		for (let j = i + 1; j < splits.length; j += 1) {
			const child = splits[j];

			const { start: childStart, end: childEnd, highlight: childHighlight } = child;
...
export default class HighlightedText extends React.PureComponent {
	...
	renderSplits = (text, splits, level = 1) => {
		...
		splits.forEach((split) => {
			...
			if (index < splitIndex) {
				result.push(
					<span key={`split-${level}-${start}`}>{text.substring(index, splitIndex)}</span>
				);
			}
			if (splitIndex === end) {
				return;
			...
			result.push(
				<Renderer
					...
					text={actualStr}
					{...otherProps}>
					{children.length > 0 ? this.renderSplits(splitStr, children, level + 1) : splitStr}
				</Renderer>
			);
		...
		if (index < text.length) {
			result.push(<span key={`split-${level}`}>{text.substring(index)}</span>);
		}
	...
	render() {
		const { className, highlights, text, startIndexSelector, endIndexSelector } = this.props;
		...
		return (
			<div className={_cs(className, styles.highlightedText)}>
				<p>{this.renderSplits(text, nestedSplits)}</p>
			</div>
		);
	}
}
