...
export default class Highlight extends React.PureComponent {
	...
	handleClick = (e) => {
		const { onClick, text, highlightKey, highlight } = this.props;
	...
	render() {
		const { color, label, tooltip, children, className: classNameFromProps } = this.props;
		...
		const className = _cs(styles.highlight, classNameFromProps, "tc-highlighted-text-highlight");
