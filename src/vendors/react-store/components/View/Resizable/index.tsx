...
export default class Resizable extends React.PureComponent {
	...
	getSeparatorClassName = () => {
		...
		const classNames = [separatorClassName, "separator", styles.separator];

		return classNames.join(" ");
	};

	getOverlayClassName = () => {
		const classNames = ["overlay", styles.overlay];
		return classNames.join(" ");
	};

	getClassName = () => {
		const { className, resizableClassName } = this.props;
		const classNames = [className, resizableClassName, styles[resizableClassName]];
	...
	getFirstContainerClassName = () => {
		...
		const classNames = [firstContainerClassName, "first", styles.first];
	...
	getSecondContainerClassName = () => {
		...
		const classNames = [secondContainerClassName, "second", styles.second];
	...
	handleMouseUp = (e) => {
		if (this.resizing) {
			...
			this.props.resizeContainers(this.firstContainer, this.secondContainer, this.initialSize + dx);
	...
	render() {
		const { firstChild, secondChild, disabled } = this.props;
