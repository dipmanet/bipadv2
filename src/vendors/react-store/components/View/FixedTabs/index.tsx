...
export default class FixedTabs extends React.Component {
	...
	getClassName = () => {
		...
		const classNames = [className, "fixed-tabs", styles.fixedTabs];
	...
	getTabClassName = (isActive) => {
		...
		const classNames = [itemClassName, styles.tab, "fixed-tab"];
	...
	renderTab = (_, data, index) => {
		const { active, tabs, useHash, modifier } = this.props;
		...
		if (!useHash) {
			...
			return (
				<button onClick={onClick} className={className} key={data} type="button">
					{content}
				</button>
			);
		}
		...
		return (
			<a onClick={onClick} href={`#/${data}`} className={className} key={data}>
				{content}
			</a>
		);
	};
	...
	render() {
		const { tabs, useHash, defaultHash, children } = this.props;
		...
		return (
			<div className={className}>
				...
				<div className={styles.blank}>{children}</div>
			</div>
		);
	}
}
