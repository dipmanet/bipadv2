...
export default class Pager extends React.PureComponent {
	...
	pagination = (totalCapacity, active, total) => {
		const { disabled, onPageClick } = this.props;
		...
		if (right.capacity > 0) {
			if (right.excess >= 0) {
				lst = [...lst, ...range(startIndex, active - 1).map(this.renderButton)];
			} else {
				lst = [
					...lst,
		...
		lst = [...lst, this.renderSpan(styles.active, active)];

		if (left.capacity > 0) {
			if (left.excess >= 0) {
				lst = [...lst, ...range(active + 1, lastIndex).map(this.renderButton)];
			} else {
				lst = [
					...lst,
	...
	renderSpan = (className, index, key) => (
		<span className={`${styles.paginateSpan} ${className}`} key={isFalsy(key, [""]) ? index : key}>
			{index}
		</span>
	);

	renderButton = (index) => {
		const { onPageClick, disabled } = this.props;
				...
				{showItemsPerPageChange && (
					<div className={styles.itemsPerPage}>
						<SelectInput
							...
							disabled={disabled}
						/>
						<div className={styles.perPage}>{perPageTitle}</div>
					</div>
				)}
				{showInfo && (
					<div className={styles.currentRangeInformation}>
						<div className={styles.showing}>{showingTitle}</div>
						<div className={styles.currentItemsStart}>{currentItemsStart}</div>
						<div className={styles.rangeIndicator}>{rangeIndicator}</div>
						<div className={styles.currentItemsEnd}>{currentItemsEnd}</div>
						<div className={styles.of}>{ofTitle}</div>
						<div className={styles.itemCount}>{itemsCount}</div>
					</div>
				)}
				<div className={styles.pageList}>{pages}</div>
			</div>
		);
	}
}
