...
export default class MultiViewContainer extends React.Component {
	...
	getHash = () => window.location.hash.substr(2);
	...
	renderContainer = (p) => {
		const { activeClassName, containerClassName } = this.props;

		const { view, isActive, ...otherProps } = p;

		const Component = view.component;

		if (view.wrapContainer) {
			const className = _cs(containerClassName, styles.container, isActive && activeClassName);
		...
		const className = _cs(isActive && activeClassName);

		return <Component className={className} {...otherProps} />;
	};

	renderView = (k, data) => {
		const { views, active, useHash } = this.props;
		...
		if (lazyMount) {
			...
			return <LazyContainer key={key} view={view} isActive={isActive} {...params} />;
		}

		const params = view.rendererParams ? view.rendererParams() : {};
		return <Container key={key} view={view} isActive={isActive} {...params} />;
	};
	...
	render() {
		...
		return <List data={viewList} modifier={this.renderView} />;
	}
}
