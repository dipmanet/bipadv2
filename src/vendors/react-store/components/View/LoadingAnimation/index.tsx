...
export default class LoadingAnimation extends React.PureComponent {
	...
	render() {
		const {
			...
			delay, // eslint-disable-line @typescript-eslint/no-unused-vars
			message,
			spinnerClassName,
			...otherProps
		} = this.props;

		const { showMessage, sizeCategory } = this.state;
		...
		return (
			<Message
				...
				{...otherProps}>
				<Spinner className={spinnerClassName} size={sizeCategory} />
