...
const defaultEmptyComponent = () => {
	const classNames = ["empty", styles.empty];
...
export class NormalListView extends React.Component {
	...
	render() {
		...
		if (isEmpty) {
			if (isFiltered) {
				content = !pending ? <EmptyWhenFilter className={styles.empty} /> : null;
			} else {
				content = EmptyComponent && !pending ? <EmptyComponent /> : null;
			}
		} else {
			content = <NormalList data={data} {...otherProps} />;
		}
