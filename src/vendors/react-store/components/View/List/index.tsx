...
export class NormalList extends React.Component {
	...
	renderListItem = (datum, i) => {
		...
		if (Renderer) {
			...
			return <Renderer className={rendererClassName} key={key} {...extraProps} />;
		}

		// If there is no modifier, then return a ListItem
		return <ListItem key={key}>{datum}</ListItem>;
	};

	renderListItemFromGroup = (datum, groupKey, i) => this.renderListItem(datum, i);
	...
	renderGroup = (groupKey) => {
		...
		const extraProps = groupRendererParams ? groupRendererParams(groupKey) : undefined;

		return <Renderer key={groupKey} className={groupRendererClassName} {...extraProps} />;
	};

	render() {
		const { data, groupKeySelector, groupComparator } = this.props;
		...
		const groups = listToGroupList(data, groupKeySelector, this.renderListItemFromGroup);
