...
export default class Headers extends React.Component {
	...
	renderHeader = (key, header) => {
		const { headers, headerModifier, highlightColumnKeys, disabled } = this.props;
	...
	render() {
		const { headers, className: classNameFromProps } = this.props;

		return (
			<thead className={_cs(classNameFromProps, "thead")}>
				<tr>
					<List data={headers} keySelector={this.getHeaderKey} modifier={this.renderHeader} />
				</tr>
			</thead>
		);
	}
