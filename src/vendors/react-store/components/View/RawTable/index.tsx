...
const DefaultEmptyComponent = ({ className, isFiltered }) => (
	<Message className={className}>
		{isFiltered ? "There are no items that match your filtering criteria." : "There are no items."}
	</Message>
);
...
export default class RawTable extends React.Component {
	...
	static getSortedHeaders = (headers = []) => [...headers].sort((a, b) => a.order - b.order);

	static headerKeyExtractor = (header) => header.key;

	constructor(props) {
		super(props);
		const { headers: headersFromProps } = this.props;
	...
	UNSAFE_componentWillReceiveProps(nextProps) {
		const { data: oldData, headers: oldHeaders, onDataSort } = this.props;
	...
	render() {
		...
		const { headers, headersOrder } = this.state;
				...
				<div className={_cs(styles.scrollWrapper, "raw-table-scroll-wrapper")}>
					...
					{data.length <= 0 && !pending && (
						<div className={styles.emptyContainer}>
							<EmptyComponent className={_cs("empty", styles.empty)} isFiltered={isFiltered} />
						</div>
					)}
				</div>
				{pending && <LoadingAnimation message="Fetching data..." />}
			</div>
		);
	}
}
