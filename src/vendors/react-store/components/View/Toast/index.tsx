...
const propTypes = {
	notification: PropTypes.shape({
		...
		actionButtons: PropTypes.oneOfType([PropTypes.node, PropTypes.arrayOf(PropTypes.node)]),
	}).isRequired,
...
export default class Toast extends React.Component {
	...
	getClassName = () => {
		const { shown, notification } = this.state;
					...
					{notification && (
						<div className={styles.container}>
							<header className={styles.header}>
								<h4 className={styles.heading}>{notification.title}</h4>
							...
							<div className={styles.mainContent}>
								<Icon className={styles.icon} name={this.getIconName()} />
								<div className={styles.message}>{notification.message}</div>
								<div className={styles.actionButtons}>{notification.actionButtons}</div>
							</div>
						</div>
					)}
				</div>
