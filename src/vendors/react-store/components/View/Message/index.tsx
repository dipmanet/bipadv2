...
const calculateDimensionCost = (width, height, factor) => width * Math.sqrt(height) * factor;
...
export default class Message extends React.PureComponent {
	...
	handleResize = (e) => {
		...
		const fontSize = calculateRelativeValue(minFontSize, maxFontSize, width, height, resizeFactor);
		...
		if (onSizeCategorization) {
			// small / medium / large
			const sizeCategory =
				sizeCategoryMap[calculateRelativeValue(1, 3, width, height, resizeFactor)];
	...
	render() {
		const { className: classNameFromProps, children } = this.props;
