...
import { getLayoutBounds, getSortedItems } from "../../../utils/grid-layout";
...
export default class GridViewLayout extends React.PureComponent {
	...
	UNSAFE_componentWillReceiveProps(nextProps) {
		const { layoutSelector: newLayoutSelector, data: newData } = nextProps;

		const { layoutSelector: oldLayoutSelector, data: oldData } = this.props;

		if (newLayoutSelector !== oldLayoutSelector || newData !== oldData) {
	...
	render() {
		const { className: classNameFromProps, keySelector, itemClassName } = this.props;
