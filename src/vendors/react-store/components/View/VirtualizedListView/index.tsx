...
import { isFalsy, _cs } from "@togglecorp/fujs";
...
const DefaultEmptyComponent = () => {
	...
	return <Message className={className}>Nothing to show.</Message>;
};
...
class VirtualizedListView extends React.Component {
	...
	UNSAFE_componentWillReceiveProps(nextProps) {
		const { itemHeight, boundingClientRect } = this.props;

		const { itemHeight: newItemHeight, boundingClientRect: newBoundingClientRect } = nextProps;
	...
	renderItem = (datum, i) => {
		...
		if (Renderer) {
			...
			return (
				<Renderer className={rendererClassName} key={key} itemHeight={itemHeight} {...extraProps} />
			);
		}
	...
	renderItems = () => {
		...
		const { itemsPerPage, offset, itemHeight } = this.state;

		if (isFalsy(itemHeight)) {
			return <div ref={this.item}>{this.renderItem(data[0], 0)}</div>;
		}
	...
	render() {
		...
		return (
			<div ref={this.container} id={id} className={className}>
				<Items />
				{data.length === 0 && (
					<React.Fragment>
						{EmptyComponent && <EmptyComponent />}
