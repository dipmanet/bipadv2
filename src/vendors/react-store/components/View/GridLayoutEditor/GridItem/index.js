...
import { addClassName, removeClassName } from "../../../../utils/common";
...
const areLayoutsEqual = (l1, l2) =>
	l1.left === l2.left && l1.top === l2.top && l1.width === l2.width && l1.height === l2.height;
...
export default class GridItem extends React.Component {
	...
	constructor(props) {
		...
		const { datum, layoutSelector, minSizeSelector } = props;
	...
	UNSAFE_componentWillReceiveProps(nextProps) {
		const { layoutSelector: oldLayoutSelector, datum: oldDatum } = this.props;

		const { layoutSelector: newLayoutSelector, datum: newDatum, minSizeSelector } = nextProps;
	...
	testLayoutValidity = (newLayout) => {
		const { itemKey, layoutValidator } = this.props;

		const { current: container } = this.containerRef;

		const { isLayoutValid, newPossibleLayout } = layoutValidator(
			itemKey,
			newLayout,
			this.isResizing
		);
		...
		if (newPossibleLayout) {
			const { isLayoutValid: isNewLayoutValid } = layoutValidator(
				itemKey,
				newPossibleLayout,
				this.isResizing
			);
	...
	handleMouseUp = (e) => {
		...
		const { onLayoutChange, itemKey } = this.props;
	...
	calcBoundsExcess = () => {
		...
		const { left: scrollLeft, top: scrollTop, width: scrollWidth, height: scrollHeight } = scroll;
	...
	renderHeader = () => {
		const { datum, headerModifier } = this.props;

		return headerModifier(datum);
	};

	renderContent = () => {
		const { datum, contentModifier } = this.props;

		return contentModifier(datum);
	};

	renderResizeHandle = () => {
		const className = [styles.resizeHandle, "resize-handle"].join(" ");
	...
	renderGhostItem = () => {
		const { width, height, left, top } = this.lastValidLayout;
		...
		return (
			<div style={style} className={styles.ghostItem}>
