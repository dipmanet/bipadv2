...
export default class GridLayoutEditor extends React.Component {
	...
	constructor(props) {
		...
		const { data, keySelector, layoutSelector } = props;
	...
	fixItemLayout = (key) => {
		...
		const compareLayouts = (k1, k2) => {
			...
			return l1.top + l1.height - (l2.top + l2.height) || l1.left + l1.width - (l2.left + l2.width);
		};
		...
		for (let i = 0; i < layoutKeyList.length; i += 1) {
			...
			if (doesIntersect(reduceLayout(currentLayout, gridSize), reduceLayout(newLayout, gridSize))) {
	...
	handleItemLayoutValidation = (key, newLayout, forResize) => {
		...
		for (let i = 0; i < layoutKeyList.length; i += 1) {
			...
			if (doesIntersect(reduceLayout(newLayout, gridSize), reduceLayout(otherLayout, gridSize))) {
				isLayoutValid = false;
	...
	handleLayoutChange = (key, layout) => {
		const { onLayoutChange, gridSize } = this.props;
	...
	render() {
		...
		return (
			<div ref={this.containerRef} className={className} style={superContainerStyle}>
