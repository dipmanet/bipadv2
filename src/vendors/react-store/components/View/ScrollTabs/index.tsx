...
export default class ScrollTabs extends React.Component {
	...
	getClassName = () => {
		...
		const classNames = [className, "scroll-tabs", styles.scrollTabs];
	...
	getTabClassName = ({ isActive, isBasic, isDisabled }) => {
	...
	handleTabClick = (key) => (e) => {
		const { onClick, useHash, replaceHistory } = this.props;
	...
	renderTab = (_, data, index) => {
		...
		const extraProps = rendererParams ? rendererParams(data, tabs[data], index) : undefined;
		...
		return (
			<a key={data} className={className} onClick={onClick} href={`#/${data}`}>
				{tabs[data]}
			</a>
		);
	};
	...
	render() {
		const { tabs, useHash, defaultHash, showBeforeTabs, children, blankClassName } = this.props;

		// FIXME: generate tabList when tabs change
		const tabList = Object.keys(tabs);
		const className = this.getClassName();
		return (
			<div ref={this.mainContainerRef} className={className}>
				<HashManager
					tabs={tabs}
					useHash={useHash}
				...
				/>
				<div ref={this.tabsContainerRef} className={styles.tabsContainer}>
					{showBeforeTabs && <div className={styles.nonBlank}>{children}</div>}
					<List data={tabList} modifier={this.renderTab} />
					<div className={_cs(styles.blank, blankClassName)}>{!showBeforeTabs && children}</div>
				</div>
				<Button
