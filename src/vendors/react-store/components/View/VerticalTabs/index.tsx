...
export default class VerticalTabs extends React.Component {
	...
	getClassName = () => {
		...
		const classNames = [className, "vertical-tabs", styles.verticalTabs];
	...
	getTabClassName = (isActive) => {
		const classNames = [styles.tab, "vertical-tab"];
	...
	renderTab = (_, data, index) => {
		...
		if (!useHash) {
			...
			return (
				<button className={className} key={data} onClick={onClick} type="button">
					{content}
				</button>
			);
		}
	...
	render() {
		...
		return (
			<div className={className}>
				...
				/>
				<List data={tabList} modifier={this.renderTab} {...otherProps} />
			</div>
		);
	}
}
