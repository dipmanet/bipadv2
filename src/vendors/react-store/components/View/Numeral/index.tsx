...
import { addSeparator, isFalsy, isTruthy, formattedNormalize } from "@togglecorp/fujs";
	...
	render() {
		...
		return (
			<span className={`numeral ${className} ${styles.numeral}`}>
				{isTruthy(prefix) && <span className="prefix">{prefix}</span>}
				{isTruthy(showSign) && value !== 0 && <span className="sign">{value > 0 ? "+" : "-"}</span>}
				<span className="number">{number}</span>
				{isTruthy(normalizeSuffix) && <span className="normalized-suffix">{normalizeSuffix}</span>}
				{isTruthy(suffix) && <span className="suffix">{suffix}</span>}
			</span>
		);
	}
}
