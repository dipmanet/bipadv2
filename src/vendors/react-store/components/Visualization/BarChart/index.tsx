...
const propTypes = {
	 ...
	 * Highlight which bar
	 */
	highlightBarX: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
	/**
	 * Chart Margins
	 */
...
class BarChart extends React.PureComponent {
	...
	constructor(props) {
		...
		this.scaleX = scaleBand().padding(barPadding);
	...
	renderBarChart() {
		...
		this.scaleY.domain([min([0, min(data, (d) => d[yKey])]), max([0, max(data, (d) => d[yKey])])]);
		...
		this.barChart
			...
			.enter()
			.append("rect")
			.attr("class", (d) => (d[xKey] === highlightBarX ? "bar bar-selected" : "bar"))
			.attr("x", (d) => this.scaleX(d[xKey]))
			.attr("y", (d) => (d[yKey] > 0 ? this.scaleY(d[yKey]) : this.scaleY(0)))
			.attr("width", this.scaleX.bandwidth())
			.attr(
				"height",
				(d) =>
					(d[yKey] > 0
						? this.scaleY(0) - this.scaleY(d[yKey])
