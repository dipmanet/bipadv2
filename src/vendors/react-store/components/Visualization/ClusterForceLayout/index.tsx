...
class ClusterForceLayout extends PureComponent {
	...
	onMouseOver = (d) => {
		...
		onHover(d);
		select(this.style).style("display", "block");
	};

	onMouseMove = (d) => {
		select(this.tooltip)
			.html(
				`<span class=${styles.id}>${d.id}</span>
	...
	drawChart = () => {
		...
		const color = scaleSequential(colorScheme).domain(range(noOfClusters));
		...
		const nodes = this.data.map((node) => {
			...
			const element = {
				...
				radius,
				group,
				x: Math.cos((group / noOfClusters) * 2 * Math.PI) * 200 + width / 2 + Math.random(),
				y: Math.sin((group / noOfClusters) * 2 * Math.PI) * 200 + height / 2 + Math.random(),
			};
		...
		this.simulation = forceSimulation()
			...
			.force("collide", forceCollide((d) => d.radius + padding).strength(0));
		...
		const node = group
			...
			.call(drag().on("start", this.dragstarted).on("drag", this.dragged).on("end", this.dragended))
	...
	render() {
		...
		const svgClassName = [styles.clusterForceLayout, "clusterforcelayout", className].join(" ");
