...
class SimpleHorizontalBarChart extends PureComponent {
	...
	getScaleY = memoize((data, top, height, labelSelector, bandPadding) => {
		...
		if (stepOffset > 0) {
			const newHeight = scale.paddingOuter() * 2 + (scale.step() + stepOffset) * data.length;
	...
	render() {
		...
		const { barsHeight, scaleY } = this.getScaleY(data, top, height, labelSelector, bandPadding);
		...
		const axisBottomData = this.getAxisBottomData(scaleX, margins, noOfTicks, tickFormat);

		const className = _cs("horizontal-bar-chart", styles.horizontalBarChart, classNameFromProps);
		...
		return (
			<div
				...
				<div
					...
					<svg className={svgClassName} width={width} height={barsHeight}>
				...
				{!hideXAxis && (
					<div
						...
						<svg className={styles.axes} width={width} height={heightXAxis}>
