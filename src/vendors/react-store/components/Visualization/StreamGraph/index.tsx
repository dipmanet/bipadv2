...
class StreamGraph extends PureComponent {
	...
	onMouseMove = (d, i, nodes) => {
		const { x, labels, tooltip, group, height } = this;

		const { data, labelSelector } = this.props;
		...
		group.select(".mouse-line").attr("d", `M${mouseXpos},${height} ${mouseXpos},${0}`);
		...
		select(nodes[i]).classed("hover", true);
	};

	onMouseOut = (d, i, nodes) => {
		const { group, tooltip } = this;

		group.selectAll(".layer").transition().duration(250).attr("opacity", 1);

		group.select(".mouse-line").style("opacity", 0);

		tooltip.style("display", "none");

		select(nodes[i]).classed("hover", "false");
	};
	...
	init = () => {
		const { boundingClientRect, margins, data, labelName, labelSelector, colorScheme } = this.props;
		...
		this.group = select(this.svg).append("g").attr("transform", `translate(${left}, ${top})`);

		this.labels = keys(data[0]).filter((d) => d !== labelName);
		const values = data.map((d) => labelSelector(d));

		this.series = stack().keys(this.labels).order(stackOrderInsideOut).offset(stackOffsetWiggle)(
			data
		);

		this.x = scaleLinear().domain(extent(values)).range([0, this.width]);
		...
		this.colors = scaleOrdinal().domain(this.labels).range(colorScheme);
	...
	drawChart = () => {
		const { boundingClientRect, data } = this.props;
		...
		this.init();
		const { group, size, colors, series, x, onMouseOver, onMouseMove, onMouseOut } = this;
	...
	render() {
		...
		const svgClassName = ["stream-graph", styles.streamGraph, className].join(" ");

		const tooltipClassName = ["stream-graph-tooltip", styles.streamGraphTooltip].join(" ");
