...
class RadialDendrogram extends React.PureComponent {
	...
	renderChart() {
		...
		const group = svg
			...
			.attr(
				"transform",
				`translate(${(width + left + right) / 2}, ${(height + top + bottom) / 2})`
			);
		...
		node
			...
			.style("fill", topicColors)
			.attr("x", (d) =>
				d.x < 180 === !d.children ? `${scaledValues(d.value) + 4}` : `-${scaledValues(d.value) + 4}`
			)
	...
	render() {
		...
		const radialDendrogramStyle = ["radial-dendrogram", styles.radialDendrogram, className].join(
			" "
		);
		return (
			<svg
