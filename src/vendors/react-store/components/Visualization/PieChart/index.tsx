...
class PieChart extends PureComponent {
	...
	drawChart = () => {
		const { boundingClientRect, data, valueSelector, colorScheme } = this.props;
		...
		const textArcs = arc().outerRadius(outerRadius).innerRadius(outerRadius);
		const arcs = arc().padRadius(outerRadius).innerRadius(0);
	...
	render() {
		...
		const svgClassName = ["piechart", styles.piechart, className].join(" ");
