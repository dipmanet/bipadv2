...
class TimeSeries extends React.PureComponent {
	...
	renderBarChart(height, width) {
		const { data, xKey, yKey, yTicks, margins, xTickFormat, yTickFormat, showArea } = this.props;
		...
		root.append("g").attr("class", "axis axis--y").call(yAxis);
		...
		if (showArea) {
			...
			root.append("g").attr("class", "time-area").append("path").data([data]).attr("d", lineArea);
		}

		root.append("g").attr("class", "time-path").append("path").data([data]).attr("d", line);
