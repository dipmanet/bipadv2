...
import { scaleOrdinal, scaleLinear, scaleBand, scalePow, scaleLog } from "d3-scale";
...
class HorizontalBar extends PureComponent {
	...
	addGrid = (group, x, y) => {
		const {
			valueLabelFormat,
			boundingClientRect: { width: containerWidth, height: containerHeight },
		...
		const yAxis = axisLeft(y).tickSizeInner(-width).tickSizeOuter(0).tickPadding(10);
		...
		group.append("g").attr("id", "yaxis").attr("class", styles.grid).call(yAxis);
	};

	handleMouseOver = (d, node) => {
		const { showTooltip, tooltipContent, labelSelector, valueSelector, valueLabelFormat } =
			this.props;

		select(node).style("filter", "url(#drop-shadow)");
	...
	drawChart = () => {
		...
		const { width: containerWidth, height: containerHeight } = boundingClientRect;
		...
		const group = select(this.svgRef).append("g").attr("transform", `translate(${left}, ${top})`);

		const maxValue = max(data, (d) => valueSelector(d));

		let x = scaleLinear().range([0, width]).domain([0, maxValue]);

		if (scaleType === "log") {
			x = scaleLog().range([0, width]).clamp(true).domain([0.1, maxValue]);
		} else if (scaleType === "exponent") {
			x = scalePow().exponent([exponent]).range([0, width]).clamp(true).domain([0, maxValue]);
		}
		...
		if (showGridLines) {
			...
			group
				...
				.attr("transform", `translate(0, ${height})`)
				.call(xAxis);
			group.append("g").attr("id", "yaxis").attr("class", `y-axis ${styles.yAxis}`).call(yAxis);
		}
		...
		const groups = group.selectAll(".bar").data(data).enter().append("g").attr("class", "bar");
		...
		group.selectAll(".x-axis").selectAll(".tick line").style("visibility", "hidden");

		group.selectAll(".y-axis").selectAll(".tick line").style("visibility", "hidden");
		...
		groups
			...
			.on("end", (d, i, nodes) => {
				...
				select(nodes[i]).attr("x", newX).style("fill", fillColor);
			});
	...
	render() {
		const { className: classNameFromProps, svgStyle } = this.props;

		const className = _cs("horizontal-bar-chart", styles.horizontalBar, classNameFromProps);

		const tooltipClassName = _cs("tooltip", styles.tooltip);
