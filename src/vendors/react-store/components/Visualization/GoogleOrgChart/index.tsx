...
class GoogleOrgChart extends React.PureComponent {
	...
	flattenData = (node, parentName) => {
		const { keySelector, childSelector, titleSelector } = this.props;
		...
		return [[{ v: key, f: title }, parentName, title], ...flattenedChildren];
	};
	...
	selectHandler = () => {
		const { singleSelect, multiSelect, value, onChange } = this.props;
	...
	drawChart = () => {
		...
		const flatData = [["key", "parent", "title"]].concat(this.flattenData(options, ""));
