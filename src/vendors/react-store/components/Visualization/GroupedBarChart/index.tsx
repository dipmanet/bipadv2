...
import { scalePoint, scaleOrdinal, scaleLinear, scaleBand } from "d3-scale";
...
class GroupedBarChart extends PureComponent {
	...
	mouseOverRect = (node) => {
		...
		const content = tooltipRenderer
			? ReactDOMServer.renderToString(tooltipRenderer(node))
			: `${key}: ${value}`;

		select(this.tooltip).html(`<div>${content}</div>`).style("display", "inline-block");
	};

	mouseOverCircle = (node) => {
		const { tooltipRenderer, lineDataSelector, groupSelector } = this.props;
	...
	drawChart = () => {
		...
		const defaultColor = scaleOrdinal().range(colorScheme);
		...
		const x1 = scaleBand().domain(columns).rangeRound([0, x0.bandwidth()]).padding(0.05);

		const y = scaleLinear()
			.domain([0, max(values, (d) => max(columns, (key) => d[key]))])
			.nice()
			.rangeRound([height, 0]);
	...
	render() {
		...
		const svgClassName = ["grouped-bar-chart", styles.groupedBarChart, className].join(" ");
