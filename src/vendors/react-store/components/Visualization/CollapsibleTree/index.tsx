...
class CollapsibleTree extends React.PureComponent {
			...
			.call(
				zoom()
					...
					.on("zoom", () => {
						...
						group.attr("transform", `translate(${x + left}, ${y + top + height / 2}) scale(${k})`);
					})
			)
			.append("g")
	...
	setupChart = () => {
		const { data, childrenSelector, boundingClientRect, colorScheme, nodeSize, margins } =
			this.props;
	...
	addNodes = (group, source, nodes) => {
		...
		let i = 0;
		const node = group.selectAll("g.node").data(nodes, (d) => {
			if (d.id) {
				return d.id;
			}
			i += 1;
		...
		group.select("text").call((d) => {
			const { margins } = this.props;
			const { top, left } = margins;
			const { x = 0, y = 0, k = 1 } = this;
			...
			group
				...
				.attr(
					"transform",
					`translate(${x + left + translateX}, ${y + top + this.height / 2}) scale(${k})`
				);
		});
		...
		nodeUpdate.selectAll("text").transition().ease(easeSinInOut).style("fill-opacity", 1);
	...
	addLinks = (group, source, links) => {
		const link = group.selectAll("path.link").data(links, (d) => d.id);
	...
	drawChart = () => {
		const { data, boundingClientRect } = this.props;
	...
	render() {
		...
		const treeStyle = ["collapsible-tree", className].join(" ");
		...
		return (
			<Fragment>
				...
				/>
				<Icon className={styles.info} name="info" title="Use Ctrl + mouse to pan and zoom" />
			</Fragment>
		);
	}
}
