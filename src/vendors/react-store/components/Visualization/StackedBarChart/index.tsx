...
class StackedBarChart extends PureComponent {
	...
	init = () => {
		const { boundingClientRect, margins, data, colorScheme, labelName, labelSelector } = this.props;
		...
		this.series = stack().keys(this.dimensions).offset(stackOffsetDiverging)(data);

		this.x = scaleBand().domain(this.labels).rangeRound([0, width]).padding(0.1);
	...
	mouseOutRect = () => {
		select(this.tooltip).transition().duration(50).style("display", "none");
	};

	drawChart = () => {
		const { boundingClientRect, data, labelSelector } = this.props;
		...
		const { group, series, x, y, colors, mouseOverRect, mouseMove, mouseOutRect, width } = this;
		...
		group.append("g").attr("class", styles.yAxis).call(axisLeft(y).tickSize(0).tickPadding(6));
	...
	render() {
		...
		const svgClassName = ["stacked-bar-chart", styles.barchart, className].join(" ");
