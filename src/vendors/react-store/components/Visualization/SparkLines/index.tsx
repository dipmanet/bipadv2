...
class SparkLines extends PureComponent {
	...
	handleMouseMove = (element, focus) => {
		const { data, onHover, xLabelModifier, yLabelModifier } = this.props;
		const { scaleX, scaleY, xValue, yValue, bisectXValue } = this;
		...
		focus.select(".crosshair").attr("transform", `translate(${scaleX(xValue(d))}, ${0})`);
		...
		select(this.tooltip)
			.html(
				`<span class=${styles.yvalue}>${yLabel}</span>
	...
	drawChart = () => {
		const { data, boundingClientRect, margins, fill, xValueSelector, yValueSelector } = this.props;
		...
		if (fill) {
			group.append("path").attr("class", `fill ${styles.area}`).datum(data).attr("d", areas);
		}
		...
		const focus = group.append("g").attr("class", styles.focus).style("display", "none");
		...
		focus.append("circle").attr("class", "circle").attr("r", circleRadius);
	...
	render() {
		...
		const sparkLinesStyle = ["spark-lines", styles.sparkLines, className].join(" ");
