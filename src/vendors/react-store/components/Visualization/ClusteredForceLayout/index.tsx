...
class ClusteredForceLayout extends PureComponent {
	...
	init = () => {
		...
		const getNumberOfLinks = (links, value) =>
			links.reduce((n, link) => {
				const matches =
					link.source === value ||
					link.target === value ||
					link.source.id === value ||
					link.target.id === value;
				return n + matches;
			}, 0);
		...
		this.scaledWidth = scaleLinear().domain(this.minmax).range([1, 3]);
		this.scaledRadius = scaleSqrt().domain(this.nodeSizeExtent).range(circleRadiusExtent);
		this.valueline = line()
			.x((d) => d[0])
			.y((d) => d[1])
	...
	groupDragStart = (node) => {
		if (!event.active) this.simulation.alphaTarget(0.3).restart();
		select(node).select("path").style("stroke-width", 3);
	};
	...
	groupDragEnd = (node) => {
		...
		select(node).select("path").style("stroke-width", 1);
	};

	updateGroups = (node, paths) => {
		const { groupIds, valueline } = this;
		...
		groupIds.forEach((groupId) => {
			...
			select(path.node().parentNode).attr(
				"transform",
				`translate(${this.centroid[0]}, ${this.centroid[1]}) scale(${1.2})`
			);
		});
	};

	drawChart = () => {
		const { idSelector, boundingClientRect, groupSelector, valueSelector } = this.props;
		...
		const groups = group.append("g").attr("class", "groups");
		...
		const node = group
			...
			.attr("r", (d) => {
				const size =
					nodeSizes.find((value) => value.id === idSelector(d)).size || nodeSizeExtent[0];
		...
		groups.selectAll(".enclosed_path").call(
			drag()
