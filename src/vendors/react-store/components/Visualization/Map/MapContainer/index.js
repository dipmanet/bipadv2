...
class MapContainer extends React.PureComponent {
	...
	renderChild = (injectedProps) => {
		...
		const { mapContainerRef } = injectedProps;

		return <div className={_cs(className, styles.map)} ref={mapContainerRef} />;
	};

	render() {
		return <MapContext.Consumer>{this.renderChild}</MapContext.Consumer>;
	}
}
