...
export default class Segment extends PureComponent {
	...
	render() {
		const { value, label, style, centerTooltip, hideLabel, enlargeOnHover } = this.props;
		...
		return (
			<Tooltip
				tooltip={
					<div className={styles.tooltip}>
						{label === "" ? (
							...
							<div>
								{`${label}: `}
								<Numeral value={value} precision={0} />
							</div>
						)}
					</div>
				}
				...
				<div className={classNames} style={style}>
					{!hideLabel && <div className={styles.value}>{value}</div>}
				</div>
			</Tooltip>
		);
	}
