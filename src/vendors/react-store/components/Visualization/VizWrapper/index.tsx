...
const wrapViz = (WrappedComponent) => {
	const WrapperComponent = class extends React.PureComponent {
		...
		UNSAFE_componentWillReceiveProps(newProps) {
			const { colorScheme: oldColorScheme, colorSchemeType: oldColorSchemeType } = this.props;

			const { colorScheme, colorSchemeType } = newProps;
		...
		getColorSchemeValues = (type) => {
			...
			const mapDiscrete = (color) => ({
				...
				image: (
					<ColorPalette
						colorScheme={
							getCategoricalColorScheme(color) || getCategoryForContinuousColorScheme(color)
						}
					/>
				),
			});
			...
			if (type === discrete) {
				return getCategoricalColorNames().map((color) => mapDiscrete(color));
			}
		...
		handleSelection = (data) => {
			...
			if (colorSchemeType === continuous) {
				...
				colorScheme = getCategoricalColorScheme(data) || getCategoryForContinuousColorScheme(data);
			}
		...
		renderHeader = ({ fullScreen }) => {
			...
			const { handleSelection, handleSave, setFullScreen, removeFullScreen } = this;

			const { colorSchemeName, colorSchemeOptions } = this.state;
				...
				<div className={styles.header}>
					...
					<div className={styles.rightContent}>
						<div className={styles.colorSchemeInputContainer}>
							<Label show text={colorSchemeLabelText} className={styles.colorSchemeLabel} />
							<SelectInput
								clearable={false}
