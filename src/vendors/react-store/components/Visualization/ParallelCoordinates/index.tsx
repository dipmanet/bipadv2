...
class ParallelCoordinates extends PureComponent {
	...
	setContext = (width, height, margins) => {
		...
		return select(this.svg).append("g").attr("transform", `translate(${left}, ${top})`);
	};
	...
	init = () => {
		const { margins, boundingClientRect, data, ignoreProperties, colorScheme } = this.props;
		...
		this.x = scalePoint().domain(this.dimensions).range([0, width]).padding(0.1);
		this.y = {};
	...
	brush = () => {
		...
		svg.selectAll(".fg").style("display", (d) => {
			const show = !actives.every((active) => {
				const dim = active.dimension;
				return active.extent[0] <= this.y[dim](d[dim]) && this.y[dim](d[dim]) <= active.extent[1];
			});
	...
	drawChart = () => {
		const { data, boundingClientRect, labelSelector, colorSelector, margins } = this.props;
		...
		const { width, height, dimensions, x, y, path, colors } = this;
		...
		axes
			...
			.attr("class", "brush")
			.each((d, i, nodes) => {
				select(nodes[i]).call(this.y[d].brush);
			})
			.selectAll("rect")
			.attr("x", -10)
			.attr("width", 20);
	...
	render() {
		...
		const svgClassName = ["parallel-coordiantes", styles.parallel, className].join(" ");
