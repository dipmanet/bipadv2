...
class DonutChart extends PureComponent {
	...
	textArch = (outerRadius, innerRadius) => arc().outerRadius(outerRadius).innerRadius(innerRadius);
	...
	addDropShadow = (svg) => {
		...
		const filter = defs.append("filter").attr("id", "drop-shadow").attr("height", "120%");
	...
	addPaths = (element, options) => {
		...
		const { valueSelector, colorSelector, labelSelector, labelModifier } = this.props;
	...
	drawChart = () => {
		...
		const pies = pie().sort(null).value(valueSelector);
	...
	render() {
		...
		const svgClassName = ["donutchart", className].join(" ");
