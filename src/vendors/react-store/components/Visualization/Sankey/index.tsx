...
class Sankey extends PureComponent {
	...
	addNodes = (element, data, colors, width) => {
		const { labelSelector, valueSelector } = this.props;
		const nodes = element.selectAll(".node").data(data).enter().append("g").attr("class", "node");
		...
		nodes.append("title").text((d) => `${labelSelector(d)}\n${valueSelector(d)}`);

		const links = select(this.svg).selectAll(".link");

		nodes
			.on("mouseover", (d) => {
				nodes.select(`#node${d.index}`).style("opacity", 1);

				links.transition().duration(300).style("opacity", 0.1);
			...
			.on("mouseout", (d) => {
				nodes.select(`#node${d.index}`).style("opacity", 0.8);

				links.transition().duration(500).style("opacity", 0.8);
			});
	};
	...
	render() {
		...
		const svgClassName = ["sankey", styles.sankey, className].join(" ");
