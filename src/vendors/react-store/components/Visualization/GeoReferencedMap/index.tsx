...
export default class GeoReferencedMap extends React.PureComponent {
	...
	componentDidMount() {
		...
		map.on("mousemove", "region-layer", (e) => {
			...
			popup.setLngLat([e.lngLat.lng, e.lngLat.lat + 0.1]).setHTML(feature.properties.name);
		});

		map.on("mouseenter", "region-layer", (e) => {
			const feature = e.features[0];
			popup.setHTML(feature.properties.name).addTo(map);
		});
		...
		map.on("mouseenter", "unclustered-point-circle", (e) => {
			const feature = e.features[0];
			map.getCanvas().style.cursor = "pointer";
			popup.setLngLat(feature.geometry.coordinates).setHTML(feature.properties.title).addTo(map);
		});
	...
	addPointsLayers = () => {
		this.map.addLayer({
			...
			paint: {
				"circle-color": ["step", ["get", "point_count"], "#bdc9e1", 10, "#74a9cf", 20, "#0570b0"],
	...
	loadGeoRegions = (geoLocations) => {
		...
		const countriesData = {
			...
			features: geoLocations.map((selection) => selection.geoJson.features[0]),
		};
		if (source) {
	...
	render() {
		...
		return (
			<div
				...
				{(pendingMap || loading) && <LoadingAnimation />}
				<div
