...
class OrgChart extends React.PureComponent {
	...
	getNodeClassName = (d) => {
		const classNames = ["node", styles.node];
	...
	addSelection = (item) => {
		...
		const settings = {
			$bulk: [{ $push: [newSelection] }, { $unique: (selection) => selection }],
		};
	...
	drawChart = () => {
		const { boundingClientRect, data, childSelector, labelSelector, disabled, nodeSize, margins } =
			this.props;
		...
		const group = svg
			...
			.call(
				zoom().on("zoom", () => {
					...
					group.attr(
						"transform",
						`translate(${x + left + width / 2}, ${top + rectWidth + y}) scale(${k})`
					);
				})
			)
			.append("g")
		...
		const nodeWidth = widthPerTreeLeaves < minNodeWidth ? minNodeWidth : widthPerTreeLeaves;
		const nodeHeight =
			heightPerTreeDepth < minNodeHeight ? minNodeHeight : heightPerTreeDepth - rectWidth;
		...
		nodes.append("rect").style("cursor", "pointer");
		...
		if (!disabled) {
			nodes.selectAll("rect").on("click", (item) => {
	...
	render() {
		...
		const chartClassName = [styles.chart, "chart"].join(" ");

		const infoIconClassName = [styles.infoIcon, "info-icon"].join(" ");

		return (
			<div className={containerClassName}>
				<Icon className={infoIconClassName} name="info" title="Use mouse to pan and zoom" />
				<svg
