...
class SimpleVerticalBarChart extends PureComponent {
	...
	getRenderData = memoize((data, height, scaleX, scaleY, labelSelector, valueSelector, margins) => {
	...
	getScaleX = memoize((data, width, labelSelector, bandPadding) =>
		scaleBand().range([width, 0]).domain(data.map(labelSelector)).padding(bandPadding)
	);
	...
	render() {
		...
		const axisLeftData = this.getAxisLeftData(scaleY, height, margins, noOfTicks, tickFormat);

		const className = _cs("vertical-bar-chart", styles.verticalBarChart, classNameFromProps);

		const svgClassName = _cs("svg", styles.svg);

		// const horizontalTextOffset = 6;
		// const minBarWidthToRenderText = 16;

		return (
			<div className={className} width={containerWidth} height={containerHeight}>
				<svg className={svgClassName} width={width + left + right} height={height + top + bottom}>
								...
								{showTicks &&
									axisLeftData.map((d) => (
										<g
											...
											<line className={_cs(styles.line, "line")} x1={0} y1={0.5} x2={-5} y2={0.5} />
											<text className={_cs(styles.label, "tick-label")} y={0.5} x={-6} dy="0.32em">
												{Numeral.renderText({
													value: d.value,
													precision: 1,
													normal: true,
