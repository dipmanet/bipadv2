...
class MultiLineChart extends React.PureComponent {
	...
	drawChart = () => {
		const { data, margins, colorScheme, tickArguments, boundingClientRect } = this.props;
		...
		const x = scaleTime()
			.domain(extent(dates.map((date) => new Date(date))))
			.nice()
			.range([0, width]);

		const y = scaleLinear()
			.domain([0, max(series, (d) => max(d.values))])
			.nice()
			.range([height, 0]);
		...
		const axisLayer = group.append("g");
		...
		axisLayer.append("g").attr("class", `${styles.yAxis}`).call(axisLeft(y));
	};
	...
	render() {
		...
		const className = [styles.multiline, classNameFromProps].join(" ");
