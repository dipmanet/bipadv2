...
class WordCloud extends PureComponent {
	...
	startWordCloudComputation = ({
		boundingClientRect: { width: containerWidth, height: containerHeight } = emptyObject,
		data,
		font,
		rotate,
		labelSelector,
	...
	calculateWordCloud = memoize(
		(width, height, font, data, rotate, labelSelector, frequencySelector) => {
			...
			const minFont = 7;
			const maxFont = Math.max(2 * minFont, Math.sqrt(renderAreaFactor));
        ...
        */
			...
			const fontSizeSelector = (d) =>
				Math.max(minFont, Math.min(frequencySelector(d) * sizeOffset, maxFont));
	...
	render() {
		...
		const className = _cs(classNameFromProps, styles.wordCloud, "word-cloud");
