...
class Organigram extends PureComponent {
	...
	colorExtractor = (item) => {
		const { selectColor, fillColor } = this.props;
	...
	drawChart = () => {
		...
		const { setContext, colorExtractor, addDropShadow, toggleSelection } = this;
		...
		node
			...
			.attr(
				"width",
				(d, i, nodes) => select(nodes[i]).node().parentNode.getBBox().width + boxPadding
			)
			.attr(
				"height",
				(d, i, nodes) => select(nodes[i]).node().parentNode.getBBox().height + boxPadding
			)
		...
		if (!disabled) {
			node.selectAll(".box").on("click", (cell) => {
	...
	render() {
		...
		const svgClassName = ["organigram", styles.organigram, className].join(" ");
		...
		return (
			<Fragment>
				...
				/>
				<Icon className={styles.info} name="info" title="Use mouse to pan and zoom" />
			</Fragment>
		);
	}
}
