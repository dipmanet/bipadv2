...
import { getStandardFilename, addClassName, removeClassName } from "../../../utils/common";
...
class ForceDirectedGraph extends React.PureComponent {
		...
		if (oldHighlightClusterId !== newHighlightClusterId) {
			if (container) {
				if (newHighlightClusterId) {
					...
					const newActiveNode = container.getElementsByClassName(highlightNodeClassName)[0];
	...
	init = ({
		boundingClientRect: { width: widthFromProps, height: heightFromProps },
		idSelector,
		...
		this.group = select(svg).append("g").attr("transform", `translate(${left}, ${top})`);
	...
	ticked = () => {
		const { boundingClientRect, margins, radiusSelector, useVoronoi } = this.props;

		const { width: widthFromProps, height: heightFromProps } = boundingClientRect;
		...
		if (useVoronoi) {
			...
			const clip = this.group
				.selectAll("clipPath")
				.data(this.recenterVoronoi(this.nodes.data()), (d) => d.data.index);
	...
	handleMouseOver = (d) => {
		const { onMouseOver, labelSelector } = this.props;
		...
		return this.tooltip.transition().duration(100).style("display", "inline-block");
	};

	handleMouseMove = () =>
		this.tooltip.style("top", `${event.pageY - 30}px`).style("left", `${event.pageX + 20}px`);
	...
	handleMouseOut = (d) => {
		...
		return this.tooltip.transition().duration(100).style("display", "none");
	};
	...
	renderChart = ({
		...
		select(container).selectAll(styles.tooltip).remove();
		...
		this.nodes = this.group
			...
			.attr("class", (d) => {
				...
				const classNames = [styles.nodes, "nodes", `nodes-${id}-${group}`];
		...
		if (useVoronoi) {
			...
			this.nodes.append("circle").attr("r", 3).attr("fill", "black");
		} else {
			this.nodes
				.append("circle")
		...
		this.simulation.nodes(data.nodes).on("tick", this.ticked);

		this.simulation.force("link").links(data.links);
	};

	render() {
		const { className: classNameFromProps, clusterSize } = this.props;
		...
		return (
			<div className={containerClassName} ref={this.containerRef}>
				...
				/>
				<svg className={svgClassName} ref={this.svgRef} />
			</div>
		);
	}
}
