...
import { SketchPicker, TwitterPicker, GithubPicker } from "react-color";
...
const propTypes = {
	/**
	 * for styling by classNa../HintAndError/index.tsx
	 */
...
class ColorInput extends React.PureComponent {
	...
	handleColorPickerInvalidate = (colorPickerContainer) => {
		...
		const optionsContainerPosition = calcFloatingPositionInMainWindow(
			parentRect,
			containerRect,
			offset
		);
	...
	render() {
		...
		return (
			<div
				className={_cs(styles.colorInput, className, disabled && styles.disabled)}
				ref={(el) => {
					this.container = el;
				}}>
				...
				<button
					...
					<span className={_cs(styles.color, "color")} style={{ backgroundColor: value }} />
				</button>
				<HintAndError
