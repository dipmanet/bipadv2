...
class SimpleListInput extends React.PureComponent {
	...
	getOptionsMap = memoize((options, keySelector) => listToMap(options, keySelector, (d) => d));

	getRendererParams = (key) => {
		const { options, labelSelector, keySelector, itemClassName, disabled, readOnly } = this.props;
	...
	render() {
		const { className, disabled, error, hint, label, showHintAndError, showLabel, value } =
			this.props;

		return (
			<div className={_cs(className, styles.listInput)}>
				<Label
