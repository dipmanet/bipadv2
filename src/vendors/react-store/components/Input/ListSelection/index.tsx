...
import { _cs, isFalsyString, caseInsensitiveSubmatch, compareStringSearch } from "@togglecorp/fujs";
...
export class NormalListSelection extends React.PureComponent {
	...
	getClassName = () => {
		const { className: classNameFromProps, disabled, error, segment } = this.props;
	...
	getFilteredOptions = memoize((searchText, options) => {
		...
		const newOptions = options
			...
			.sort((a, b) => compareStringSearch(labelSelector(a), labelSelector(b), searchText));
		return newOptions;
	});
	...
	renderParams = (key, itemData) => {
		const { labelSelector, tooltipSelector, value, disabled, readOnly, segment } = this.props;
	...
	renderInput = () => {
		const { options, keySelector, listClassName, searchText } = this.props;

		const filteredOptions = this.getFilteredOptions(searchText, options);

		return (
			<ListView
				className={_cs(styles.options, "list-selection-options", listClassName)}
	...
	render() {
		const { label, hint, error, showHintAndError, showLabel, disabled } = this.props;
		...
		return (
			<div className={className}>
				<Label show={showLabel} text={label} disabled={disabled} error={!!error} />
				<Input />
