...
export default class Options extends React.PureComponent {
	...
	getClassName = () => {
		...
		const classNames = [className, styles.options];
	...
	renderOption = (k, data) => {
		...
		const label = optionLabelSelector ? optionLabelSelector(data) : labelSelector(data);
	...
	renderEmpty = () => {
		const { renderEmpty: EmptyComponent, data } = this.props;
	...
	render() {
		const { onBlur, onInvalidate, parentContainer, data, show } = this.props;
		...
		return (
			<FloatingContainer
				...
				className={className}>
				<List data={data} modifier={this.renderOption} />
				<Empty />
			</FloatingContainer>
		);
	}
