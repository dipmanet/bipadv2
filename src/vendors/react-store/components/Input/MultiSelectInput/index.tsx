...
import RawInput from "../RawInput/index.js";

import {
	calcFloatPositionInMainWindow,
	defaultOffset,
	defaultLimit,
} from "../../../utils/bounds.tsx";
...
export class NormalMultiSelectInput extends React.PureComponent {
	...
	findPlaceholderValue = memoize((options, labelSelector, keySelector, value = []) => {
		const optionsMap = listToMap(options, keySelector, (element) => element);
	...
	filterOptions = memoize((options, labelSelector, value) => {
	...
	handleSelectAllButtonClick = () => {
		const { options, keySelector, onChange } = this.props;
	...
	render() {
		...
		const finalPlaceholder = inputTitle || placeholder;

		const finalSearchValue = searchValue || "";

		const filteredOptions = this.filterOptions(options, labelSelector, searchValue);
