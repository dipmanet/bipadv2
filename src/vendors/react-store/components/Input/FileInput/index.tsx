...
export default class FileInput extends React.PureComponent {
	...
	handleChange = (e) => {
		...
		const files = filesFromInput.filter((file) =>
			FileInput.isValidFile(file.name, file.type, this.props.accept)
		);
			...
			<div className={`file-input ${className} ${styles.fileInputWrapper}`}>
				{showPreview && (
					<div className={`${styles.preview} image-input-preview`}>
						{preview ? (
							<img alt="No preview available" className={`img ${styles.img}`} src={preview} />
						) : (
							<p className="no-preview-text">No preview available</p>
						)}
					</div>
				)}
				<label className={`label ${styles.label}`} htmlFor={this.inputId}>
					{children}
				</label>
				<input
				...
				/>
				{showStatus && (
					<p className={styles.status}>{files.length > 0 ? files[0].name : "No file choosen"}</p>
				)}
			</div>
		);
	}
