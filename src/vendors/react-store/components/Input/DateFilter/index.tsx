...
import { formatDateToString, encodeDate, decodeDate } from "@togglecorp/fujs";
...
class DateFilter extends React.PureComponent {
	...
	static calculateOptionsAndValue = memoize((value) => {
		...
		if (startDate === endDate) {
			return {
				options: [...options, { key: "selectedExact", label: startDate }],
				value: "selectedExact",
			};
		}
	...
	getClassName = () => {
		...
		const classNames = [className, "date-filter"];
	...
	render() {
		...
		const { options: selectInputOptions, value: selectInputValue } =
