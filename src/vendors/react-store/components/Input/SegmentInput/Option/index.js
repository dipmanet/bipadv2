...
import { _cs, randomString, isFalsy } from "@togglecorp/fujs";
...
export default class SegmentOption extends React.PureComponent {
	...
	render() {
		...
		const { containerClassName, ...otherProps } = extraProps;
		...
		return (
			<label htmlFor={this.inputId} className={classNames}>
				<input
					...
					name={name}
				/>
				{Renderer ? <Renderer className={rendererClassName} {...otherProps} /> : label}
			</label>
		);
	}
}
