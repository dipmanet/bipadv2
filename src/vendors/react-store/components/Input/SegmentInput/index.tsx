...
class SegmentInput extends React.PureComponent {
	...
	getClassName = () => {
		const { className, disabled, error, required } = this.props;

		const classNames = [className, "segment-input", styles.segmentInput];
	...
	handleInputChange = (changeEvent) => {
		const { onChange, disabled, options, keySelector, readOnly } = this.props;
		const { value } = changeEvent.target;

		const finalValue = options.map((val) => keySelector(val)).find((val) => String(val) === value);
	...
	render() {
		const { error, hint, label, showLabel, showHintAndError, options, keySelector, disabled } =
			this.props;
