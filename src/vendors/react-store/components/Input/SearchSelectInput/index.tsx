...
import { caseInsensitiveSubmatch, compareStringSearch, _cs } from "@togglecorp/fujs";
import { FaramInputElement } from "@togglecorp/faram";

import DangerButton from "../../Action/Button/DangerButton.tsx";
import handleKeyboard from "../../General/HandleKeyboard.tsx";
import HintAndError from "../HintAndError/index.tsx";
import Label from "../Label/index.tsx";
import RawInput from "../RawInput/index.tsx";

import { defaultLimit } from "../../../utils/bounds.tsx";
// import { calcFloatPosition../HintAndError/index.tsxt, defaultLimit } from "../../../utils/bounds";
...
class SearchSelectInput extends React.PureComponent {
	...
	filterOptions = memoize((options, labelSelector, value, maxDisplayOptions) => {
		...
		const newOptions = options
			.filter(
				(option) => value === undefined || caseInsensitiveSubmatch(labelSelector(option), value)
			)
			.sort((a, b) => compareStringSearch(labelSelector(a), labelSelector(b), value))
			.slice(0, maxDisplayOptions);

		return newOptions;
	});

	findDefaultSearchValue = memoize((options, labelSelector, keySelector, value) => {
	...
	render() {
		...
		const { showOptionsPopup, focusedKey, inputInFocus, searchValue } = this.state;
		...
		const inputClassName = _cs(styles.inputAndActions, "input-and-actions");
		...
		return (
			<div className={className} ref={this.containerRef} title={title}>
				{showLabel && (
					<Label
						text={label}
						error={!!error}
				...
				{showHintAndError && <HintAndError error={error} hint={hint} />}
				<Options
