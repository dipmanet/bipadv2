...
import FloatingContainer from "../../View/FloatingContainer/index.tsx";
import Delay from "../../General/Delay.tsx";
...
import DigitalInput from "../DigitalInput/index.tsx";
...
import ActionButtons from "./ActionButtons/index.tsx";
...
class DateInput extends React.PureComponent {
	...
	getClassName = () => {
		const { disabled, className, value, error, separator, readOnly } = this.props;

		const { yearInputFocused, monthInputFocused, dayInputFocused } = this.state;

		const classNames = [className, "date-input", styles.dateInput];
	...
	handleDatePickerInvalidate = (datePickerContainer) => {
		...
		const { current: container } = this.containerRef;
		const parentRect = container ? container.getBoundingClientRect() : this.boundingClientRect;
	...
	handleChange = (valueToOverride) => {
		const { onChange, value: valueFromProps, separator } = this.props;
	...
	renderDatePicker = ({ y, m, d }) => {
		...
		return (
			<FloatingContainer
				...
				<DatePicker value={datetime} onChange={this.handleDatePickerDatePick} />
			</FloatingContainer>
		);
	};
	...
	render() {
		...
		const { showDatePicker, containerHover } = this.state;
		...
		return (
			...
			<div
				...
				{showDatePicker && <FloatingDatePicker y={yearValue} m={monthValue} d={dayValue} />}
			</div>
		);
	}
}
