...
class RadioInput extends React.PureComponent {
	...
	// XXX: use isTruthy
	getSelectedOption = memoize(
		(options, value, keySelector) => options && options.find((d) => keySelector(d) === value)
	);
	...
	rendererParams = (key, option) => {
		const { labelSelector, keySelector, options, className, value, onChange, ...otherProps } =
			this.props;
	...
	render() {
		const { className, options, keySelector, value, name } = this.props;
