...
export default class DayPicker extends React.PureComponent {
	...
	UNSAFE_componentWillReceiveProps(nextProps) {
		const { year, month, value } = this.props;

		if (year !== nextProps.year || month !== nextProps.month || value !== nextProps.value) {
	...
	renderDay = (day) => (
		<div key={day.key} className={this.getDayClassName(day.key)}>
			{day.value && (
				<Button onClick={() => this.handleDayChange(day.value)} transparent>
					{day.value}
				</Button>
			)}
		</div>
	...
	render() {
		const { className, month, year, onMonthClick } = this.props;
		const monthName = DayPicker.monthNames[month - 1];

		const classNames = [className, "day-picker", styles.dayPicker];
		...
		return (
			<div className={classNames.join(" ")}>
				<header className={styles.header}>
					...
					/>
					<Button className={styles.month} onClick={onMonthClick} transparent>
						{`${monthName} ${year}`}
					</Button>
					<Button
				...
				<div className={styles.weeks}>{this.weeks.map(this.renderWeek)}</div>
			</div>
		);
	}
}
