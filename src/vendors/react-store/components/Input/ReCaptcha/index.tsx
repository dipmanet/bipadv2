...
class ReCaptcha extends Component {
	...
	renderGrecaptcha = () => {
		const { onloadCallback, siteKey, theme, size, tabindex, type, hl, badge } = this.props;
	...
	renderError = () => {
		const { showHintAndError, error } = this.props;
		...
		return (
			<p className={`${styles.empty} ${styles.error}`} key="empty">
				-
			</p>
		);
	};
	...
	render() {
		const { className, showHintAndError, error } = this.props;
		...
		const reCaptchaStyle = `${styles.gRecaptcha} ${
			showHintAndError && error ? styles.errored : ""
		}`;
		...
		return (
			<div className={containerStyle}>
				<div
					ref={(recaptchaDom) => {
						this.recaptchaDom = recaptchaDom;
					}}
