...
const isValidTimeString = (value, separator) => {
	...
	return !(+h < MIN_HOUR || +h > MAX_HOUR || +m < MIN_MINUTE || +m > MAX_MINUTE);
};
...
class TimeInput extends React.PureComponent {
	...
	getClassName = () => {
		const { disabled, readOnly, className, value, error, separator } = this.props;

		const { hourInputFocused, minuteInputFocused } = this.state;

		const classNames = [className, "time-input", styles.timeInput];
	...
	render() {
		...
		const { h: hourValue = "", m: minuteValue = "" } = decodeTime(value, separator);
