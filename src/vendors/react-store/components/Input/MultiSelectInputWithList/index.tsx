...
class MultiSelectInputWithList extends React.PureComponent {
	...
	render() {
		...
		const listClassName = _cs(listClassNameFromProps, styles.list, "list");

		const selectClassName = _cs(selectClassNameFromProps, styles.input, "select");

		const Item = hideRemoveFromListButton || readOnly || disabled ? ListItem : DismissableListItem;

		const Input = maxDisplayOptions === undefined ? MultiSelectInput : SearchMultiSelectInput;
