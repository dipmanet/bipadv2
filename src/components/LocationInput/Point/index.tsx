...
class DraggablePoint extends React.PureComponent<Props, State> {
	...
	private setMapEvents = memoize((map: unknown, pointShape: "rect" | "circle") => {
		...
		map.on("mousedown", shapeName, (e: { preventDefault: () => void }) => {
			e.preventDefault();
			map.on("mousemove", this.handleMove);
			map.once("mouseup", this.handleEnd);
		});

		map.on("touchstart", shapeName, () => {
	...
	private handleRegionSelectInputChange = (newRegion: { adminLevel: number; geoarea: number }) => {
		const {
			municipalities,
			districts,
			provinces,
		...
		const regions = newRegion.adminLevel ? regionMap[newRegion.adminLevel] : undefined;
	...
	private handleLatInputChange = (lat: number) => {
		const { geoJson = defaultGeoJson, onPointMove } = this.props;
		...
		const features = this.context.map.queryRenderedFeatures(point, {
	...
	private handleEnd = (e: unknown) => {
		...
		const { geoJson, onPointMove } = this.props;
	...
	public render() {
		...
		const { lng, lat } = this.getFormData(geoJson);
