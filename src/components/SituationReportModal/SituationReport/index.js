...
import { _cs, isDefined, listToMap } from "@togglecorp/fujs";
...
import { generatePaint, hazardTypesList } from "#utils/domain";
...
const pieColors = ["#003f5c", "#7a5195", "#ef5675", "#ffa600"];
...
const requestOptions = {
	incidentsGetRequest: {
		...
		query: ({ props: { selectedReportDetails } }) => {
			...
			const { fromDate, toDate, event, type } = selectedReportDetails;
...
class SituationReport extends React.PureComponent {
	...
	render() {
		const { className, selectedReportDetails, requests, mapStyle, regions, hazardTypes } =
			this.props;

		const {
			incidentsGetRequest: { pending: incidentsPending, response },
		} = requests;
		...
		if (!selectedReportDetails) {
			return (
				<div className={_cs(className, styles.situationReport)}>
					<Translation>
						{(t) => <Message>{t("Please select a situation report")}</Message>}
					</Translation>
				</div>
			);
		}
		...
		if (incidents.length < 1) {
			return (
				<div className={_cs(className, styles.situationReport)}>
					<Translation>
						{(t) => <Message>{t("There are no incidents in this situation report")}</Message>}
					</Translation>
				</div>
			);
		}
		...
		const { hazardGroupedIncidents, mapState, aggregatedStat, minCasuality, maxCasuality } =
		...
		const { peopleLossPieData, familyLossPieData } = this.getDataForPieCharts(aggregatedStat);
			...
			<div className={_cs(styles.situationReport, className)}>
				...
				<div className={styles.situationReportFull}>
					<header className={styles.header}>
						<Translation>{(t) => <h2>{t("Incident Situation Report")}</h2>}</Translation>
					</header>
					<div className={styles.map}>
						<Translation>
							{(t) => <h3 className={styles.mapHeading}>{t("Incident Statistics")}</h3>}
						</Translation>
						...
						<Map
							...
							<div className={styles.legend}>
								<Translation>
									{(t) => <div className={styles.legendTitle}>{t("Hazards Legend")}</div>}
								</Translation>
								<HazardsLegend
					...
					<div className={styles.vizContainer}>
						<Translation>
							{(t) => <h3 className={styles.heading}>{t("Disaster Summary")}</h3>}
						</Translation>
						<div style={{ height: hazardSummary.length * 32 }}>
							<ResponsiveContainer>
								<BarChart data={hazardSummary} layout="vertical">
									<XAxis dataKey="value" type="number" hide />
									<Bar fill="#e04656" dataKey="value" layout="vertical">
										<LabelList dataKey="label" position="insideLeft" />
									</Bar>
					...
					<div className={styles.map}>
						<Translation>
							{(t) => <h3 className={styles.mapHeading}>{t("Casuality Statistics")}</h3>}
						</Translation>
					...
					<div className={styles.charts}>
						<div className={styles.pieCharts}>
							<div className={styles.pieChart}>
								<Translation>{(t) => <h4>{t("People Loss")}</h4>}</Translation>
								<PieChart width={240} height={240}>
									<Pie data={peopleLossPieData} dataKey="value" cx={120} cy={120} outerRadius={60}>
							...
							<div className={styles.pieChart}>
								<Translation>{(t) => <h4>{t("Family Loss")}</h4>}</Translation>
								<PieChart width={240} height={240}>
									<Pie data={familyLossPieData} dataKey="value" cx={120} cy={120} outerRadius={60}>
