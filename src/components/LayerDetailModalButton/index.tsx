...
import { _cs, camelToNormal } from "@togglecorp/fujs";
...
class LayerDetailModal extends React.PureComponent<ModalProps> {
				...
				if (!layer.longDescription) {
					return (
						<div className={styles.details}>
							<Message>{language === "en" ? "Not available" : "उपलब्ध छैन"}</Message>
						</div>
					);
				}
						...
						{orderedKeyList.map((groupKey) => {
							...
							return (
								<div key={groupKey} className={styles.group}>
									<div className={styles.groupTitle}>{camelToNormal(groupKey)}</div>
									<div className={styles.rows}>
										{Object.keys(orderGroup).map((mk) => {
											...
											return (
												<div className={styles.metadataItem} key={mk}>
													<div className={styles.label}>{camelToNormal(mk)}</div>
													<div className={styles.value}>{metadata || "-"}</div>
												</div>
											);
										})}
									</div>
	...
	public render() {
		const { className, closeModal, layer } = this.props;
	...
	public render() {
		...
		return (
			<ModalButton
				...
				transparent
				iconName="info"
				modal={<LayerDetailModal layer={layer} language={language} />}
			/>
		);
	}
}
