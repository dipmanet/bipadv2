...
import { AppState, EventElement, Flow, Organization } from "#types";

import { Field, Event } from "#store/atom/page/types";
...
const requestOptions: { [key: string]: ClientAttributes<PropsWithRedux, Params> } = {
	...
	reliefFlowAddRequest: {
		url: ({ props }) => (props.value ? `/relief-flow/${props.value.id}/` : "/relief-flow/"),
...
class AddFlowForm extends React.PureComponent<Props, State> {
	...
	public render() {
		const {
			...
			eventList,
			requests: {
				reliefFlowAddRequest: { pending: addReliefPending },
				eventTypesGetRequest: { pending: eventTypesGetPending },
				organizationGetRequest: { response, pending: organizationsGetPending },
				fiscalYearsGetRequest: { response: fiscalYearsResponse, pending: fiscalYearsGetPending },
			},
			language: { language },
		} = this.props;

		const { organizationList, faramValues, faramErrors, pristine } = this.state;
		...
		const pending =
			addReliefPending || eventTypesGetPending || organizationsGetPending || fiscalYearsGetPending;
					...
					{(t) => (
						<Faram
							...
							<ModalBody>
								...
								<TextInput autoFocus faramElementName="description" label={t("Description")} />
								...
								/>
								<DateInput faramElementName="date" label={t("Date")} language={language} />
							</ModalBody>
							<ModalFooter>
