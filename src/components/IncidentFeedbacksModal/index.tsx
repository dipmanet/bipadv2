	...
	return (
		<div className={_cs(className, styles.incidentFeedback)}>
			<div className={styles.iconContainer}>
				<Icon className={styles.icon} name="chatBox" />
			</div>
			<div className={styles.detailContainer}>
				...
				<div className={styles.details}>
					<DetailItem
						iconName="calendar"
						value={
							<FormattedDate className={styles.createdOn} value={createdOn} mode="yyyy-MM-dd" />
						}
					/>
					<DetailItem iconName="user" value={name} />
					<DetailItem iconName="email" value={email} />
					<DetailItem iconName="telephone" value={mobileNumber} />
				</div>
				{acknowledged && <div className={styles.acknowledgementStatus}>Acknowledged</div>}
				{!acknowledged && (
					<ModalAccentButton
...
class IncidentFeedbacksModal extends React.PureComponent<Props, State> {
	...
	public render() {
		const {
			...
			closeModal,
			requests: {
				incidentFeedbacksGet: { pending, response },
			},
		} = this.props;
		const { incidentFeedbacks } = this.state;
...
export default createRequestClient(requestOptions)(IncidentFeedbacksModal);
