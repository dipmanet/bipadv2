	...
	render() {
		...
		const valueComponent =
			value === "-" ? (
				...
				finalData &&
				(finalData.label === "Estimated loss (NPR)" ||
					finalData.label === "अनुमानित आर्थिक क्षेति (रु )") ? ( // finalData.value === 0
					finalData.value === 0 ? (
					...
					<span className={_cs(styles.value, valueClassName)}>{value}</span>
				)
			) : (
				<div className={_cs(styles.value, valueClassName)}>{value}</div>
			);
		return (
			<div className={_cs(classNameFromProps, styles[type])}>
				{iconLabel ? (
					<div className={_cs(styles.iconLabel, label)} />
				) : (
					<div className={_cs(styles.label, labelClassName)}>{label}</div>
				)}
				{valueComponent}
			</div>
		);
