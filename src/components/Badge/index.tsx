...
export default class Badge extends React.PureComponent {
	...
	render() {
		const { className, titleClassName, noBorder, title, icon, iconClassName, iconStyle, tooltip } =
			this.props;
		...
		return (
			<Tooltip tooltip={tooltip}>
				<div
					...
					{isTruthyString(icon) && <Icon name={icon} style={iconStyle} className={iconClassName} />}
					{isTruthyString(title) && (
						<div className={_cs(styles.title, titleClassName)}>{title}</div>
					)}
				</div>
			</Tooltip>
		);
