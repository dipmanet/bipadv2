...
const SidebarLogo = ({ data, selectedCategory, onClick,
    manual, searchData, handleManualType, language, faqs, yearSelection,
    handleYearFilter, handleReset, filterManualTypeKey, filterYearKey }) => {
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <h3>{language === 'en' ? 'Manuals' : 'पुस्तिकाहरू'}</h3>
                                    {
                                        (filterManualTypeKey || filterYearKey) && (
                                            <div title={language === 'en' ? 'Reset' : 'रिसेट'} onClick={() => handleReset(manualRef, yearRef)} style={{ cursor: 'pointer' }}>
                                ...
                                <div className={styles.sidebarCategories}>
                                    <>
                                        ...
                                        <select name="type" id="manual" onChange={handleYearFilter} ref={yearRef}>
                                            <option value="">{language === 'en' ? 'Select Year' : 'वर्ष चयन गर्नुहोस्'}</option>
                                            {yearSelection && yearSelection.length
