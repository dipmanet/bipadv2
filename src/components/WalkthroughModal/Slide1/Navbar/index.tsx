...
const Navbar = (props) => {
    const { requests: { logoutRequest }, language: { language }, authState: { authenticated }, children } = props;
    ...
    const { activeRouteDetails: { name: routeName } } = useContext(RiskInfoLayerContext);
        ...
        <div className={styles.mainContainer}>
            <div className={styles.navLinks}>
                {routeName !== 'homepage'
                    ? (
                        <>
                            <Link to="/about/"><span style={{ marginLeft: '0px' }} className={routeName === 'about' ? styles.active : ''} onClick={() => handleButtonClick(1)}>{language === 'en' ? 'About' : 'बारेमा'}</span></Link>
                            <Link to="/developers/"><span className={routeName === 'developers' ? styles.active : ''} onClick={() => handleButtonClick(2)}>{language === 'en' ? 'Developers' : 'विकासकर्ताहरू'}</span></Link>
                            <Link to="/manuals/"><span className={routeName === 'manuals' ? styles.active : ''} onClick={() => handleButtonClick(3)}>{language === 'en' ? 'Manuals' : 'पुस्तिकाहरू'}</span></Link>
                            {/* <Link to="/manuals/"><span className={routeName === 'subDomain' ? styles.active : ''} onClick={() => handleButtonClick(4)}>Sub-Domains</span></Link> */}
                            <Link to="/faqs/"><span className={routeName === 'faqs' ? styles.active : ''} onClick={() => handleButtonClick(5)}>{language === 'en' ? 'FAQs' : 'सोधिने प्रश्नहरू'}</span></Link>
                        </>
                    )
                    : ''}
            </div>
            ...
            <div className={styles.languageButton}>
                ...
                {showLoginForm ? <NewLoginModal homepageLogin setShowLoginForm={setShowLoginForm} routeName={routeName} /> : ''}
                ...
                {authenticated ? (
                    ...
                    : (
                        <div role="presentation" className={styles.loginButtonImage} onClick={() => setShowLoginForm(true)}>
