...
const Faqs = ({ language: { language } }) => {
	...
	useEffect(() => {
		if (content.length) {
			const filteredDescription = sidebar
				.filter((d) => d.id === headerCategory)[0]
				.childs.find((i) => i.id === selectedCategory);
			...
			<div className={styles.mainContainer}>
				...
				<div className={styles.mainBody}>
					...
					<div className={styles.content}>
						<h1>
							{language === "en"
								? "Frequently asked question for BIPAD Portal"
								: "विपद् पोर्टलका बारे बारम्बार सोधिने प्रश्नहरु"}
						</h1>
						{filteredCategory &&
							filteredCategory.childs &&
							filteredCategory.childs.length &&
							filteredCategory.childs.map((item) => (
								<div key={item.id}>
									<div className={styles.questionList}>
										<div className={styles.question}>
											<span>{language === "en" ? item.questionEn : item.questionNe}</span>
											<div
												onClick={() => {
...
export default connect(
	mapStateToProps,
	null
)(createConnectedRequestCoordinator<ReduxProps>()(createRequestClient()(Faqs)));
