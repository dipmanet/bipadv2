...
const FooterButton = ({ data, selectedCategory, onClick, lastPageId, firstPageId, language }) => {
	const sideViewData = [];
	const sideList = data.length && data.map((item) => item.childs.map((d) => sideViewData.push(d)));
	return (
		<div
			className={
				selectedCategory === firstPageId
					? styles.footerButton
				...
				sideViewData.map((item, i) =>
					selectedCategory !== firstPageId && selectedCategory === item.id ? (
						<div key={item.id}>
							<button
								type="button"
								onClick={() => onClick(sideViewData[i - 1].parent, sideViewData[i - 1].id)}>
