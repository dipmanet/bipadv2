...
class PastDateRangeInput extends React.Component<Props> {
	...
	private pastDataLabelSelector = (d, language) => (language === "en" ? d?.label : d?.labelNe);
	...
	private handleStartDateInputChange = (newStartDate: string) => {
		...
		onChange({
			rangeInDays: "custom",
			startDate:
				language === "en" ? newStartDate : convertDateAccToLanguage(newStartDate, language),
			// endDate: value ? value.endDate : undefined,
	...
	public render() {
		...
		const { customActive, customState, previousLanguage, changedStartDate, changedEndDate } =
			this.state;
		const activePage = this.context?.activeRouteDetails?.name;

		const test = convertDateAccToLanguage(value.startDate, language);
		...
		return (
			<div className={_cs(styles.pastDateRangeInput, className)}>
				<RadioInput
					...
					options={
						activePage === "dashboard" ? pastDateRangeDashboardOptions : pastDateRangeOptions
					}
									...
									value={
										language === "en"
											? value && value.startDate
												? changedStartDate
													? BSToAD(value.startDate)
													: value.startDate
												: ""
											: convertDateAccToLanguage(value.startDate, language)
									}
									// value={value.startDate}
									language={language}
								/>
								...
								<DateInput
									...
									value={
										language === "en"
											? value && value.endDate
												? changedEndDate
													? BSToAD(value.endDate)
													: value.endDate
												: ""
											: convertDateAccToLanguage(value.endDate, language)
									}
									// value={value.endDate}
									language={language}
								/>
...
export default FaramInputElement(connect(mapStateToProps)(PastDateRangeInput));
