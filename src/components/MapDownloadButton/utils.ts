...
const setRealTimeTitle = (
    ...
    realtimeList.forEach((singleRealtime) => {
        const { key, titlePart } = singleRealtime;
...
// Hazard
const setRiskInfoHazardTitle = (activeLayer: ActiveLayer, regionName: string) => {
    // Risk Info - Hazard
...
// Vulnerability
const setRiskInfoVulnerabilityTitle = (activeLayer: ActiveLayer, regionName: string) => {
...
// Risk
const setRiskInfoRiskTitle = (activeLayer: ActiveLayer, regionName: string) => {
...
// Climate Change
const setRiskInfoClimateChangeTitle = (activeLayer: ActiveLayer, regionName: string) => {
...
export const getRouteWiseTitle = (
    ...
    if (pageContext && pageContext.activeRouteDetails) {
        ...
        if (routeName === 'dashboard') {
            return setDashBoardTitle(titleContext, regionName);
        }

        // Incident
        if (routeName === 'incident') {
            return setIncidentTitle(titleContext, regionName);
        }

        // Damage and Loss
        if (routeName === 'lossAndDamage') {
            return setDamageAndLossTitle(titleContext, hazardList, regionName);
        }

        // RealTime
        if (routeName === 'realtime') {
            return setRealTimeTitle(titleContext, realtimeFilters, regionName);
        }

        // Profile
        if (routeName === 'profile') {
            return setProfileTitle(titleContext, regionName);
        }
        ...
        if (routeName === 'riskInfo') {
            ...
            */
            if (riskInfoSubModule === 'capacity-and-resources') {
                return setCapacityAndResourcesTitle(titleContext, regionName);
            }
            ...
            if (riskInfoSubModule === 'hazard') {
                return setRiskInfoHazardTitle(activeLayer, regionName);
            }

            // Risk Info - Vulnerability
            if (riskInfoSubModule === 'vulnerability') {
                return setRiskInfoVulnerabilityTitle(activeLayer, regionName);
            }

            // RiskInfo Risk
            if (riskInfoSubModule === 'risk') {
                return setRiskInfoRiskTitle(activeLayer, regionName);
            }

            // RiskInfo Climate change
            if (riskInfoSubModule === 'climate-change') {
                return setRiskInfoClimateChangeTitle(activeLayer, regionName);
            }
        }
    }
