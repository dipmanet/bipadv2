...
import Faram, { requiredCondition, lengthGreaterThanCondition } from "@togglecorp/faram";
...
const requestOptions: { [key: string]: ClientAttributes<ReduxProps, Params> } = {
	loginRequest: {
		...
		onSuccess: ({ response, props, params }) => {
			const { setAuth, setUserDetail } = props;
			const {
				profile: { otpMode },
			} = response;
...
class Login extends React.PureComponent<Props, State> {
	private static schema = {
		fields: {
			...
			password: [requiredCondition, lengthGreaterThanCondition(4)],
		},
	};
	...
	private submitNewPassword = (newpassword: string) => {
		...
		const {
			requests: { newPasswordSetRequest },
		} = this.props;
	...
	public render() {
		...
		const {
			...
			homepageLogin,
			requests: {
				loginRequest: { pending: pendingProp },
			},
		} = this.props;
		let displayElement;
							...
							<div className={styles.mainLoginContainer}>
								<div className={styles.signIn}>
									<div className={styles.signinTitles}>
										...
										<p>
											{t(
												"An integrated and comprehensive DIMS platform to support disaster risk management through informed decision making."
											)}
										</p>
										<hr />
									</div>
									...
									<div className={styles.formElements}>
										...
										<div className={styles.loginBtn}>
											<PrimaryButton type="submit" pending={pending} className={styles.newsignIn}>
												{t("Login")}
											</PrimaryButton>
										</div>
								...
								<div className={styles.pwdRequestContainer}>
									<div className={styles.closeBtn}>
										<DangerButton
											...
											<Icon name="times" className={styles.settingsBtn} />
										</DangerButton>
									</div>
		...
		if (pageAction === "thankyouPage") {
			displayElement = (
				<ThankYouPage closeModal={closeModal} pending={pending} updatePage={this.updatePage} />
			);
		}
		...
		return <Modal className={_cs(styles.newloginModal, className)}>{displayElement}</Modal>;
	}
}
...
export default connect(
	mapStateToProps,
	mapDispatchToProps
)(createConnectedRequestCoordinator<ReduxProps>()(createRequestClient(requestOptions)(Login)));
