						...
						<header className={styles.header}>
							...
							<DateOutput
								...
								value={convertDateAccToLanguage(
									new Date(incidentOn).toLocaleDateString(),
									language
								)}
								language={language}
							/>
						...
						<div className={styles.actions}>
							...
							<div className={styles.right}>
								{showEditIncident && onEditIncident && (
									<AccentButton className={styles.button} transparent onClick={onEditIncident}>
										{t("Edit")}
									</AccentButton>
								)}
						...
						<div className={styles.content}>
							<div className={styles.general}>
								...
								<TextOutput
									...
									valueClassName={styles.value}
									value={
										<FormattedDate mode="yyyy-MM-dd hh:mm" value={reportedOn} language={language} />
									}
								/>
							</div>
							{wardNames.length > 0 && (
								<div className={styles.wards}>
									<h4 className={styles.heading}>{t("Wards")}</h4>
									<div className={styles.wardList}>
										{wardNames.map((wardName) => (
											<div className={styles.ward} key={wardName}>
												<Icon name="checkmarkCircle" className={styles.icon} />
												<div className={styles.name}>{wardName}</div>
											</div>
										))}
									</div>
								</div>
