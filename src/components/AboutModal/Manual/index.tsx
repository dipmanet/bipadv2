...
import { arrayGroupBy } from "#utils/common";
...
const ManualItem = ({ data, className }: { className?: string; data: ManualElement }) => (
	<a
		href={data.file}
		...
		<div className={styles.left}>
			<img src={data.image || ManualIcon} alt="M" className={styles.image} />
		</div>
...
class Manual extends React.PureComponent<Props> {
	public render() {
		const {
			requests: {
				manualListRequest: { pending, response = {} },
			},
			className,
		} = this.props;
		...
		if (isEmpty(yearWiseList)) {
			return (
				<div className={styles.message}>
					<Translation>{(t) => <Message>{t("No Publications to display")}</Message>}</Translation>
				</div>
			);
		}
		return (
			<div className={_cs(className, styles.manual)}>
				{pending && <LoadingAnimation />}
				{Object.keys(yearWiseList)
					.sort((a, b) => b - a)
					.map((key) => {
						const orderedManauals = yearWiseList[key].sort((a, b) => (a.order < b.order ? -1 : 1));
						return (
...
export default createConnectedRequestCoordinator<Props>()(createRequestClient(requests)(Manual));
