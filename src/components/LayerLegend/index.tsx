...
const RasterLegend = (props: RasterLegendProps) => {
	...
	useEffect(() => {
		...
		imageUrlToDataUrl(url, (dataUrl: string) => {
	...
	return (
		<Translation>
			{(t) => (
				<>
					{loadingLegend && <div className={styles.loadingMessage}>{t("loading legend...")}</div>}
					{rasterLegendDataUrl && (
						<div className={styles.rasterLegend}>
							<img className={styles.rasterLegendImage} src={rasterLegendDataUrl} alt={layerName} />
						</div>
					)}
				</>
			)}
				...
				return (
					<div className={styles.legendElement} key={color}>
						...
						<div className={styles.value}>
							<Numeral normal value={value} precision={getPrecision(value)} />
						</div>
					</div>
				);
			})}
			<div className={styles.legendElement} key={lastElementKey}>
				<div className={styles.color} style={{ backgroundColor: lastElementKey }} />
				<div className={styles.value}>
					<Numeral normal value={lastElementValue} precision={getPrecision(lastElementValue)} />
					{maxValueCapped && "+"}
				</div>
			</div>
		</div>
...
const LayerLegend = (props: Props) => {
	const { className, layer, language } = props;
	...
	return (
		<div className={_cs(className, styles.legend, "map-legend-container")}>
			...
			{layer.type === "raster" && <RasterLegend layerName={layer.layername} />}
