...
class Filters extends React.PureComponent<Props, State> {
	...
	public constructor(props: Props) {
		...
		const {
			requests: { inventoryItemGetRequest },
		} = this.props;
	...
	public UNSAFE_componentWillReceiveProps() {
		const { filters: faramValues, municipalities, districts, provinces } = this.props;

		const { locRecv } = this.state;
		...
		if (
			...
			const municipalityMatch = municipalities.filter((item) => item.code === subDomain);
			const districtMatch = districts.filter((item) => item.code === subDomain);
			const provinceMatch = provinces.filter((item) => item.code === subDomain);
	...
	private views = {
		...
		rainBasin: {
			component: () => (
				<div className={styles.activeView}>
					<RainBasinSelector faramElementName="rainBasin" basins={this.props.rainFilters.basin} />
					<RainStationSelector
						faramElementName="rainStation"
						stations={
							this.state.filteredRainStation.length > 0
								? this.state.filteredRainStation
								: this.state.allStationsRain
						}
					/>
				</div>
			),
			...
			component: () => (
				<div className={styles.activeView}>
					...
					<RiverStationSelector
						...
						stations={
							this.state.filteredRiverStation.length > 0
								? this.state.filteredRiverStation
								: this.state.allStationsRiver
						}
					/>
				</div>
			),
		...
		if (type === "river") {
			if (mainVal && mainVal.riverBasin && Object.keys(mainVal.riverBasin).length > 0) {
				...
				if (mainVal.riverBasin.title !== undefined) {
					const filteredStation = this.state.allStationsRiver.filter(
						(r: { basin: string }) => r.basin === mainVal.riverBasin.title
					);
		...
		if (type === "rain") {
			if (mainVal && mainVal.rainBasin && Object.keys(mainVal.rainBasin).length > 0) {
				...
				if (mainVal.rainBasin.title !== undefined) {
					const filteredStation = this.state.allStationsRain.filter(
						(r: { basin: string }) => r.basin === mainVal.rainBasin.title
					);
	...
	private handleSubmitClick = () => {
		const { setFilters, carKeys, FilterClickedStatus, setDataArchiveRiverFilter } = this.props;
			...
			if (activeView === "riverBasin") {
				setDataArchiveRiverFilter({
					dataArchiveRiverFilters: {
						...
						municipality: faramValues.riverStation && faramValues.riverStation.municipality,
			...
			if (activeView === "rainBasin") {
				setDataArchiveRiverFilter({
					dataArchiveRiverFilters: {
						...
						municipality: faramValues.rainStation && faramValues.rainStation.municipality,
	...
	private checkRealTimeFilter = (filterValue: number) => {
		...
		if (
			realTimeFilters.faramValues &&
			realTimeFilters.faramValues.realtimeSources &&
	...
	private getTabs = (
		...
		if (
			(activeRouteDetails && activeRouteDetails.path !== "/realtime/") ||
			...
			<div className={_cs(styles.filters, className)} id="component-filter">
				<header className={styles.header}>
					<h3 className={styles.heading}>
						<Translation>{(t) => <span>{t("Filters")}</span>}</Translation>
					</h3>
					<Translation>
						{(t) => (
							<Button
				...
				<div className={styles.content}>
					...
					<Faram
						...
						{validActiveView && (
							<header className={styles.header}>
								<Translation>
									{(t) => <h3 className={styles.heading}>{t(`${tabs[validActiveView]}`)}</h3>}
								</Translation>
								<Button
					...
					{validActiveView && activeView !== "others" && (
						<div
							...
							role="presentation">
							<Translation>{(t) => <span>{t("Submit")}</span>}</Translation>
						</div>
					)}
				</div>
			</div>
