			...
			<div className={styles.rangeWrapper}>
				{riskRange.map((rangeData) => (
					<div className={styles.legendElements}>
						<div className={styles.colorElement} style={{ backgroundColor: rangeData.color }} />
						<p className={styles.scoreStream}>{rangeData.title}</p>
					</div>
				))}
			</div>
			<div className={styles.streamSource}>
				<p style={{ margin: 0 }} className={styles.scoreStream}>
					Source:
				</p>
				<a className={styles.streamLink} href="http://tethys.icimod.org/apps/streamflownepal/">
					ICIMOD
				</a>
			</div>
		</div>
