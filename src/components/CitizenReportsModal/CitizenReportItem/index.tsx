...
class CitizenReportItem extends React.PureComponent<Props, State> {
	...
	private handleReportExpansion = () => {
		const { setExpandedReport, data, isExpandedReport } = this.props;
	...
	public render() {
		const {
			...
			isExpandedReport,
			requests: {
				citizenReportPatchRequest: { pending: patchPending },
			},
		} = this.props;

		const { faramValues, faramErrors, disabled } = this.state;
			...
			<div className={_cs(className, styles.citizenReport)}>
				<div className={styles.rowContainer}>
					...
					<div className={styles.details}>
						<div className={styles.detailsTopContainer}>
							...
							/>
							{data.verified && <Badge title="Verified" icon="check" />}
						</div>
						<div className={styles.description}>{data.description || "No description"}</div>
					</div>
				...
				{isExpandedReport && data.image && (
					<div className={styles.expandedContent}>
						<img className={styles.image} src={data.image} alt="report" />
					</div>
				)}
			</div>
		);
...
export default createRequestClient(requestOptions)(CitizenReportItem);
