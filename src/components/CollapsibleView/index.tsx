...
export default class CollapsibleView extends React.PureComponent {
	...
	componentDidUpdate() {
		const { collapsing, expanding } = this.state;
		...
		if (expandedContainer && collapsedContainer) {
			...
			if (expanding) {
				const { width: initialWidth, height: initialHeight } = collapsedContainerBCR;

				const { width: finalWidth, height: finalHeight } = expandedContainerBCR;
				...
				const { width: initialWidth, height: initialHeight } = expandedContainerBCR;

				const { width: finalWidth, height: finalHeight } = collapsedContainerBCR;
	...
	animate = (
		{ expandedContainer, collapsedContainer, initialWidth, initialHeight, finalWidth, finalHeight },
		currentTimestamp,
		initialTimestamp = 0
	) => {
		...
		if (initialTimestamp === 0) {
			...
			const newWidth = initialWidth + (finalWidth - initialWidth) * animationProgress;
			const newHeight = initialHeight + (finalHeight - initialHeight) * animationProgress;
	...
	render() {
		...
		const { expanding, collapsing } = this.state;

		const className = _cs(styles.collapsibleView, classNameFromProps);
		...
		return (
			<div className={className}>
				{(expanded || collapsing) && (
					<div ref={this.expandedContainerRef} className={expandedContainerClassName}>
						{expandedView}
					</div>
				)}
				{(!expanded || expanding) && (
					<div ref={this.collapsedContainerRef} className={collapsedContainerClassName}>
						{collapsedView}
					</div>
				)}
			</div>
